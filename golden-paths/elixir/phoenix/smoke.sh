#!/bin/sh
# Ephemeral smoke (Elixir/Phoenix) — boot the release (PHX_SERVER/PORT/SECRET_KEY_BASE come from the
# image ENV), poll /ready, assert /hello, exit.
set -u
bin/golden_phoenix start &
pid=$!
