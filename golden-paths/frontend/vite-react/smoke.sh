#!/bin/sh
# Ephemeral smoke (Frontend) — start the static server, poll /ready, assert /hello + the rendered
# demo route both carry the known payload, exit.
set -u
node server.mjs &
pid=$!
...
if curl -sf http://127.0.0.1:8080/hello 2>/dev/null | grep -q 'hello, weyland' \
   && curl -sf http://127.0.0.1:8080/ 2>/dev/null | grep -q 'golden-frontend-vite-react'; then
