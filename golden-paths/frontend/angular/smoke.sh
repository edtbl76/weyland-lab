#!/bin/sh
# Ephemeral smoke (Frontend/Angular) — start the static server, poll /ready, assert /hello + the served
...
if curl -sf http://127.0.0.1:8080/hello 2>/dev/null | grep -q 'hello, weyland' \
   && curl -sf http://127.0.0.1:8080/ 2>/dev/null | grep -q 'golden-frontend-angular'; then
