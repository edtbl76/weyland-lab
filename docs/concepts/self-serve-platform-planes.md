...
3. ~~**No asset autodiscovery**~~ **— DONE (follow-up F, 2026-09-05).** `all_assets` and `all_asset_checks` are now DERIVED from the imports in `assets/__init__.py` (every imported `AssetsDefinition` / list of them; every `*_checks` list) — the manual second list that drifted at B113 is gone, verified 1:1-equivalent to what it replaced. The registration guard (`tests/test_asset_registration.py`, 4 pytest) keeps the imports honest, and a wrong derived set fails the code-server load loudly.
...
- **Follow-up F — DONE (2026-09-05):** full autodiscovery — `all_assets`/`all_asset_checks` are derived from the imports (`assets/__init__.py`, verified 1:1-equivalent to the old hand-lists), removing the double-registration; the registration guard (`tests/test_asset_registration.py`, 4 pytest) keeps the imports honest.
...
| F | ~~Asset autodiscovery — close the silent-load footgun~~ | 2 | Small | **DONE 2026-09-05** — full autodiscovery: `all_assets`/`all_asset_checks` derived from the imports (verified 1:1) + the registration guard (4 pytest). |
