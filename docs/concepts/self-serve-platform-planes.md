...
3. ~~**No asset autodiscovery**~~ **— silent-load footgun CLOSED (follow-up F, 2026-09-05).** A static registration guard (`tests/test_asset_registration.py`, stdlib ast, no dagster) fails CI when a `build_land_asset` symbol is not both imported and in `all_assets` — so the `edgar_land`-class miss is loud at CI, not in production. Full autodiscovery (dropping the manual lists) is the elegant follow-on, deferred because it changes the dagster load path and needs materialization testing.
...
| **ODCS contracts** | Partial (foundation, follow-up E) | ODCS adopted as the canonical shape (a lab subset — [decision doc](data-contracts-odcs.md)); the finance domain is fully expressed as `contracts/finance/*.odcs.yaml` with a CI conformance gate. Other domains + live-schema conformance are the tracked continuation. |
...
2. ~~**Contracts not unified under one standard**~~ **— FOUNDATION shipped (follow-up E / B157, 2026-09-05).** ODCS adopted (lab subset); the finance domain's four products are expressed as `*.odcs.yaml` mapped from the existing dbt/Soda/DomainConfig substance, gated in CI by `check-odcs-contracts.sh` (10 bats). Music/health migration + live-schema conformance are tracked follow-ons in the [decision doc](data-contracts-odcs.md).
...
- **Follow-up E — FOUNDATION DONE (2026-09-05, B157):** ODCS adopted as the canonical contract shape (lab subset — [decision doc](data-contracts-odcs.md)); the finance domain's four products contracted (`contracts/finance/*.odcs.yaml`) + `check-odcs-contracts.sh` conformance gate (10 bats, shellcheck clean, CI-wired). Other domains + live-schema conformance are tracked follow-ons.
- **Follow-up F — DONE (2026-09-05):** a static asset-registration guard (`tests/test_asset_registration.py`, 4 pytest) fails CI when a `build_land_asset` symbol isn't imported + in `all_assets`, closing the silent-load footgun (full autodiscovery deferred — a dagster load-path change).

**All six follow-ups (A–F) shipped.** The mesh's self-serve planes are now: infra invisible at use *and* self-provisioning (D); the product-experience edges paved (B ingestion, C operate) with the split single-sourced; and the mesh-experience plane governable by CI (A catalog-coverage) with contracts under one standard (E) and registration guarded (F).
...
| E | ~~ODCS adoption — unify contracts under one standard + conformance gate~~ | 3 | Large | **FOUNDATION DONE 2026-09-05** (B157) — subset adopted, finance contracted, CI gate; other domains + live-schema conformance tracked. |
| F | ~~Asset autodiscovery — close the silent-load footgun~~ | 2 | Small | **DONE 2026-09-05** — static registration guard (4 pytest) fails CI on an unregistered land asset; full autodiscovery deferred (load-path change). |
