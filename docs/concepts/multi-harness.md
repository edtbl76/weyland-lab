...
| **VS Code** (1.139, secondary) | **Codex** extension (`openai.chatgpt`) — reads `~/.codex/config.toml`, so Linear + Bifrost come with it; **Claude Code** extension (`anthropic.claude-code`, installed 2026-09-25) | via the harness extensions; also a native `linear` entry in `~/.config/Code/User/mcp.json` (used only by Copilot Chat); **Linear Connect** (`linear.linear-connect`) provides a shared Linear sign-in to other VS Code extensions | Copilot Chat stays available but is not the path |
