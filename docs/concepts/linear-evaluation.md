...
| **Agent (coding sessions) + Skills** | **DON'T ADOPT the Agent (paid cloud, duplicates local Claude Code); DO invest in portable skills (git SoT + Bifrost registry)** | Linear's "Agent" = **coding sessions**: delegate an issue → Linear spins a coding session in its **managed cloud sandbox** (Claude Code/Codex), drafts a PR. It **costs AI credits** (not $0) and **runs in the cloud, not the LAN** — duplicating the local Claude Code workflow the lab already runs free. Fails both hard constraints → don't adopt. **Covers the self-hosted variants too (2026-09-25):** e.g. **Cyrus** (Apache-2.0, runs Claude Code on your own machine, BYOK) fixes cost + cloud but is still *delegate an issue → coding session → PR* — the duplication of the local Claude Code workflow stands, and it adds a new cost: Linear's webhooks would need an ingress into the LAN. Same for the rest of Linear's agent directory (Codex, Cursor cloud agents, Copilot agent, Factory, Devin, Charlie, …); **cto.new** is genuinely free (frontier models, no keys) but fails on cloud (clones repos, incl. private, into its VMs) + duplication. **Integrations-directory sweep, 2026-09-25 — 12 more, all DON'T ADOPT, researched with sources:** *cloud agent → PR (the rejected pattern):* Tembo, [code]smith (free ended 2026-08-15), Blocks (free 250 min/mo + BYO keys; 7 of 9 repos are public so exposure is limited — but it is the rejected pattern, UNATTENDED; kept as a **comparator in B179** — Warp vs Blocks, what a "software factory" gives a solo lab that interactive Claude Code doesn't), TierZero (incident agent, paid, acquired by Cognition), Stilla (meeting agent + PR drafts; being acquired by Meta); *duplicates a self-hosted system:* Reflag (hosted flags, JS/TS-first + Ruby, others via HTTP API/OpenFeature — a cloud copy of the self-hosted Unleash, which is language-agnostic), Dash0 (hosted OTel — have Prometheus/Loki/Tempo/Pyroscope/Grafana + the Grafana MCP), Larridin (enterprise dev analytics — have Port + SonarQube + LiteLLM/Bifrost cost); *needs something the lab doesn't use:* Sanity Content Agent (a Sanity CMS), Minds (a research audience), Ranger (originally: cloud-browser QA can't reach LAN apps — **superseded 2026-09-26, see the Ranger re-evaluation below**); CellCog (one-time credits; duplicates the local LLM + Bifrost). Only Reflag and Sanity have a real $0 plan — both fail on fit, not cost. **Sweep continued (same day) — 6 more, all DON'T ADOPT:** *cloud agent → PR:* Sinatra (real free tier: 5 tasks/day on your own model key — still the rejected pattern), Replicas (paid, $50/mo); *duplicates a self-hosted system:* Solo (enterprise codebase Q&A from $750/mo — have Sourcebot + Zoekt + graphify + `context_ask`), Testifly (AI E2E tests; the Linear integration needs its $89/mo tier and it can't reach `*.weyland.lab` for free — have Playwright MCP + Claude Code, Keploy for APIs); *built for teams/business functions the lab doesn't have:* Coco by Cotera (sales/ops agents, one-time credits), Jellyfish Agent (DevEx survey on every closed issue — no team to survey). **Third sweep (same day) — 15 more, all DON'T ADOPT:** *cloud agent → PR:* Jules MCP (Google; real free tier, 15 tasks/24h — free doesn't fix the pattern, same as cto.new), Producta (1-month trial only), Tusk (unit tests as PRs; Linear on the paid Team plan), Builder.io MCP (Fusion visual agent → UI PRs; free 60 credits/mo; overlaps Figma Make + the Figma MCP, B39); *duplicates what the lab already runs:* CleanSlate (agents over Linear issues = Claude Code + the Linear MCP), Postman Agent Mode (files issues via the Linear MCP; Keploy covers API tests), 11.ai (ElevenLabs voice assistant, free in alpha, cloud — the operator's Telegram lane covers chat-ops), Alkemi (team reporting — views + Port), APX (Datadog/Postgres agents — no Datadog; Prometheus alerting + the Postgres MCP), Lovable MCP (Figma Make on the existing Figma Pro is the prototyping path, B181), Qodo (PR vs issue-requirements compliance — Qodo was cancelled in B106; its OSS PR-Agent already runs via the gateway); *needs what the lab doesn't have:* Fellow, Spellar AI (meeting capture — no meetings; Fellow's Linear link is paid-only), Attio Agents (a CRM). **CodeRabbit** is the near-miss: it is ALREADY adopted (B106, free public-repo tier), and its Linear integration (PR checked against the issue's acceptance criteria, coding plans, issue creation) is useful — but CodeRabbit's docs gate it to Essentials and above, excluding Free/OSS, so it stays unconnected. **Fourth sweep (same day) — 14 more, 1 ADOPTED:** **Codex MCP — ADOPTED** (config, not a product: Codex CLI → Linear's hosted MCP, OAuth; `codex exec` verified `linear/get_issue` on EMA-239; setup in `docs/runbooks/coding-agents.md`, which also records the Ubuntu 24.04 bubblewrap/AppArmor fix Codex's sandbox needed). DON'T ADOPT: *MCP-client entries for editors the lab doesn't use:* Zed, Notion Custom Agents (no Notion; Business plan); *duplicates what the lab already runs:* ContextStream (hosted agent memory; re-judged 2026-09-25 after "cloud is fine if free": the free tier is real — 10k credits/mo, 15 projects — but per-operation credit cost is unpublished and it would be a second, proprietary memory store. The need it names is real, though: **shared memory across harnesses** is now **B182** (EMA-240), candidate = an MCP memory server behind Bifrost), Gemini Enterprise (paid; Claude + the Linear MCP), Super (team Q&A over Linear), Systemprompt.io (MCP management from mobile — Bifrost manages MCP servers, the operator's Telegram lane is the mobile path; also judged vapor on inspection 2026-09-25 — interesting idea, no usable product yet, revisit only if it ships), Lovable (prototyping path already decided, B181); *paid or built for teams/functions the lab doesn't have:* Graph (team insights; its agents need paid Slack), Kraftful (customer feedback), Reclaim.ai (Linear is paid-only, not on Lite), Rize (paid time tracking), Hookshot Proteges ($99/mo+). **Warp** is not judged here — it is B179's own subject, with Blocks as its comparator. **Figma Make — ADOPTED (2026-09-25):** Linear's verified MCP connector inside Figma Make's chat — reads issues/projects/documents into a Make session and creates/updates issues back; $0 extra on the existing Figma Pro plan (Make needs a Full seat on a paid plan; connectors are on all plans), but connector context counts toward Make's 3,000 AI credits/mo, so pull the specific issue, not a project. Enabled together with the GitHub connector. It also supersedes the Figma Make half of B181 (a URL launcher): Make fetches the issue itself. Evaluating Make's Connectors / Designs / Skills properly is **B183** (EMA-241). **VS Code MCP — ADOPTED (2026-09-25, reversing the earlier DON'T at the owner's call):** VS Code reaches Linear through the **Codex extension** (`openai.chatgpt`, which uses `~/.codex/config.toml` — Linear + Bifrost already wired), with the Claude Code extension (`anthropic.claude-code`) installed beside it; the native `linear` entry in `~/.config/Code/User/mcp.json` (`code --add-mcp …`) serves Copilot Chat only. Editors are hosts, not harnesses — see `concepts/multi-harness.md`. **Also 2026-09-25:** Gemini Enterprise (paid-only product; the lab's free Gemini is a different thing) and Reclaim.ai (Linear is paid-only, not on Lite) confirmed DON'T; **PagerDuty — DON'T** (not used; alerting is Alertmanager → Telegram); **Sentry — DEFERRED to B53** (the Sentry ↔ Linear integration is folded into the B53 Sentry spike, where it is weighed against GlitchTip). **Linear Connect (VS Code) — DON'T (installed, tried, uninstalled same day):** Linear's official VS Code extension (`linear.linear-connect`) is only an authentication provider — no UI of its own; it lends a Linear sign-in to other VS Code extensions, and none installed use it (Codex and Claude Code reach Linear through their own MCP configs). Side items spun out of the walk: B183 (Figma Make), B184 (Slack), B185 (time tracking), B186 (GitLens — WON'T DO 2026-09-26: not used enough to pay for). **incident.io — DON'T (deferred to B184):** the free Basic plan is real (Slack-native incidents, 1 schedule, 1 status page, 1 workflow, 2 integrations) but everything that would actually be used — API/webhooks, MCP, AI summaries/root cause, multiple schedules, private incidents — is paid; free self-hosted alternatives (Keep, OneUptime) and the Telegram ↔ Slack routing split are now part of B184's audit. **Honeybadger — DON'T:** bundles error tracking (free 5k/mo, 15-day retention), logs (50 MB/day), uptime (1 monitor), cron check-ins and a status page — each already self-hosted here (GlitchTip, Loki, blackbox + Uptime Kuma, the hand-rolled cron-freshness guards, Kuma's status page); free-tier integrations are GitHub/Slack/email only, and the commercial-vs-GlitchTip comparison slot is already B53 (Sentry). **Retool — DON'T:** a real free plan (5 users, unlimited apps, 500 workflow runs/mo; self-hosting NOT on Free) and its Linear resource can create/report/update issues — but a cloud-hosted builder cannot reach the LAN-only `*.weyland.lab` services, so it could only join SaaS APIs (Linear/GitHub/Port), and Port already is the lab's self-service UI + actions layer; no concrete internal app is waiting on a builder (if one appears, a self-hosted builder such as Appsmith/ToolJet is the lab-native evaluation). **Aldena — DON'T:** an AI-agent platform (agents in "rooms" read/update Linear and do the engineering work) — the rejected cloud-agent pattern, and only a 14-day trial with a card, no free tier. **Sweep continued (2026-09-25) — 6 more, all DON'T:** All Quiet (incident management with Alertmanager + Slack + Linear integrations, but plans start at $4.99/user/mo — no forever-free tier; the free incident candidates are Keep / OneUptime in B184), GitHub Enterprise Cloud (paid GitHub tier; the free GitHub ↔ Linear integration is already connected), Iteration X (browser bug capture, Jira-first; the lab has no outside users filing bugs), Marker.io (website feedback/bug reports; 15-day trial only), Merge (a unified API for SaaS vendors building integrations into their own product — not a lab use), Neat (free Linear/GitHub notifications app, but macOS-only — no Linux build). **Sweep continued — 5 more, all DON'T (Qodo re-listed, already judged):** Netlify (the lab hosts nothing on Netlify — apps are LAN services; the blog is on Vercel, see the Vercel correction), **PostHog** (free cloud tier, creates/links Linear issues from analytics/replays/errors — no product analytics exist here, but nothing needs them yet; deferred as **B188** (LOW, `parked:held` on OJay Floyd / MyBodyGraph having real users), together with Iteration X / Marker.io-style feedback capture), Testmo (paid test management; tests are code in CI), Truto (a unified API for SaaS vendors, like Merge), Warestack (GitHub events as Linear comments — the connected GitHub ↔ Linear integration already links PRs/commits). **Sweep continued — 9 more:** DON'T: Delvyn Studio (AI spec writing — AI-DLC + the backlog), Deviera (CI failures / stale PRs → issues — `check-pr-lifecycle.sh` + CI alerting), Trigger.dev (Apache-2.0 background jobs — a fourth scheduler beside Dagster / CronJobs / Woodpecker), Spike (one free month, then paid), Testsigma (paid AI test generation), Testomat (free for 2 users / 2 projects, but tests are code in CI), groundcover (free forever but 12-hour retention in your own cloud account; duplicates the LGTM + Pyroscope stack), Todo comments for Linear (a paid VS Code extension). **Runframe — candidate for B184** (hosted incident response free for up to 5 users: on-call, escalation, status page, AI post-incident reviews, Linear follow-ups — the hosted, zero-RAM counterpart to the self-hosted Keep / OneUptime). **Sweep continued — 8 more, all DON'T (CodeRabbit re-listed, already judged):** *coordination between people:* Axolo (a Slack channel per PR synced to its Linear issue — now a B184 audit candidate), Livecycle (PR reviews + preview environments → Linear), Opsgenie Triage (Opsgenie not used), Pagerly (Slack triage rotations, $19/team/mo). **Correction (owner, same day):** rotation was first dismissed as meaningless for one person — wrong: the lab should RUN and TEST on-call, so **B184 now requires at least one exercised pager rotation** (e.g. between two Telegram channels), provided by the chosen incident tool (OneUptime / Runframe schedules) rather than a paid rotation add-on; *already covered:* GitHub Enterprise Server (regular GitHub + the connected GitHub ↔ Linear integration), Kualitee (paid test management — the question is now B189), Pensar (AI security findings → Linear — the scan suite + SonarQube + Trivy are the DoD Pillar 7 lane); *needs users:* Modem (customer feedback → issues — joins the B188 held stack). **Sweep continued — 10 more:** **Testiny → B189 candidate** (hosted test management, free for up to 3 users, Linear integration on the free plan — the only hosted free test tool with a Linear link). **SpecBot — ADOPTED (tried same day):** scores a Linear issue's readiness for an AI coding agent on 8 dimensions (objective, expected behavior, acceptance criteria, edge cases, technical context, dependencies, repro, priority/scope), threshold 80; free 25 analyses/mo; invoked by mentioning `@SpecBot` in the issue (`recheck` after fixes). First run: **EMA-240 (B182) scored 51/100 — NOT READY** (acceptance criteria 12, edge cases 10, priority/scope 12). The gap was systemic (every AI-drafted issue used Why/Scope only), so it was fixed at the source: the Backlog item / Spike / Bug templates gained Acceptance criteria + Edge cases + Out of scope sections, and `AGENTS.md` now requires them of every harness. A self-hosted replacement is **B190**. DON'T: Runway (mobile release management — no mobile apps), Shake (in-app bug reports — needs users; B188 stack), Spades (team planning-poker estimation), Rootly (incident management, $20/user/mo — the free incident candidates are in B184), AppDeploy (publish web apps from Linear — unverified; overlaps Figma Make / B183), TestOrchestrator (test cases from Linear — B189 covers test management), Graphite (stacked PRs; you commit to main, and GitHub is the PR surface), Highlight (session replay + errors — needs users; B188 stack; its status after the LaunchDarkly acquisition is unverified). **Core integrations screen (2026-09-26) — 12:** KEEP GitHub + Figma (both enabled). → B184: Slack (Linear's own Slack integration, the first thing B184 connects) and **Linear Asks for Slack** (requests from Slack/email → issues, helpdesk flows — a demo candidate; plan tier unverified). DON'T: PagerDuty Triage (not used; B184's rotation comes from the free incident tool — listed in its rotation table), Zapier (the lab already runs self-hosted **n8n**, which has a Linear node), Airbyte (a heavy extra service — but the idea behind it is noted: **Linear as a data-mesh source** via a small Dagster asset over the Linear API would feed cycle time (B185), DORA (EMA-172) and OKR progress (B119.1) from one table), GitLab / Notion / Microsoft Teams (not used), Zendesk / Intercom (customer support, no customers — the B188 held stack). **Core integrations screen 2 (2026-09-26):** KEEP Google Sheets (enabled). Already judged: Sentry (→ B53), VS Code / Linear Connect (DON'T), GitHub Enterprise Cloud (DON'T). **Create issues via email** (pre-installed) — a team/template intake address turns an email into an issue; noted as a **B184 fallback** for alert → Linear (an Alertmanager email receiver; needs outbound SMTP and does not deduplicate). Descript / Loom (pre-installed embeds; neither tool is used, so they never activate — left as is; **reconsidered under B191** and set aside there: YouTube covers external sharing and is automatable, Descript only matters for narrated demos). DON'T: Discord (not used — Telegram, Slack per B184), Front / Gong / Salesforce (support, sales calls, CRM — no customers; Salesforce is an Enterprise add-on), Jira (a migration path from a tool never used). **YouTube — ADOPT via B191 (2026-09-26):** pre-installed; plays YouTube videos inline in Linear issues and documents. B191 publishes its scripted demo recordings **unlisted to YouTube** by API (MinIO keeps the master copy), so each B-item's demo plays inline on its Linear issue. GitHub Enterprise Server and Opsgenie Triage on the same screen were already judged (DON'T). **Bug-capture screen (2026-09-26):** repeats already judged — Linear Asks (→ B184), Sentry (→ B53), incident.io (→ B184), Honeybadger, All Quiet. **Jam — SPIKE B192 (Medium, EMA-251; site jam.dev) — free tier far too small on its own:** a Chrome extension that captures screen recording + console + network into a Linear issue, with an MCP server agents can read captures through; free = 30 Jams, 5 recording links, 5-minute recordings. The lab files issues at ~30 an hour during a working session, so the cap is gone in an afternoon. The idea (capture UI evidence straight into Linear) stays useful — B191's recording pipeline is where a lab-owned version would come from. DON'T: Bird Eats Bug (same idea; free tier 15 uploads and Linear likely paid), Vercel (preview-deployment comments → Linear issues; **correction 2026-09-26: the blog emangini.com IS on Vercel** — still DON'T, the only commenter on its previews is the owner, so comment→issue adds nothing over filing directly; revisit if others review previews), Arc (pre-installed, inert — Arc has no Linux build; Chrome is the browser). **Bug-capture screen 2 (2026-09-26):** repeats already judged (Vercel, Arc, All Quiet, Iteration X, Marker.io, Qodo). **Ranger — RE-EVALUATED (2026-09-26), the first verdict withdrawn:** Ranger is three things and only one was judged. (1) The **Linear integration** — `@ranger` in a comment reformats a bug issue and generates a feature test plan; it runs inside Linear on issue text, so reachability never applied (overlaps the Bug template + SpecBot/B190 and B189's test plans). (2) The **cloud QA agent** — the original DON'T (can't reach `*.weyland.lab`). (3) A **local CLI** (`npm install -g @ranger-testing/ranger-cli` → `ranger setup` in a project → `ranger go`) that launches a browser against a locally built UI, tests a feature and returns a verdict, shown on a **Feature Reviews** dashboard. If that browser runs on rogueone it can reach the LAN — to be proven. Cost: custom annual contracts by suite size, no published free tier (the owner has an account; what it allows is unknown). **→ comparator in B191** (agent-free feature verification vs the Playwright walk / Playwright MCP). **BetterBugs → comparator in B192** (free 10 recording links/mo, console + network, Linear on free unconfirmed, MCP paid-only). **BugHerd — DON'T** (client website feedback; 7-day trial, then from $42/mo). **Testing & feedback screen (2026-09-26) — 15, 4 repeats (Testmo, Spike, APX, Kualitee):** **QA Sphere → B189 candidate** (hosted test management, free to 3 users with integrations incl. Linear). **TestDino → B189 comparator** (Playwright test reporting; free 5,000 runs/mo, 1 project, 90-day retention; Linear on free unverified; relevant to B191's Playwright recordings). **Claap → B192 comparator** (bug videos → Linear; free 10 videos/user — too few at the lab's rate). Supercut (recordings → Linear; unverified) noted in B192. DON'T: TalkThrough (voice/drawing annotation; unverified, overlaps B192), Ybug / Capture.dev / Feedbucket / Lay (website feedback — the B188 users stack; Ybug's integrations are paid), Flare (PHP/Laravel error tracking, trial — no PHP here, GlitchTip covers errors), Dixa (customer service). **Testing & feedback screen 2 (2026-09-26) — 14, 3 repeats (Shake, Testiny, Rootly):** all DON'T. Test tools → B189's "also seen" table: Currents (Playwright/Cypress dashboard, from $49/mo — TestDino covers it free), TestCollab (trial), TestLodge (from $34/mo), TestMu AI (LambdaTest renamed 2026-01; tiny free tiers), Reflect (AI e2e; cloud runner likely can't reach the LAN — same question as Ranger's CLI). Subtext (session-evidence triage — its Linear integration needs Linear Business/Enterprise because it uses Loops; no user sessions yet). Usersnap / Sleekplan / Userback (customer feedback — B188 stack). Laravel Nightwatch (no PHP). Thena (B2B support). **Automation & forms screen (2026-09-26) — 15, 4 repeats (Zapier, Create issues via email, Jira, Trigger.dev):** all DON'T. Hosted automation builders — Integrately, IFTTT, viaSocket (with Zapier) — can't reach the LAN; the self-hosted **n8n** already running has a Linear node and can. **This gave n8n a real job:** U14 (EMA-16) now makes n8n the lab's Linear automation hub, starting with a **Linear weekly digest** to Telegram — n8n had zero active workflows and was a retirement candidate. Raycast (no Linux build). Fivetran (cloud ELT can't reach the LAN lakehouse — third tool after Airbyte / Sheets whose job is getting Linear data out; strengthens the case for a small Dagster ingestion job feeding B185 / EMA-172 / B119.1). Fillout / Jotform (intake forms — no outside requesters; B188 stack). Harvest (time entries per issue; reportedly 1 seat / 2 projects free — noted in B185 as a manual-timer comparator beside Clockify). Supernormal / Tactiq (meetings → issues — no meetings). NEXUPIRA (AI agents acting on Linear — the cloud-agent pattern). **Automation & enforcement screen (2026-09-26) — 15, 5 repeats (NEXUPIRA, Trigger.dev, viaSocket, Testomat, Axolo):** **LineGuard + Required → B190 comparators** (they enforce required fields per workflow state by moving the issue back with a comment — the missing "gate" beside templates and scoring; pricing unknown). DON'T: hosted automation builders Latenode, Monkedo, Pipedream, SureTriggers (can't reach the LAN → n8n, U14), Capybara (Jira), Joyn (customer-facing updates), Tally (forms — B188 stack), Circleback (meetings). **Scheduling & automation screen (2026-09-26) — 15, 3 repeats (Opsgenie Triage, Reclaim.ai, Testiny):** **n8n → U14** (the Linear automation hub — but its Linear Trigger is webhook-based and the lab's n8n is LAN-only, so U14 builds on a Schedule trigger + Linear API polling; recorded in U14). **SnoozeThis — candidate:** a comment ("wait until [date]" or a GitHub issue URL) snoozes an issue, watches the blocker and restores it when resolved — automates the lab's `parked:held` convention for date/GitHub blockers (not for B188's "has users" hold); GitHub app free, Linear tier unverified. DON'T: Formcrafts (forms — B188 stack), Google Calendar (out-of-office → Linear status; availability signal for teammates, none here), Granola / Steady (meetings, team check-ins), Jet Admin (no-code app builder — Port), LangSmith Agent Builder (hosted agent builder — LangGraph operator + Langfuse), Lindy / Make (hosted builders → n8n), Quicktion (emails → issues — Linear's own email intake), Shipam (launch pages — nothing public yet). **Screen (2026-09-26) — 3, 1 repeat (TestLodge):** Traqq (manual timer from Linear issues; free 3 users / 1 project; Linux app; no automatic tracking) → noted in B185 beside Clockify / Harvest. Unito (2-way sync with HubSpot / Salesforce / ServiceNow / GitLab / Jira — no other tracker or CRM; 14-day trial) — DON'T. **Customer Experience category (2026-09-26) — 15, 5 repeats (Zendesk, Intercom, Front, Salesforce, Gong):** all customer-facing → the B188 held stack or DON'T: Attio (CRM), Productlane (helpdesk / public roadmap / changelog, $29/user/mo), Arkweaver (go-live messages to customers), Cycle / Olvy (feedback), Fernand / Pylon (support), Index (Productboard-style discovery), Lightfield (customer conversations). **Lindie — candidate:** private read-only links to one Linear project, viewable without a Linear account (Linear itself shows projects only to members or paid guests); free = 1 shareable project — a fit for showing the lab to family / a portfolio showcase beside B191's YouTube demos. **Customer Experience category, screen 2 (2026-09-26) — 15, all new, all DON'T (need customers):** SupportBee, HappyFox, HubSpot Sync (IssueLinker), Startdeliver, Lane, Atlas Support, BuildBetter, Canny, Deskpro, Formspree, Leanbe (support / feedback / forms / signals), ProdPad, Fibery, Craft.io (a second planning tool — Linear is the planner; goals go in initiatives per B119.1). **Chatwoot → noted in B188** as the open-source, self-hostable customer-chat option if the product apps ever need one. **Customer Experience category, screen 3 (2026-09-26) — 15, 3 repeats (Dixa, Kraftful, Solo):** DON'T (need customers / support / research): Dovetail, Ducalis (vote-based prioritization + changelog — prioritization is the backlog order + priority field), Featurebase, FeatureOS, Harvestr, Help Scout, initdesk, Plain, Survicate, Unthread. **Sharing Linear outward — comparators to Lindie:** Helium Rooms (share Linear boards with outsiders in read-only rooms) and Feedvote (client portal + public roadmap on Linear). With Lindie they form the one Customer-Experience need that doesn't wait on users: showing the lab's work to people outside Linear (family, portfolio). → **spike B193** (Low, EMA-252). **End of Customer Experience + start of Collaboration (2026-09-26) — 9, 5 repeats (Slack, Linear Asks, Notion, Discord, Attio):** DON'T: Hipporello Service Desk, SparrowDesk, Feedovate (support / feedback — B188 stack), Glean (paid enterprise search — `context_ask` / Sourcebot + the Linear MCP in every harness already search Linear). **Collaboration category, screen 2 (2026-09-26) — 15, 7 repeats (Productlane, Vercel, Cycle, Descript, Fellow, Fillout, Index):** all DON'T: Range (async team check-ins), Slab (knowledge base — the MkDocs docs site is it), Cardinal (PRDs + customer feedback — specs live in the backlog / design docs), Commented / Hotjar (feedback / heatmaps — need users; B188 stack), Fireflies.ai (meetings), Guru (AI answers — `context_ask` + the Linear MCP), Kollabe (planning-poker estimation — estimates are off by choice, and it needs several estimators). **Collaboration category, screen 3 (2026-09-26) — 15, 4 repeats (Lindie, Loom, Olvy, Fibery):** **Roadmark, SteelSync, Alignear → B193 comparators** (sharing Linear outward; Roadmark turns an initiative into a shareable roadmap — fits B119.1's OKRs and avoids exposing raw issues). DON'T: LinCal / Morgen (calendar time-blocking), Locu (personal task app — Linear is the list), Miro (no whiteboard in use; FigJam/Figma covers design), NEXT (user research — B188), PlanningPoker.live (estimates off by choice), Read AI (meetings), Teamplify (team analytics). **Collaboration category, screen 4 (2026-09-26) — 15, 6 repeats (Joyn, Leanbe, Claap, Craft.io, Ducalis, Featurebase):** **SuperDuty → B184 rotation table** (auto-assigns the on-duty member to Linear triage; free tier to verify). DON'T: Sunsama / Superlist (daily planner / personal task app — Linear is the list), Timing (macOS only — B185 has the Linux options), Tana (meetings), Whimsical (diagrams are LikeC4 + Mermaid), Clay (personal CRM), ClearFeed (Slack support desk), DailyBot (async standups — no team). **Collaboration category, screen 5 (2026-09-26) — 15, 6 repeats (FeatureOS, Feedvote, Granola, Helium Rooms, Kraftful, Livecycle):** **Korl — noted beside B193** (auto-generates product presentations/updates from Linear project data — a slide-deck form of showing the lab outward; free tier unverified). DON'T: Float (team capacity planning — no team, no estimates), GitBook / Slite (doc embeds — MkDocs is the docs platform), himala / Hypercontext / Parabol (meetings, agendas, retros), One Horizon (daily Linear recaps — U14's n8n digest is that job), Screenpresso (Windows only; bug capture is B192). **End of Collaboration + start of Media & Design (2026-09-26) — 13, 8 repeats (Steady, Thena, Usersnap, Hipporello, Userback, Figma, Descript, Loom):** all 5 new DON'T: Agiled (a second business-management tool), Team O'clock (planning poker — estimates off by choice — plus team retros), TeamRetro (team retros), Plandeck (cross-team critical path — one team; blocking links already show the path), Reforge Insights (customer-feedback mining — B188 stack). **End of Media & Design + start of Analytics (2026-09-26) — 12, 9 repeats (Miro, YouTube, Claap, Screenpresso, Airbyte, Google Sheets, Fivetran, Retool, Jellyfish):** all 3 new DON'T: Tella (hosted screen recorder — B191 records by script and publishes to YouTube, already embedded by Linear), Canva AI Connector (Canva not used; Figma is the design tool), Span (paid DORA-style team analytics — DORA is EMA-172). The Analytics category is mostly tools for getting Linear data out for metrics — the same unbuilt Linear-as-a-data-source Dagster job again (B185 / EMA-172 / B119.1). **Analytics category, screen 2 (2026-09-26) — 15, 2 repeats (NEXT, Teamplify):** → B185 "also seen": Everhour, TrackingTime, TMetric (timers on Linear issues). → B119.1 comparators: Hill Charts (Curious Lab; Routific) — visual progress for initiatives; DoubleLoop — strategy/impact maps. DON'T: Basedash (Linear Q&A/dashboards — Grafana/Superset + the Linear MCP; data step is the Dagster idea), Cycle Report (client sign-off), Multitudes (team analytics), Screenful (scheduled reports — U14's n8n digest; charts in Grafana), Secoda (data catalog — DataHub), Linage (cycle narratives — no cycles run), minware (paid team-scale AI/dev analytics — the lab's agent metrics are Port's ai_session, B62). **End of Analytics + start of Security & Compliance (2026-09-26) — 15, 2 repeats (One Horizon, Survicate):** Timely → B185 "also seen" (automatic AI time tracking, paid). Swarmia → comparator for EMA-172 (DORA-style developer-productivity insights from Linear + GitHub; team-scale, likely paid — EMA-172 builds DORA from the lab's own data). DON'T: Polymer (dashboards — Grafana/Superset), Stepflo (user-journey friction — B188), Phase (biology-based personal scheduling), Fencer / SecureSlate (security/compliance tickets — the scan suite + SonarQube + Trivy are the DoD Pillar 7 lane), Kawach AI (org compliance policies for the workspace), Vanta / Drata (paid compliance-certification automation — no certification pursued), Orca Security (paid cloud-account security — the lab is not on a cloud provider). **Security & Compliance, screen 2 (2026-09-26) — 10, 6 repeats (Fencer, Kawach AI, SecureSlate, Vanta, Drata, Orca):** **SimpleBackups + Cloudback — surfaced a real gap:** nothing backs up the Linear workspace (status, comments, templates, views, labels, soon the B119.1 OKRs) while Postgres / MinIO / rogueone are all backed up; free tiers unverified. The lab-native answer is the same nightly Dagster export seen five times before (Linear API → MinIO): a restorable backup AND the data source for B185 / EMA-172 / B119.1. → **B194** (High, EMA-253). DON'T: Aikido Security (AppSec platform — the scan suite + SonarQube + Trivy, DoD Pillar 7), Expiration Reminder (cert expiry is already a blackbox-exporter metric; no contracts). **Tally: 294 directory integrations checked, 4 adopted (Codex MCP, Figma Make, VS Code MCP, SpecBot) + 3 already enabled (GitHub, Figma, Google Sheets); the only live evaluation thread is B179.** BUT its "skills" are just **repo files** (`skills.md` / Claude Code skills), which the lab already owns: `register_bifrost_skills.py` is the git SoT, and Bifrost **serves them as a Claude Code marketplace**. So a skill is a portable artifact with a swappable consumer — invest there ($0, LAN, local Claude Code now; the Linear Agent later if ever paid). Seeded 4 loop-skills (DoD gate, master-the-tool walk, pr-lifecycle reconcile, full-guard-suite) — executes B175. |
...
| **Team Home (Overview / Team resources / Documents)** | **ADOPT MINIMAL — a one-line description + resource links in one section PER INITIATIVE (as built 2026-09-25); DON'T ADOPT Linear Documents** | Home is the team's landing page. It earns a place only as a *pointer* to where truth lives outside Linear: **Docs** (`https://docs.weyland.lab` — `mkdocs-site.yml` builds all of `docs/`, so the backlog, DoD, runbooks and demos are already there), **Port** (`https://app.port.io` — the IDP), **Bifrost** (`https://bifrost.weyland.lab` — the AI gateway + MCP/prompt/skill registry). Description: *"Status lives here. Order and detail live in git: docs.weyland.lab (backlog, DoD, runbooks)."* — the `check-linear-sync` rule in plain words. **As built:** sections mirror the 6 initiatives — Lab & Systems (Docs, Port, Bifrost, GitHub weyland-lab) · Music Studio (stud.io repo) · Helper Tools (OJayFloyd, startme-curator repos) · Learning (freejack) · My Work (emangini.com + Vercel, Algopedia, ServiceTransformation, blog repos) · Health and Fitness (MyBodyGraph) — so Home is the portfolio's launchpad, grouped the same way the initiatives drive scope. **Documents: DON'T ADOPT** — authoring docs in Linear would fork the git record (guarded by the doc checks) into an unguarded copy. **Settings → Documents → Templates (2026-09-26): left empty** — with no Linear documents there is nothing for a document template to shape. **Views not linked** — the 19 views are one click away in the sidebar. Set in the UI: team resources are `TeamPinnedResource` and the API has **no pin mutation** — `entityExternalLinkCreate(teamId)` succeeds but makes an UNPINNED, invisible link with no list query to find it again (3 such orphans were created trying, 2026-09-25). The description WAS set via `teamUpdate`. |
| **Customer requests** | **OFF — until B188 un-holds** (2026-09-26) | Links customers (with revenue / tier attributes) to issues and projects so work is prioritized by who asks. With no customers the owner would be the only one — i.e. the backlog again. Unlike on-call rotation (exercisable with stand-ins, B184), a demo with fake customers exercises only the UI, not the prioritization it exists for. It is the Linear-native landing spot for the B188 stack (Intercom / Zendesk / Canny / Chatwoot all feed it) — turn on when OJay Floyd or MyBodyGraph has real users. |
| **Pulse** | **ON (2026-09-26)** — personal schedule **Weekly** (workspace default Daily; the personal setting is the one that applies) | A workspace feed of **project and initiative updates** with optional daily/weekly morning summaries. Turned on now (three reversible toggles — no reason to wait); the feed fills once B119.1's OKR check-ins are posted. Complements U14's n8n digest: Pulse = goal/update progress, the n8n digest = issue movement (opened/closed, High lane, stale). Chain: Initiatives → Update schedule (reminders) → Pulse (summary) → B184 Slack channel (distribution). |
| **Team settings → General: Estimates + Create issues by email** | **Estimates: Not in use (by choice). Email intake: ON — address treated as a secret** (2026-09-26) | Estimates stay off (owner's choice; B190 must not penalize their absence). The team intake address is on and is B184's alert → Linear fallback. It is a capability URL — anyone holding it can file issues unauthenticated — so it is kept out of this public repo, stored in a SealedSecret if Alertmanager ever uses it, and rotated if it leaks. |
| **Team settings → AI & Agents** (5 settings) | **Team agent guidance: ADOPT (point it at `AGENTS.md`). Agent skills: none. Agent connectors: DON'T. Project updates (auto-generate): TRY on Weyland Lab. Resolved thread summaries: N/A on Free** (2026-09-26) | The workspace is on the **Free plan** (`organization.subscription` is null). **Guidance** = markdown instructions every installed agent receives on EMA issues (team guidance outranks workspace guidance), and it costs nothing. It is the Linear-side copy of the `AGENTS.md` rules that apply to issue work: implementation-ready sections, a B-number per issue, the priority field and no estimates, no secrets in issues. Keep it a short pointer to `AGENTS.md` so the two can't drift. **Agent skills** are saved Linear Agent chat workflows; zero exist, and the lab's skill source of truth is Bifrost (B175), so a second copy would only drift. **Agent connectors** are MCP servers for Linear Agent and Loops. Linear's cloud can't reach the LAN MCP fleet (Bifrost), and Loops are paid, so the same LAN/$0 rejection applies as for the Agent row. **Project updates** has the agent draft updates from recent activity plus the rules you define. It is not billed against AI credits (only coding sessions and Loops are), and it feeds Pulse and the Update schedule. Try it on Weyland Lab and keep it only if the drafts beat writing by hand. It overlaps the U14 n8n weekly digest: Linear writes the update, n8n distributes it. **Update rules (set in the UI, 2026-09-26):** a weekly, factual update with five sections in order: Shipped (grouped by Kind, with B-numbers), Decisions (closed Spikes and their verdicts), Blocked/parked (`parked:held`), Next (Priority, then backlog order), Risks. Cite issues by EMA id and B-number, never estimates, never secrets. **Health definitions:** *On track* = work completed this week and no blocker older than 7 days. *At risk* = a HIGH issue blocked or `parked:held`, or CI red for more than a day. *Off track* = a HIGH item has slipped two updates in a row, or a required system is down. **Resolved thread summaries** is a Business/Enterprise feature; comment threads are few on a solo board anyway. |
| **Workspace settings → AI & Agents + MCP connectors** | **Share usage data: OFF. Linear Agent web search: ON. Linear Agent MCP connectors: OFF** (2026-09-26) | **Usage data** is feedback on AI results, sent to Linear and not used for training. It does nothing for the lab, so leave the privacy default. **Web search** lets the Linear Agent cite current sources, which is the kind of lookup a Spike needs. Chat is included in the plan (not AI credits), and issues carry no secrets by rule (`AGENTS.md`), so a query leaks nothing sensitive. **MCP connectors** is the workspace switch behind the team-level "Agent connectors" verdict above: Linear's cloud can't reach the LAN MCP fleet (Bifrost), and deep agent work runs in the local harnesses. Leave it off. |
