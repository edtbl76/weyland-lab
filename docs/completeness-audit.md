...
- **[gitops] tool-server MCP image (`weyland-tool-server:local`) not reconstructible** — `imagePullPolicy: Never`, hand-built → ErrImageNeverPull on rebuild → `/mcp`+`/mcp-act` never start — Fix: local registry / Woodpecker pipeline / committed bootstrap.
...
- **[trigger] Woodpecker has no real build/test/deploy pipeline and no cron** — Fix: Woodpecker cron trigger, or land B57 registry pipelines; document no auto-trigger today.
