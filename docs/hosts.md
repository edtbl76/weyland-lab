...
| **mother** | VM (`vm-101`) on weyland | `192.168.1.243` ✅ | `emangini@mother` | k3s AI platform (tool-server, Postgres/pgvector, Qdrant, Weaviate, Neo4j, Dagster, n8n, Grafana, MinIO, CoreDNS, Traefik, **LiteLLM model gateway** — NodePort 30400, **Istio service mesh** (istiod + Kiali + Jaeger; B8 slice — tool-server + 4 backends meshed, PERMISSIVE mTLS)) |
