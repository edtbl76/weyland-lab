...
|---|---|---|
| weyland-tool-server (v0.7.0) | `mother:30080` | **3Gi limit** (raised from 2Gi 2026-07-21 — OOMKilled under sustained `/context/ask` load at retrieval depth 8; a **503** from this service means the pod went away, check `lastState.terminated.reason`). RAG retrieval (4 backends) + `/context/ask` (RAG gen) + `/evals/*` + `/pipeline/trigger` + health, **+ `/mcp` system-view MCP server** (read-only tools, **FastMCP** Streamable HTTP — migrated off `fastapi-mcp` 2026-08-01 / image v14), **+ B14 guards via the shared `weyland-guard` service** (B70 Part 2 — `/context/*` POST INPUT/OUTPUT hooks to `weyland-guard`, **fail-open**; guard models no longer in-process). **B100: MLflow Traces on `/context/ask`** (`tool-server-rag` experiment, manual spans — the raw-httpx→Ollama gen isn't autolog-able) **+ the RAG system prompt fetched live from the MLflow Prompt Registry** (`rag_system`, TTL-cached, fail-safe to a baked default). Consumers: **Claude Code** (rogueone, validated 2026-06-14) + the **B66 operator** (`weyland-operator`, live 2026-07-24; Hermes retired 2026-07-23). The platform's HTTP boundary. |
...
| **JupyterHub** (B1.8 L8, 2026-07-12) | `jupyter.weyland.lab` (ns `jupyterhub`) | **Data-science notebook layer** — completes B1.8 (Feast + Ray were the ML tiers; this is the interactive one). Zero-to-JupyterHub; hub+proxy tiny always-on + **KubeSpawner singleuser pods on-demand** (1h idle-cull → scale-to-zero). Keycloak OIDC (`GenericOAuthenticator`; mkcert-CA back-channel or the token exchange 500s). Custom **`registry.weyland.lab/weyland-jupyter`** image (polars/s3fs/pylance/duckdb) — **registry-served, `pullPolicy: IfNotPresent`** (was `:local`/`ctr import`/`Never`, which the weekly image-prune deleted → `ErrImageNeverPull`; registry+IfNotPresent is prune-safe); singleuser pod **mesh-joined** (istio sidecar via `extraLabels`) so it reaches the meshed stores (GizmoSQL/MySQL/Cube) over mTLS, and NetworkPolicy `egressAllowRules.privateIPs:true` for the unmeshed mesh + public egress (GitHub git-sync). **Notebook library = B81, DONE 2026-09-02: 26 notebooks** (formats→storage→query→vector/graph→transform/semantic→feature/ML→AI/RAG→governance→streaming), git-synced into `~/notebooks` on spawn, each validated end-to-end in-pod. Needed the node **max-pods 110→250** bump. `k8s/jupyterhub/`, [runbooks/jupyterhub.md](runbooks/jupyterhub.md). |
...
| Data | **JupyterHub notebook library** — spawn -> Keycloak OIDC -> KubeSpawner (mesh-joined) -> git-sync -> query the live mesh (B81) | [flow-jupyter-notebook-library.md](diagrams/flow-jupyter-notebook-library.md) |
