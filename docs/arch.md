...
- **CockroachDB (B65 Tier-2 #8)** — the grid's **distributed-SQL** cell (`brfss` + `nhis`, geo-partitioned in
  ...
  but NOT pg-*dialect* — SQLAlchemy's postgres dialect (and DataHub's native postgres source) AssertionError on
  its version string, so the loader uses `cockroachdb://` and cataloging uses a raw-psycopg2 `emit_cockroach`. See
  [runbooks/datasets-hydration.md](runbooks/datasets-hydration.md).
