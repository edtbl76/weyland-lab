...
| MLflow (B10+B16) | `mlflow.weyland.lab` | Experiment tracking + model registry. **Postgres** backend store + **MinIO** `mlflow` artifact bucket (proxied via `--serve-artifacts`). Meshed (STRICT Postgres); **Keycloak SSO** (forward-auth, B1.1). `k8s/mlflow/`. **Two-plane note:** big models must upload artifacts **direct to MinIO** (experiment `artifact_location=s3://…`) — the serve-artifacts proxy times a multi-GB `model.pkl` out through the 1Gi pod. See remote-training below. |
| **Container registry** (2026-07-06) | `registry.weyland.lab` | **MinIO-backed OCI registry** (`distribution/registry`, blobs in MinIO `registry` bucket → stateless, no PVC). **No auth** (LAN-only). The platform's private Docker registry (reusable; the in-cluster registry B57 wanted). Docker API only — no web UI. `k8s/registry/`. |
| **Remote training** (2026-07-06) | rogueone (compute) → `registry.weyland.lab` → `mlflow.weyland.lab` | Heavy model training runs **on rogueone** (RAM + RTX 5000 Ada), not the k3s box. A training job is a **self-contained container** in the registry: given only a mounted kubeconfig it self-fetches creds from k8s Secrets + opens its own port-forwards (`kubectl`), reads lakeFS silver, trains, and logs to MLflow with the **artifact going direct to MinIO**. First consumer: `genre-trainer` (genre classifier, `genre_classifier` v1 single / **v2 Ray-Tune-best** f1 0.312). **`--tune`** runs a **Ray Tune** sweep on a local Ray cluster (dashboard `:8265`, loopback-only — unauth Jobs API = RCE). The pragmatic single-container form of B1.8's deferred JupyterHub/Ray tier; **persistent Ray (head on mother + rogueone worker) = the KubeRay follow-up.** **Gotcha:** Docker Desktop caps container RAM at the VM's allocation (hides rogueone's 128 GB). [runbooks/remote-training.md](runbooks/remote-training.md), [runbooks/mlflow-training.md](runbooks/mlflow-training.md). `services/genre-trainer/`. |
