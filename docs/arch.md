...
| weyland-guard | `weyland-guard.weyland.svc:8080` (ClusterIP) | **B70 Part 1** — shared B14 guard service extracted from the tool-server. 3 typed routes `/guard/{input,output,act}`; **SHADOW** default (record-only), **fail-open** callers; 3 baked models (injection + toxicity + `nli-deberta-v3-small` ≈1.5 Gi) load **once** here for the tool-server + the coming `weyland-agent` + future B66. **B115: now also runs the `llama_guard.safety` Classify validator** (Llama Guard content-safety) on **INPUT+OUTPUT** (shadow, fail-open) — not baked; it POSTs to the external `llama-guard` svc via `LLAMA_GUARD_URL` (below). Verdicts → own `/metrics` + `guardrail_verdicts`. The first clean seam of the tool-server decomposition (→ B31). [runbooks/guardrails.md](runbooks/guardrails.md). |
| llama-guard (Classify classifier) | `llama-guard.weyland.svc:8080` (ClusterIP, no ingress) | **B115 Classify layer** of the guardrails platform (Scan → **Classify** → Dialog → Structure) — Meta's **Llama Guard** run as a model-based content-safety classifier over its full safety taxonomy (S1 violent crime, S9 weapons, S11 self-harm, …), a SECOND opinion alongside the single-purpose llm_guard scanners. Called by `weyland-guard` as `llama_guard.safety`. **Tier 1** (always-on) = **Llama-Guard-3-1B on CPU (mother)**, served by **llama.cpp** (`ghcr.io/ggml-org/llama.cpp:server`, OpenAI-compat, model `QuantFactory/Llama-Guard-3-1B-GGUF:Q8_0`, temp 0 — the GGUF embeds the safety-taxonomy chat template). **UNMESHED** (keeps the on-boot HF model pull off the mesh); Argo-managed, `k8s/llama-guard/`. **Tier 2** (on-demand escalation) = **Llama-Guard-3-8B on the rogueone GPU** (llama.cpp `server-cuda`, `:8003`, `scripts/llama-guard-8b.sh`) — NOT wired into Bifrost; reached by repointing `LLAMA_GUARD_URL` to `http://192.168.1.230:8003`, no rebuild. $0, no GPU for tier 1. [runbooks/guardrails.md](runbooks/guardrails.md). |
...
- **Code quality & security scanning (B43 → B69/B89/B90):** two weekly k8s CronJobs (ns `weyland`) scan the repo —
  **`code-scan-suite`** runs **10 checks** in one image (gitleaks, secret-files, checkov, kubescape, hadolint, bandit, osv-scanner,
  shellcheck, semgrep, trivy) + **code-maat** change-hotspots; **`sonar-scan`** runs SonarQube. All self-hosted OSS
