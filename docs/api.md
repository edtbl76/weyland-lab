...
| MySQL (health/wellness) | `mysql.data-mesh.svc:3306` (in-cluster); IntelliJ via k8s port-forward | **B65 Tier-2 #5** — health datasets (current 8-dataset roster: NHANES, Big Five, WHO GHO, CDC Physical Activity, BRFSS, NHIS, USDA FoodData, Open Food Facts — NHIS replaced UK Biobank; MyFitnessPal dropped); user `weyland` / dev password. **Dataset load = pending (data-store-mageddon);** the lake (lakeFS+Iceberg) is the current source of truth. |
