...
| **vLLM** (LLM, GPU — **B111 bench**) | rogueone | `http://192.168.1.230:8001/v1` (Bifrost `vllm`) | ✅ (on-demand) | On-demand throughput/continuous-batching bench — `Qwen2.5-7B-Instruct-AWQ`; **native Docker engine only** (`DOCKER_HOST=…docker.sock`) + VRAM-capped. `scripts/vllm-bench.sh`. See [runbooks/gpu-inference.md](runbooks/gpu-inference.md) + [demos/gpu-inference.md](demos/gpu-inference.md). |
| **SGLang** (LLM, GPU — **B111 bench**) | rogueone | `http://192.168.1.230:8002/v1` (Bifrost `sgl`) | ✅ (on-demand) | On-demand **prefix-caching** bench — `Llama-3.2-1B`, **RadixAttention** (~6.2× faster TTFT on cache hits) for agent/RAG. Native Docker engine only; `--mem-fraction-static` goes HIGHER not lower. `scripts/sglang-bench.sh`. See [runbooks/gpu-inference.md](runbooks/gpu-inference.md). |
| **rag-embed** (embeddings, GPU) | rogueone (GPU) | `http://192.168.1.230:8900` | ✗ | **B-RAG-STREAM** warm embedding service - native systemd unit `rag-embed.service` holding `bge-base-en-v1.5` (768-dim, B74) resident on the RTX 5000 Ada. `GET /health` → `{status,model,dim,device}`; `POST /embed {"texts":[str,...]}` → `{"vectors":[[float,...],...],"dim":768,"model":"BAAI/bge-base-en-v1.5"}` (L2-normalized, cosine-ready). Sole client = the Dagster `rag_stream_produce` producer. LAN-only. See [diagrams/flow-rag-stream.md](diagrams/flow-rag-stream.md), [demos/rag-stream.md](demos/rag-stream.md). |
| **LiteLLM gateway** (hosted models) | mother (NodePort 30400) | `http://mother:30400/v1` (`192.168.1.243`) | ✅ | Fronts the hosted-model fleet **and is the use-case ROUTER (B111)**: 9 `wl-*` aliases (`wl-coding`/`wl-agentic`/`wl-rag`/`wl-search`/`wl-reason`/`wl-judge`/`wl-default`/`wl-speed`/`wl-big-oss`), each a primary + server-side **fallback chain** (network/5xx/429/timeout). Providers via `envFrom bifrost-provider-keys` (groq/anthropic/gemini/openrouter/perplexity/opencode-zen/deepseek/cerebras/openai/xai + ollama-local). Wildcards `gemini/*`·`openrouter/*`·`anthropic/*` + aliases `gemini-flash`/`gemini-pro`/`claude-haiku`. Bearer = `LITELLM_MASTER_KEY`. Egress valve + spend alerts. **B17 (2026-08-01): the hosted lanes now egress THROUGH Bifrost** (LiteLLM→Bifrost→provider — `api_base` at the in-cluster Bifrost `/v1` + the `realm-llm` VK, model ids are Bifrost's e.g. `anthropic/claude-haiku-4-5`) so provider cost/tokens/latency + per-VK attribution land in Bifrost's Observability — no LiteLLM DB (it's master-key-only, can't do VKs). Local ollama lanes (`wl-rag`/`wl-reason`/`wl-judge`) stay direct. Map [llm-routing-map.html](llm-routing-map.html); runbook [model-gateway.md](runbooks/model-gateway.md). |
...
| `/mcp` | MCP | **B2 system-view MCP server** (Streamable HTTP via **FastMCP** — migrated off `fastapi-mcp` 2026-08-01 / image **v14** [B111 "Option C"]; fastapi-mcp 0.4.0 hung under starlette 1.x, `await request.body()` → ClientDisconnect) — read-only tools: `status`, `context_search`, `context_ask`, `list_models`. Consumers: the **weyland-mcp-compositor** `context` upstream (→ Bifrost fleet, 95 tools) + **Claude Code** (`http://192.168.1.243:30080/mcp`) + the **B66 operator**. (Hermes retired 2026-07-23.) |
...
(the 95 fleet tools) and re-exposes them through **one `/mcp` endpoint**, so a coding agent (Cline / Cursor / Claude Code)
points a single URL at the whole read-only lab. Verified 2026-07-30: Bifrost `/mcp` returns all **95/95** tools end-to-end.
...
See [bifrost-provider-loadout.md](design/bifrost-provider-loadout.md).
...
| ClickHouse (columnar OLAP) | `clickhouse.data-mesh.svc:8123` HTTP / `:9000` native (in-cluster) · `192.168.1.243:30123` (NodePort `clickhouse-lan`, HTTP) | **B65 Tier-2** — columnar OLAP over silver; user `default` / dev password; UNMESHED. Native client `clickhouse-connect` (HTTP 8123). See [[clickhouse-tier2-hydration]]. |
| Cassandra (wide-column) | `cassandra.data-mesh.svc:9042` CQL (in-cluster) · `192.168.1.243:30942` (NodePort `cassandra-lan`) | **B65 Tier-2** — wide-column store; **AllowAllAuthenticator (no auth)**; UNMESHED. Client `cassandra-driver`. Empty partition keys map to `__UNKNOWN__`; table names are long. See [[cassandra-tier2-hydration]]. |
| MongoDB (document) | `mongodb.data-mesh.svc:27017` (in-cluster) · `192.168.1.243:30017` (NodePort `mongodb-lan`) | **B65 Tier-2** — document store; user `weyland` / dev password; UNMESHED. Client `pymongo`. |
| CockroachDB (distributed SQL) | `cockroachdb.data-mesh.svc:26257` pg-wire (in-cluster) · `192.168.1.243:30257` (NodePort `cockroachdb-lan`); Admin UI `cockroachdb.weyland.lab` | **B65 Tier-2** — distributed SQL, **insecure dev mode** (user `root`, no password, `sslmode=disable`); UNMESHED. pg-WIRE, NOT a Postgres dialect — see [[cockroachdb-pg-wire-not-dialect]]. Client `psycopg`. |
| TimescaleDB (time-series) | `timescaledb.data-mesh.svc:5432` (in-cluster) · `192.168.1.243:30543` (NodePort `timescaledb-lan`, DataGrip/clients) | **B65 Tier-2 #4** — Postgres extension for time-series; 5 hypertables (eval_scores_ts, guardrail_verdicts_ts, dagster_run_durations, unleash_feature_metrics, datahub_ingestion_runs); db `timeseries`, user `weyland` / dev password. See [runbooks/timescaledb.md](runbooks/timescaledb.md). |
| MySQL (health/wellness) | `mysql.data-mesh.svc:3306` (in-cluster) · `192.168.1.243:30306` (NodePort `mysql-lan`, DataGrip/clients) | **B65 Tier-2 #5** — health datasets, hydrated from silver Parquet (data-store-mageddon, 2026-07-01). 6 databases = the grid `MySQL=Y` set: NHANES, Big Five, WHO GHO, CDC Physical Activity, BRFSS, NHIS (USDA + Open Food Facts are `MySQL=N`; NHIS replaced UK Biobank). 32 tables (dataset→db, parquet file→table). user `weyland` / dev password. See [runbooks/datasets-hydration.md](runbooks/datasets-hydration.md). |
...
| guardrails-structure (Structure validator) | `guardrails-structure.weyland.svc:8080` (ClusterIP, no ingress) | **B115 Structure layer** — **Guardrails AI** output-schema validation + re-ask, run as a **standalone meshed service** (image `registry.weyland.lab/guardrails-structure:v1`, `k8s/guardrails-structure/`) because guardrails-ai pins `click<=8.2.0`, which can't co-install with the Dagster/dbt/huggingface stack (`click>=8.4.2`) that produces the output. **`POST /structure/judge-scores`** `{raw, model, api_base?, num_reasks?}` validates the judge's raw output against a Pydantic `JudgeScores` (3 floats in [0,1]: faithfulness/answer_relevancy/context_relevancy) and, on a schema miss, **re-asks the same judge model** (litellm→Ollama) to repair → `{scores, source∈guarded\|reasked\|failed, validation_passed}`; `/health` `/ready`. Called by the Dagster RAG eval judge (`eval_scores._judge`, fail-safe to a best-effort parse if down). Argo-managed. See [runbooks/guardrails.md](runbooks/guardrails.md). |
| nemo-guardrails (Dialog layer) | `nemo-guardrails.weyland.svc:8080` (ClusterIP, no ingress, meshed) | **B115 Dialog layer** — **NeMo Guardrails** conversational/topical control, run as a **standalone service** (image `registry.weyland.lab/nemo-guardrails:v3`, `k8s/nemo-guardrails/`, Argo) because NeMo's dep tree (langchain, fastembed) is too heavy to embed. A **FastAPI wraps the NeMo library** (`LLMRails.generate`) behind a plain OpenAI **`POST /v1/chat/completions`** (+ `GET /v1/models`, `/health`, `/ready`) — NeMo's native server wants `config_id` nested under a `guardrails` object a vanilla OpenAI client won't send. **Consumed by Open WebUI** (`chat.weyland.lab`) as an OpenAI connection → the guarded **`weyland-operator`** chat model appears alongside the raw (unguarded) Ollama models. Rails = input safety + **topical control via a strengthened `self check input` LLM-judge rail** (blocks off-topic + jailbreak with a custom operator refusal; the Colang dialog rail wouldn't fire). See [runbooks/guardrails.md](runbooks/guardrails.md). |
...
**STUD.io masterdb backup** (production durability): nightly `pg_dump -Fc masterdb` (rogueone `studio_db`) → `wl/studio-backups/masterdb/` via the `studio-masterdb-backup.timer` system unit (daily + `Persistent`; script `~/Documents/Studio/STUD.io/scripts/backup-masterdb.sh`). 180-day retention (MinIO ILM). masterdb stays local (hot-path/CI-heavy); this is its off-laptop copy → chain rogueone → weyland MinIO → root-disk mirror. Full plan: `design/b127-desktop-to-native-migration.md`.
...
| **Woodpecker CI** (CI/CD — GitHub OAuth login; **mixed fleet**: `kubernetes` backend for weyland jobs + `local`-backend agents on rogueone for STUD.io CI, B57b; manual/cron triggers) — **Keycloak forward-auth** on the UI + **two LAN NodePorts** (`192.168.1.243:30980` HTTP = `woodpecker-cli`/REST, bypasses forward-auth which 302s Bearer calls to login; `192.168.1.243:30900` gRPC `woodpecker-grpc-lan` = off-cluster local agents register). CLI: `woodpecker-cli pipeline create edtbl76/stud.io --branch main`. | `https://woodpecker.weyland.lab` · `http://192.168.1.243:30980` (CLI/REST) · `192.168.1.243:30900` (agent gRPC) |
| **Argo CD** (GitOps CD — local admin; app-of-apps, **81 apps** as of 2026-08-21) — **Keycloak forward-auth** on the UI + a **LAN NodePort** (`mother:30880` HTTP = the `argocd` CLI, bypasses forward-auth, which answers a CLI's `POST /session.SessionService/Create` with **400** because it cannot browser-SSO; `k8s/argocd/argocd-lan.yaml`, hand-applied — Argo does not manage itself). Login: `argocd login mother:30880 --insecure --grpc-web`. Consumed by `scripts/ship-images.sh` for targeted `argocd app sync`, never `--all` (B134). | `https://argocd.weyland.lab` · `http://mother:30880` (CLI) |
...
| **Port.io** (IDP — SaaS; **launcher/catalog**, not status board) | `https://app.port.io` — EU org; **Launcher** dashboard (`endpoint` bp). Integrations: K8s, Istio, GitHub, Linear, Unleash (`feature_flag`), code-quality suite (weekly 21-tool `code-scan-suite` + `sonar-scan` → `code_quality` / `security_scan` / `code_hotspot`; Code Health dashboard) |
| **MLflow** (experiment tracking + model registry; B10+B16, **v3.14 — B47**) — **Keycloak SSO** (forward-auth) + **LAN NodePort** (`mlflow-lan`, unauth, `externalTrafficPolicy: Local`, iptables-pinned to rogueone — lets the external Ray worker log runs + register models; artifacts two-plane → direct to MinIO) | `https://mlflow.weyland.lab` · `http://192.168.1.243:30500` |
| **Container registry** (MinIO-backed OCI registry; remote-training capability) — **no auth** (LAN-only) | `https://registry.weyland.lab` — Docker API (browse `/v2/_catalog`; **web UI → `registry-ui.weyland.lab`**). Blobs in MinIO `registry` bucket. See [runbooks/remote-training.md](runbooks/remote-training.md) |
| **Registry UI** (joxit/docker-registry-ui — browse images/tags; nginx-proxy → the in-cluster registry svc, no CORS) — **Keycloak SSO** (forward-auth) | `https://registry-ui.weyland.lab` — `k8s/registry/registry-ui.yaml` |
| **Feast UI** (feast registry browser — feature views / entities / services + SavedDatasets, read-only) — **Keycloak SSO** (forward-auth) | `https://feast-ui.weyland.lab` — `feast ui` + a registry.json-dump launcher (works around feast 0.58's empty-UI bug); `k8s/data-mesh/feast-ui.yaml`. See [diagrams/flow-feast.md](diagrams/flow-feast.md) |
| **Ray head** (persistent Ray cluster — dashboard + Jobs API; submit training / HP-sweep jobs) — **Keycloak SSO** (forward-auth) **+ Ray token auth (B47)** | `https://ray.weyland.lab` — GCS `mother:6379` (edge workers dial this); in-cluster Jobs API `ray-head.weyland.svc:8265`. rogueone joins as a native worker (Python 3.11.14 exact match). **Token auth on (Ray 2.56):** paste the token at the dashboard prompt (30-day cookie); the Jobs API requires it (401 without) — `kubectl -n weyland get secret ray-auth -o jsonpath='{.data.token}' \| base64 -d`. **Metrics** → Prometheus (head + worker `:8080`), Grafana `Dashboards → Ray`. See [runbooks/remote-training.md](runbooks/remote-training.md) |
| **Uptime Kuma** (live status board — own auth; **37 monitors**, Telegram paging; Port webhook retired) | `https://kuma.weyland.lab` |
...
| **Unleash** (feature flags; OSS self-hosted; → Port `feature_flag` webhook) — **Keycloak SSO** (forward-auth; own login behind = double) | `https://unleash.weyland.lab` — Python SDK for the tool-server; see [runbooks/unleash.md](runbooks/unleash.md) |
| **SonarQube** (code quality / static analysis; → Port `code_quality` webhook) — **Keycloak SSO** (forward-auth; own login behind = double; native OIDC possible later) | `https://sonarqube.weyland.lab` — meshed Postgres backend; fed by the weekly `sonar-scan` CronJob (part of the 21-tool scan-suite → Port). See [runbooks/code-quality.md](runbooks/code-quality.md) |
| **Sourcebot** (B166 — code search/nav: Zoekt-backed search + web UI + MCP + NL Q&A for agents; the $0 Sourcegraph replacement) — **Keycloak forward-auth** (own first-user-is-owner login behind) | `https://sourcebot.weyland.lab` — reuses `weyland-postgres` (`sourcebot` DB) + `valkey`; Sourcebot indexes continuously. See [runbooks/code-intelligence.md](runbooks/code-intelligence.md) |
| **Zoekt** (B166 — standalone trigram code-search engine; the lightweight programmatic search endpoint for scripts/agents) — **NO forward-auth** (by design — the open JSON API) | `https://zoekt.weyland.lab` — search API `GET /search?q=<term>&num=N&format=json`; daily 03:35 NY index cron of `weyland-lab`. See [runbooks/code-intelligence.md](runbooks/code-intelligence.md) |
...
| **lakeFS** (data-mesh **B1.2** — file/dataset versioning) — **Keycloak SSO** (forward-auth; own access-key auth behind) | `https://lakefs.weyland.lab` — Programmatic: `lakefs.data-mesh.svc.cluster.local:8000` (in-cluster, no gate — forward-auth is browser-only, so CLI/pipelines use the svc directly) + **LAN NodePort** `mother:30800` (`lakefs-lan`) |
| **DataHub** (data catalog / discovery / lineage / governance UI — B1) — **Keycloak forward-auth** | `https://datahub.weyland.lab` — GMS `datahub-datahub-gms.data-mesh.svc:8080` (in-cluster; the fleet datahub-mcp + coverage guards read it) |
| **OpenSearch Dashboards** (RAG `opensearch` backend browser — one of the 5 RAG index stores) — **Keycloak forward-auth** | `https://opensearch.weyland.lab` — `k8s/opensearch/dashboards-ingress.yaml` |
