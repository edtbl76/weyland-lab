...
| MySQL (health/wellness) | `mysql.data-mesh.svc:3306` (in-cluster); IntelliJ via k8s port-forward | **B65 Tier-2 #5** — health, wellness, and personality profiling datasets (NHANES, Big Five, WHO GHO, CDC Physical Activity, BRFSS, MyFitnessPal, UK Biobank, USDA FoodData, Open Food Facts); 9 databases; user `weyland` / dev password. |
