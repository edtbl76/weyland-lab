...
| **Port.io** (IDP — SaaS; **launcher/catalog**, not status board) | `https://app.port.io` — EU org; **Launcher** dashboard (`endpoint` bp). Integrations: K8s, Istio, GitHub, Linear, Unleash (`feature_flag`), code-quality suite (weekly 9-tool `code-scan-suite` + `sonar-scan` → `code_quality` / `security_scan` / `code_hotspot`; Code Health dashboard) |
...
| **SonarQube** (code quality / static analysis; → Port `code_quality` webhook) — **Keycloak SSO** (forward-auth; own login behind = double; native OIDC possible later) | `https://sonarqube.weyland.lab` — meshed Postgres backend; fed by the weekly `sonar-scan` CronJob (part of the 9-tool scan-suite → Port). See [runbooks/code-quality.md](runbooks/code-quality.md) |
