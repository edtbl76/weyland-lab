...
| **vLLM** (LLM, GPU — **B111 bench**) | rogueone | `http://192.168.1.230:8001/v1` (Bifrost `vllm`) | ✅ (on-demand) | On-demand throughput/continuous-batching bench — `Qwen2.5-7B-Instruct-AWQ`; **native Docker engine only** (`DOCKER_HOST=…docker.sock`) + VRAM-capped. Spin up/down via `nodes/rogueone/services/gpu-inference/`. See [runbooks/gpu-inference.md](runbooks/gpu-inference.md) + [demos/gpu-inference.md](demos/gpu-inference.md). |
