...
| Lightdash (dbt-native BI) | `https://lightdash.weyland.lab` (own login) — **first cut, deploy pending** | dbt-native BI: metrics/explores built from the dbt project over the marts. See [runbooks/lightdash.md](runbooks/lightdash.md). |
...
| **Ray head** (persistent Ray cluster — dashboard + Jobs API; submit training / HP-sweep jobs) — **Keycloak SSO** (forward-auth) | `https://ray.weyland.lab` — GCS `mother:6379` (edge workers dial this); in-cluster Jobs API `ray-head.weyland.svc:8265`. rogueone joins as a native worker. **Metrics** → Prometheus (head + worker `:8080`), Grafana dashboards under `Dashboards → Ray`. See [runbooks/remote-training.md](runbooks/remote-training.md) |
