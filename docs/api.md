...
| **LiteLLM gateway** (hosted models) | mother (NodePort 30400) | `http://mother:30400/v1` (`192.168.1.243`) | ✅ | Fronts the hosted-model fleet **and is the use-case ROUTER (B111)**: 9 `wl-*` aliases (`wl-coding`/`wl-agentic`/`wl-rag`/`wl-search`/`wl-reason`/`wl-judge`/`wl-default`/`wl-speed`/`wl-big-oss`), each a primary + server-side **fallback chain** (network/5xx/429/timeout). Providers via `envFrom bifrost-provider-keys` (groq/anthropic/gemini/openrouter/perplexity/opencode-zen/deepseek/cerebras/openai/xai + ollama-local). Wildcards `gemini/*`·`openrouter/*`·`anthropic/*` + aliases `gemini-flash`/`gemini-pro`/`claude-haiku`. Bearer = `LITELLM_MASTER_KEY`. Egress valve + spend alerts. Map [llm-routing-map.html](llm-routing-map.html); runbook [model-gateway.md](runbooks/model-gateway.md). |
