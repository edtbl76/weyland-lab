...
| **02:50** | k8s CronJob | `dashboard-coverage` (observability — the sibling of `servicemonitor-coverage`: that guard proves a service is SCRAPED, this one proves what is scraped is also VISUALIZED. Reconciles every Prometheus scrape job against the Grafana dashboards — a job is covered when a dashboard scopes a panel to it by `job=` OR charts a metric **distinctive** to it (exported by ≤5 jobs, so `up`/`process_*` shared metrics confer nothing). **Exists because dashboards had only a one-time audit (B109) while ServiceMonitors had a standing guard (B148)** — a new metrics-exposing service could ship with no board and every check stayed green; found 2026-09-01 that minio/lakefs/flink/apisix/pve + the agent services + loki/tempo/alloy were scraped-but-unvisualized, and that the `bifrost` board charted non-existent metrics. Runs **unmeshed** (Prometheus ClusterIP + API server, no sidecar). Read-only SA scoped to `list configmaps` in `monitoring` (all 34 dashboard ConfigMaps live there); Prometheus reached via ClusterIP (no RBAC). Logic is the byte-identical `scripts/check-dashboard-coverage.sh` embedded in the `dashboard-coverage-logic` ConfigMap, a bats case asserting no drift; `scripts/tests/dashboard-coverage.bats` (11 cases) runs in CI. `spec.timeZone: America/New_York`. | daily | **light** — two Prometheus GETs + one API GET, ~2s; 64Mi/25m requests. Slotted at 02:50 adjacent to its sibling `servicemonitor-coverage` (02:45, ~2s, finishes before this starts), 10m before the 03:00 DataHub Neo4j scan and 33m after the 02:17 HEAVY ingestion — inside the Design Rule #5 pre-dawn window. |
| **02:55** | k8s CronJob | `alert-coverage` (observability — the THIRD coverage guard: `servicemonitor-coverage` proves a service is SCRAPED, `dashboard-coverage` proves it's VISUALIZED, this proves a down/absence is ALERTED. A job is covered by a down/absent alert scoped to it (`up{job="J"}==0` / `absent(...)`, e.g. `LancedbExporterDown`, `KubeletDown`) OR by the **BLANKET net** — kube-prometheus-stack's `TargetDown` (`count(up==0) BY (job) …`) fires for any job going down, so one rule protects all 38. **The drift it catches is the blanket net being removed** — a values change disabling default rules, a bad PrometheusRule edit — after which every service silently loses down-alerting at once with no positive signal. Deliberately does NOT demand a dedicated Down alert per service (that would flag ~32 of 38 and add ~30 alerts redundant with TargetDown). Read-only SA scoped to `list prometheusrules` cluster-wide (rules span monitoring/weyland/data-mesh/…); Prometheus via ClusterIP. Byte-identical `scripts/check-alert-coverage.sh` in the `alert-coverage-logic` ConfigMap + a bats drift case; `scripts/tests/alert-coverage.bats` (10 cases) runs in CI. `spec.timeZone: America/New_York`. | daily | **light** — one Prometheus GET + one API GET, ~2s; 64Mi/25m requests. Slotted at 02:55 after its two siblings (02:45/02:50), 5m before the 03:00 DataHub Neo4j scan — inside the Design Rule #5 pre-dawn window. |
