...
|---|---|---|---|---|
| ~~02:00~~ | **PARKED** | `data-mesh-scaledown` — **no CronJob manifest exists**; auto scale-down is parked (Argo selfHeal reverts `replicas:0` within ~3 min), so store sleep is the manual store-scaler easy-button, not a timer | — | n/a (parked) |
...
| 04:50 | Dagster | `datasets_finance_land` | daily — **STOPPED** (B158 follow-up C — the code `default_status` is STOPPED and it stays off: finance is static snapshots, run on demand. Generated by `build_domain_jobs(FINANCE_CFG)`, not hand-authored.) | light — SEC/FRED/yfinance across ~50 companies, self-skips if fresh (30-day window). **STOPPED, so no live slot contention; if ever enabled, pick a clear slot first — the 04:00–06:00 pre-dawn window is dense.** |
...
| **03:05** | k8s CronJob | `datahub-coverage` (B158 follow-up A — the FOURTH coverage guard, the first on the DATA estate rather than infrastructure: `servicemonitor`/`dashboard`/`alert-coverage` prove a service is SCRAPED/VISUALIZED/ALERTED; this proves every mesh dataset is CATALOGED. Reconciles two planes — REALITY (the `iceberg.datasets_*.*` silver/gold + `iceberg.dbt.mart_*` tables Trino exposes) against CATALOGED (the dataset URNs DataHub GMS holds, via `scrollAcrossEntities`) — and a table in Trino but absent from DataHub is drift. **Exists because data-governance completeness had no positive signal**: nothing failed when a mesh dataset was never emitted, and the B158 audit even caught the `data-mesh-map` product tile drifting from the emit's real `_PRODUCTS` by hand. **Fails closed**: an empty catalogued set while Trino is non-empty is "GMS unreachable / token missing" → exit 2, never "all uncatalogued"; the GMS scroll also asserts it fetched all `total` datasets before grading. Runs **unmeshed** in the `weyland` ns (verified 2026-09-05: data-mesh has no PeerAuthentication and datahub-gms/trino/trino-noauth carry no sidecar) — needs the `DATAHUB_GMS_TOKEN` from the local `datahub-token` Secret, no kubectl/RBAC. Byte-identical `scripts/check-datahub-coverage.sh` embedded in the `datahub-coverage-logic` ConfigMap (`scripts/embed-datahub-coverage.sh` regenerates it) + a bats drift case; `scripts/tests/datahub-coverage.bats` (12 cases) runs in CI. `spec.timeZone: America/New_York`. | daily | **light** — one Trino REST query + a paged GMS GraphQL read + a set diff, ~5s; 64Mi/25m requests. Slotted at 03:05 just past the 03:00 DataHub Neo4j scan (its siblings sit at 02:45/02:50/02:55; 03:00 is taken), 10m before the 03:15 DataHub Postgres scan — inside the Design Rule #5 pre-dawn window. |
| **03:10** | k8s CronJob | `image-provenance` (B88 image-signature follow-up — the FIFTH coverage guard, and the live counterpart to the CI guard `scripts/check-image-provenance.sh`: it proves every DECLARED workload's image is from a REVIEWED source. The Gatekeeper `require-signed-images` constraint only AUDITS running pods in dryrun, so "0 violations" is transient — it never sees a non-running CronJob/scaled-to-zero store, nor an image baked in a Helm chart Argo renders. This enumerates every deployment/statefulset/daemonset/cronjob/job/pod cluster-wide and judges each image with the same allow/exempt/official rule as the constraint's Rego, read from the LIVE `K8sImageSignature` constraint (single source of truth). It is the invariant the eventual dryrun→deny flip rests on. **Fails closed**: the kubectl init-dump failing fails the Job; no images enumerated → exit 2; an unreviewed image → exit 1 (the failed Job raises `kube_job_status_failed` → `ScheduledJobFailed` → Telegram). Runs **unmeshed** in `monitoring` (only target is the API server); read-only cluster RBAC (workload controllers + the constraint). Byte-identical `scripts/check-image-provenance.sh` embedded in the `image-provenance-logic` ConfigMap (`scripts/embed-image-provenance.sh` regenerates it) + a bats drift case; `scripts/tests/image-provenance.bats` (10 cases) runs in CI. `spec.timeZone: America/New_York`. | daily | **light** — two kubectl dumps + a set walk, ~5s; 64Mi/25m requests. Slotted at 03:10 with the coverage-guard family (02:45/02:50/02:55/03:05), between the 03:05 `datahub-coverage` and the 03:15 DataHub Postgres scan — inside the Design Rule #5 pre-dawn window. |
| **03:20** | k8s CronJob | `api-drift` (B155 API lifecycle — the runtime half: fetches each catalogued API's LIVE spec (FastAPI `/openapi.json` on the `.svc` FQDNs + the realm A2A card) and diffs it against the committed contract snapshot (`docs/api/specs/`) with the breaking-change engine (`scripts/lib/api_spec_diff.py`). Catches a deployed API that drifted from its governed contract — a route removed, a field dropped, a new required field — without the `apis.yaml` catalog + snapshot + version being updated. PR-time governance is `check-api-lifecycle.sh` over `apis.yaml`. **Reachability-honest**: a scaled-to-zero/down source is SKIPPED (not drift); a breaking diff on a reachable API → exit 1 → `kube_job_status_failed` → `ScheduledJobFailed` → Telegram. Runs **unmeshed** in `monitoring`, **no kubectl/RBAC** (HTTP GETs only, like `datahub-coverage`). Logic ConfigMap (`api-drift-configmap.yaml`) is generated by `scripts/embed-api-drift.sh` (engine + runner + `apis.yaml` + snapshots, mounted flat at `/opt/api-drift`); `scripts/tests/api-drift.bats` (6 cases, incl. byte-identity) runs in CI. `spec.timeZone: America/New_York`. | daily | **light** — a handful of HTTP GETs + a spec diff, ~5s; 64Mi/25m requests. Slotted at 03:20 just past the 03:15 DataHub Postgres scan, extending the coverage-guard family's pre-dawn cluster — inside the Design Rule #5 window. |
| **03:35** | k8s CronJob | `zoekt-index` (B166 code-intelligence — re-indexes the public `weyland-lab` repo for **standalone Zoekt** (`zoekt.weyland.lab`, the un-authed JSON search endpoint for scripts/agents). Shallow-clones from GitHub egress + runs `zoekt-git-index` into the shared PVC the `zoekt-webserver` reads; NO Istio sidecar (touches no in-mesh service, so the Job terminates). **Daily, not `*/30`** — Design Rule #5 (no mid-day auto-runs); Sourcebot is the continuously-fresh primary via its own internal indexer (not a k8s CronJob, so unbound by Rule #5), Zoekt is the lightweight daily mirror. Immediate freshness after a push is a manual `kubectl -n weyland create job --from=cronjob/zoekt-index zoekt-index-now`. Covered by the daily `ScheduledJobStale` budget (26h) in `cron-freshness-rules.yaml`. `spec.timeZone: America/New_York`. | daily | **light** — a shallow clone + trigram index of a ~small repo, seconds; 256Mi/100m requests. Slotted at 03:35 after the 03:20 `api-drift` (the coverage-guard family ends there) and before the 04:00 `datasets_health_land` — 15 min clear either side, inside the Design Rule #5 pre-dawn window. |
| **04:30** | k8s CronJob | `cron-freshness-check` (B135 observability — asks the **Woodpecker API** whether the `nightly-images` cron is `enabled` and whether `next_exec` has gone stale, and POSTs a synthetic `ScheduledWorkNotRunning` alert to Alertmanager v2). **Exists because a Woodpecker cron is not a Kubernetes object**: no CronJob, no Job, no pod, so kube-state-metrics cannot see it and no `kube_cronjob_*` metric ever will — which is why `nightly-images` sat `enabled:false` for four days unnoticed. Decision logic lives in the `cron-freshness-logic` ConfigMap so `scripts/tests/cron-freshness.bats` runs the same text the cluster does. `spec.timeZone: America/New_York`. Sibling `PrometheusRule` `cron-freshness` (k8s/monitoring/cron-freshness-rules.yaml) covers the 18 **k8s** CronJobs with per-cadence freshness budgets. | daily | **light** — one Woodpecker API call + jq, ~2s; 32Mi/25m requests. Slotted at 04:30 because it is 3.5h after the 01:00 `nightly-images` cron (a genuine run has long finished) and clear of 05:30 `docs-site-rebuild` / 05:45 `pr-staleness-check` — stacking two watchdogs on one minute means one failure mode hides the other in the job log. |
...
| **09:00 (Sun)** | k8s CronJob | `code-scan-suite` (the `quality-tools.yaml` roster — 21 scan-suite tools → Port + code-maat hotspots; one `scan-suite` image) | weekly (Sun) | **HEAVY** — semgrep auto + trivy fs (13:00 UTC) |
| **10:00 (Sun)** | k8s CronJob | `sonar-scan-repos` (B138 — SonarQube source-level analysis for the 6 non-Java active repos; alpine/git initContainer clones each with the pr-lifecycle Contents:read token, scanner loops one Sonar project per repo) | weekly (Sun) | med — 6 source-level scans (14:00 UTC) |
| **11:00 (Sun)** | k8s CronJob | `sonar-scan-gradle` (B138 — SonarQube for ServiceTransformation, the Gradle multi-module Java repo; clone → `./gradlew classes` compile → sonar-scanner with java.binaries) | weekly (Sun) | med — Gradle build + scan (15:00 UTC) |
...
| **03:45** (+≤15m jitter) | node systemd (rogueone) | `machine-inv-drift` (B170 — nightly machine-inventory drift check: for each **reachable** host `collect \| machine_inventory.py merge --prune` inside an **isolated git worktree** off `origin/main`, open/update **one** inventory PR (`chore/machine-inventory-drift`) when the catalog changed, and push a **Kuma heartbeat** (`down`→Telegram on drift or an unreachable host, `up` when clean); `emit`+`verify` the committed SoT → Port first (B169 gate). Runs on **rogueone** because that is where discretionary installs happen and it already holds the fleet SSH keys + `gh` auth + `PR_TOKEN`/Port/`KUMA_INVENTORY_PUSH_URL` in `scripts/.env` — so **no SSH key in the cluster**. **Unreachable host = skipped, never pruned** (merge refuses empty stdin). Freshness is the **Kuma dead-man's-switch** (a rogueone timer is not a k8s object, so `ScheduledJobStale` cannot see it — same as the Woodpecker crons), and `pr-staleness` backstops an ignored PR. `decide_signal` bats-tested (`scripts/tests/machine-inv-drift.bats`); `merge --prune` in `machine-inventory.bats`. `nodes/rogueone/systemd/machine-inv-drift.*`; shell-tests lane in CI. `Persistent=true` so a missed run fires on next boot. | daily | light — 3 SSH scans + a worktree diff + maybe one `gh` PR, ~30-60s; runs on **rogueone**, no single-node contention |
...
- **Down is manual** (the store-scaler easy-button) — auto scale-down is **PARKED**: no `02:00` CronJob manifest exists, and Argo selfHeal would revert a `replicas:0` within ~3 min anyway.
