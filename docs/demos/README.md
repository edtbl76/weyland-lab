...
| 47 | **AI-DLC v2 workflow** (B133 — `/aidlc` forwarding loop; Method retired, knowledge-repos decoupled) | ✅ `flow-aidlc-workflow` | 🟡 `aidlc-workflow.md` (Part A engine validation RUN 2026-08-20 — doctor 47/0, graph clean, Bedrock-strip + absolute-bun verified, 511 KB skills / 116 prompts regression green, retired-generator guard exits 1; Part B reshaped to a **disposable `/tmp` fixture under `bugfix` scope** — fixture defect reproduction verified 2026-08-20, **full gated run NOT YET RUN** — needs a human at the approval gates) | ✅ Part A read-only (`--dry`/`--check`); Part B fixture in `/tmp` + intent record → both torn down, verified via `status` + `doctor` |
