...
sequenceDiagram
    ...
    Note over Srv,Ag: notify-port (build status → Port ci_pipeline) landed in B63 (flow-ci-reliability-signal). The merge is still the gate — but since B135 it is a MACHINE gate, not a human click: see flow-ship-loop.
```
