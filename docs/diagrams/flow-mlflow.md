...
A client (a training or eval run) logs to MLflow at `mlflow.weyland.lab`. Traefik gates with Keycloak SSO via
the `traefik-forward-auth` middleware (was a basicAuth dev-password middleware), then proxies to the MLflow
server. Run metadata (params/metrics/tags) goes to the
...
sequenceDiagram
    ...
    TR->>ML: forward-auth (Keycloak SSO) OK, proxy
