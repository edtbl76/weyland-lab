# AI-DLC upstream contributor guide (archived copy)

> Moved here from the repo-root `AGENTS.md` on 2026-09-25. This is the **upstream AI-DLC framework's** contributor
> guide (it describes `core/`, `harness/`, `dist/`, the CHANGELOG policy of the framework repo) — none of which exist in
> weyland. It sat at the root as `AGENTS.md`, so every harness that reads `AGENTS.md` (Codex, OpenCode, Pi) was told
> the wrong project conventions. Kept for reference on how the framework is structured; the root `AGENTS.md` is now
> weyland's harness-neutral instructions. See [aidlc-v2-migration.md](aidlc-v2-migration.md).
