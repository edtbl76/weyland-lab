...
- **A2. Run it in real CI once.** Everything below has only run locally in containers. The B148 lesson: a control that has never actually run is not yet a control. This is the real remaining risk, and it clears itself on the next pipeline — watch that run.
- **A3. scan-suite image ships mypy+shfmt.** Self-resolves on next ship (in images.tsv); verify the running CronJob image carries both after.
- **A4. Istio long-connection watch** on DataHub↔Redpanda (first meshed client of the unmeshed broker; `neo4j-istio-bolt-longconn-stall` precedent — stall shows under SUSTAINED load; fix = DestinationRule keepalive).
...
**5 Tracking** — `check-linear-sync` green; B88 stays **IN PROGRESS** — Track A operator tails remain (A1 Woodpecker `cosign_key` secret [sign/attest break non-fatally until then], A3 scan-suite mypy+shfmt, A4 Istio long-conn watch); not flipped to Done.
