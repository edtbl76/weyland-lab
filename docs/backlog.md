...
**Dagster ✅** (custom emitter) · **Grafana ✅** (recipe codified, SA token via DataHub Secret → B69) · **Iceberg/Nessie ✅** (+ Dagster→Iceberg lineage) · **MLflow ✅** (+ eval→MLflow tracking, `eval_mlflow_log` → `weyland_rag_eval`) · **Neo4j ✅** (graph schema via apoc) · **Postgres ✅** (ALL DBs, `weyland` superuser, profiling on) · Kafka · **S3/MinIO ✅** (B72 custom-emit) · **Qdrant ✅** · **Weaviate ✅** (custom emit via `datahub_emit.py` emit_qdrant/emit_weaviate, lineage ← `*_write`; native connectors give no pipeline lineage / none for Weaviate) · OpenSearch (playground) · lakeFS. _(CSV moved out → [B68].)_ Recipes + emitters in `k8s/data-mesh/datahub-ingestion/` + `datahub_emit.py`. **9 of 12 done (2026-06-27); remaining HAVE: Kafka, OpenSearch (own — populate w/ BM25 hybrid index first), lakeFS (populate w/ dataset versioning first).**
...
**Trino ✅** (1st Tier-2 done, 2026-06-27 — single-node, native-Nessie catalog + postgres; in DataHub w/ sibling/upstream lineage to iceberg; full gate closed; [runbooks/trino.md](runbooks/trino.md)) · DuckDB · TimescaleDB (B1.4) · dbt Core · Flink (B1.5) · ClickHouse · Cassandra · CockroachDB · MySQL/MariaDB · MongoDB · Superset (B1.7) · Feast (B1.8). _(Kafka pairs here w/ Flink. DuckDB/TimescaleDB pending the set-confirm.)_
