...
- **B-RT** — **Remote model training (rogueone) + persistent Ray** — **✅ DONE 2026-07-06.** MinIO-backed OCI **registry** (`registry.weyland.lab`) → self-contained **trainer container** on rogueone (kubeconfig-only: self-fetches creds + own port-forwards) → **MLflow with artifacts DIRECT to MinIO** (bypasses the serve-artifacts proxy that timed a big model out through the 1Gi pod). **Persistent Ray cluster:** always-on **head** on mother (`ray.weyland.lab`, `k8s/ray/`, **plain Ray not KubeRay**, hostNetwork, `--num-cpus=0` coordinator) + **rogueone as a permanent native systemd edge worker** (`ray-worker.service`; not-always-up → drops on sleep, auto-rejoins on wake). Submit via `ray job submit`; **`--tune`** = a Ray Tune HP sweep **across the cluster**; the **winner retrains + registers on the worker** (`@ray.remote` task — keeps the big fit off the 4Gi head). `genre_classifier` ~**v6** (Ray-Tune-best f1 **~0.308**). **Hardened:** MLflow **LAN NodePort `:30500`** iptables-pinned to rogueone (`externalTrafficPolicy: Local` for source-IP); MinIO TLS **verified via `AWS_CA_BUNDLE`** (mkcert root — replaced `MLFLOW_S3_IGNORE_TLS`); Ray dashboard Keycloak-gated. **Parity lesson:** the native worker must exactly match the head's Python patch (3.11.9) + Ray + serialization libs (pyarrow/numpy/**boto3/botocore/s3transfer**) — install from the head's `pip freeze`. Runbooks: [runbooks/remote-training.md](runbooks/remote-training.md), [runbooks/mlflow-training.md](runbooks/mlflow-training.md). `services/genre-trainer/`, `services/ray-head/`, `k8s/ray/`, `k8s/registry/`. **OPEN follow-ups:**
    ...
    4. **Ray observability → Prometheus + Grafana — ⏳ PARTIAL (2026-07-07).** **Metrics DONE:** head exports `ray start --metrics-export-port=8080` (+ container/svc port) and `k8s/ray/servicemonitor.yaml` scrapes it → **350 `ray_*` metrics in Prometheus** ✅ (query in Grafana Explore). **Still TODO (tomorrow):** (a) the Ray dashboard Metrics tab is empty ("Set up Prometheus/Grafana" banner) — needs `RAY_PROMETHEUS_HOST` + `RAY_GRAFANA_HOST` + `RAY_GRAFANA_IFRAME_HOST` env on the head **AND** Grafana iframe/anon access, which fights the **Keycloak-OIDC Grafana** (the embed hits auth — the real wrinkle); (b) import Ray's shipped Grafana dashboards (cluster/node/actor/task) into `grafana.weyland.lab`; (c) the **external rogueone worker** node metrics need a static-target scrape on `.230:8080` (off-cluster — rides item 1's edge-hardening). Head is `hostNetwork` so its `:8080` is on `mother:8080` directly.
    5. **Feast UI — ⚠ DEPLOYED but EMPTY (2026-07-07).** `feast ui` at `feast-ui.weyland.lab` **serves** (uvicorn 200s, Keycloak forward-auth, meshed for the STRICT-mTLS SQL registry; `k8s/data-mesh/feast-ui.yaml`) but renders **no feature views**. The slim image needed the gRPC registry-server deps — **`grpcio grpcio-health-checking grpcio-reflection`** added to `Dockerfile.feast` (feast 0.58 `feast ui` runs a REST/gRPC mode `feast serve` doesn't). **Open bug (tomorrow):** the frontend `GET /api/v1 → 404` — bundled-UI ↔ 0.58-REST-backend **API-path mismatch** (feast UI version churn), or it's not reading the populated registry. Diagnose: `kubectl -n data-mesh exec deploy/feast-ui -- feast -c /app/feast_repo feature-views list` (is the registry populated from the pod's view?) + inspect what path the UI JS calls; may need a feast/UI-mode pin. Cross-ref **B1.8**. (Items 3+5 = the "give the capability a face" batch; item 4 = its observability sibling — #3 done, #4 partial, #5 needs the empty-UI fix.)
