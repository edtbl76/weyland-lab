...
- **B-RT** — **Remote model training (rogueone) + persistent Ray** — **✅ core DONE 2026-07-06.** MinIO-backed OCI **registry** (`registry.weyland.lab`) → self-contained **trainer container** on rogueone (kubeconfig-only: self-fetches creds + own port-forwards) → **MLflow with artifacts DIRECT to MinIO** (bypasses the serve-artifacts proxy that timed a big model out through the 1Gi pod); `genre_classifier` **v1** (single) / **v2** (Ray Tune best, f1 0.312). **`--tune`** = Ray Tune sweep on a local Ray cluster. Persistent Ray **head** live at `ray.weyland.lab` (Phase 1 — jobs *submitted* to it, dashboard SSO-gated). Runbooks: [runbooks/remote-training.md](runbooks/remote-training.md), [runbooks/mlflow-training.md](runbooks/mlflow-training.md). **OPEN follow-ups (building now, in order):**
    1. **Ray Phase 2 — rogueone as a DMZ edge worker.** `hostNetwork` head + a segmented VLAN/firewall allow-list (head Ray ports + MLflow/MinIO/lakeFS only) + Ray TLS / join-token + **native Ray on rogueone** (also escapes the Docker-Desktop RAM/GPU/loopback traps). Rationale: Ray is *not* a security boundary → constrain at the network (DMZ).
    2. **Feast source (`--source feast`).** Train from `get_historical_features` (point-in-time). Wrinkle: the offline store = Postgres = **STRICT mTLS**, so the trainer needs a mesh-reachable path (a rogueone container can't just port-forward it).
    3. **Registry web UI.** A front-end for `registry.weyland.lab` (e.g. `joxit/docker-registry-ui`) — it's API-only today.
