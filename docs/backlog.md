...
| `incrementalSyncEnabled` | reads `true` at `spec.appSpec.incrementalSyncEnabled` after the backfill — it is a deliberate temporary override, and leaving it off means every sync is full every 4h |
