...
**COUPLED TO EMA-186 — the decisions hinge on each other, in both directions (linked in Linear 2026-08-27).** Downward: you cannot judge whether to buy, replace or supplement a GPU box while the incumbent's failure mode is open — rogueone is the current GPU host, and its whole-box freeze is parked mid-investigation (8 incidents; #8 on 2026-08-25 was the first positive detection, a stack-canary smash in the iwlwifi RX path 227ms after docker veth churn — kernel memory corruption, NOT GPU/RAM/kernel-version). Upward: the strategy chosen here changes what "fixing rogueone" is even worth. If the answer is hybrid-with-burst-rent, rogueone stops being the lab's compute floor and becomes one replaceable node, which lowers the bar for retiring rather than repairing it; if the answer is stay-local, its reliability becomes load-bearing and the freeze must be driven to root cause. Neither issue should be closed on its own reasoning.
