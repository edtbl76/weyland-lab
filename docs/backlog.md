...
- **A1. Woodpecker CI secrets** `cosign_key` / `cosign_password` — UI-only state, operator adds in Woodpecker (Repo → Settings → Secrets). Until then builds ship UNSIGNED (logged, non-fatal). See runbooks/supply-chain.md.
