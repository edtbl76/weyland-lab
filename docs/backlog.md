...
- **B2** — Hermes agent platform (OpenClaw sibling) — **CLOSED 2026-08-05 — superseded by B66** (`weyland-operator`; Hermes CT-104 destroyed 2026-07-23, the operator lane rebuilt in B66 + the local-brain work). Historical: **v1 LIVE 2026-06-14**: Hermes CT 104 on `qwen3-coder:30b` (MoE); read-only **system-view MCP server** (4 tools: status/context_search/context_ask/list_models) built into the tool-server via `fastapi-mcp` `mount_http()`, registered in Hermes, **validated end-to-end** (agent → MCP → live backend health). **Telegram gateway front door LIVE 2026-06-14** (allowlisted DM → agent reply). Runbook docs/runbooks/agent-hermes.md · design docs/concepts/agent-platform-design.md.
...
11. **B1** — **Data mesh — the Iteration-2 platform build.** **DONE (core, 2026-08-05)** — the mesh is built + live: all slices shipped (B1.1 Keycloak SSO · B1.2 Nessie/lakeFS · B1.5 dbt · B1.6 governance DataHub/Ranger/Soda · B1.7 Cube · B1.8 Jupyter) + the stores (ClickHouse/Cassandra/Qdrant/Weaviate/Neo4j/Feast), Trino, OpenLineage, and the operator queries it via the fleet MCPs. **Maturity tails split into their own items** (B77 DQ, B78 vector polish, B80 DataHub, B81 Jupyter, B113 financial domain) — **TODO: collapse those into one "data-mesh maturity" bucket** when we reach them in the triage. Historical design: FULL design locked in `design/data-mesh-design.md` (~30 techs across 8 layers; single-node **run-mode tiered** — always-on foundation vs KEDA/operator on-demand). The **3 data products are the *output*** — they ride on the platform below, they are NOT the build. Sequenced into dependency-ordered slices (storage→catalog/gov→query→transform→consumption); **Keycloak pulled to the front** (lab-wide SSO, not mesh-specific — pays off immediately):
 ...
 - **B1.9 — The 3 data products → REFRAMED 2026-07-16 (platform-complete; B1 DONE).** The mesh already publishes **9 data products** (`_PRODUCTS` in `datahub_emit.py`: Spotify Audio · Artist Popularity · Genre Taxonomy · Chronic Health Trends · Global Health Indicators · Personality Profiles · Weyland Docs · AIDLC KB · Genre Classifier) + **AI-Dev Usage** (B62, Port) — the *output* the platform was built to enable already exists. The original trio doesn't survive scrutiny: (2) store-inventory → lineage/observability is **already shipped** (OpenLineage + DataHub lineage + Port + Grafana/Kuma); (3) model-tuning feed is **parts that already exist** (Feast DONE + MLflow DONE + Ray DONE — wiring, not a build). Only (1) **model-eval** warrants a real build, and even that *productizes* the existing **B4** judge-panel leaderboard rather than starting fresh → spun out to **B84**. Building "3 products" to hit the number 3 = box-checking; declined. **B1.9 closed as platform-complete; B1 = DONE.**

### Maturity / Hardening / Polish
- **B161** — **Investigate Dify** — **HIGH (2026-09-08).** Evaluate [Dify](https://dify.ai) (open-source LLMOps / LLM-app platform: visual workflow + agent builder, RAG pipeline, prompt IDE, model management, built-in observability) against the lab's already-built AI plane. The question is **overlap vs. unique value**, not adoption-by-default: the estate already has the gateway (Bifrost/LiteLLM), agent frameworks (LangGraph — operator, Realm, agentic-RAG), a full RAG stack (Qdrant/Weaviate/LanceDB + bge embed), eval (promptfoo/MLflow/golden-set), prompt federation (Bifrost prompt-repo), and LLM observability (Langfuse). Decide: does Dify **replace** a hand-rolled piece, **complement** as a low-code front door for non-code workflow authoring, or **duplicate** what's there? Must be $0 + self-hosted + LAN-only (Keycloak forward-auth, meshed Postgres) to qualify. Deliverable: a per-capability overlap map + a keep/skip call with rationale. Relates B17+B19 (MCP/agents), B103 (Langfuse), B84 (eval), B66 (operator). See detail below.
...
- **B186** — **Configure GitLens in VS Code (Community; Pro only if it earns its cost)** — **WON'T DO (2026-09-26, Linear EMA-244 canceled — not used enough to pay for; Pro's first-year deal doesn't change that, since the full price after year one won't be kept up; was HIGH 2026-09-25).** VS Code is now a harness host (Codex + Claude Code extensions); GitLens is its git layer. Community is free and covers Commit Graph / Visual File History / Worktrees on public + local repos (most lab repos are public); Pro is ~$8/user/mo billed annually (~$96/yr — not $0, the owner's call) and mostly duplicates what's in place (PR triage, Claude/Codex AI) and it has **no Linear integration**. Upgrade only if a private repo needs it. See detail below.
...
- **B163** — **Golden-path capability coverage: exercise each framework's modules semi-exhaustively** — **MEDIUM (↓ High→Medium 2026-09-08 — same-day demote: on scoping it is EXTREMELY large, a matrix of 21 paths × framework-native modules × the estate's whole integration surface (Postgres/Trino/10 Tier-2 stores/3 vector stores/Neo4j/Redpanda/Valkey/MinIO + auth/OTel/secrets). It is a PROGRAM to decompose into an umbrella of sub-items, not a single High deliverable; the decomposition/design pass comes first. Linear EMA-220).** B153's 21 golden paths currently satisfy the MINIMAL contract (`/health` `/ready` `/metrics` `/hello`) — enough to prove the paved road builds, tests, scans, and serves, but a thin slice of what each framework offers. Go through ALL 21 paths and deepen each to **semi-exhaustively cover its framework's module capabilities**, so a scaffolded service inherits a real demonstration of the framework rather than a stub: routing (params/query/wildcards), middleware/filters/interceptors, request validation + typed binding, structured error handling, dependency injection / app context, config + env, background/lifecycle hooks, and the framework's testing idioms — the specific surface differs per framework (e.g. NestJS pipes/guards/modules, FastAPI dependencies/Pydantic, Spring `@RestControllerAdvice`, Axum extractors/middleware, Fiber middleware groups). Each added capability keeps the contract green AND gains lane tests + coverage. Semi-exhaustive, not exhaustive: cover the modules a real service actually reaches for, skip the exotic. Deliverable: a per-framework capability checklist + the deepened paths (tests + coverage baselines updated), all still green through the B153 lanes + golden-path-smoke. Depends on B153 (the paths + harness); relates B160 (extended language suite — new paths inherit the deeper template), B154/B155 (onboarding + API lifecycle exercise the richer surface). See detail below.
- **B164** — **Extended golden paths: mobile clients + additional-language wave** — **DONE 2026-09-13 (Linear EMA-221) — CI-confirmed: all 58 test+scan lanes green (#126) + all 9 new service golden paths built+served in-cluster (#134) + 4 mobile clients; plus a delivered on-demand golden-path-smoke capability (`--var SMOKE_ONLY=smoke`). On-device iOS/SwiftUI carved to [B168] (Very Low).** TWO workstreams: (1) MOBILE clients + (2) an additional-language SERVICE wave (9 langs, below). **(1) Mobile:** cover MOBILE, which neither B153 nor B160 includes and which does NOT fit the existing golden-path contract: that contract is an HTTP SERVICE contract (`/health` `/ready` `/metrics` `/hello`, verified by build-image → run k8s Job → curl `/hello`), but a mobile app is a CLIENT — it serves no endpoints and deploys to a device/store, not a Job. So mobile needs an ADAPTED contract (as the frontend paths adapted it for SSR): an app skeleton that bundles + builds, a test lane, a scan lane (every language gets one), and a "smoke" that is a headless bundle/render check (`expo export` / metro / `flutter test`), NOT the golden-path-smoke image+curl Job. **Targets:** **React Native** (flagship — jest + `@testing-library/react-native`, Linux-CI-buildable ✓) · **Flutter** (`flutter test` + Android/web bundle, Linux-CI-buildable ✓) · **Swift/iOS** (SwiftUI — **HARDWARE-GATED**: needs macOS + Xcode, which the $0 Linux lab has no runner for; authorable as a template but its lane can't run until a Mac exists — ties the hardware-gated set. If Swift is wanted Linux-CI-verifiable, server-side Swift/**Vapor** is an HTTP service → **B160**, not here). Deliverable: RN + Flutter lane-verified; Swift/iOS authored + parked on macOS; the client/mobile contract variant documented in `docs/design/golden-paths.md`. **(2) Additional-language SERVICE wave (added 2026-09-11):** extend the B153/B160 golden-path SERVICE suite (same HTTP contract + B160 prove-in-Docker cadence; each = a net-new test+scan lane) with 9 more languages — **clean keeps (5):** server-side Swift (Vapor/Hummingbird — Linux toolchain, NOT hardware-gated iOS Swift), Erlang (Cowboy — same BEAM as the shipped Elixir), Julia (Genie/Oxygen), Lua (OpenResty/Lapis — APISIX's stack), server-side Dart (shelf/dart_frog — distinct from Flutter above, shared Dart toolchain); **optional/breadth (4):** R (plumber), Perl (Mojolicious/Dancer2), Haskell (Servant/Yesod), Ada (AWS); **shitcanned (7):** SQL, Fortran, Delphi (proprietary), Scratch, Assembly, Objective-C, COBOL. Relates B153, B160, B163. See detail below.
- **B167** — **Evaluate + integrate Plannotator** — **DONE 2026-09-11 (Linear EMA-228).** Eval + impl [Plannotator](https://plannotator.ai/) (`backnotprop/plannotator`, OSS) — a **local visual plan + code-diff review/annotation** tool for coding agents (Claude Code, OpenCode, Codex, Pi, Droid): annotate an agent's proposed plan or a git diff with pen/arrows/highlights, then push that back to the agent as structured feedback in one click (no copy-paste). Runs **local, data stays local, no telemetry** — $0 + LAN-clean by design. Directly in the **B104** coding-agent-tooling family and pairs with **Emdash** (parallel-agent supervisor) and **Serena/B166** (agent structural context): Emdash runs many agents, Plannotator is the human review/steer surface over their plans+diffs. Deliverable: install (plugin/hook + VS Code extension), a per-harness fit finding (which of Claude Code / OpenCode / Codex it wires to cleanly here), an eyes-on UAT on a real weyland plan+diff, and a keep/skip call. Relates B104, B166, B15 (local-model agents). See detail below.
- **B168** — **iOS/SwiftUI on-device golden path (hardware-gated)** — **VERY LOW (2026-09-11, Linear EMA-229).** A documented **limitation**, carved out of B164: a real SwiftUI app running on an iOS **simulator or device** needs **macOS + Xcode**, which the $0 Linux-only lab has no runner for — and cloud-macOS (GitHub Actions macOS / Xcode Cloud / MacStadium / Cirrus) breaks the $0 / self-hosted / LAN-only bar. So the on-device iOS surface stays **parked** and is unlikely to be actioned unless a Mac enters the lab. **This is NOT a coverage gap in the paved road** — the Swift-UI programming model + build chain are already proven on Linux by **B164**'s `golden-paths/mobile/swift-tokamak` (Tokamak, a SwiftUI-compatible API, → WebAssembly via SwiftWasm/carton), and server-side Swift by `golden-paths/swift/vapor-app`. What B168 does NOT cover is exactly and only: on-device/simulator iOS execution, UIKit/SwiftUI-on-iOS rendering, and App Store packaging. Revisit only if a macOS runner ever exists. Relates B164 (parent), B149/B150 (the hardware-gated set). See detail below.
...
- **B78** — **Data-mesh maturity (bucket)** — **DONE 2026-09-05** — all three threads complete: (a) OFF vector hydration DONE 2026-08-31, (b) B81 JupyterHub notebooks DONE, (c) B113 finance domain DONE 2026-09-05; EMA-69 Done (2026-09-01). _(Priority history, now moot: was ↑ Medium→High 2026-08-11 rebalance; B81 folded in [High — floored to parent 2026-08-13], B113 folded in [High, EMA-110].)_ Three polish/expansion threads on the built mesh: **(a)** OFF → vector hydration (below); **(b)** the **JupyterHub notebook library** [B81 — full scope in its entry]; **(c)** a **financial/economic datasets domain** [B113 — full scope in its entry]. DataHub maturity (B80) + DQ (B77) stay their own buckets. — **(a) Open Food Facts → vector stores (Qdrant/Weaviate/LanceDB):** **PLAN WRITTEN 2026-08-27 → [docs/concepts/open-food-facts-vector-hydration.md](concepts/open-food-facts-vector-hydration.md). Re-scoped: this is NOT "just add the spec + run."** Scoping found (1) `_build_vectors` reads every silver parquet **whole** into pandas — OFF's single 4.5M-row × 211-column all-string file OOMs the user-code pod (limit 12Gi, on a ~98%-committed node), and a cap applied *after* the read does not help; (2) a `text` spec whose columns are absent is **silently** dropped, so a wrong column name hydrates identical vectors and reports success — Step 1 read the real silver schema (2026-08-31) and **inverted** the suspicion: the file HAS `categories_en` (+ `categories`, `categories_tags`) and does **not** have `categories_fr`; `datasets_field_docs.OPEN_FOOD_FACTS` is stale, so the backlog's `categories_en` was correct and the intuitive "fix" to `categories_fr` would have been the silent-empty bug; (3) `lancedb_allow` defaults to `vector_allow`, so one entry targets **three** backends; (4) `weyland-dagster` has **no tests at all**, and `team.md` mandates TDD. Real scope = a bounded projected/capped read in the shared loader + fail-closed on empty column resolution + the service's first pytest suite, then the spec. Original note follows. Deferred from the B1 vector-store hydration (2026-07-03): every other grid Qdrant/Weaviate=Y set went in one pass, but OFF is **~4.5M docs** → a full text-embed (`product_name`+`brands`+`categories_en` via bge-small) is *hours* on CPU and **~7 GB of vectors per store** (4.5M × 384 × 4B × 2) — not a lab footprint. Do it capped: filter to non-empty `product_name`, cap ~200k, embed the concatenated product text → ~600 MB/store, ~20–30 min. Reuses the `build_vector_load_assets` loader + `vector_spec` (`{text: [...], cap: 200000}`) already built for the other sets; just add the OFF spec + run. (Its nutrition columns are `large_string` in silver — string-parsed — so text similarity is the meaningful vector, not the nutrition numbers.) — **THREAD (a) DONE 2026-08-31:** OFF hydrated to all three vector backends via the bounded read — **Qdrant 200,000 · LanceDB 200,000 · Weaviate 195,792**, all dim 384 — with the user-code pod peaking **~6 Gi against its 12 Gi limit** (the whole-read OOM retired, proven live not estimated: the projected+capped read of the real 4.5M×211 file peaked 18.6 MB). Work: CI python lane made per-project dep-aware (`requirements-test.txt` installed by `run-lang-tests.sh`); weyland-dagster's FIRST pytest suite; `_build_vectors` reworked to a projected+capped `iter_batches` read in a dagster-free `parquet_read.py` (fail-closed on absent columns) with `build_records` extracted + tested; the OFF spec added (`text: product_name+brands+categories_en` — categories_en confirmed present against real silver, field docs were stale re categories_fr). Two fixes found en route: `ship-images.sh` SMOKE now **retries** the transient Recreate rollout (it was false-failing on the dagster code-server's ~40s definitions load), and `_load_dataset_to_weaviate` now reports the **true landed count** + logs batch failures (the 195,792-vs-200,000 gap was a silent over-count). Threads **(b) B81** (JupyterHub notebook library — DONE) + **(c) B113** (finance domain — DONE 2026-09-05, all 5 phases) are now complete, so with **(a) OFF vector hydration DONE 2026-08-31** all three threads (a/b/c) are done — **B78 COMPLETE** (EMA-69 flipped Done 2026-09-01; B80 DataHub-maturity and B77 DQ remain their own separate buckets, never part of B78).

**DoD — B78(a) + vector-store observability & quality (2026-08-31).** **1 OK** — `datasets-hydration.md` (new "Vector-store observability & quality" section + the 3 hydration markers), `open-food-facts-vector-hydration.md` (status→DONE), `arch/weyland.likec4` (`lancedbExporter` obs component + edge), `backlog.md`, Linear EMA-69; api/hosts/schedules N/A (no endpoint/ingress/timer — the exporter is a continuous loop, not a CronJob), verified. **2 OK** — LikeC4 gained the exporter component + relation; 150 mermaid blocks parse; no new sequence flow warranted (data through existing assets). **3 OK** — `demos/vector-stores.md`, the hydration RUN end-to-end per backend (Qdrant/LanceDB 200k · Weaviate 195,792, dim 384, pod peak ~6 Gi) + live-verified checks/exporter/dashboard/alerts. **4 OK** — throwaway Weaviate class + scan/verify Jobs deleted; single exporter pod; no hand-edited config. **5 OK** — `check-linear-sync` green (31 refs); EMA-69 thread (a) DONE, B152 filed EMA-209. **6 OK** — the lancedb-exporter (new service) onboarded: `applications.yaml` pure-compute entry (`check-app-registry` green), ServiceMonitor + Grafana dashboard + `LancedbExporterDown`/`LancedbRepoUnreachable` PrometheusRule (promtool SUCCESS), logs via Alloy→Loki; traces/profiles/synthetic/Kuma/backup **N/A** (internal metrics exporter, no LAN ingress, stateless — health is the ServiceMonitor + Down alert); GitOps ✓ (Argo, registry image, lockstep tag). **7 OK** — scan-suite RUN: **0 CRITICAL**; the new Python (`parquet_read.py`, `checks.py` vector checks, exporter) adds **no new HIGH** (verified per-file with bandit — the B608 MEDIUMs are pre-existing `loaders.py` Tier-2 SQL; the 1H bandit/1H semgrep are the documented pre-existing aidlc-swarm/langfuse highs); mypy 158M via the per-directory fix. **8 OK** — additions only (parquet_read, vector checks, exporter, dashboard, alerts, registry entry); shared-code changes are backward-compatible — `_load_dataset_to_weaviate`'s return is consumed only as the load-count metadata (now honest), `_build_vectors`'s bounded path is gated on `cap`/`filter` so every existing dataset is byte-identical; no removals/renames to reverse-sweep. **Port registration (corrected + executed 2026-08-31).** The exporter entry is `datahub_application: false` — deliberately NOT a DataHub Application (owns no cataloged data), so `emit_applications` (DataHub-only; emits ONLY `datahub_application: true` rows) correctly never emits it — the earlier "reaches both Port and DataHub via emit_applications" note was WRONG on both counts. The Port `component` entity has **no auto-sync path**: B137 removed the tofu `port_entity` for_each, `emit_applications` never touches Port, and `check-app-registry` only guards Argo↔registry (git-side) — so a `false` component can pass every git check and never reach Port. Closed by pushing the entity via the Port API directly and reconciling live Port to the registry: **66/66**, which also backfilled the 38-day-orphaned `port-k8s-exporter` (B145). The stale claim in `tofu/port/applications.tf` ("pushed to Port through the same path") is corrected in-file. Separately re-shipped the dagster image to `git-cc3a2918` (exporter/dbt-docs/user-code lockstep) — note that bake is inert for the registry (nothing reads the baked yaml for Port/DataHub); the alert + docs deploy via Argo on push. **Gap fixed:** the registry→Port drift now cannot recur silently — `emit_port_components` (dagster-free `port_components` module, unit-tested; wired into `datahub_catalog_emit_job` after `emit_applications_op`) upserts every registry entry to Port as a `component` on each catalog emit (idempotent, merge-safe). Validated live by hand (66/66, no-op). Separately, the `ship-images.sh` FR1.5 race is fixed: the tag check now retries transient mid-rollout states internally to a timeout covering Argo self-heal latency + roll (was a 300s clock started at sync time, before selfHeal began the roll), failing fast only on permanent states — 85/85 bats green.
...
- **B82** — **Application taxonomy (cross-surface app classification)** — **DONE 2026-08-05; nested under B80.** DataHub emit `(29, 4157)`, 54 Port components (29 data-app / 25 pure-compute) each with `is_data_application` + DataHub link + a `capabilities` array, Argo-completeness check green (72 apps accounted). Enrichment live: Documentation link, group Tag, Domain, customProperties, + a 30-term **Application Capabilities** glossary (DataHub terms on the 29; the same capabilities as a Port array on all 54). Registry = `services/weyland-dagster/weyland_pipeline/applications.yaml`; DataHub `emit_applications()` + `tofu/port/applications.tf` (for_each) + `scripts/check-app-registry.sh` (drift guard). **DoD COMPLETE:** arch §7f · `concepts/application-catalog.md` · `flow-application-taxonomy` · LikeC4 · platform-map · demo (RUN, eyes-on) · drift guard. Follow-up (non-blocking): point `emit_applications` at the registry `capabilities` to retire the mirrored `_APP_CAPABILITIES` on the next Dagster rebuild. **Gate PASSED** — `acryl-datahub 1.6.0.15` / GMS v1.6.0 model the `Application` entity (`ApplicationPropertiesClass`/`ApplicationsClass` import clean). **Scope grew beyond DataHub into a cross-surface taxonomy** (the DoD's new "one source of truth" process): adopt DataHub's **Application** entity as an app-centric lens *alongside* Domains + Data Products, but the same app-classification lines must hold across **four surfaces — DataHub · Port · docs · diagrams — with zero drift.** The line: an app is a DataHub Application iff it **owns cataloged data now OR plausibly will** (→ DataHub Application **+** Port component, linked); pure-compute (KEDA scaler, TTS, stateless proxies, query engines) is **Port-component-only**. **Mechanism = one canonical registry** `services/weyland-dagster/weyland_pipeline/applications.yaml` (baked into the Dagster image + read by Tofu) that `datahub_emit.py`, `tofu/port/applications.tf`, and the docs/diagrams all consume or are DoD-checked against. **Final categorization: 29 DataHub Applications + 25 pure-compute = 54 components** (reclassified during the Argo-driven roster pass: grafana → data-app [owns ~367 grafana datasets], nessie + lakefs added as data-apps [cataloged DBs], ranger confirmed operational data-app; the earlier "~26 + borderline" framing is superseded). **Port is a first-class part of this body of work — not an afterthought.** Producer-based URN rules (mirror `_DOMAIN_RULES`); MLflow agent-traces attach by explicit tag. Design + full roster: `design/application-taxonomy.md`. Build sequence: registry → `emit_applications()` → Port codification → docs (`arch.md` + `concepts/application-catalog.md`) → LikeC4 + platform-map → DoD sweep. Relates B80 (umbrella), B43/B60 (Port), the DataHub governance layer.
- **B81** — **JupyterHub notebook library (B1.8 follow-on)** — **FOLDED INTO B78 (2026-08-05)** — thread (b) of the data-mesh maturity bucket. Full scope retained here: grow the single seed notebook (`datasets_lake.ipynb` — 4 formats + polars/DuckDB, done 2026-07-12) into a **semi-exhaustive, runnable library of notebooks demonstrating the whole data/ml/ai stack**, baked into (or git-synced to) the JupyterHub singleuser image so opening JupyterLab is a real, self-documenting playground. **(a) Per-format deep dives** — one notebook EACH for **Parquet · Arrow/IPC · Avro · Lance**: format internals (columnar vs row, encoding/compression, zero-copy for Arrow, dataset versioning + vector-index for Lance, schema evolution for Avro), read/write, and when-to-use — beyond the one combined datasets_lake notebook. **(b) Full-stack library** — a curated set spanning every layer: **storage/versioning** (lakeFS branching, Nessie/Iceberg time-travel + snapshots); **query/federation** (Trino cross-catalog, DuckDB/GizmoSQL, + one per Tier-2 store: ClickHouse/Cassandra/MySQL/Mongo/Cockroach/Timescale); **vector/graph** (Qdrant/Weaviate/Lance similarity search, Neo4j Cypher); **transform/semantic** (dbt marts, MetricFlow `mf query`, Cube REST/SQL); **feature/ML** (Feast online + point-in-time retrieval, Ray training/HP-sweep submission → MLflow tracking/registry); **AI/RAG** (LlamaIndex retrieval over the vector stores, embeddings, the eval harness, LiteLLM/Ollama); **governance/quality** (DataHub lineage/GraphQL queries, Soda scans, Ranger-masked queries); **streaming** (Redpanda consume, Debezium CDC tail). **Distribution:** notebooks live in the repo (`k8s/jupyterhub/singleuser/notebooks/`); consider **nbgitpuller / git-sync** so the library updates without an image rebuild (the current `cp -n /opt/examples` postStart is fine for one seed but doesn't scale). Each notebook doubles as a living demo + doc of that layer. See runbooks/jupyterhub.md, [[cube-semantic-layer-b1.7]]. — **DONE (EMA-71, 2026-09-02):** (1) **distribution mechanism shipped** — notebooks moved to `singleuser/notebooks/`, `postStart` now **git-syncs** the dir into `~/notebooks` on every spawn (git is in the scipy base → library grows by `git push`, no image rebuild; baked `/opt/examples` is the offline fallback; singleuser NetworkPolicy confirmed to allow GitHub egress); (2) **per-format wave (a) DONE** — `01_format_parquet` · `02_format_arrow_ipc` · `03_format_avro` · `04_format_lance`, all self-contained and **headless-execute clean** (re-validated fresh, 0 error-outputs each) + a `notebooks/README.md` index; (3) **stack-layer wave (b) — storage/versioning DONE (2026-09-01)** — `10_storage_lakefs` (git-for-data: zero-copy branch/commit/diff/merge/log + commit-id time-travel over the live `music` repo, scratch-branch-only, self-cleaning) · `11_storage_nessie_iceberg` (Iceberg snapshots/schema-evolution/atomic commits + Nessie git-like catalog branching + commit-hash time-travel via `StaticTable`; reads live `dbt.mart_*` read-only, writes a scratch `nb_demo` namespace, self-cleaning) — both **headless-execute clean** against the live mesh (0 error-outputs; catalog independently re-verified clean post-run). Added Iceberg warehouse creds to the singleuser pod via SealedSecret `jupyterhub/iceberg-s3-creds` (mirrors data-mesh `nessie-secret` s3 keys). Found: this stack versions at the Nessie catalog-commit level (one live Iceberg snapshot per table), so time-travel is by commit hash, not snapshot id. (4) **query/federation wave STARTED (2026-09-01)** — `20_query_trino_federation` (Trino federated SQL: catalog/schema discovery + a real cross-catalog join `iceberg.eval.eval_scores` ⋈ `postgresql.public.eval_results` in ONE query + predicate/column pushdown via `EXPLAIN`; read-only, env-driven, `%pip install trino`), **headless-execute clean** (0 error-outputs) against the live coordinator. **Catalog reality caught live:** only `iceberg` + `postgresql` + `system` are wired as Trino catalogs — the Tier-2 stores are NOT Trino connectors, so the planned "Tier-2 through Trino" notebook is re-scoped to `22` = per-store NATIVE clients; `21` = DuckDB/GizmoSQL. Trino is unmeshed (plain HTTP, no sidecar), so no secret/values change for nb 20. `21_query_duckdb_gizmosql` (DuckDB two ways: EMBEDDED over lakeFS Parquet with window fns/projection-pushdown + true zero-copy Arrow interop proven by identical buffer address, and SERVED via GizmoSQL Arrow Flight SQL/ADBC over the persisted silver base tables incl. USDA relational JOINs), **headless-execute clean** (0 error-outputs) against live lakeFS + GizmoSQL via NodePort. Embedded half reuses existing `lakefs-creds`; served half adds SealedSecret `jupyterhub/gizmosql-creds` (mirrors data-mesh `gizmosql-secret`) + a values env block. `22_query_tier2_native` COMPLETES the query wave: the six Tier-2 stores by NATIVE client (NOT Trino connectors) — ClickHouse/Cassandra/MongoDB/CockroachDB/TimescaleDB/MySQL, each a real read on hydrated data, **headless-execute clean** (0 error-outputs). Four password stores share `TIER2_DB_PASSWORD` (SealedSecret `jupyterhub/tier2-creds`); Cassandra (AllowAllAuthenticator) + Cockroach (insecure) need none. Added `k8s/data-mesh/tier2-lan.yaml` — six `-lan` NodePort services (30123/30942/30017/30257/30543/30306) so rogueone reaches these previously-in-cluster-only stores (lab is **NodePorts-only, never port-forward**); api.md rows updated. **Mesh caveat:** `data-mesh` is istio-injected with inconsistent per-store opt-in/out, so a store gets a sidecar on pod restart and an unmeshed client then can't reach it — nb 22 was validated from INSIDE the mesh; the singleuser pod must be mesh-joined for the meshed stores (MySQL, and GizmoSQL from nb 21) at spawn-UAT. **Query wave DONE (20 Trino federation · 21 DuckDB/GizmoSQL · 22 native Tier-2).** **Vector/graph wave DONE (2026-09-01): 30 Qdrant · 31 Weaviate · 32 LanceDB · 33 Neo4j**, all four **headless-execute clean** against live stores via NodePort. `31_vector_weaviate` is **the U16 deliverable** — class/object browse (the dropped-UI replacement) + vector/BM25/hybrid/GraphQL, so **[U16] is now satisfied**. Qdrant open, Weaviate anonymous, LanceDB reuses `LAKEFS_*` (embedded, opens lakeFS-backed Lance tables via object-store), only Neo4j adds a secret (`jupyterhub/neo4j-creds` ← weyland `neo4j-secret`). Also added the singleuser pod **mesh-join** (`sidecar.istio.io/inject:"true"` via `extraLabels`) so in-pod spawns reach the meshed stores (MySQL, GizmoSQL). **Transform/semantic wave DONE (2026-09-02): 40 dbt-marts+MetricFlow · 41 Cube**, both **headless-execute clean** validated IN-CLUSTER (ephemeral injected data-mesh pod) — Trino + Cube are ClusterIP-only, so NOT via NodePort (a Trino NodePort would expose the no-auth engine on the LAN) and NOT port-forward. nb 40 queries the 7 marts via Trino + shows the MetricFlow metric definitions + compiled Trino equivalent (real dbt project is two-layer staging→marts); nb 41 uses Cube's pg-wire SQL API + the `MEASURE()` contract. Trino no creds; Cube adds `jupyterhub/cube-creds` (← data-mesh `cube-secret`). **Feature/ML wave DONE (2026-09-02): 50 Feast · 51 MLflow(+Ray)**, both **headless-execute clean**. nb 50 (Feast) validated IN-CLUSTER (weyland-postgres is STRICT-mTLS) — online Valkey + historical Postgres point-in-time over `track_audio_features`/`state_health_risk`, real train/serve parity; nb 51 (MLflow) via NodePort 30500 — 28 experiments/400 runs, `genre_classifier` registry v1-v9, Ray shown as the genre-trainer pattern (247 Ray-Tune runs) not a live client (version-pinned). Consolidated creds `jupyterhub/mlplat-creds` (WEYLAND_PG_PASSWORD + MLFLOW_TRACKING_USERNAME/PASSWORD). **AI/RAG wave DONE (2026-09-02): 60 RAG · 61 LiteLLM gateway · 62 RAG eval**, all three **headless-execute clean**. nb 60 = real retrieve→augment→generate (local bge-base embed matching the corpus, Qdrant `weyland_chunks`, LiteLLM `wl-rag`; grounded-vs-no-context contrast); nb 61 = the LiteLLM `wl-*` gateway (chat/streaming/live routing); nb 62 = RAG eval leaderboard (Trino over iceberg.eval+postgresql) + one live `wl-judge` faithfulness score. 60/61 via NodePort, 62 in-cluster. New cred `jupyterhub/litellm-creds` (LITELLM_MASTER_KEY). **Governance/quality wave DONE (2026-09-02): 70 DataHub · 71 Soda · 72 Ranger**, all three **headless-execute clean** in-cluster. nb 70 = DataHub catalog/lineage (search + schema + bidirectional lineage + domains/glossary, acryl-datahub SDK); nb 71 = Soda contract scan over the marts (trino-noauth, 8/8 checks, fail-closed guard + py3.12/http-auth fixes); nb 72 = Ranger column mask (`analyst` sees `depression_pct` NULL vs `dbt` real, column-scoped proven). New cred `jupyterhub/datahub-creds` (DATAHUB_TOKEN); Soda/Ranger need none. **Streaming wave DONE (2026-09-02): 80 Redpanda · 81 Debezium CDC**, both **headless-execute clean** in-cluster, no creds (Redpanda plaintext/unmeshed). nb 80 = Kafka consume + Avro/Schema-Registry (`confluent-kafka[avro]`, bounded read-only consume, live offset watermarks); nb 81 = Debezium CDC (connector RUNNING, Envelope change events, Iceberg mirror `cdc_demo_live` via Trino — CDC→lakehouse loop; live consume showed 0 events [demo topic aged out past 7d retention, no periodic writer] so it proved the decode against the real schema + showed the populated mirror rather than fabricating). **B81 NOTEBOOK LIBRARY COMPLETE: 25 numbered notebooks + the `datasets_lake` seed** — formats → storage → query → vector/graph → transform/semantic → feature/ML → AI/RAG → governance/quality → streaming, each validated end-to-end against the live mesh, git-synced into `~/notebooks` on spawn. **OPERATOR UAT PASSED (2026-09-02): 26/26 notebooks run clean IN-POD** (full headless batch in the real singleuser pod, Keycloak-spawned) — git-sync distribution, the singleuser mesh-join (21/22/41 GizmoSQL/MySQL/Cube over mTLS), public egress (60 HF + LiteLLM), all eight injected sealed creds, and the registry-served image all confirmed live in-pod. Two defects caught + fixed by the UAT: (1) the singleuser image was `pullPolicy:Never`/ctr-imported and got deleted by the weekly `weyland-image-prune` → `ErrImageNeverPull` → moved to `registry.weyland.lab/weyland-jupyter:v1` + `IfNotPresent` (prune-safe); (2) nb 33's GDS cell used GDS 1.x anonymous-projection syntax that fails on GDS 2.x (the path had been dead during out-of-pod validation because GDS read absent then) → rewritten to a named projection + `gds.degree.stream` + drop, guarded. **Thread (b) / B81 is DONE** — flip EMA-71 → Done + mark the DoD demo pillar. U16 also delivered (nb 31). Two follow-ups closed 2026-09-02: nb 81's consume now shows a real live `op=u` CDC envelope (triggered by a one-row UPDATE to the `cdc_demo` demo table); and the nb 51 MLflow model-LOAD in-pod was **deliberately NOT wired** — it stays a graceful note, because the MinIO artifact creds it needs are read as generic `AWS_*` and would collide across every notebook kernel + widen every spawn's blast radius for one already-degrading cell (scoped path documented in runbooks/jupyterhub.md if ever wanted).
- **B113** — **Financial / economic datasets domain (data-mesh)** — **FOLDED INTO B78 (2026-08-05)** — thread (c) of the data-mesh maturity bucket. Full scope retained here: extend the datasets-lake beyond health/music to a **financial + economic** domain, ingested + maintained the SAME way (a `DomainConfig` → `build_transform_assets` → `build_asset_checks` → `build_store_load_assets` → silver/gold Iceberg → Trino / DataHub / Tier-2 stores; the agents reach it via the existing `trino` / `datahub` fleet MCPs — **no paid third-party MCP** like Bigdata.com/RavenPack). **Free sources:** **SEC EDGAR** (company filings + XBRL company-facts API, free, no key), **FRED** (Federal Reserve economic series, free API key), **Alpha Vantage** / **Finnhub** (market/equity data, free tiers). Static snapshots into the mesh like every other dataset; **optional** later: a small scheduled live-refresh or a tiny self-built MCP if *currency* (today's filings/prices/news) is wanted — the free APIs above beat a paid institutional feed. Decided 2026-07-31 (vs Bigdata.com): self-host the open data, skip the enterprise vendor. Relates B1 (data mesh), B77 (DQ `@asset_check`/GE), B73 (dataset uses), B80. — **SCOPED 2026-09-02 → [docs/design/finance-domain.md](design/finance-domain.md).** Locked: **full domain, phased** (Phase 1 FRED skeleton [gate] → EDGAR XBRL + Neo4j graph → EDGAR filings-text RAG notebook → market OHLCV). Sources: FRED (~12-15 macro series), SEC EDGAR (XBRL company-facts + 10-K/10-Q text for ~50 companies), market daily OHLCV (~30-50 tickers; bulk-snapshot via stooq/yfinance — Alpha Vantage's 25/day is too tight, Finnhub reserved for later live-refresh). Richest domain by design: exercises **Timescale** (series), **vector/RAG** (filings), **Neo4j** (company graph), Iceberg/dbt/ClickHouse. Static snapshots only (live-refresh deferred). Contracts = **ODCS** (ties B157); onboarding it is the live test of **B156**'s 5 capabilities. 8-pillar DoD acceptance criteria in the design doc. — **PHASE 1 (FRED skeleton) DONE 2026-09-03.** FRED macro (13 series / 40,930 obs, 1919→2026) end-to-end: `datasets_finance_fred_land` → silver (parquet/arrow/avro/lance) → Iceberg gold + Nessie commit → **Timescale + ClickHouse** → dbt `mart_macro_indicators` (latest + YoY). **Consumption (Lightdash + Superset + Cube — NOT Grafana; the first-cut design named Grafana for BI, corrected to the mesh's actual BI plane):** Cube `macro_indicators` cube; Lightdash `Finance — marts overview`; Superset `Weyland Marts — Finance` (snapshot) + `Weyland Finance — Macro Time Series` (raw fred_macro line charts over ClickHouse). DataHub **Finance** domain (21 assets) + **Macro Indicators** product (8) + glossary (Finance Concepts + 4 terms); Soda `weyland_finance` 10/10 green; finance sections in the per-store query cookbooks (trino/dbt-marts/timescaledb/clickhouse/gizmosql) + `flow-finance-ingestion.md` + `demos/finance-domain.md`. **Platform fixes found + fixed en route:** soda↔Trino TLS regression (all domains — trino-client 0.339 auth-over-http → `auth_type: NoAuthentication`); Timescale hypertable **chunk-explosion segfault** (loader default 7-day chunks → 3,891 chunks on the 107-yr span crashed the shared `timeseries` DB → `chunk_time_interval => 5 years`); Superset seed pagination + dashboard-chart association; Lightdash metric/dimension name collision. — **PHASE 2 (SEC EDGAR XBRL + company graph) DONE 2026-09-04.** EDGAR company-facts + submissions for ~49 mega-caps end-to-end: `datasets_finance_edgar_land` → **company_financials (20,741 facts) · company_meta (49) · company_filings (1,144)** → Iceberg + ClickHouse + **CockroachDB + MySQL + MongoDB** → dbt `mart_company_financials` (latest-annual pivot, revenue-union fix so recent revenue isn't dropped) + a **Neo4j** graph — `(:Company)-[:IN_INDUSTRY]->(:SIC)` (49/28) + `(:Company)-[:FILED]->(:Filing)` (1,144). DataHub **Company Financials** product (20 assets), Finance domain now 39; Soda **22/22** finance checks; Lightdash + Superset + **Cube** (`company_financials` cube) consumption; cookbook (trino/dbt-marts/neo4j/gizmosql/cockroach) + storage-grid rows + flow-diagram Phase-2 flowchart + demo. **Bugs found + fixed en route:** revenue first-present-wins → union both us-gaap concepts; edgar_land not registered in assets/__init__; Neo4j shared-`:Company`-label cross-clear wiped FILED edges → `clear_labels`. **MySQL + MongoDB fan-out FIXED + shipped (2026-09-04, image `git-8659183e`):** the two general loader defects that had blocked EDGAR are solved and validated live (MySQL 20,741+49 rows, MongoDB 20,741+49 docs) — the MySQL loader self-provisions its database (`CREATE DATABASE IF NOT EXISTS` + a `mysql.yaml` `--init-file` schema grant, so no per-dataset root grant ever again) and the MongoDB loader casts date columns to timestamp so BSON encodes them (`mongo_encode.to_bson_encodable`, unit-tested). company_financials + company_meta now fan out to **five** stores. — **PHASE 3 (SEC EDGAR filings-text RAG) DONE 2026-09-04.** Each mega-cap's latest **10-K** narrative → `datasets_finance_edgar_text_land` (bs4 HTML-strip → `edgar_text_parse` **section-aware** chunker: Business / Risk Factors / Legal Proceedings / MD&A / Market Risk, whole-doc fallback; the section regex anchors on canonical Item titles + last-occurrence-skips-TOC, validated against a real Apple 10-K) → `filings_text` silver + Iceberg gold → the **vector fan-out** (bge-small 384 → Qdrant/Weaviate/LanceDB, collection `datasets_finance_filings_text`, payload = ticker/accn/section/chunk_id/text for citations) → **`63_rag_sec_filings.ipynb`** (retrieve→cite→answer via `wl-rag`, grounded-vs-no-context, section-filtered retrieval). DataHub **SEC Filings** product; Soda `filings_text` checks; new dep `beautifulsoup4`; TDD `test_edgar_text_parse` (7 tests) + real-filing validation. Design call: 10-K only (not 10-Q — quarterly deltas 4× the corpus for little narrative gain), narrative Items only (Item 8 tables already in the XBRL mart). — **PHASE 4 (market OHLCV) DONE 2026-09-04.** Full daily price history for the same ~50 mega-caps → `datasets_finance_market_land` (**yfinance** — the design's other free source **stooq** now gates its CSV behind a JS proof-of-work wall, verified; `market_parse` drops NaN/in-progress bars, `auto_adjust=False` keeps raw close + adj_close) → `price_daily` (ticker/date/OHLC/adj_close/volume) → Iceberg + **TimescaleDB hypertable** on `date` + ClickHouse + **Cassandra** (partition by ticker — the one net-new store) → dbt `mart_price_daily` (latest close, daily return, 30d volatility daily+annualized, 52-week hi/lo, pct-off-high). Consumption: Lightdash/Superset price charts + Cube `price_daily` cube. DataHub **Market Prices** product; Soda `price_daily` checks (incl. `duplicate_count(ticker,date)=0`); TDD `test_market_parse` (6 tests) + real-yfinance validation (128 raw→127 clean); new deps yfinance+curl_cffi+lxml+multitasking+peewee. Verified in-cluster: Yahoo egress works from the weyland ns. — **PHASE 5 (ML lane) DONE 2026-09-05** — the finance genre-classifier analogue, and **B113 IS NOW COMPLETE (all 5 phases)**. `mart_price_features` (471,354 per-(ticker,date) trailing features — lagged returns / realized vol / volume / range / SMA, no leakage) → Feast `price_features` view (entity ticker, time-varying on date; loaded silver→feast-Postgres by `feast_setup.py`) → `price_feast_training_set` Dagster asset (MESHED Feast `get_historical_features` point-in-time join + the forward-5d-vol target in the entity_df; the leakage-prone target math extracted to `ml_targets.py`, **TDD 5 tests**) → **122,208** training rows → lakeFS → the external **`services/finance-trainer/`** (mirrors genre-trainer: reads the lakeFS parquet, `--task both`, Ray-Tune-capable) → **BOTH models registered live in MLflow**: `price_volatility_regressor` v1 (RMSE 0.0104, **R²=0.43**) + `price_volatility_classifier` v1 (vol-regime HIGH/LOW at per-ticker median, **acc 0.636 / F1 0.637**). Design call vindicated: volatility clusters and IS learnable (R²>0, acc≫0.5), whereas a returns/direction model would be the ~coin-flip trap. En route the FR1.5 ship gate was fixed for real (feast rebuilds byte-identically → kubelet reports a stale digest tag; `deployed_tags_for` now reads declared `.spec` images, bats-guarded). **Deferred/tracked:** ODCS contracts (B157). **B113 DONE — EMA-110 flipped to Done (2026-09-05).** — **FULL 8-PILLAR DoD AUDIT 2026-09-05.** Re-verified end-to-end, mechanism not box-check. **Guards (12, all green):** mermaid (157) · doc-counts (78) · app-registry · sa-automount · quality-tools · secret-placeholders · linear-sync (38 refs, no project-less) · port-iac · cron-freshness (14) · servicemonitor (33) · dashboard-coverage (38) · alert-coverage (38). **Data plane (live via Trino/Neo4j):** marts `mart_macro_indicators` 13 · `mart_company_financials` 42 · `mart_price_daily` 50 · `mart_price_features` 471,354; silver `datasets_finance.*` — fred_macro 40,930 · company_financials 20,741 · company_meta 49 · company_filings 1,144 · filings_text 8,851 · price_daily 472,354/50 tickers; Neo4j 49 companies/28 SIC/1,144 filings/1,144 FILED — every count matches the phase claims, and the named Soda invariant `duplicate_count(ticker,date)=0` re-derived independently (holds). **Pillar 5:** EMA-110 = Done (project Weyland Lab, High, parent EMA-69). **Pillar 7:** scan-suite RUN — 0 CRITICAL; bandit/semgrep 1H each = the documented pre-existing pair, bandit on the B113 finance code is clean, the 1 pip-audit exception justified+wired. **6 GAPS FOUND + FIXED (Pillar 8 reverse-sweep / Pillar 1):** (1) `data-mesh-map.html` undercounted the mesh (2→3 domains, 9→13 products, finance prose added); (2) LikeC4 modelled `genre-trainer` but not `finance-trainer` → `financeTrainer` component + lakeFS/MLflow edges added; (3) `platform-map.html` had no finance-trainer row → added; (4) query cookbooks `weaviate.md`/`mysql.md`/`mongodb.md` held finance data but had no finance section → real-column sections added (`DatasetsFinanceFilingsText`; MySQL `company_financials`/`company_meta`; Mongo `datasets_finance`); (5) design-doc OpenSearch `Y*` aspiration reconciled to "not shipped — filings served via vector RAG"; (6) `demos/finance-domain.md` H1 was Phase-1-only → retitled to all 5 phases. DataHub live-count not machine-verified this session (fleet datahub MCP tools down on the gateway; GMS needs a token) — verified by the codified emit + the data plane it mirrors + the map-count corrections, with eyes-on UAT at `datahub.weyland.lab` the standing Pillar-3 step.
- **B114** — **Stud.IO → Figma design system (code → design)** — **FOLDED INTO B39 (2026-08-05)** — the code→design half of the Figma↔code workflow bucket. Full scope: extract Stud.IO's UI into a **Figma design system** and manage the experience there, keeping it linked back to code. Bidirectional via the **official Figma MCP** (the Claude Code `figma` plugin — **not** a Bifrost MCP-library install; that entry is agent read-context only): **`figma-generate-library`** discovers Stud.IO's components + design tokens *from the codebase* and builds the Figma library; **`figma-generate-design`** assembles the pages/screens in Figma; **Code Connect** (`figma-code-connect`) maps Figma components ↔ Stud.IO code so design and code stay in sync as UX is managed in Figma. An interactive Claude Code workflow (point it at the Stud.IO repo), not a standing agent tool. Relates **B46** (Stud.io product backlog).
- **B116** — **chat.weyland.lab (Open WebUI) workspace review** — **MEDIUM (2026-08-05 — promoted HIGH as an "active user-facing surface"; ↓ High→Medium 2026-08-27 — the premise was wrong, it isn't in use yet. Polish on an idle UI. Re-promote when chat.weyland.lab becomes a surface that actually gets opened).** go through the Open WebUI workspace end-to-end and curate it into a coherent lab assistant rather than a raw model list: **model connections** (Ollama native + the B115 `nemo-guardrails` guarded `weyland-operator` lane — incl. why a new OpenAI-connection model may not surface without an admin-panel refresh / per-user model visibility), **personas / system prompts**, the **prompt library**, **tools / functions**, **RAG / knowledge** collections, **voice** (STT whisper shim + a TTS lane, e.g. Kokoro), **users / RBAC + Keycloak SSO** defaults, and general **UX / settings**. Prompted 2026-08-03 while wiring the B115 Dialog guarded model (the guarded lane didn't appear in the picker — a workspace-config gap, not just the connection). Relates B13 (Open WebUI), B115 (guardrails Dialog).
...
- **B119.1** — **OKRs in Linear initiatives (objective = initiative, key results + check-ins native)** — **MEDIUM (2026-09-25; ↓ High→Medium 2026-09-26 — important, but build work comes first; KR drafts, format and cadence are parked in EMA-247 ready to resume). Linear EMA-247, sub-issue of B119, which is DONE — B119.1 is its open follow-up.** No free OKR tool has both a Linear integration and an MCP server (Tability has both but is trial-only; Perdoo / Weekdone / Profit.co have neither), while Linear Initiatives (6, adopted 2026-09-24) already hold the goals and the work and are MCP-readable. Objective = initiative; key results = measurable targets in a fixed format in each initiative's description (metric, baseline → target, date); check-ins = native initiative updates (On track / At risk / Off track). Only gap: no KR score — a small Linear-API script if numbers are wanted. Comparators seen in the walk (2026-09-26): Hill Charts (Curious Lab — hill charts for issues, projects or initiatives; Routific — Shape Up flavor) as a visual progress view for initiative OKRs, and DoubleLoop (impact-oriented strategy maps — what initiatives + OKRs do natively). Scope: KR format + cadence → draft 2–4 KRs per initiative for the owner to approve → write them in → decide on scoring (+ an optional guard that every initiative has KRs). **The cadence is enforced natively:** Settings → Initiatives → **Update schedule** (currently "No expectation for updates") sends the initiative owner reminders to post check-ins — set it as part of this item; once B184's Slack lands, initiative updates can also post to a Slack channel. **Pulse** is already on (2026-09-26, personal schedule Weekly) — the check-ins will surface there.
...
- **B53** — **Sentry SaaS** (free tier) — **LOW (2026-08-05; reviewed 2026-08-27 — KEPT, and re-framed as a candidate SLICE of [B104] rather than a standalone build; linked in Linear).** **Dropping it was proposed and rejected — both arguments for dropping were wrong.** ① *"No learning delta, GlitchTip is Sentry-compatible"* is **false**: Sentry-*compatible* is not *equivalent*, GlitchTip is deliberately a lightweight subset, and the delta is substantial — **performance monitoring/tracing** (GlitchTip has essentially none) · **cron / check-in monitoring**, which this lab **hand-rolled** as `check-cron-freshness-budgets.sh` + the Dagster freshness CronJob + `ScheduledJobStale` rules · session replay · profiling · release health/adoption · **suspect commits** (error → introducing commit, adjacent to the EMA-172 DORA work) · source-map handling · richer alert routing. ② *"Ships data off-LAN, counter to the LAN-only choice"* is **overstated**: the lab already sends data off-LAN where it matters — **Port.io is cloud and is the IDP**, **Linear is cloud and is the declared status source of truth**, and Bifrost egresses to ~15 external providers. "LAN-only" means *the platform is self-hosted*, not that nothing may touch a cloud. **Where the value actually is: the comparison** — *what does the heavy commercial version give you over the lightweight self-hosted one* is precisely B104's question, and the free tier (5k errors/month) makes it genuinely $0. **Still LOW because GlitchTip covers the core need and nothing is failing** — which is a reason for Low, not for drop. (The original note below said the Port↔Sentry edge was "moot with Port at LOW"; [B60] is **Medium**, confirmed 2026-08-27.) Deferred from B51 to Extras. Would add a native Port↔Sentry integration (Ocean exporter, richer than the GlitchTip webhook) + a cloud sink. **Why deferred:** GlitchTip already covers error tracking AND GlitchTip→Port works, so the only delta is shipping app errors **off-LAN to a third-party cloud** — counter to the self-hosted/LAN-only choice that picked GlitchTip over SaaS. Optional/demo-only; revisit only if a Port-native Sentry demo is specifically wanted. **+ Sentry ↔ Linear integration (added 2026-09-25, from the B119 integrations walk):** Linear's official Sentry integration turns a Sentry exception into a Linear issue (or links an existing one), shows the linked issue in Sentry's sidebar and the Sentry link on the Linear issue, and can **auto-create issues from Sentry alert rules**; it cannot target private Linear teams. Enabled from Linear workspace settings. Part of the comparison: does GlitchTip offer any error → Linear issue path, or is this another piece of the commercial delta? Plan tier for the integration on Sentry's free Developer plan is unverified — check before relying on it.
...
21. **U13** — Slim sentence-transformers image / ONNX evaluation — **DONE 2026-09-14 (option (a) ONNX-only; torch dropped from all four mother CPU images, deployed + prod-verified).** Was HIGH (↑ Medium→High 2026-09-06). Decision resolved to (a) swap to ONNX only: all four images (tool-server, agent, dagster, guard) run bge embedders + DeBERTa classifiers on raw ONNX Runtime, byte-equivalent so no vector re-hydration; dagster 8.38→3.77GB. See detail below.
...
**The sourcing problem — DONE DECIDED 2026-06-18 → Option 2 via MinIO + brand-neutral.** The content is the
...
**Architecture SCOPED 2026-06-20 (directional)** → `design/data-mesh-design.md` (full 8-layer stack +
...
### B2 — Hermes (agent platform) — DONE built · RETIRED 2026-07-23
...
- **DONE Slice A — Software Catalog (LIVE 2026-06-19).** Scaffolded Backstage (`services/weylandidp/`, new
...
- **DONE Slice B — TechDocs + Catalog Graph (LIVE 2026-06-19).** `docs/` renders in-app as TechDocs: built
...
- **PAUSED — Slice C — Scaffolder golden-path template (BUILT, PARKED → B42).** The "New k8s service" template
...
- **Observability / tracing** — watch prompts, responses, tool calls, latency, cost, RAG retrievals:
 - **Langfuse** [TOP] — go-to self-host LangSmith replacement: tracing + prompt mgmt + eval + datasets;
 framework-agnostic (works with the OpenAI-compatible Ollama/tool-server); Postgres-backed.
 - **Arize Phoenix** [TOP] — best for RAG debugging: OpenInference/OTel traces, retrieval/embedding
...
- **Evaluation** — score output quality:
 - **Ragas** [TOP] — RAG-specific metrics (faithfulness, answer relevancy, context precision/recall) —
...
`design/b8-istio-design.md`. Shape: **Approach 1** — sidecar mode, first slice =
...
- **DONE SLICE 1 MESHED & VALIDATED 2026-06-18:** istiod (minimal) + Kiali + Jaeger up; **qdrant + weaviate +
...
- **DONE Slice 1b DONE 2026-06-18 — neo4j + Postgres meshed.** The TCP-protocol break (Bolt 7687 / pgvector
 ...
 meshed (PERMISSIVE).**
- **DONE Slice 2 (STRICT) DONE 2026-06-18.** Dagster meshed (all 3 pods — gRPC code-location via the named
...
- **DONE Observability consolidated 2026-06-18.** (Was: Kiali on the addon Prometheus, a deliberate bring-up
...
- **Kiali security follow-up (2026-06-18):** the Istio addon ships demo-grade defaults. Hardened: `view_only_mode:
 true` (read-only) + ClusterRole read-only on Istio CRDs + non-placeholder signing key. **DONE Ingress auth DONE
...
### B9 — Refactor Python scripts → Go binaries — LOW (kept deliberately; see the strategic rationale)
...
**STRATEGIC RATIONALE — recorded 2026-08-27 (operator).** This item is **kept on purpose even though it will not outrank anything for a long time**, and it should not be scored on present-day utility. The lab is expected to split into a **cohesive platform + applications**, and that split means *code* — at which point managing the system as a pile of shell scripts and Python becomes the wrong substrate rather than merely an unloved one. Stated plainly by the operator: *"I hate shell, and I think that managing this system as a bunch of shell files and Python is bad juju."* The value curve is back-loaded; a Low that never rises is normally a drop candidate, but **that heuristic does not apply here** — this one is waiting on an architectural event, not on interest.

**The bats investment is NOT in tension with this — it is the enabler.** `team.md` mandates shell tests and this repo now carries substantial bats coverage (guards, CI checks, reconcilers). Those suites are the **behavior contract**: they are what makes a port verifiable rather than a rewrite-and-hope. Each tested script arrives at the migration with an executable spec its replacement must satisfy. Do not read "we are hardening shell" as evidence against replacing it later.

**When this activates:** the platform/application split becoming real work. At that point the open questions above (which scripts, Go vs alternatives) get answered against the split's actual boundaries rather than in the abstract — which is also why answering them *now* would be premature.
...
**DONE Status (2026-06-15) — DONE; both halves shipped in shadow.** The enforcement *promotions* below are
...
`shadow→flag/block` → **B35** (DONE done); (2) the enforcing act policy gate (allowlist/rate-limit/`block`) →
...
### B104 — AI dev-tooling landscape evaluation ("tech nerd" survey) — **DONE 2026-09-10.** (An earlier same-day DONE was retracted for missing the DoD Pillar-3 UAT — eyes-on every UI — and reinstated only after every UI/operator deliverable was actually eyes-on confirmed by the operator: **Grafana dashboard**, **Bruno** desktop app, **Emdash**, and the **Keploy** 7/7 replay. Lesson kept in the sub-items below.) **Delivered + validated:** (1) **perf baseline** — k6, on-demand bounded harness → `tests/perf/baseline.tsv` + runbook; (2) **survey refresh** ([concepts/ai-dev-tooling-survey.md](concepts/ai-dev-tooling-survey.md)); (3a) **coding-agent eval harness** (`eval/coding-agents/` + `scripts/coding-agent-eval.sh`, fail-closed, 14 bats, real codex contract observed); (3d) **regression ratchet** (`scripts/perf-ratchet.sh`, 11 bats). **Built but NOT yet UAT'd / finished:** ~~**Bruno**~~ — **DONE 2026-09-10.** `bruno/weyland/` = **27 requests / 58 assertions across 9 services** (incl. the authed example), real read/health coverage of the LAN serving+backend surface [tool-server boundary incl. all 5 vector backends + a functional retrieval POST · LiteLLM gateway · Qdrant · Weaviate · Neo4j · Ollama · rag-embed · ClickHouse · MLflow gateway], CLI-validated green AND **desktop-app eyes-on confirmed by the operator** (requests render + send green). **Gotcha recorded:** the Bruno **snap** renders its file dialogs as tofu (□) — its confined 2018-era `gnome-3-28-1804` GTK platform can't resolve host fonts; fix is the unconfined **.deb/AppImage** (v4.1.0), not the snap. Steps in the runbook + README.; ~~**Keploy**~~ — **UAT GREEN 2026-09-10**: recorded `golden-python-fastapi` → `keploy test` **7/7 pass, dependency-free** (`verified_green`); corpus at `keploy/keploy/golden-fastapi-smoke/` (config + mappings + 7 tests; no mocks — no downstream deps; reports gitignored). Gotchas found + fixed: Ray owns rogueone:8080 incl. its metrics → app moved to **8091**; **same-port** mapping required (a host≠container remap breaks keploy's readiness probe + replay routing); `healthPath: /health` so replay skips the 3-min readiness timeout. **CI-step attempt TESTED + reverted (pipeline #115):** a var-gated k8s `keploy-api-tests` step failed with exit 127 — **keploy needs Docker** (installer refuses without it; keploy runs its agent inside a docker container on Linux), and a k3s step pod has containerd/no docker. Not the eBPF/privileged worry (privileged pods run fine here — buildkitd). So keploy stays **on-demand on the host** via `scripts/keploy-verify.sh` (rogueone has docker + proven keploy 7/7); the CI path — a **local-backend** workflow on rogueone via a `.woodpecker/` split — is documented in [runbooks/woodpecker-step-menu.md](runbooks/woodpecker-step-menu.md) as the option if a real (deps-having) service ever makes it worth the restructure. ~~**Emdash**~~ — **DONE 2026-09-10.** Installed (`generalaction/emdash` v1.2.4, the `.deb`; NOT the npm `emdash` CMS) + **eyes-on confirmed by the operator**: added the weyland project → created a task → it opened a **git worktree** and drove a live **Claude Code agent** in it. Recipe in [runbooks/parallel-agent-supervisor.md](runbooks/parallel-agent-supervisor.md); ~~**Grafana dashboard**~~ — **DONE 2026-09-10**: pushed → Argo-synced into Grafana (uid `k6-perf`), a fresh k6 remote-write datapoint pushed, all 6 panels' queries return live data, and **eyes-on confirmed by the operator** (panels render with data). See the "remaining to DONE" checklist appended below. _(was HIGH; ↑ Low→Medium 2026-08-11; ↑ Medium→High 2026-08-27. Pairs with [B86].)_
...
- **Security — SAST / SCA / cloud** — Snyk · Veracode · Checkmarx · **Semgrep** (DONE B47) · Wiz. Semgrep OSS already runs;
...
slot vs which get desk-noted." Realistic build candidates = the OSS ones (SemgrepDONE, Playwright, k6/JMeter/Gatling,
...
### B108 — Generalize the demo-mode toggle across shadow systems — LOW (↑ Low→Medium 2026-08-11 rebalance; ↓ Medium→Low 2026-08-27 — the entry's own gate ("don't build a framework before there are ≥2 real consumers") is still unmet: weyland-guard remains the ONLY system with a mode concept, so this is not "generalize a toggle" but "invent a mode primitive for four systems that lack one." No driver. Let B87 (E2E demo sweep) promote it if the sweep surfaces a second real consumer)
...
### B127 — Migrate rogueone off Docker Desktop → native Docker Engine (+ STUD.io data-plane consolidation) — **DONE (2026-08-27)**

**⛔ THE PACKAGE IS DELIBERATELY NOT UNINSTALLED — removing `docker-desktop` BREAKS the native Docker engine.** The original scope item (3) said "uninstall Docker Desktop" and that instruction is WRONG; it stayed in this entry long after the decision was reversed and caused the plan to be re-proposed twice on 2026-08-27. Desktop is **retired in place** instead: the systemd user unit is `disabled` + `inactive`, and the default docker context is `default` → `unix:///var/run/docker.sock` (native). The package and `/opt/docker-desktop` stay on disk permanently. Do not "finish the job" by removing them.

**THE 574 GB WAS RECLAIMED WITHOUT UNINSTALLING (2026-08-27)** — the space was never in the package, it was in one sparse VM disk image:
```
rm -f /home/edwardmangini/.docker/desktop/vms/0/data/Docker.raw
```
`~/.docker/desktop` 574G → **57M**; `/` 54% → **33%** (1.4T used → 856G, 1.8T free). Native engine unaffected (29.7.1, 8 containers running); every STUD.io container stayed `Up` through it without restarting. If Desktop is ever started again it simply builds an empty disk.

**Pre-delete verification — what was inside, and why it was safe.** The VM held a full second Docker namespace: ~24 images (~35 GB, all re-pullable from `registry.weyland.lab`), ~140 anonymous scratch volumes, and named volumes including **stale duplicates of `studio_studio_db_data` and `studio_studio_minio_data`**. Confirmed before deleting, not assumed:
- **STUD.io database** — live copy is the NATIVE volume (`/var/lib/docker/volumes/studio_studio_db_data/_data`, `studio_db` healthy). Plus two checksummed dumps at `~/Documents/Studio/backups/{b127-20260813,cutover}/masterdb.dump` with a `row-counts.baseline.txt` (gear=5, scanner_plugin_links=912, …) and the nightly `studio-masterdb-backup.timer` → MinIO (180-day ILM).
- **STUD.io photos — the near-miss.** The native `studio_studio_minio_data` volume is **116 KB containing only `.minio.sys`** — an empty MinIO nothing uses. The live photos are in **weyland's** MinIO: the running app's env is `MINIO_ENDPOINT=http://192.168.1.243:30990`, `MINIO_BUCKET=studio-photos`. Listed the bucket directly and confirmed all 5 gear photos present (103/132/50/74/44 KiB, UUIDs matching `photos.manifest.txt`), plus the same 5 on disk in the backup. **Checking only the database would have looked reassuring and proved nothing about the photos** — the empty native volume is exactly the artifact that would have caused panic *after* an irreversible delete.
- **Accepted loss:** `studio_sonarqube_*` / `dev_sonarqube_*` existed ONLY in the VM, so pre-2026-08-14 SonarQube analysis history is gone permanently. Deliberate (operator, 2026-08-27). Sonar itself moved to weyland (`:30969`, `controlroom` project) so nothing breaks forward.

**Original freeze justification is void — see [B150].** This entry once argued Desktop caused rogueone's freezes; that was falsified here on 2026-08-13, and B150 has since traced them conclusively to **failing P-core 8 (CPUs 4/5)**, with the segfaults on core 12 (CPUs 6/7). B127 was still worth doing for the disk and the redundant engine — just never for that reason.

**Remaining tail (small, tracked here, not blocking):** `scripts/gpu-docker` still exists though native is now the default context, so the `DOCKER_HOST` wrapper is redundant. `docs/runbooks/gpu-inference.md` was corrected 2026-08-27 (it still claimed Desktop was the default context and that a bare `docker` would fail); `hosts.md` / `api.md` / `arch.md:175` were already accurate. `arch.md:152` and `runbooks/remote-training.md` rows 7/10 retain Desktop RAM-cap gotchas as historical record.

Linear: EMA-147. Relates [B150] (the freeze root cause this entry once mis-attributed), B111 (GPU serving on native), B79.

---

### B127 (original entry) — MEDIUM (2026-08-12; expanded 2026-08-14)
**WARNING — SCOPE EXPANDED + PROGRESS (2026-08-14). Full plan: `design/b127-desktop-to-native-migration.md`; memory `b127-docker-native-migration`.** B127 grew from "uninstall Desktop" into **retire Desktop + consolidate STUD.io's shared infra onto weyland** (SonarQube + MinIO central; masterdb stays local but backed up). **ADDITIVE WORK DONE — no STUD.io downtime:** (a) MinIO S3 LAN NodePort **:30990** + scoped `studio-svc` user + buckets `studio-{photos,downloads,backups}`; (b) **nightly masterdb backup** (`studio-masterdb-backup.timer` → `wl/studio-backups`, 180-day ILM) — **production is now off the laptop**; (c) Sonar API NodePort **:30969** + `controlroom` project + analysis token; (d) genre-trainer repointed to native. **REMAINING = the STUD.io-quiet-window cutover:** app `compose`→native (adopts the pre-loaded + verified `studio_studio_db_data`), sonar/minio/agent-`.env` repoints, drop `studio_minio`+`sonarqube` from compose, uninstall Desktop → **reclaim ~574 GB** (its qcow VM disk). Backup floor: `~/Documents/Studio/backups/b127-20260813/` (checksummed).
...
### B18 — Spotify integration (Hermes tool) — RETIRED CLOSED 2026-07-29 (Hermes retired)
...
`design/mcp-gateway-design.md`. **DONE Anti-spoof DONE 2026-07-29** — the tool-server act endpoints are
...
still passes. **DONE MCP server fleet DONE 2026-07-29** — 6 **read-only** MCP servers (grafana · trino · k8s · postgres ·
...
design `design/mcp-server-fleet-design.md`. This *earns* the aggregation trigger (≥2 MCP servers).
**DONE Composition DONE 2026-07-30** — `weyland-mcp-compositor` (FastMCP `create_proxy`) aggregates the 6 servers → ~90
namespaced tools; gateway routes **`/mcp-fleet` → compositor** (`/mcp` stays → tool-server for RAG). **DONE Operator wired
...
The B14 read+act slice (`design/b14-readact-design.md`) deliberately built the *boundaries*
...
### B20 — Home Assistant integration — LOW (↓ Medium→Low 2026-09-06) — Maturity
...
design doc: **[docs/concepts/home-assistant.md](concepts/home-assistant.md)** — **moved out of the gitignored `aidlc-docs/` on 2026-08-27**, where it was untracked and therefore existed on exactly one disk while this entry cited it as the source of truth (same class as the live-only Port config [B137] fixed). $0 except an optional one-time **$5** Nest Device Access; **no Nabu Casa**.
**Lands on weyland (Proxmox), not mother** — so it costs nothing on the capacity-contended k8s node and is unaffected by the rogueone hardware situation ([B150]).
...
### B22 — SearXNG (self-hosted search) — privacy-max — LOW (reviewed 2026-08-27)
Optional replacement for **Tavily**. Web search is inherently off-LAN, but SearXNG removes the **cloud
middleman**: queries go from your box straight to search engines — self-hosted, free, no key. **Cost:**
another service to run + rawer (non-agent-tuned) results vs Tavily's clean search+extract. **Not urgent** —
Tavily works fine indefinitely. **Revisit if the Tavily middleman/quota becomes a concern**, or for
privacy/learning.

**CONSUMER LIST CORRECTED 2026-08-27.** This entry said "current search backend for **OpenClaw + Hermes**"
and that "**both agents** re-point at `SEARXNG_URL`" — **Hermes is retired** ([B66]'s operator replaced it),
so that list named a system that no longer exists. The real consumers to re-point are the **B66 operator**,
the **[B17] realm-of-agents** fleet, and the **tool-server**. Confirm the live set before scoping; do not
trust the pair named above.

**Why it stays LOW, stated plainly so it is not re-litigated:** unlike [B53] — where Sentry has a genuine
feature delta over GlitchTip — **SearXNG's delta versus Tavily is negative** by this entry's own admission
(rawer, non-agent-tuned results). The gain is *privacy*, and that argument is weak in a lab whose IDP
(Port.io), status source of truth (Linear) and LLM egress (Bifrost → ~15 providers) are all cloud. By the
[B85] admission bar it is also **not a new category** — search is covered; this is a different implementation
of a solved problem. Kept because the trigger above is real, not because a case has been made.
...
### B27 — Enable Hermes Kanban skills — RETIRED (→ Linear)
...
**Design (finalized 2026-06-17): `design/b27-kanban-design.md`** — scope (a) self-management
...
**DONE EVALUATED 2026-08-03 → ADOPT.** Real trial (`scripts/nemo-guardrails-trial/`) confirmed NeMo's niche is
...
### B36 — Hermes dashboard performance (Maturity / Hardening / Polish) — RETIRED CLOSED 2026-07-29 (Hermes retired; the dashboard it optimized no longer exists)
...
### U13 — sentence-transformers image slimming — HIGH (↑ Medium→High 2026-09-06)
...
- **DONE 2026-09-14 — decision: OPTION (a), ONNX-only (torch fully dropped from every in-scope image). Deployed + verified in prod across all four services.** Migrated all four mother CPU images off `sentence-transformers`/`torch` to raw ONNX Runtime (`onnxruntime` + `tokenizers` + numpy). Embeddings/logits proven **byte-equivalent**, so **NO vector re-hydration** (Qdrant/Weaviate/Neo4j/pgvector/LanceDB untouched):
  - **weyland-tool-server** (bge-base 768): 2.57→**1.38 GB**; torch absent; embed 768/norm 1.0.
  - **weyland-agent** (bge-base 768, `retrievers.py` `OnnxBge`): **2.06 GB**; torch + sentence-transformers absent, `llama-index-core` KEPT (BaseRetriever/NodeWithScore/schema); embed 768/norm 1.0.
  - **weyland-dagster** (bge-base 768 + bge-small 384): **8.38→3.77 GB (−4.6 GB)** — pruned torch's entire CUDA-13 wheel stack from `requirements.txt` (torch/transformers/sentence-transformers/triton/nvidia-\*/cuda-\*, 22 pins; +onnxruntime). New `resources/onnx_embedder.py` (`OnnxEmbedder`, a SentenceTransformer-shaped `.encode`) drives BOTH `SentenceTransformerResource` (KB, bge-base) and `datasets_lib/loaders._embedder` (hydration, bge-small); base=768/small=384 both norm 1.0; resource + loaders import clean; single==batch-row diff 0.0.
  - **weyland-guard** (two DeBERTa classifiers): **2.38 GB**; torch/transformers/sentence-transformers absent, Presidio+spaCy KEPT. New `guardrails/validators/onnx_runtime.py`: `OnnxCrossEncoder` (grounding.nli 3-logit NLI — logit diff **0.0** vs sentence-transformers CrossEncoder) + `OnnxTextClassifier` (prompt_guard.injection — scores match the transformers pipeline to ~1e-9). Every SHADOW verdict + calibrated threshold (`GROUNDING_THRESHOLD=0.15`, `PROMPT_GUARD_THRESHOLD=0.5`) stays valid unchanged; **18/18 guard unit tests pass** (grounding + safety + config; prompt_guard test seam switched from a `transformers.pipeline` stub to a `classifier=` injection). `llama_guard.safety` = remote httpx svc, `pii.presidio` = spaCy — neither pulled torch, untouched.
  - **Scope notes (all six build contexts accounted for):** `weyland-dagster/Dockerfile.feast` was already torch-free (feast-only — no change); `rag-embed/requirements.*` **deliberately EXCLUDED** — resident GPU embed service on rogueone where torch/CUDA is justified.
  - **Pattern:** each image is multi-stage — a throwaway builder stage exports the model(s) → ONNX (torch lives ONLY there, thrown away), the runtime stage runs the export on raw onnxruntime + tokenizers. bge pooling = CLS token + L2 normalize; DeBERTa classifiers feed input_ids + attention_mask (no token_type_ids). Verified locally in-docker per [B160]/CLAUDE.md (torch absent + equivalence + size) before hand-off; CI confirms.
  - **Deploy + prod verification (2026-09-14):** **tool-server + dagster** shipped via CI (`ship-images.md`) at `git-5de488fe` (PR #93); **guard + agent** hand-built+pushed on rogueone (NOT in `images.tsv`, B135 ph2) as `weyland-guard:v12` + `weyland-agent:v8` with manifest bumps. Prod checks (read-only, in-cluster): tool-server `/context/search` retrieves (similarity 0.749 against the ST-hydrated index → byte-equivalence holds in prod, no re-hydration); dagster code location `LOADED` with the `onnx_embedder` resource; guard `/ready` shows BOTH ONNX validators active (`grounding.nli` OnnxCrossEncoder + `prompt_guard.injection` OnnxTextClassifier) and a grounding check scored clean; agent `/ready` ok with OnnxBge loaded; `torch` confirmed ABSENT in every running pod. CI hardening done along the way: `.woodpecker.yml` 7 toolchain downloads + the `alr` install got `--retry`/fail-closed loops (transient GitHub 504s were failing the ship), and `ship-images.sh` `SHIP_POLL_TIMEOUT` 60m→120m (the FR1.3 poll was bailing on the ~50-min pipeline and orphaning bump PRs). Stale `weyland-tool-server.yaml` "keep the CPU-only torch layer" comment corrected.
- **DONE 2026-09-14.** Scope was six build contexts; all resolved (`Dockerfile.feast` was already torch-free, `rag-embed` deliberately excluded — GPU). Linear EMA flipped. See the deploy bullet above for the per-service prod evidence.
...
- **2026-09-26 — n8n HAS A REAL JOB (found in the B119 Linear integrations walk).** The directory lists four hosted
  ...
  **Constraint (found 2026-09-26):** n8n's **Linear Trigger** node is webhook-based — Linear must reach n8n's URL, and
  the lab's n8n is LAN-only, so trigger-driven workflows will not fire. Build on a **Schedule trigger + Linear API
  queries** (polling), the same no-webhooks rule as the rest of the lab.
...
- **Schema browsing** → **DataHub**, live today. `emit_weaviate()` enumerates every class with description + lineage back to `datasets_health_weaviate_load` / `datasets_music_weaviate_load`.
- **Object inspection + search** → **[B81]'s notebook library**, which already lists *"vector/graph (Qdrant/Weaviate/Lance similarity search)"* in its full-stack set. **This fold makes that notebook the explicit deliverable for U16's scope.** — **DELIVERED 2026-09-01: `31_vector_weaviate.ipynb`** (class/object browse, `near_vector`, BM25, hybrid, raw GraphQL over the 12 live classes), validated headless-clean against live Weaviate. U16's scope is met.
- **Precedent** → LanceDB got the Lance Data Viewer, so "I want to look at vectors" has an answer that did not require hand-building a second frontend.

**Why the notebook wins for a lab:** no build, no deploy, no image to patch, no forward-auth wiring — and it doubles as *documentation of how to query the store*, which a React app does not. A bespoke frontend is the most maintenance-heavy option available for a store queried occasionally.
...
Doris (OLAP variety, on-demand) · Spark (big-data compute, on-demand) · RDF/triplestore (semantic-web, lightweight) · Okta · BigQuery · DynamoDB (cloud SaaS — zero local cost). Dropped candidates → [B67]. **Tier-3 re-eval folded into [B85] (data-store-mageddon v2, 2026-07-16) — verdicts: RDF [YES] graduate-when-needed; Doris ~ marginal; Spark/Okta/BigQuery/DynamoDB [NO].**
...
**DONE Dagster (Tier 1) — DONE via custom emitter (2026-06-26).** 17 datasets + lineage in DataHub (15 original + the 2 iceberg_export assets).
...
### B66 — Operator Agent Platform (consolidation — supersedes 13 fragmented agent items) — DONE CORE DONE 2026-07-24
**DONE SHIPPED 2026-07-24 — `weyland-operator`, a LangGraph pod on mother.** Text the lab from anywhere → it acts.
...
`design/operator-agent-design.md`. Brain = `gpt-oss:20b` (bake-off winner); Hermes/OpenClaw retired.
...
- **Workstreams (each absorbs an old item):**
 ...
 - **Ops** — ~~B36 (dashboard perf), B52 (Hermes error tracking)~~ both **RETIRED CLOSED/MOOT 2026-07-29** (Hermes retired).
...
### B68 — CSV / Google Sheets ingestion (DataHub) — CLOSED 2026-08-05 — won't-do (niche polish, no driver; the mesh's real sources are already ingested)
...
### B72 — Datasets landing zone + CSV→Parquet pipeline (music data) — DONE (date unrecorded — predates the DoD, introduced 2026-07-14)
...
### B74 — Retrieval precision (MATURITY) — DONE SOLVED 2026-07-28 (bge-base 768 + topic-prefix)
**Added 2026-06-27. RESCOPED to MATURITY 2026-07-21** after B96. **DONE SOLVED 2026-07-28.**

**DONE SOLVED 2026-07-28 — the middle rung won: bge-base (768-dim) + topic-prefix = clean sweep, no trade.** Swapped
...
**DONE Phase 1 DONE 2026-07-27 — diagnosis + first fix attempt (null result).** Inspected `/context/search` retrieval for
...
**Added 2026-06-26. Designed + started 2026-07-22** (full design: `design/agentic-rag-langgraph-design.md`). A **LangGraph agentic-RAG loop** — retrieve → grade → *reflect/re-retrieve if weak* → answer — with per-step **MLflow Traces** (`mlflow.langchain.autolog()` + `mlflow.llama_index.autolog()`), more capable than the single-shot `/context/ask`. **Feeds B66** (this is the LangGraph framework-viability spike).
...
- **DONE Part 1 — `weyland-guard`:** the B14 guard layer extracted into a shared FastAPI service (3 typed routes `/guard/{input,output,act}`, SHADOW default, fail-open, models load once ≈1.5 Gi). Live + verified (injection→block, grounding→flag, toxicity→pass in shadow). `services/weyland-guard/`, `k8s/weyland-guard/`, [runbooks/guardrails.md](runbooks/guardrails.md).
- **DONE Part 2 — tool-server migration:** `weyland-tool-server` v0.5.0 drops llm-guard + the guard-model bakes, calls `weyland-guard` over HTTP (fail-open). Verified: `/context/ask` unchanged, verdicts flow via the service (grounding discriminated real-vs-hallucinated), fail-open held (guard@0 → still answers). First clean seam of the tool-server decomposition (→ B31).
- **DONE Part 3 — `weyland-agent`:** the LangGraph loop (retrieve→grade→reflect→generate) + 4 custom LlamaIndex retrievers + dual MLflow autolog, calling `weyland-guard` for outer I/O. Live + verified: `/agent/ask` returns grounded answers, guards flow via the shared service, and **MLflow Traces capture per-step spans** (`agentic-rag` experiment). WARNING — autolog needs the full `langchain` package (langchain-openai/langgraph only pull langchain-core → silent no-op otherwise). `agent.weyland.lab`; [runbooks/agentic-rag.md](runbooks/agentic-rag.md).
...
### B101 — Registry manifest-finalization defect (large pushes don't land the manifest) — DONE MITIGATED 2026-07-24
...
`Prompt: <name> - vN`). Design `design/prompt-federation-design.md`, memory `prompt-federation-b103`. **Phase 2 DONE (2026-08-10) — bidirectional + auto:** `sync_prompts.py` runs
...
`langfuse-evaluation-b103`; design `design/langfuse-evaluation-design.md`. — Evaluators (LLM-as-judge on a sampled
...
**Concepts**; LikeC4 model re-validated (`[YES] Valid`). Renders after push + docs-site rebuild.
...
### B75 — Additional music datasources — **DONE (2026-08-27)** — data expansion
**DONE 2026-08-27.** Every dataset in scope landed (list below, all marked DONE); Spotify Charts was **removed by decision** (Kaggle requires login, no free public source); and the two things it could not do — the **full** MSD and the **gated** datasets — were explicitly delegated to [B76] at the time and are tracked there. Closed during the 2026-08-27 triage, where it was found sitting at MEDIUM as open work with nothing left in it. **This was a status error, not a duplicate of B76** — B75 is the completed parent that handed off its own remainder.

**Added 2026-06-28. Updated 2026-06-29.** Music datasets added to the `music` lakeFS repo:
- **MSD (UCI subset)** DONE — UCI YearPredictionMSD (515k songs, 90 audio features). Full MSD → B76.
- **Last.fm** DONE — `matthewfranglen/lastfm-360k` (HuggingFace, 13.9M rows, user listening history).
- **MusicBrainz** DONE — `seungheondoh/music-wiki` (HuggingFace, 11 entity configs: artist/release/genre/etc.).
- **GTZAN** DONE — `marsyas/gtzan` (HuggingFace, 1k songs, 10 genres, genre classification benchmark).
- **LP-MusicCaps-MC** DONE — `seungheondoh/LP-MusicCaps-MC` (HuggingFace, 5.5k rows, music captioning).
- **LP-MusicCaps-MTT** DONE — `seungheondoh/LP-MusicCaps-MTT` (HuggingFace, 22k audio/88k captions, audio tagging).
- **AudioSet (balanced)** DONE — `agkphysics/AudioSet` balanced (HuggingFace, 35k clips, 527 audio event labels, CC-BY-4.0).
...
### B76 — Full MSD + Music4All + MTG-Jamendo — LOW (↓ High→Low 2026-09-14 — externally gated: Music4All/MTG-Jamendo-full access requests + MSD needs AWS + ~300GB vs mother at 78.6% disk; hold until B134 frees capacity + access is queued; data expansion)
...
- **WON'T DO Original lockdown scope OBSOLETE (the work below is void) — reframed to retirement (2026-06-17).** B25b replaced the Dagster
...
**DONE P1 (model-eval product) DONE 2026-07-24.** The B4/B96 judge-panel leaderboard is now a first-class **DataHub Data
...
**DONE P2 (eval engines) DONE 2026-07-25 — kept as a *complementary suite*, NOT a bake-off.** The "pick one engine" framing
...
### B85 — data-store-mageddon v2 — MEDIUM (2026-08-05; comprehensive datastore re-evaluation; ↑ Medium→High 2026-08-08 rebalance — the disk-stall/timeout incident (B123) makes a systematic datastore re-eval more earned; ↓ High→Medium 2026-08-27 — stays OPEN: the point is to **re-evaluate the 27 candidates below**, not to close them out. The verdicts are a snapshot, not a decision log; re-run them when the estate's shape changes)
...
**[NO] Cloud / paid / proprietary — can't or won't run ($0 + LAN-only):**
...
**[NO] Already declined with recorded rationale (pure duplicate of the committed stack):**
...
**[YES] Genuine new-category candidates (free, single-node-feasible, a slot the grid lacks):**
...
2. **Spec Kit constitution + `analyze` consistency gate** → a checkable baseline. Closest fit to what the repo already does with sensors, and the most likely to earn its keep.
3. **Kiro EARS notation** → Requirements Analysis, plus the steering-file pattern.
4. **BMAD context-engineered story/unit files.**

**⚠ THE TARGET MOVED.** The 2026-08-11 evaluation compared against the user-authored `.methodaidlc/` Method. The project migrated to **AWS AI-DLC v2 on 2026-08-20**, so any borrowing now lands in `core/` (stages, agents, sensors, memory layers) — a different, more structured host than the one that was evaluated. Re-check each borrowing against the v2 shape before implementing; some may already be satisfied by v2 primitives (the `analyze` gate against `core/sensors/` in particular).

**[B126] MERGED IN 2026-08-27 (was EMA-146, Low).** B126 was "trial / adopt a spec-driven framework (B86 follow-up)" and its part **(b) was literally these same four notations** — so it was never a separate item, it was this item's execution half filed twice. Folded here in full:
- **(a) Optional trial** — run **OpenSpec** (brownfield delta model) or **Spec Kit** (constitution + `analyze` gate) against **one small unit** to feel the discipline first-hand. $0 / MIT / agent-agnostic / reversible. Genuinely optional: B86's verdict was already *cross-pollinate, don't migrate*, so a trial mostly re-tests a settled decision. **Explicitly NOT Kiro** — `$0` violation (managed AWS, Bedrock-locked, no BYOK, capped free tier).
- **(b) The four adoptions** — as listed above.
- **GATE (from B126, and it still governs):** low-priority / **opportunistic — do this when the Requirements/Units stages are next being revised, not as a standalone push.** That gate is why B86 sits reopened rather than active.
- **v2 makes it EASIER, not harder** (per [B133]): the borrowings now target v2's stage files, and `core/` → `dist/<harness>/` is an **explicit customization seam** the old fork never had.
...
**Added 2026-07-17.** The 12 end-to-end walkthroughs (`docs/demos/*-e2e.md` + `soda-dq.md` / `ranger-masking.md`; ledger rows **E1–E12** in `docs/demos/README.md`) are **MEDIUM authored, pending a live validation run**. Per the demos DoD a demo isn't DONE until it's been executed **straight through against live infra**. This item = the last mile of the docs-relevance + E2E-demos pass, and the tracker for it.
- **Run each E2E demo end-to-end** (UI + CLI) on the live platform; flip MEDIUM → DONE in the ledger as each passes.
...
### B88 — Per-language build lanes: tests + scanners + software supply chain — DONE 2026-08-30 (Linear EMA-80 Done — Track B gaps #1–#6 done + all Track A tails A1–A4 verified) (was REOPENED 2026-08-28 — closing it was then premature; see OUTSTANDING below) (2026-08-05; gated on Stud.IO CI track, B57/B118; ↑ Medium→High 2026-08-27; SCOPE EXPANDED 2026-08-27 from "test runners" to the whole per-language build surface — see the three phases below)

**FOLLOW-ON — declared-image provenance invariant (2026-09-07, surfaced by B156).** The image-signature admission control (Phase 3) ships in `dryrun`, and the B156 capabilities audit found its `require-signed-images` constraint at **0 violations** — every running image already from `registry.weyland.lab` or a reviewed publisher. But a Gatekeeper *audit* is transient (running pods only, on an interval), so "0 violations" was an observation, not the invariant the eventual dryrun→deny flip needs. Closed in-pass (fix-don't-file) with two guards enforcing *every declared image is from a reviewed source*, reading the SAME allow/exempt/official rule as the Rego from the SAME policy: **`scripts/check-image-provenance.sh`** (CI, pre-merge, wired into `repo-guards`, fail-closed 0/1/2) scans the repo manifests, and the nightly **`image-provenance` CronJob** (`k8s/monitoring/image-provenance.yaml`, 03:10, `monitoring` ns, read-only cluster RBAC, byte-identical embedded guard) enumerates EVERY declared workload live — running or not, incl. chart-rendered images the git scan and the running-pods audit both miss — against the live constraint. Verified 2026-09-07: CI git scan clears all **120** declared images (exit 0), negative case names an unreviewed image (exit 1), fail-closed on no-images/no-policy (exit 2); the enumerator clears **569** live across every declared workload. Tests: `scripts/tests/image-provenance.bats` **10** cases (incl. byte-identity drift, TDD Red→Green), shellcheck 0, promtool SUCCESS on the extended `ScheduledJobStale`/`ScheduledJobFailed` regexes, `kubectl --dry-run=client` 5 resources OK, cron-freshness-budgets green (16 CronJobs). Docs: runbook `supply-chain.md § the provenance invariant`, `flow-image-provenance.md`, `demos/image-provenance.md` (ledger row 59), schedules row 03:10. **This is the invariant the dryrun→deny flip rests on; the flip itself stays a Phase-3 follow-up — it needs the actual signing of not-recently-rebuilt images + two clean audit cycles first.**

**CORRECTION 2026-09-22 — the in-cluster enumerator CronJob never actually ran until now; the "569 live" above did NOT come from the deployed Job.** The CI git-scan guard (`check-image-provenance.sh` in `repo-guards`, 120 images) always worked. But the nightly `image-provenance` CronJob was non-functional from ship — **three stacked bugs, each masking the next**: (1) its `dump` initContainer image drifted to a **shell-less** `rancher/kubectl:v1.35.6` (`exec /bin/sh: no such file` → `Init:StartError`) → fixed to `alpine/k8s:1.35.6`; (2) its ClusterRole granted the constraint resource **plural** `k8simagesignatures` while the real API name is **singular** `k8simagesignature` → `Forbidden` on the policy read since inception → fixed; (3) the `check` container ran `/opt/ip/run.sh` — the wrapper that turns the dumped policy+workloads JSON into the guard's `IMAGE_LIST`/`POLICY_*` env contract — but **that wrapper was never written** → fixed (added `run.sh` to the ConfigMap + a `scripts/embed-pr-lifecycle`-style embed + 3 bats cases). The scheduled enumerator's **first successful in-cluster run was 2026-09-22** ("all 143 declared images reviewed", exit 0). **Lesson (recurred 3× this session):** a green CI guard + a clean `kubectl --dry-run` do NOT prove the *runtime* works — verify the running Job/pod, not just the build.

**⚠ THE DEFER TRIGGER HAS FIRED, AND THERE IS NOW A CONCRETE DEFECT (2026-08-27).**

**Trigger fired.** B57 (EMA-46) and B118 (EMA-107) are both DONE — the Stud.IO bridge is live — and Woodpecker is no longer idle: it runs weyland's own image CI, `port-pr-reconcile`, and the `linear-sync` DoD gate. The "PAUSED" note below is superseded.

**The defect — the Python lane is path-hardcoded and cannot discover new suites.** The two test steps in `.woodpecker.yml` are not built the same way:
```
shell-tests  ->  bats scripts/tests/                       # a DIRECTORY — auto-discovers
unit-tests   ->  cd .../services/weyland-guard && pytest   # ONE service, named by path
```
The shell lane scales with the repo; the Python lane does not. **[B78]'s Open Food Facts work adds `weyland-dagster`'s first pytest suite, and nothing would run it** — the tests would sit *green by absence*, passing because they are never executed. That is the same absence-as-success class recorded repeatedly in `project.md`, this time located in the test harness itself, where it is hardest to notice.

**Also entirely uncovered:** `k8s/flink/sql-runner/pom.xml` and `k8s/flink/health-job/pom.xml` — two Maven modules with ZERO test execution in CI.
...
**CLOSED since the reopen, cont'd (2026-08-28/29):**
2. ~~No cosign SealedSecret~~ → **key generated on mother, round-trip-verified (both keys base64-decode back identical), sealed** into `k8s/sealed-secrets/sealed/weyland__cosign-signing-key.yaml` (both keys, `managed` annotation, correct ns). Runbook gained an **install step** — cosign was on NEITHER host — the real repo path (`~/lab/weyland-platform/...`, mother has no checkout), and a warning about the stale loose `~/seal-secrets.sh` that predated the allow-list entry.
5. ~~Gatekeeper policy never applied~~ → **live**. First dryrun audit reported **270 violations**, which was the point: it proved the exemption model wrong in two directions only live data could show — `registry.istio.io` sidecars (injected into every meshed ns, so a namespace exclusion never covered them) and implicit Docker-Hub refs with no registry prefix. Then a **security review caught a real bypass I introduced**: I widened `docker.io/library/` → `docker.io/` to lower the count (suppressing a true signal), and my `implicit_docker_hub` rule admitted `attacker/evil` — my own test suite had asserted that bypass as correct. Fixed: single-segment official images only, org-scoped ones explicit, publisher-scoped exemptions (44 reviewed publishers, tiered VENDOR/COMMUNITY/SOLO). **Audit now 0** — every running image is registry-local, official, or a reviewed publisher, so any future violation means an unreviewed image appeared. Rego tests grew to 15 (typosquat + unreviewed-publisher cases); the harness now reads the SHIPPED constraint's parameters, not a synthetic literal.

**BONUS — the audit found and fixed a real security gap.** The one FLAGGED publisher, `docker.io/bitnamilegacy/kafka`, is Bitnami's frozen namespace (no security updates). Rather than allowlist it, traced it to DataHub's bundled `prerequisites-kafka` broker — a **redundant second Kafka** the lab didn't need, since Redpanda already runs in-cluster and speaks the Kafka API. Repointed DataHub at Redpanda (`global.kafka.bootstrap.server`), disabled the bundled chart broker (`kafka.enabled: false`), verified end-to-end: all 8 topics recreated, GMS ready/0-restarts, both system-update Jobs Completed, mae/mce/usage consumer groups Stable, **catalog intact throughout** (175,388 Postgres aspects / 4,196 OpenSearch datasets — Kafka is the event bus, not the source of truth). Unmaintained image **gone from the cluster**; the exemption list **shrank 50 → 49** — a control removed because the risk was eliminated, not tolerated. **⚠ new Istio watch:** DataHub is the first meshed client of the unmeshed Redpanda; the `neo4j-istio-bolt-longconn-stall` precedent showed up under SUSTAINED load, so watch for a consumer stall and apply a DestinationRule keepalive if it appears. Also verified Redpanda was NOT the operator's deliberate broker being removed — cluster-wide search: kafka-connect + Flink both point at Redpanda, and the deleted broker held only DataHub's 8 topics.
...
- ✔ **#1 Coverage ratchet.** Only Go emitted coverage and nothing read it — a number produced and discarded. Built `scripts/coverage-ratchet.sh`: per-project line coverage vs a committed baseline (`tests/lang/coverage-baseline.tsv`), failing ONLY on a regression beyond a 0.1pt float-noise tolerance — a **ratchet, not an 80% floor** (a hard threshold on mostly-fixture lanes either fails day one or forces fake tests; % is the most gameable metric). Extractors for 8 lanes verified against REAL toolchains: go `-coverprofile`, pytest-cov, node built-in + jest, jacoco, cargo-llvm-cov — each fails CLOSED (exit 2) if its tool is missing, never a silent pass. **Shell deliberately excluded, loudly** (kcov absent from Alpine + line-coverage of a decision-asserting orchestrator measures ~nothing). Real baselines now protected: weyland-guard **74%**, Flink health-job **9.8%**, sql-runner **33.3%**. 11 bats + wired into all test lanes. Regression path proven live (seeded 90% → real 74% → exit 1, baseline NOT rewritten down). ✔ #2 integration/e2e tier (2 black-box lanes, verified CI) · ✔ #3 per-build image scan + delta (verified CI) · ✔ #4 post-deploy verification (tool-server txn, verified live) · ✔ #5 Unleash→CI wiring (ship kill-switch, exercised live) · ✔ #6 CI toolchain caching (cold→warm ~9 min saved, verified #49/#50). **All six Track B gaps done.** Lower tier remains: DAST/mutation/perf (parked).

## B88 — REMAINING PLAN (written 2026-08-29, survives compaction)

Two tracks left. Track A = the original integration gaps (CI secrets, real-CI-run, scan-suite ship). Track B = the CI/CD HARDENING gaps found while mapping the pipeline. Work Track B one item at a time; each is TDD-first (bats/JUnit Red before impl), each verified against REAL toolchains (not stubs — the repo's own rule), each cascaded through the DoD.

**TRACK A — integration/operator (mostly outside this session's reach):**
- **A1. Woodpecker CI secrets** `cosign_key` / `cosign_password` — **DONE, verified in CI #54 (2026-08-30).** Operator provisioned the `cosign_key` secret; five stacked defects fixed to make sign/attest actually work in-pipeline: (1) `--key "$PEM"` treats the key content as a filename → `env://COSIGN_KEY` via `cosign_key_ref`; (2) the deprecated `secrets:` list injected nothing → `environment: COSIGN_KEY: {from_secret: cosign_key}`; (3) empty-value secret 422 → `COSIGN_PASSWORD` moved to a plain env (`""`); (4) the secret's event-restriction excluded `manual` → `woodpecker-cli repo secret update --event manual --event cron`; (5) cosign could not push `.sig`/`.att` to the mkcert registry → `--allow-insecure-registry` + real-stderr surfacing. Build #54 signed + SLSA-attested all four built images (dagster-user-code, feast-server, flink, flink-py), zero `SIGN FAILED` / `shipped UNSIGNED`. See runbooks/supply-chain.md.
- **A2. Run it in real CI once.** — **DONE.** The whole loop has now run end-to-end in real CI across pipelines #54/#55/#56: build → per-build vuln scan → sign+attest → bump PR → merge → Argo sync → FR1.5 live-check → TXN. No longer local-container-only.
- **A3. scan-suite image ships mypy+shfmt.** — **DONE, verified live in the deployed `git-7e5b424a` CronJob (2026-08-30).** shfmt worked from the first ship (`0C/0H/0M/35L`). mypy shipped PRESENT but broken: a single SRC-wide pass hit `Duplicate module named "app"` (ten `app.py` plus colliding `config.py`/`guard.py`/… across service dirs) → **exit 2, silent miss** — and the runner logged a BLANK cause because mypy writes that fatal to stdout while the code printed only stderr. Fixed in `scan.py`: run mypy **per-directory** (collisions are all across dirs; within a dir module names are unique) and read stdout+stderr. Validated against real mypy in the image, then confirmed in the deployed CronJob: `= mypy: 0C/0H/158M/0L`, no `could not run`. Both tools now run.
- **A4. Istio long-connection watch** on DataHub↔Redpanda — **DONE.** Watched ~26h under real event-bus traffic with 0 sidecar resets / 0 connection stalls; the neo4j-bolt keepalive failure mode did not appear on the DataHub↔Redpanda path. No DestinationRule keepalive needed.
- **Gate fix found while shipping A3 (2026-08-30).** `ship-images.sh` FR1.5 stopped the git-91789258 ship with `weyland-flink-py(absent)`: flink-py is a BOUNDED application-mode FlinkDeployment that is DELETED at rest ("no steady-state cost"), so `deployed_tags_for` finds neither a pod nor a CronJob template and reads `absent` forever — the same no-steady-state class as scan-suite's CronJob, which was already special-cased. Added `is_ondemand_image` + `registry_has_tag`: an on-demand image absent from live workloads is verified by registry presence of the new tag (FR1.3 pushed it) instead of a running pod; a missing tag still fails closed. TDD: 2 new bats (pass + fail-closed control), 83/83 green, shellcheck clean at `--severity=warning`.
...
- **B/#2 — integration/e2e/smoke tier (2026-08-29 — DONE + VERIFIED IN REAL CI. 2 integration lanes: guard + DataHub↔Redpanda, both ran GREEN inside a real Woodpecker pipeline (#41) from k8s-backend pods, confirmed by their printed OK lines — 5/5 assertions and topic-spine+MAE/MCE-Stable, not just exit codes. Flink MiniCluster + any future test-tier depth deferred → **B151**.)**
  ...
  **`scripts/integration/guard-blackbox.sh`** — a curl-only (no framework, no jq; flat JSON via grep/sed)
  ...
  23 steps). It POSTs real payloads across all three hooks. **The by-hand live observation (2026-08-29)
...
- **B/#3 — per-build image scan — DONE + VERIFIED IN REAL CI 2026-08-29 (pipeline #44).** `trivy image
  --scanners vuln` per pushed image in the build step (`supply-chain.sh vuln`), loud + non-fatal: prints the CVE
  count and exits 0 even with findings; only a scan that could not run is exit 2. Verified on 5 images in one real
  build — dagster-user-code 327 (12C/73H), feast 169, flink 314, flink-py 3088, scan-suite 2082 (48C/754H) — a
  base-image CVE is now caught at build time instead of waiting for Sunday's scan-suite. **Making it actually work
  took three fixes the real pipeline surfaced that local checks could not:** (1) **`all` was not resilient** — sign's
  hard-exit on the missing `COSIGN_KEY` (A1) aborted the whole script, so licences and vuln had NEVER run in CI;
  steps now `return` (never `exit`) and `all` collects the worst outcome. (2) **syft's SBOM had been silently failing**
  — the tools could not pull from the mkcert-TLS lab registry; `SYFT_REGISTRY_INSECURE_SKIP_TLS_VERIFY` + trivy
  `--insecure` fixed it (SBOM now written). (3) **the scans OOMKilled the build pod (exit 137)** — best-effort QoS on
  RAM-tight mother; gave the build step a **1Gi request / 4Gi limit** (memory only, to avoid the cpu-requests wall),
...
- **B/#6 — CI toolchain caching — DONE + VERIFIED IN REAL CI 2026-08-29 (cold #49 vs warm #50, both green).**
  Measured speedup: **scan-rust 125s→6s** (~20×, cargo no longer recompiling audit/deny), **test-rust 52s→8s**,
  **test-java 68s→19s** (maven plugins cached), **test-go 18s→8s**, **build 682s→370s** (trivy DB not re-downloaded)
  — ~9 min saved per warm run. **Two real-CI-only blockers surfaced (lint-passed both):** (1) the Woodpecker SERVER
  rejected step `volumes` with `Insufficient trust level` — `woodpecker-cli lint` doesn't check the repo's trust
  setting; fixed by setting `trusted.volumes:true` on repo 2 (the operator had already granted the STRONGER
  `security:true`, so this is a consistent least-privilege increment; `network` stays false). (2) The PVCs must be
  applied before the run or step pods hang Pending. Both now green; PVCs all Bound. Every lane reinstalled its
  toolchain per run: `cargo install cargo-llvm-cov/cargo-audit/cargo-deny` (compiled from source, minutes each),
  maven plugins, go modules, npm packages, and (since #3) the 109MB trivy vuln DB. Added **5 persistent cache PVCs**
  (`k8s/woodpecker/ci-caches.yaml` — ns woodpecker, RWO local-path, mirroring `buildkitd-cache`) mounted per-lane in
  `.woodpecker.yml` at each toolchain's cache dir with the matching env var: cargo (`CARGO_HOME=/cache/cargo` +
  literal PATH so the scripts' `command -v cargo-llvm-cov` finds the cached bin — no `$PATH`-in-YAML), maven
  (`/root/.m2/repository`), trivy (`TRIVY_CACHE_DIR`), go (`GOMODCACHE`/`GOCACHE`), npm (`npm_config_cache`). k8s
  backend syntax is the step-level `volumes: [<pvc>:<path>]` (confirmed from the Woodpecker docs, not guessed);
  woodpecker-cli lint passes. **RWO is safe** (single-node sequential steps → one writer, which cache dirs need
  anyway). **OPERATOR PREREQUISITE:** `kubectl apply` the PVCs BEFORE the next run, or step pods stay Pending on a
  missing PVC. Verification = run the pipeline twice (cold populates, warm should be materially faster on
  rust/build/java). Pure speed; lowest of the real gaps.

**DoD — Track B gaps #1–#5 (2026-08-29).** **1 OK** — docs cascaded per gap + this run: `arch.md` (CI §), `ci-architecture-map.html` (23 steps), runbooks (`supply-chain.md`, `ship-images.md`, `unleash.md`), `backlog.md`, LikeC4; doc-counts/mermaid/app-registry/quality-tools guards green; no new host/endpoint (unleash already in `hosts.md`), no timer change (schedules N/A, verified). **2 OK** — new `diagrams/flow-cicd-hardening.md` with **5 sequence diagrams** (ratchet · integration · vuln+Δ · txn · flag-gate); the two stale flows (`flow-build-lanes`, `flow-ship-loop`) are pointed at it via the extended demos; 149 mermaid blocks parse. **3 OK** — `demos/cicd-hardening.md`, **RUN end-to-end**: ratchet 80→71.5 exit 1 (baseline held) · guard 5/5 + datahub MAE/MCE Stable (live in-cluster) · vuln 327 [12C/73H] Δ:no-new (CI #47) · `txn_tool_server` TXN_OK (live) · flag ON→ALLOW/OFF→HELD/ON→ALLOW (live); ledger row B88-B added; `ship-images.md` + `build-lanes-supply-chain.md` pointers keep them non-stale. **4 OK** — demo teardown documented (read-only except `/tmp/base.tsv` rm, `--rm` pods, flag restored ON). **5 OK** — `check-linear-sync` green (30 refs reconciled, no project-less open issues); B88 stays IN PROGRESS (#6 + operator items A1–A4 remain), not flipped to Done. **6 OK** — the new capabilities are CI/ship-loop STEPS, inherently durable (they run every pipeline/ship — proven in real runs #41/#47), not standing services needing a new alert; the `weyland-ship-enabled` flag degrades fail-open if Unleash resets. **7 OK** — shellcheck clean across the whole shell surface (the shell-batch scan; CodeScene/semgrep N/A for `.sh`, the documented shell equivalent, same as B137); no new Python; the Unleash backend token default is the documented LAN-only read-only token already committed in `runbooks/unleash.md`; #3 additionally adds per-build image vuln scanning. **8 OK** — additions only (no removals/renames to reverse-sweep); no shared function changed (no code cascade); the new surfaces (`test-integration-*` steps, the `weyland-ship-enabled` flag, the vuln/txn/flag functions) are all reflected in the map/runbooks/demo.

**FULL B88 DoD — the ENTIRE item, all tracks (2026-08-30).** Covers the original build lanes + supply chain + the DataHub→Redpanda migration + Track B #1–#6; supersedes the interim #1–#5 block above.
**1 Docs** — `arch.md` §CI, `ci-architecture-map.html` (**23 steps** + toolchain-caches card; stale "22 steps" narrative fixed), runbooks (`supply-chain.md`, `ship-images.md`, `unleash.md` [+deploy kill-switch], `woodpecker.md` [+caches +trust gotcha]), `hosts.md`/`api.md`/`schedules.md` verified N/A (no new host/endpoint/timer), LikeC4; doc-counts (78), mermaid (**150 blocks**), app-registry, quality-tools all green.
**2 Diagrams** — `flow-build-lanes` (Track A) + `flow-cicd-hardening` (**6 sequence diagrams**, #6 added); all parse.
**3 Demos** — `build-lanes-supply-chain.md` (Track A, RUN) + `cicd-hardening.md` (**#1–#6, all RUN live**; #6 cold→warm timing table added: scan-rust 125s→6s, build 682s→370s); ledger row B88-B updated to #1–#6.
**4 Cleanup** — demo teardowns documented; the ad-hoc DoD scan Job I created was deleted; the old `prerequisites-kafka` StatefulSet (kept for rollback during the migration) is **gone** from `data-mesh`.
**5 Tracking** — `check-linear-sync` green. Track A tails all closed 2026-08-30: **A1** sign/attest verified in CI #54 (five stacked cosign/secret fixes), **A2** whole loop run in real CI (#54–#56), **A3** mypy+shfmt both verified live in the deployed `git-7e5b424a` CronJob (mypy per-directory fix), **A4** 26h Istio watch clean. B88 work is COMPLETE; the only remaining step is the operator flipping B88 → Done in Linear (the act lane).
**6 Ops** — Gatekeeper `require-signed-images` LIVE in **dryrun, 0 violations** (observable via TOTAL-VIOLATIONS); CI/ship-loop mechanisms are event-triggered steps, durable (proven across real runs #41–#50); scan-suite is a weekly CronJob under cron-freshness monitoring; the #6 caches degrade safely (fail-open, PVCs).
**7 Security** — **fresh 19-tool scan-suite RUN 2026-08-30 (3m7s)**: **0 CRITICAL across every tool**; B88's own shell is **0C/0H** (shellcheck); the HIGHs are all pre-existing + traced (semgrep 1 = aidlc-swarm, bandit 1 = langfuse_eval, osv 8 = Flink poms, kubescape 546 = the systemic k8s-control baseline that B88's added manifests join at 0 critical). No B88 file introduces a new critical/high.
**8 Cascade** — the DataHub→Redpanda migration reverse-swept: the `docker.io/bitnamilegacy/` allowlist entry **removed** from `image-signatures.yaml` (0 active, only an explanatory comment), `kafka.enabled:false` in the prereqs, the broker StatefulSet reaped; every new surface reflected in map/runbooks/demo; no shared-function change (no code cascade).
...
**STILL OUTSTANDING:**
1. **Woodpecker CI secrets `cosign_key` / `cosign_password`** — declared in `.woodpecker.yml`, not provisioned. Operator-only (Woodpecker secrets are UI state). Until then every build logs the signing failure and ships unsigned.
2. **Nothing has run in real CI.** 6 test lanes + 3 scan lanes + `rego-policies` + the supply-chain step have only run locally in containers. Fires on the next pipeline; unproven until then. The B148 lesson: a control that has never actually run is not yet a control.
3. **scan-suite image still `git-a7c56951`** — `mypy`/`shfmt` declared + wired but not in the running image. Self-resolves on the next ship (it's in `images.tsv`).
5. **Istio long-connection watch** on DataHub↔Redpanda (see BONUS above).
...
### B153 — Golden paths per supported language — DONE 2026-09-08 (Linear EMA-210)

**DONE 2026-09-08 — pipeline #95 `golden-path-smoke` GREEN end-to-end: all 21 golden paths built via buildkitd and served in-cluster ("OK — all 21 golden path(s) served in-cluster and were torn down").** The paved road is proven: each blessed path builds to `registry.weyland.lab/golden-<lang>-<fw>`, runs as a run-to-completion Job in the `golden-paths` namespace under the dedicated `golden-path-runner` SA, serves the contract (`/health` `/ready` `/metrics` `/hello`), and tears down — replacing the B88 hello fixtures as the CI lane fixtures. Getting the CI wiring green flushed a chain of latent bugs that had NEVER run in CI (each fixed): repo-guards (doc-count 78→79 + registry entry + dedicated-SA RBAC off `default` + the Woodpecker per-step-SA flag), the Java lanes (JDK 17→21 to match the golden paths' release-21 target), the node test runner (`node --test` branch now installs deps for Fastify), the Go Dockerfile bases (`golang:1.23`→`1.26`), buildkit build-context cross-contamination (identical Dockerfiles → build each path from a fresh `mktemp` context), and the Java runtime-stage smoke.sh copy (from context, not the build stage). Follow-ons split out: **B160** (extended-language suite), **B163** (semi-exhaustive per-framework module coverage). Full build/fix history below.

**Design LOCKED + FastAPI reference DONE 2026-09-07.** Design doc [docs/design/golden-paths.md](design/golden-paths.md): a golden path is **runnable + ephemeral + extendable** — one artifact that is both the blessed per-language service template AND the CI build-infra probe (replaces the B88 hello fixtures), conforming to one framework-agnostic **contract** (`/health` `/ready` `/metrics` `/hello` + lane self-test + selfcheck + coverage + buildkit Dockerfile + a run-to-completion k8s Job that smoke-serves then tears down + an onboarding-declaration template so a scaffolded real service passes the B154/B155 gates by construction). The "one paved road" is the CONTRACT (the altitude); the framework lives beneath it, so a language has several golden paths all conforming. **Locked matrix — 21 paths:** Python (FastAPI/Flask/Litestar/Django) · Java (Spring Boot/Quarkus/Micronaut) · Go (net/http/Gin/Echo/Fiber) · Rust (Axum/Actix-web/Rocket) · Node (Express/Fastify/NestJS) · Frontend (Next.js/Remix/Vite+React/Astro). **FastAPI reference built + verified** (`golden-paths/python/fastapi/`): the lane discovers + runs it (4 tests pass, selfcheck fails, coverage **100%** baselined), `smoke.py` spawns real uvicorn + asserts `/ready`+`/hello` (RUN, exit 0), multi-stage non-root Dockerfile, README carries the onboarding-declaration template. Lane hardened: a `selfcheck/` dir is never discovered as its own project (`is_excluded`, guarded by a new `lang-tests.bats` case — 26 green). Ephemeral-Job harness `scripts/run-golden-path-jobs.sh` (buildkit build → Job → wait → assert → delete; shellcheck 0, dry-run OK) — the live in-cluster build+run is an operator step. **Python COMPLETE (4/4) 2026-09-07:** FastAPI (reference) + Flask + Litestar + Django, all lane-verified (fixture + 4 projects green, coverage 100% each via a `.coveragerc` that scopes coverage to the service code, smoke.py serves for each). **Java COMPLETE (3/3) 2026-09-07:** Spring Boot + Quarkus + Micronaut, each verified via `mvn test` (4 contract tests + BUILD SUCCESS, `-Pselfcheck` fails, java lane discovers all 3); REST controller + micrometer `/metrics`, multi-stage maven→JRE Dockerfile + `smoke.sh`. The ephemeral-Job runner now reads a per-path `.smoke` command (language-agnostic via `sh -c`). **A second wave of languages is split to [B160] (EMA-217, HIGH): Angular · C#/.NET · C/C++ · PHP · Ruby on Rails · Elixir · Clojure · Kotlin · Scala.** **Go COMPLETE (4/4) 2026-09-07:** net/http + Gin + Echo + Fiber, each verified on go 1.26 + `-race` (4 contract tests pass, coverage baselined 86–92%, `deliberate`-tagged selfcheck fails, go lane discovers all 4); prometheus `/metrics`, go.mod/go.sum committed, alpine-runtime Dockerfile + `smoke.sh`. **Rust COMPLETE (3/3) 2026-09-08:** Axum + Actix-web + Rocket, each verified on rust:1-slim (4 contract tests pass, `#[ignore]` selfcheck fails under `cargo test -- --ignored`, rust lane discovers all 3); prometheus `/metrics`, Cargo.lock committed, debian-slim Dockerfile + `smoke.sh` (Rocket binds 8080 via ROCKET_* env). **Node COMPLETE (3/3) 2026-09-08:** Express + Fastify + NestJS, each verified on node:24-alpine (the CI image: 4 contract tests pass, selfcheck exits non-zero, coverage baselined); three framework-idiomatic strategies on the one node runner — Express jest+supertest (100%), Fastify node:test + `fastify.inject()` dependency-light (100%), NestJS @nestjs/testing+supertest via ts-jest (78.31%, Nest decorator metadata counted-not-testable); `prom-client` `/metrics`, alpine Dockerfile (NestJS tsc→`dist/`), package-lock.json committed + node_modules/dist/coverage gitignored. **Known lane consequence (flagged, not yet fixed):** the four node lanes (ts/js/react/nextjs) share one repo-wide glob, so each node path is discovered under all four — baseline recorded once per path under its semantic lane; the others auto-record and pass. **Frontend COMPLETE (4/4) 2026-09-08 — ALL 21 GOLDEN PATHS BUILT:** Next.js + Remix + Vite+React + Astro, each verified on node:24-alpine (contract tests pass, selfcheck exits non-zero, coverage 100% baselined) AND built + served in-container with the smoke curling `/health` `/ready` `/metrics` `/hello` + the demo route. The adapted frontend contract met four ways: Next.js (App Router route handlers + SSR page, `output: standalone`, `force-dynamic`), Remix (resource routes + SSR via Remix+Vite, `json()` helper), Vite+React (SPA + a stdlib static server as the "sidecar route"), Astro (SSR `@astrojs/node` endpoints + SSR page); `Hello` uses one template literal so the SSR HTML carries the greeting contiguously. Two real bugs the in-container serve caught + fixed: Remix's server `Response` has no static `.json()` (→ `json()` helper), and React SSRs `hello, {name}` as split text nodes with a `<!-- -->` marker (→ template literal). Same quad-lane baseline note as Node (recorded under react; nextjs→nextjs). **FIXTURE-SWITCH DONE 2026-09-08:** each lane's fixture is now its golden path via a shared `resolve_fixture()` (`scripts/lib/lang-fixtures.sh`, sourced by run-lang-tests/run-lang-scan/coverage-ratchet); `WEYLAND_LANG_FIXTURE_DIR` override preserved (bats seam), only the default changed; resolved fixture excluded from real-project discovery; go lane verified end-to-end (fixture nethttp + gin/echo/fiber pass, selfcheck propagates). The 8 B88 hello fixtures under `tests/lang/` are retired (shell + baseline kept) — the directories `tests/lang/{python,java,go,rust,typescript,javascript,react,nextjs}` are now inert (under FIXTURE_DIR → excluded from discovery) and can be removed; the automated `rm` was blocked, so their deletion is left to the operator. Stale baseline rows pruned; woodpecker "dependency-free" comment corrected. **SCAN-LANE HARDENING (prerequisite, DONE):** shipping the golden paths silently regressed the node scan lanes AND surfaced two fail-open bugs in `run-lang-scan.sh` — `run_tool` matched only old npm "missing scanner" strings (current npm's `npx canceled due to missing packages` slipped through as a finding), and `scan_node/scan_rust/scan_java` returned only the LAST tool's status (masking an earlier missing scanner). Both fixed + pinned by `scripts/tests/lang-scan-guard.bats` (3 tests); `scan_node` tsc/next-lint made capability-driven (tsconfig / `next` dep) so the quad-lane discovery doesn't demand Next's linter on an Express app; all 7 node/frontend golden paths given the estate's eslint config + scan devDeps and verified scan-clean on node:24-alpine. **SCAFFOLDER DONE:** `scripts/new-service.sh <golden-path> <name> [dest]` copies a golden path (minus build output + the selfcheck probe), rewrites the `golden-<lang>-<fw>` token to the service name (SERVICE_NAME + image ref + manifest name), writes a minimal README, and prints the B154/B155 onboarding declaration; fail-closed on bad name / missing path / existing dest; pinned by `scripts/tests/new-service.bats` (5 tests). **FLOW DIAGRAM + CI-WIRED JOB RUNS DONE 2026-09-08:** the DoD flow diagram is in `docs/design/golden-paths.md` (§ Lifecycle flow — the dual scaffold-vs-fixture/build/Job path from one contract). The in-cluster Job runs are now a **CI step**, not a manual operator step: `.woodpecker.yml` `golden-path-smoke` reuses `scripts/run-golden-path-jobs.sh` (build each path via the estate's buildkitd + run its run-to-completion smoke Job, fail-closed) in a dedicated `golden-paths` namespace, with least-privilege RBAC (`k8s/golden-paths/golden-paths-rbac.yaml` + Argo app `k8s/argocd/applications/golden-paths.yaml`) granting a **dedicated** SA (`golden-path-runner`, ns woodpecker — NOT `default`, which the B95 automount hardening + `check-sa-automount-collisions.sh` forbid) Job management ONLY there; the step runs as it via `serviceAccountName` (agent flag `WOODPECKER_BACKEND_K8S_SERVICE_ACCOUNT_NAME_ALLOW_FROM_STEP=true`). Verifying the CI wiring caught + fixed **three real bugs in the exerciser** (all latent because the step had never run): buildkitd addressed at `weyland.svc` instead of the actual `woodpecker.svc`; the `buildctl` build missing `registry.insecure=true` (push would fail against the plain-HTTP LAN registry) + the registry buildcache; and the Jobs defaulting to `weyland` instead of the sidecar-free `golden-paths` ns. **Pipeline #88 (2026-08-28, first run with the golden-path-smoke step) FAILED at `repo-guards` — root-caused + fixed 2026-09-08.** Two file-static guards had gone red the moment the golden-paths Argo app was added, both invisible until then because `check-doc-counts` (which failed first) aborts the step before `check-sa-automount-collisions` runs: (1) the golden-paths Application made the real Argo count 79, so 14 docs hardcoding "78 apps" + the app-registry header drifted — bumped to 79 and `golden-paths` added to the registry `excluded:` (plumbing: namespace + RBAC only, no workload); (2) the RBAC bound the `default` SA, a B95 anti-pattern the collision guard fails closed on — reworked to a dedicated `golden-path-runner` SA (ns woodpecker, automount on), the step opting into it via `backend_options.kubernetes.serviceAccountName` (enabled by the new agent flag `WOODPECKER_BACKEND_K8S_SERVICE_ACCOUNT_NAME_ALLOW_FROM_STEP=true` in woodpecker-values.yaml — safe on this solo single-repo farm). All 11 repo-guards scripts pass locally. **Pipeline #89 (2026-09-08) cleared repo-guards + the unit lanes through test-shell, then failed at `test-java`** — the Java golden paths target release 21 but the `test-java`/`scan-java` lanes ran `maven:3.9-eclipse-temurin-17` (JDK 17 can't compile `--release 21`); invisible until now because the paths were verified in their temurin-21 Dockerfile builder, not the lane image, and `test-java` had never actually run in CI (every prior pipeline died earlier). Fixed by bumping both Java lanes to `maven:3.9-eclipse-temurin-21` — the image the Java Dockerfiles already build on; JDK 21 still compiles the real Flink modules' release-17 target (backward-compat) and their tests are pure JUnit (arithmetic extracted out of the operators, no MiniCluster). Audited the other lanes to avoid a ping-pong: python (no version pin), go (`go 1.26` == lane), rust (edition 2021), node (no engines pin) all match their images. **Pipeline #90 (2026-09-08): test-java now GREEN on JDK 21, then failed at `test-node`** — the node test runner's `node --test` branch assumed those projects are dependency-free and skipped `npm install`, but the Fastify golden path uses node:test AND requires the `fastify` module → MODULE_NOT_FOUND in a fresh clone (node_modules gitignored). It only ever passed because a manual install left node_modules behind; the lane never ran clean in CI. Fixed `scripts/run-lang-tests.sh`: the `node --test` branch now installs deps (package.json present + node_modules absent → npm install, fail closed), mirroring the jest branch and `scan_node`. Fastify is the ONLY node/frontend path on that branch with deps (the other six declare a `test` script → already installed); verified locally (fresh install + `node --test` → 4 pass, exit 0). **Pipeline #91 (2026-09-08): test-node GREEN, golden-path-smoke REACHED and the mechanism PROVEN — 4/4 frontend paths SMOKE OK** (astro/nextjs/remix/vite-react each built via buildkitd, ran as a Job in the golden-paths ns under the dedicated `golden-path-runner` SA, served + SSR-rendered). The step then failed on the FIRST Go build: `golden-go-echo` Dockerfile pins `golang:1.23-alpine` but go.mod requires `go 1.26` — a stale Dockerfile BUILD BASE (a separate toolchain declaration from the test lane's `golang:1.26`, which passed). Audited every golden-path Dockerfile base against its toolchain: only the 4 Go Dockerfiles were stale (rust `rust:1-slim`, java temurin-21, node `node:24-alpine`, python `python:3.12-slim` all match). Fixed all 4 Go Dockerfiles → `golang:1.26-alpine`. **Pipeline #92 (2026-09-08): frontend 4/4 + go/echo SMOKE OK, then go/fiber BUILD FAILED — buildkit cache cross-contamination.** go/echo built fine on `golang:1.26-alpine`; go/fiber's `go build` then failed compiling `github.com/labstack/echo/v4` (ECHO's package) despite fiber's source cleanly importing gofiber. Root cause: the "one contract, many frameworks" design gives every framework in a language a BYTE-IDENTICAL Dockerfile (all 4 Go, all 3 Rust, all 4 Python, all 3 Java); built back-to-back on the ONE shared buildkitd, identical Dockerfiles collide in its layer cache, so the 2nd path reused the 1st's `COPY . . && go build` layer and compiled the wrong source. The estate image pipeline never hits this (distinct Dockerfiles). Frontend passed because its Dockerfiles differ per-framework. **Pipeline #93 (2026-09-08): `--no-cache` was INSUFFICIENT — same go/fiber-builds-echo failure — revealing the true mechanism.** Not the build-vertex layer cache (which `--no-cache` bounds) but buildkit's local-source INCREMENTAL context sync: with byte-identical Dockerfiles AND identical filenames (main.go/go.mod/...) across a language's paths, buildkit reuses a same-named file from the previously-built path by name+size+mtime, so fiber's context kept echo's main.go. `test-go` never sees it (plain checkout, no buildkit); the estate pipeline never hits it (distinct Dockerfiles). Real fix: the exerciser now builds each path from a FRESH `mktemp` copy of its context (unique dir path + fresh mtimes from `cp`), so buildkit sees an entirely new context and cannot reuse a prior path's files — kept `--no-cache` + no registry buildcache alongside. Corrects the #92 note (the `--no-cache`-only fix was wrong). **Pipeline #94 (2026-09-08): the mktemp context fix WORKED — 8/21 SMOKE OK (frontend 4 + go 4, incl. the previously-failing go/fiber/gin/nethttp) — then java/micronaut BUILD FAILED on a per-path Dockerfile bug.** micronaut + spring-boot did `COPY --from=build /app/app.jar /app/smoke.sh ./` — copying smoke.sh FROM THE BUILD STAGE, but smoke.sh lives in the context and was never copied into the build stage (only `COPY src ./src`). First time the Java Dockerfiles were ever docker-built (prior verification was `mvn test` only). quarkus was already correct (`COPY smoke.sh ./` from context). Fixed micronaut + spring-boot to match quarkus. Then AUDITED all 21 Dockerfiles for smoke-file copy (now all from context) + app-artifact copy (node node_modules+src, python deps+main.py, rust server binary — all sound) + build bases (all match toolchains) — no other Dockerfile bug found. **Remaining:** re-push the 2 java Dockerfile fixes; trigger → smoke replays the proven 8 + builds java/node/python/rust from isolated contexts → first fully-green golden-path-smoke = the demo evidence → flip B153 to Done. (12 in-cluster smoke passes proven across runs; each remaining failure has been a distinct per-path build bug that never ran in CI, not a mechanism problem.) Original scope below.
...
### B154 — Self-service onboarding for applications / repos — DONE 2026-09-07 (Linear EMA-211)

**DONE (Phase 1 — the paved road + the DoD §6 onboarding gate).** The guard `scripts/check-onboarding-completeness.sh` is the standing gate for the DoD's "deployed service" cascade row (codified in `definition-of-done.md`): declare-and-account over the FULL §6 checklist — every deployed service declares `deployed`/`metrics`/`ingress`; the guard VERIFIES the file-checkable gates (SCHEMA incl. declaration completeness, PORT `port_component`, PLACEMENT in the single LikeC4 model) and ACCOUNTS the rest per service in a `--list` matrix (ServiceMonitor/dashboard/`*Down` LIVE via the coverage guards, Kuma MANUAL, logs AUTO, arch§6 curated) so nothing is silently skipped. The scaffolder `scripts/onboard-service.sh` writes a paved registry entry + a matching LikeC4 element from one command so a new service lands onboarding-complete. Registry schema gained `deployed`/`metrics`/`ingress`/`likec4`; 3 genuinely-missing LikeC4 nodes added on the guard's first run (weyland-agent/port-k8s-exporter/promptfoo). Live invariant 59/59; 22 bats (14 guard + 8 scaffolder), shellcheck 0, wired into repo-guards. DoD: `flow-onboarding-completeness.md`, `demos/onboarding-completeness.md` (ledger rows 60/61), `application-catalog.md`, `definition-of-done.md`. **Phase 2 (real onboarding SERVICES) = [B159] (Medium).** Original scope below.
...
**PHASED (2026-09-07).** **Phase 1 (this item, HIGH)** = the paved-path SCAFFOLDING + a completeness guard — management tooling: from one declaration, generate/stub every required surface (registry entry, ServiceMonitor, dashboard stub, `*Down` alert, Kuma monitor, cron-freshness budget where a CronJob, LikeC4 component, arch.md §6 row), plus a guard that a registered service actually HAS all its required surfaces (the ONBOARD-completeness check, complementing B138's audit). Motivated firsthand 2026-09-07: onboarding the `image-provenance` CronJob through DoD §6 by hand (freshness budget · schedules row · ScheduledJobFailed regex · LikeC4 node · arch.md clause) is exactly the manual drift this eliminates. **Phase 2 → [B159] (MEDIUM)** = the APPLICATION half: writing actual onboarding SERVICE(S) that represent the flow as real code, per the platform-vs-application split — deliberately Medium and later, done once the Phase 1 paved path proves the shape. This item stays scoped to Phase 1.

**PHASE 1a DONE 2026-09-07 — the onboarding-completeness guard (the first surface).** The coverage family already proves a service is scraped/visualized/alerted/cataloged/registered; the ungoverned surface (which the DoD itself flags, and which bit the image-provenance CronJob the same day) was **placement in the single LikeC4 model**. Shipped `scripts/check-onboarding-completeness.sh` (repo-guards, fail-closed 0/1/2): every `deployed: true` registry service must resolve to a real LikeC4 element — by an explicit `likec4: <id>` (for a subsumed/renamed element, e.g. dbt→dagster) or a kind-agnostic normalized key/name match. Registry schema gained **`deployed`** (59 true / 7 false — the code-review SaaS/IDE tools) + **`likec4`** override (3). The guard **caught 3 genuinely-missing elements on first run** — `weyland-agent`, `port-k8s-exporter`, `promptfoo` — all added to `weyland.likec4` (✓ Valid), so the live invariant holds **59/59**. TDD `scripts/tests/onboarding-completeness.bats` **10** cases (incl. the live invariant), shellcheck 0, wired into `.woodpecker.yml` repo-guards. DoD: `flow-onboarding-completeness.md`, `demos/onboarding-completeness.md` (ledger row 60), schema documented in `application-catalog.md` + the registry header. **PHASE 1b DONE 2026-09-07 — the scaffolder.** `scripts/onboard-service.sh` writes both surfaces the Phase-1a guard checks from one command: it appends a paved registry entry (`deployed: true`, `likec4: <camelId>`, group/capabilities/port_component) and inserts a matching LikeC4 element (id = camelCase(key)) as the first element in a chosen zone, then runs `check-onboarding-completeness.sh` to confirm the service lands onboarded-complete. Validates key(kebab)/group/kind/zone against the real files, refuses a duplicate key or element id (no mutation), supports `--dry-run` (verified git-clean on the real files), and leaves edge-wiring as a stated human follow-up (places the element, never guesses relationships). TDD `scripts/tests/onboard-service.bats` **8** cases (incl. dry-run-writes-nothing + end-to-end guard-passes), shellcheck 0. DoD: scaffolder section in `flow-onboarding-completeness.md` + `demos/onboarding-completeness.md` (ledger row 61) + `application-catalog.md`. **Phase 1 (paved road + guard) MVP is now complete.**

**GUARD EXTENDED to the FULL DoD §6 checklist 2026-09-07 (declare + account).** Onboarding is a DoD gate: a newly-onboarded service must clear the whole §6 "deployed service" row, so the guard now covers ALL of it, not a subset. Each deployed service DECLARES its conditional gates (registry gained **`metrics`** + **`ingress`**, backfilled — 19 metrics, 31 ingress). The guard **VERIFIES** the file-checkable gates (hard-fail): SCHEMA (`deployed` declared everywhere; `metrics`+`ingress` declared on every deployed service — an *undeclared* conditional gate is an unaccounted gate, and a missing `deployed` would silently skip the service), PORT (`port_component`), PLACEMENT (a real LikeC4 element — caught + fixed `weyland-agent`/`port-k8s-exporter`/`promptfoo`). It **ACCOUNTS** the rest per service in a `--list` DoD §6 matrix so nothing is silently skipped: ServiceMonitor/dashboard + `*Down` alert = **LIVE** (reconciled at runtime by servicemonitor-/dashboard-/alert-coverage, new services included — a metrics service with no ServiceMonitor surfaces there as `blind`), Kuma = **MANUAL** (UI-configured, not in git), logs = **AUTO** (Alloy), arch.md §6 = **curated** (only 14 of 31 ingress services live in that table — no clean predicate). Codified in `definition-of-done.md` (the deployed-service cascade row names this guard as its standing gate) + the scaffolder writes `metrics`/`ingress` too. Live invariant: 59/59 onboarding-complete. `onboarding-completeness.bats` **14** cases (SCHEMA-metrics + matrix added), `onboard-service.bats` **8**, shellcheck 0.
...
**Approach (2026-09-08): all 9 languages, no pilot (the pattern is proven by B153's Java/Go/Rust/Node/Python).** Each language = flagship framework first (idiomatic, cleanest to bless), additional frameworks as expansion; rolled out one complete language at a time through CI (each toolchain is unverifiable locally, so CI is the proving ground, exactly like B153). Flagships: .NET ASP.NET Core minimal API · Kotlin Ktor · Scala http4s · PHP Slim · Ruby Sinatra · Elixir Phoenix · Clojure Ring+Compojure · C++ Crow · Angular (+ SSR) · Vue (Vue 3 + Vite) — the last two reuse the existing node lanes (no net-new toolchain). **Progress: .NET ✓ (#97), Kotlin ✓ (#100), Scala ✓ (#102), PHP ✓ (#104), Ruby ✓ Rails+Sinatra (#106), Elixir ✓ Phoenix+Plug (#107), Clojure ✓ Compojure+Ring (#108), C++ ✓ cpp-httplib (#109), C ✓ libmicrohttpd (#109), Angular ✓ + Vue ✓ (#111); **ALL 14 golden-path builds CI-VERIFIED across 11 ecosystems (35 paths total).****

**DONE 2026-09-09 — 8-pillar DoD (repo-tooling lens; golden paths deploy NOTHING durable — they are blessed templates + CI-lane fixtures + ephemeral smoke Jobs torn down each run):** **1 docs** — `design/golden-paths.md` §"B160 — extended language wave" (14 paths, the three contract variants, per-ecosystem engineering notes) + matrix-heading pointer; `golden-paths-map.html` all-verified + [maps.md](maps.md); README/arch counts unchanged (`check-doc-counts` green); **schedules N/A** — `golden-path-smoke` is push/manual-triggered, not a timer, so no `schedules.md` row. **2 diagrams** — `flow-golden-paths.md` is mechanism-level + language-agnostic (unchanged, still valid); `check-mermaid` green (176 blocks); LikeC4 **N/A** (deploys no workload to model); the map is the visual index. **3 demo** — the demo IS the CI smoke (build→Job→curl→teardown), RUN per ecosystem #97–#111 ("all N golden path(s) served in-cluster and were torn down"); `demos/golden-paths.md` + demos-ledger **row 63** updated to 35 paths. **4 cleanup** — ephemeral by construction (every smoke Job created+deleted per run; images built+discarded, never shipped); local docker verify-images removed, no stray `build/`/`deps/`/`_build/` on disk, `.gitignore` gained elixir `deps`/`_build`/`cover`, C/C++ `build`, Angular `.angular`. **5 tracking** — this flip + EMA-217→Done; `check-linear-sync` green; **tier rebalance PROPOSED to the human, not applied**. **6 ops (repo-tooling restated)** — no ServiceMonitor/dashboard/alert/backup because nothing durable is deployed; the operational surface is the CI lanes themselves (test-*/scan-*/golden-path-smoke), which run every pipeline; the `golden-paths` ns + `golden-path-runner` SA pre-exist from B153 (no new k8s). **7 scan** — shellcheck 0 on all edited scripts; the **per-language scan lanes** (dotnet-format · ktlint · scalafmt · phpstan · rubocop · credo · clj-kondo · cppcheck) ran over every golden path in CI (advisory findings only, all lanes green); **CodeScene** (direct-to-main AI lane) 10.0 / no smells on the reviewed new source (C dispatch, Clojure core); `check-sa-automount-collisions` green (no SA/RBAC change); `check-quality-tools` green (4 new scanners registered, parity holds); **code-scan-suite RUN 2026-09-09 (`scan-suite-adhoc`, 19 tools over the repo): 0 Critical everywhere, and 0 NEW High from B160** — the repo-wide Highs (kubescape 583H k8s-workload controls, bandit 1H Python, hadolint 4H) are all pre-existing in categories B160 does not touch (it adds no k8s manifest, no Python; osv-scanner clean — no committed lockfiles); B160's 14 Dockerfiles produce **0 hadolint error**, only advisory warnings (16 DL3008 + 2 DL3018 apt/apk version-pin + 1 DL3064) document-accepted as the estate's standard lab-canary convention (base images pinned at `FROM`; golden-path images are ephemeral so dep-CVEs are not a deployed exposure anyway). **8 cascade** — new **scanners** → `quality-tools.yaml` (registered, parity green); lane-script edits → their **bats** (487 green) + `is_excluded` (`deps`/`_build`/`build` added); **no** new repo, service, endpoint/host, timer, or persistent image (golden-path images are ephemeral, so **no `images.tsv` rows** — consistent with B153); reverse sweep — the **Crow→cpp-httplib** and **lein-cljfmt→clj-kondo** substitutions leave no dangling refs (removed from project.clj + READMEs); mobile stays **B164**. Suite green locally: repo-guards (12) · shellcheck 0 · bats 487 · check-mermaid 176 · check-linear-sync.**
Angular + Vue are FRONTEND paths under `golden-paths/frontend/` that reuse the EXISTING node lanes (auto-discovered by package.json + `*.test.ts`; no net-new lane/runner/woodpecker step) — each is an SPA (Angular `ng build`, Vue `vite build`) fronted by the same stdlib+prom-client `server.mjs` sidecar the B153 frontend wave uses, with the service name in `<title>` for the smoke. Angular swaps its default Karma/browser `ng test` for **headless jest** on the pure greeting logic (the node lane has no browser); the built app is proven by the curl smoke. Both verified locally (npm test + tsc + eslint + build + image serve). **Total golden paths: 35** (B153's 21 + 14 from B160).
Clojure: scan = clj-kondo (the lein-cljfmt plugin ArityExceptions on current cljfmt), run in the cljkondo/clj-kondo image; uberjar lands at `target/`, not `target/uberjar/`. **C++** (`cpp/httplib`) + **C** (`c/libmicrohttpd`) are TWO distinct languages (separate `cpp`/`c` lanes, cmake runner, cppcheck scan): C++ uses cpp-httplib (single-header, CMake-fetched — swapped from the backlog's Crow example to dodge the asio build + avoid vendoring a 10k-line header that would blow up the LOC count) + doctest; C uses GNU libmicrohttpd + a CHECK-macro harness (not assert(), which -DNDEBUG compiles away). Both statically resolve the JVM-free contract and were caught needing a **trixie-slim** runtime (gcc:14 is GLIBC 2.41; bookworm-slim's 2.36 is too old — the C++ binary won't start). C/C++ tests unit-test the pure payload-builders (no in-process HTTP mock exists) with the real server proven by the curl smoke. Elixir: Phoenix skeleton generated via `mix phx.new` in docker then trimmed (force_ssl dropped for the LAN plain-HTTP canary, root-scope contract controller) + a pure-Plug lean sibling; both `mix release` on debian-slim; the elixir selfcheck is fail-closed by asserting the failure REASON (`mix test --only <tag>` exits non-zero on zero-match too, so a bare exit check would fail open), and `deps/`/`_build/` were added to `is_excluded` (they carry deps' own `*_test.exs`). Clojure: Ring+Compojure flagship + pure-Ring lean, Leiningen `lein uberjar` on temurin-21-jre; scan = clj-kondo (the lein-cljfmt plugin is broken — ArityException on current cljfmt), run in the cljkondo/clj-kondo image. Every language's whole chain (tests + selfcheck + scan + image build&serve) is proven locally in docker before the CI run. Ruby shipped BOTH frameworks (Rails flagship = the lane fixture + Sinatra lean baseline) — Rails needed `~> 8.0` not 7.2 (ActiveSupport 8 dropped the `quirks_mode` arg to `JSON.generate` that json 3.x removed; a 7.2 bump can't fix it); the whole chain (both `rake test`, selfcheck, rubocop, image build + puma serve as non-root) was proven locally via docker before the CI run, which caught the json break AND a `COPY /app/.bundle` Dockerfile bug (the ruby image redirects `bundle config --local` to `/usr/local/bundle`) before a wasted round-trip. **Every language gets BOTH a test lane AND a scan lane** (correcting B88's gap where go/python have test lanes but no scan — those two need scan lanes added too). **.NET wired (language 1/9), pending first CI run:** `golden-paths/dotnet/aspnet/` (App/ minimal API + prometheus-net · tests/ xunit+WebApplicationFactory · golden.sln · multi-stage Dockerfile mcr sdk→aspnet · smoke.sh); `run-lang-tests.sh`/`run-lang-scan.sh` gained the `dotnet` dispatch (`dotnet test --filter Category!=/=selfcheck`, `dotnet format` scan) + both `LANGS`; `resolve_root` made glob-marker-aware (dotnet's `*.sln`, via `find -name` so it survives the `set -f` in discovery); `.woodpecker.yml` `test-dotnet` + `scan-dotnet` lanes (mcr sdk image); `golden-path-smoke` auto-discovers the Dockerfile. Deferred fast-follows: dotnet coverage-ratchet (collector wiring per toolchain), NuGet cache PVC, and the go/python scan lanes.

Extend **B153**'s golden paths to a second wave of languages, each conforming to the SAME framework-agnostic contract ([docs/design/golden-paths.md](design/golden-paths.md) — `/health` `/ready` `/metrics` `/hello` + lane self-test + selfcheck + coverage + buildkit Dockerfile + run-to-completion Job + onboarding-declaration template; runnable, ephemeral, extendable). **Languages:** Angular (frontend/TS) · Vue (frontend/TS) · C#/.NET · C/C++ · PHP · Ruby on Rails · Elixir · Clojure · Kotlin · Scala. Multiple relevant frameworks per language, locked at build time like B153's matrix — e.g. **.NET** ASP.NET Core (minimal API + MVC) · **PHP** Laravel/Symfony/Slim · **Ruby** Rails/Sinatra · **Elixir** Phoenix/Plug · **Clojure** Ring+Compojure/Pedestal · **Kotlin** Ktor/Spring/http4k · **Scala** Play/Akka-HTTP/http4s/ZIO · **C++** Drogon/Crow/Pistache · **C** libmicrohttpd/civetweb · **Angular** SPA + Universal SSR · **Vue** Vue 3 + Vite. **Angular + Vue are FRONTEND paths in the TypeScript/node ecosystem (added 2026-09-09 at the user's request) — they reuse the EXISTING node lanes + `node:24-alpine` (no net-new toolchain), and serve the contract the way the B153 frontend wave does (a stdlib/SSR sidecar for `/health` `/ready` `/metrics` `/hello`), unlike the JVM/native second-wave languages which each need a net-new lane + runner image.** Each non-frontend language also needs a **new CI test + scan lane** in `run-lang-tests.sh`/`run-lang-scan.sh` (net-new toolchains → new runner images). Depends on **B153** establishing the contract + the ephemeral-Job harness + the golden-paths-as-fixtures switch. Relates **B88** (per-language lanes), **B154** (onboarding gates the paths pass), **B155** (API lifecycle captures their OpenAPI).

---

### B161 — Investigate AND implement Dify — HIGH (2026-09-08, scope widened 2026-09-09, Linear EMA-218)

Evaluate **Dify** (open-source LLMOps platform — visual workflow/agent builder, RAG pipeline, prompt IDE, model/provider management, dataset management, built-in tracing) against the lab's existing AI plane, and produce a keep/skip call. **Method (per the reverse-engineering rule — read the governing reality first):** map Dify's capabilities onto what the estate already runs and find the actual delta, not the marketing surface. Existing coverage to compare against: **gateway** (Bifrost + LiteLLM, 232 MCP tools, prompt/skills repos), **agents** (LangGraph — B66 operator, B17 Realm of Agents, B70 agentic-RAG), **RAG** (Qdrant/Weaviate/LanceDB + bge embed + the streaming indexer), **eval** (promptfoo B84, MLflow gateway, golden-set B96), **observability** (Langfuse B103, MLflow Traces), **workflow** (n8n, Dagster). Open questions: (1) does Dify's low-code workflow/agent canvas give a non-code authoring surface nothing else does, or duplicate LangGraph + n8n? (2) is its RAG/dataset layer additive or a redundant fourth store? (3) can it front the existing Bifrost/Ollama providers rather than its own? (4) $0 + self-hosted + LAN-only (Keycloak forward-auth, meshed Postgres/Redis) — does it fit the node capacity budget? **Two phases — investigate THEN implement (scope widened 2026-09-09):**

**Phase 1 — investigate. DONE 2026-09-09 → [concepts/dify-evaluation.md](concepts/dify-evaluation.md).** Verdict: **adopt as a COMPLEMENT — the low-code LLM-app front door, deployed thin.** Dify's one genuine delta is its visual non-code canvas (LangGraph is code, n8n isn't LLM-native); every other plane it bundles (providers/vectors/tools/tracing/prompt/eval) overlaps the estate and is wired to REUSE it — providers→LiteLLM/Bifrost (OpenAI-compat), knowledge→external Qdrant, tools→the MCP gateway (Dify is an MCP client; it can also publish apps AS MCP servers), traces→Langfuse+OTLP/Tempo. Its cost is its own multi-container runtime, which lands against the ~80Gi swapless `mother` ceiling — so the central Phase-2 call is **on-demand/store-scaler-parkable vs always-on** (recommend on-demand). Open Phase-2 decisions in the doc.

**Phase 2 — implement. PARKED 2026-09-09 — BLOCKED BY B134 (EMA-218 blockedBy EMA-195).** Phase-1's node-capacity finding is a hard gate: `mother` has ~4.6Gi memory headroom at peak (B134 re-measure 2026-08-27 — memory is the binding constraint now, not CPU), so Dify's multi-container runtime can't run always-on; and the on-demand alternative it would need is precisely what B134 **deferred** — there is no robust on-demand workload manager (ceding `/spec/replicas` to store-scaler/KEDA was rejected on mechanism: Argo self-heal reverts `replicas:0`, a sleeping pod reports Healthy, KEDA was retired for it). Dify can go **neither always-on (no memory) nor cleanly on-demand (no manager)** until B134 delivers a workload manager without that carve-out (or frees steady memory). **Resume Phase 2 when B134 unblocks.** The plan is pre-staged below.

**Phase 2 (pre-staged) — implement.** Stand Dify up in the lab as a first-class deployed service, per its Phase-1 positioning: **GitOps** (Argo app, image on `registry.weyland.lab` — mirror the upstream `langgenius/dify-*` images, no `:latest`), **meshed** (Istio STRICT — in-cluster Postgres + Redis + the vector store it needs; reuse an existing Postgres/Valkey where the plane fits rather than a redundant fourth store), **Keycloak forward-auth** at `dify.weyland.lab` (+ `/etc/hosts`, blackbox probe), **fronting the existing model plane** (point Dify's providers at Bifrost/LiteLLM/Ollama rather than direct vendor keys), and sized within the node budget (it's a multi-container app — api/worker/web/sandbox — watch the mother RAM/CPU walls). Then the full **8-pillar DoD** it owes as a real deployed service: docs (arch/hosts/api), LikeC4 placement, `applications.yaml` + Port component, ServiceMonitor+dashboard or a documented N/A, `*Down` alert, Kuma monitor, backup if it holds non-reproducible state, a demo + ledger row, scan, cascade. **Deliverable:** the Phase-1 positioning table AND a running, governed `dify.weyland.lab`. Relates B17+B19, B103, B84, B66; onboarding via **B154** (`onboard-service.sh`), API capture via **B155**.

---

### B162 — System-complexity bucket: reward DEEP modules, re-evaluate method line-count thresholds (Ousterhout) — DONE 2026-09-21 (Linear EMA-219)

**DONE 2026-09-21.** Built the lab's machine-checkable reading of complexity as a 3-stage triage engine
(`scripts/lib/complexity_triage.py`): **length only NOMINATES**, then **structure** (cyclomatic density + true AST
max-nesting) and a **self-calibrating codebase-relative z-score** decide, and the **inverse over-split smell** is
...
profile is server-side; confirming/adjusting it is an in-cluster operator step in the runbook). **First run
surfaced 20 TANGLED + 2 SHALLOW → 19 refactored to deep modules + 2 SHALLOW fixed + 1 adjudicated acceptable
(`build_store_load_assets`, a low-density 0.17 per-store registry), ZERO behavior change** — verified per function
(138 datahub + 37 loader + 8 tool + 7 kindle tests green; bats + compile+triage for the untested `assets/*.py`
integration code); the tool now passes its own triage. **8-pillar DoD (repo-tooling lens):** 1 docs —
[concepts/reading-complexity.md](concepts/reading-complexity.md); 2 diagram —
[diagrams/flow-complexity-triage.md](diagrams/flow-complexity-triage.md) (check-mermaid ✓); 3 demo —
[demos/complexity-triage.md](demos/complexity-triage.md) (ledger #82, the run IS the evidence); 4 cleanup — N/A
(repo tooling); 5 this flip + EMA-219; 6 ops — the advisory lane + runbook incl. the remediation log; 7 scan —
engine tests 8/8 + wrapper 7/7 in the CI toolchain images; 8 cascade — no shared contract changed, all refactors
behavior-preserving. Runbook: [runbooks/complexity-triage.md](runbooks/complexity-triage.md). Original scope below.

A standing **complexity bucket** to steer the codebase toward **deep modules** in the sense of John Ousterhout's *A Philosophy of Software Design*: the best modules present a **simple interface** over **substantial implementation** (high depth = functionality ÷ interface surface); the failure mode is **shallow modules** and reflexive over-decomposition ("classitis", "temporal decomposition") that multiply interface cost — passing files, threading state, splitting a coherent operation across many tiny methods — for no information-hiding gain. **The problem to fix:** the lab's scan suite grades method **length** and **cyclomatic complexity** on conventional thresholds (SonarQube quality profile, CodeScene code-health, the B106 stack) that penalize a long method *per se*, which nudges toward exactly the chopped-up, shallow decomposition Ousterhout argues against. **Re-evaluate those thresholds** so they reward depth and information hiding rather than raw line counts: distinguish a method that is long **because it hides real complexity behind a clean signature** (keep — that is a deep module) from one that is long because it is tangled (fix); and surface the inverse smell — too many shallow pass-through methods / over-split classes — which today's rules never flag. **Deliverable:** a reviewed rule/threshold set (CodeScene `rules_config_set_threshold` + SonarQube quality profile) plus a short design note (`docs/concepts/` or `docs/design/`) on how the lab reads complexity, so scanner findings are triaged against depth, not line count. Ties to the **platform-vs-application split** (B9 — real deep services are the destination; management shell/Python is the interim "bad juju"). Relates B106 (code-review stack), B120 (quality-tools registry), B47 (findings triage), B9.

---

### B163 — Golden-path capability coverage: exercise each framework's modules semi-exhaustively — MEDIUM (↓ High→Medium 2026-09-08, Linear EMA-220)

**Scoping note (2026-09-08): this is a PROGRAM, not an item — decompose before executing.** At full scope it is a matrix explosion: 21 paths × each framework's native modules × the estate's entire integration surface (Postgres · Trino · the 10 Tier-2 stores · 3 vector stores · Neo4j · Redpanda/Kafka · Valkey · MinIO/S3 · auth/OIDC · OTel/logging · secrets · feature flags). The reframe that makes it tractable — the matrix is NOT square, it splits into two tiers with different multipliers: **(1) framework-native modules** (routing/middleware/validation/DI/error-handling/lifecycle) are genuinely **per-framework** (21 × ~7); **(2) integrations** are genuinely **per-language, NOT per-framework** — the client library is shared across a language's frameworks (FastAPI/Flask/Litestar/Django all use the same psycopg/trino/qdrant clients), so integration coverage collapses from "21 × N backends" to "**6 language ecosystems × N backends**," and each golden path inherits its language's integration layer. Recommended shape: re-frame B163 as an **umbrella/bucket** (like B49/B78) spawning sub-items — per-language integration suites + per-framework module depth — each independently shippable and sequenced, reference-path-first then per-language batches (the B153 cadence). **The next step is a decomposition/design pass — no code** — producing the capability catalog (in-scope modules + backends, explicit skips) and the sub-item breakdown; that design is the deliverable that prevents building into the explosion.


B153 shipped 21 golden paths conforming to the MINIMAL framework-agnostic contract (`/health` `/ready` `/metrics` `/hello` + lane test/selfcheck/coverage + buildkit Dockerfile + run-to-completion smoke Job). That proves the paved road builds, tests, scans, and SERVES — but it exercises a thin slice of each framework. **B163 deepens all 21 paths to semi-exhaustively cover their framework's module capabilities**, so `scripts/new-service.sh <path>` scaffolds a real demonstration of the framework, not a stub. Per framework, cover the surface a real service actually reaches for: **routing** (path/query params, wildcards, grouping), **middleware / filters / interceptors**, **request validation + typed binding**, **structured error handling** (framework-idiomatic exception mapping), **dependency injection / app context**, **config + env**, **lifecycle / background hooks**, and the framework's **testing idioms**. The concrete surface is framework-specific — e.g. NestJS pipes/guards/modules/DI, FastAPI dependencies + Pydantic models, Spring `@RestControllerAdvice` + `@ConfigurationProperties`, Axum extractors + `tower` middleware, Fiber middleware groups, Flask blueprints, Litestar controllers/DI, Astro/Remix/Next route conventions. Every added capability MUST keep the contract green and gain its own lane tests + updated coverage baselines (`tests/lang/coverage-baseline.tsv`), and all 21 must stay green through the B153 lanes + `golden-path-smoke`. **Semi-exhaustive, not exhaustive** — cover the modules a real service uses, skip the exotic. **Deliverable:** a per-framework capability checklist (what "covered" means for each) + the deepened paths. Depends on **B153** (the paths + ephemeral-Job harness); relates **B160** (extended-language paths inherit the deeper template), **B154**/**B155** (onboarding + API-lifecycle gates now exercise a richer surface).

---

### B164 — Extended golden paths: mobile clients + additional-language wave — DONE 2026-09-13 (Linear EMA-221)

Mobile is absent from B153 and B160, and it does NOT fit the golden-path contract as written. The contract is an **HTTP service** contract — `GET /health` `/ready` `/metrics` `/hello`, proven by build-image → run-to-completion k8s Job → curl `/hello` (`golden-path-smoke`). A mobile app is a **client**: it serves no endpoints, has no `/health`, and ships to a device/app-store, not a Job — so it can satisfy neither the endpoint contract nor the image+curl smoke. Covering mobile therefore means an **adapted contract**, exactly as the frontend paths adapted the contract for SSR: **(1)** an app skeleton that bundles + builds (**React Native** flagship — Expo or bare RN; **Flutter** a possible second); **(2)** a **test lane** (net-new toolchain + runner image — jest + `@testing-library/react-native`) covering a component that renders the `hello, weyland` payload; **(3)** a **scan lane** (eslint/tsc — the estate rule that every language gets one); **(4)** a **"smoke" adapted to a client** — a headless **bundle/render** check (`expo export` / metro bundle / `flutter test`), NOT the HTTP-serve Job. The design doc gains a **client/mobile contract variant** alongside the service and frontend variants. **Targets (extended 2026-09-08 at the user's request):** **React Native** (flagship — jest + `@testing-library/react-native` + metro/expo; **Linux-CI-buildable ✓**) · **Flutter** (`flutter test` + Android/web bundle; **Linux-CI-buildable ✓**) · **Swift/iOS** (SwiftUI — **HARDWARE-GATED**: a SwiftUI iOS app needs **macOS + Xcode**, which the $0 Linux homelab has no runner for, so the template is authorable but its lane **cannot run** until a macOS runner exists — this thread ties the hardware-gated set [B149/B150]. Distinct from **server-side Swift** (Vapor/Hummingbird), which IS a Linux-buildable HTTP service and would belong in **B160**, not here). Deliverable: **RN + Flutter lane-verified**, **Swift/iOS authored + parked on macOS**, + the documented variant. Relates **B153** (contract + harness), **B160** (language suite), **B163** (per-framework depth). Prompted 2026-09-08 by the "is mobile covered?" check during the B160 rollout; Flutter + Swift added same-day.

**Workstream (2) — additional-language SERVICE wave (added 2026-09-11, operator).** B164 is broadened from mobile-only to the "extended golden paths" umbrella: alongside mobile clients it carries a **B160-shaped wave of 9 more SERVICE languages**, each conforming to the same framework-agnostic HTTP contract + the B160 "prove the whole chain (test + selfcheck + scan + image build & serve) in Docker before the CI push" cadence; each non-frontend language = a **net-new CI test AND scan lane + runner image**. Triaged from a 16-language proposal against the bar (serves the HTTP contract · $0 OSS Linux-CI buildable · a language you'd realistically start a new service in):
- **Clean keeps (5):** **Swift** server-side (Vapor/Hummingbird — official **Linux** toolchain, builds in the $0 Linux CI with **no cloud/macOS needed**; this supersedes the workstream-1 aside that server-Swift "belongs in B160" — it lives here now. NOT the hardware-gated iOS Swift, which stays workstream 1) · **Erlang** (Cowboy — same BEAM VM as the shipped Elixir/Phoenix [B160], high coherence) · **Julia** (Genie/Oxygen/HTTP.jl — ML/data-lab relevant) · **Lua** (OpenResty/Lapis — the lab's own APISIX gateway is OpenResty/Lua) · **Dart** server-side (shelf/dart_frog — a distinct HTTP-service path from Flutter in workstream 1, sharing one Dart toolchain/lane).
- **Optional / breadth (4):** **R** (plumber) · **Perl** (Mojolicious/Dancer2) · **Haskell** (Servant/Yesod) · **Ada** (AWS — Ada Web Server; conformant + OSS but zero lab relevance, lowest priority).
- **Shitcanned (7, do NOT build):** SQL (query lang, not a service — its road is dbt/Trino/ODCS) · Fortran (no real HTTP framework) · **Delphi (proprietary/Windows — fails $0)** · Scratch (visual teaching lang) · Assembly (novelty) · Objective-C (legacy Apple, superseded by Swift) · COBOL (legacy mainframe, no realistic new service).
- **Cloud-macOS note:** only *iOS* Swift (workstream 1) is hardware-gated; cloud-macOS runners (GitHub Actions macOS / Xcode Cloud / MacStadium / Cirrus) exist but break the $0/self-hosted/LAN model, so iOS stays parked — server-side Swift here needs none of that.

**Progress — clean keeps 5/5 BUILT + docker-verified (2026-09-11):** `golden-paths/{erlang/cowboy, julia/oxygen, lua/openresty, swift/vapor-app, dart/shelf}` each built + verified on rogueone in its toolchain image — tests pass, selfcheck fails, multi-stage non-root image builds, and all four contract endpoints served live (`/hello` = `{service,message}` with `hello, weyland`, consistent across all five). **Test lanes WIRED + verified:** `run-lang-tests.sh` (LANGS + runner/glob/root-marker + dispatch cases with each lang's verified selfcheck command — e.g. dart `-t selfcheck --run-skipped`, swift `GOLDEN_SELFCHECK=1 … --filter`, lua `busted --run=selfcheck`, erlang `rebar3 as selfcheck eunit`, julia `GOLDEN_SELFCHECK=1 Pkg.test()`) + `lib/lang-fixtures.sh` (5 fixture mappings); `--list-roots` confirms all 5 discover their golden path. **Fixed inline:** julia's agent-authored nested `test/Project.toml` was a discovery marker (double-registered `test/` as a 2nd project) → moved test deps to the main `Project.toml` `[extras]`+`[targets]`, re-verified `Pkg.test()` 11/11. **REMAINING for the 5:** `.woodpecker.yml` 5 `test-*` + 5 `scan-*` lanes (toolchain images: erlang:27 · julia:1.11 · lua+busted · swift:6.0 · dart:stable); `run-lang-scan.sh` scan funcs + `quality-tools.yaml` registration (scan commands: dart `dart analyze` ✓verified, lua `luacheck` ✓verified; swift `swift-format`, erlang `elvis`, julia `JuliaFormatter` — need docker-verify before wiring); DoD (design-doc §, demos row, guards). `golden-path-smoke` auto-discovers the new Dockerfiles → builds+serves them in-cluster on the next push. **UPDATE 2026-09-11 — ALL 9 language-wave langs BUILT + docker-verified + WIRED:** the 4 optional (R/plumber · Perl/Mojolicious · Haskell/Scotty · Ada/AWS) joined the 5 clean keeps — each verified in its toolchain image (tests pass · selfcheck fails · non-root image · 4 endpoints served) and wired into all 5 shared surfaces (`run-lang-tests.sh` + `lib/lang-fixtures.sh` + `run-lang-scan.sh` + `quality-tools.yaml` [30 lang-scan tools] + `.woodpecker.yml` [`test-*`/`scan-*` per lang]). Scanners: erlang `rebar3 xref` · julia JuliaFormatter · lua `luacheck` · swift `swift format` · dart `dart analyze` · r `lintr` · perl `perlcritic` · haskell `hlint` · ada `-gnatwa/-gnaty` (gnatcheck deferred). All local guards green (quality-tools 30 · doc-counts 81 · discovery for all 9). Notable fixes caught inline: julia nested-Project.toml discovery bug; the Ada lane pins Alire binary GNAT 14.2.1 (Ubuntu FSF GNAT can't compile AWS 25.x). **CI-CONFIRMED 2026-09-13:** all 58 test+scan lanes green (pipeline #126), and all 9 Dockerfiles built via buildkit + served in-cluster as k8s Jobs + torn down (#134, "OK — all 9 golden path(s) served in-cluster and were torn down"). Matrix **35 → 44** (service paths). CI churn resolved along the way: alpine lua lanes needed `apk add bash`; SwiftPM `.build/` added to `is_excluded` + the golden-path-smoke Dockerfile-prune (dep-checkout leak); `swift/vapor` renamed to `swift/vapor-app` (SwiftPM 6.0 identity collision with the `vapor` dep); `test-swift`/`test-haskell`/`test-ada` given memory requests + `-j` caps (best-effort OOM on the RAM-tight node); repo pipeline timeout 60→180.

**Workstream (1) — MOBILE clients BUILT + docker-verified + WIRED (2026-09-11).** All three mobile paths built under `golden-paths/mobile/` and verified on rogueone; mobile is a CLIENT (no HTTP contract, no Dockerfile, no k8s Job → `golden-path-smoke` auto-skips these Dockerfile-less dirs), so each lane's smoke is a HEADLESS BUNDLE/RENDER instead: **React Native**/Expo (`jest-expo` 4/4 · selfcheck fails · bundle smoke `expo export --platform web` · `eslint` scan; lane image `node:24` Debian/glibc — expo's export pulls native modules that fail on musl-alpine) · **Flutter** (`flutter test` widget pass · selfcheck forced with `-t selfcheck --run-skipped` · render smoke `flutter build web` · `flutter analyze` scan; lane image `ghcr.io/cirruslabs/flutter:stable`, image existence verified HTTP 200) · **Swift-iOS** (Linux-verifiable part only — a pure-Swift `Greeting` module, `swift test` 5/5; SwiftUI/iOS surface `#if canImport(SwiftUI)`-guarded so the Linux build stays green — rides `test-swift`'s discovery, verified as 1 discovered project, needs NO lane of its own; the iOS/SwiftUI UI is PARKED on the Mac hardware gate). **Discovery-collision handling (the reason this was more than "add a lang"):** react-native shares `package.json`+`*.test.tsx` with the node lanes and flutter shares `pubspec.yaml`+`*_test.dart` with the dart lane — fixed both directions: a path exclusion in `is_excluded()` keeps node/dart lanes off the mobile fixtures (which run directly via `resolve_fixture`, no coverage lost), and a distinctive test glob keeps the mobile lanes off their siblings (flutter keys on the naturally-unique `widget_test.dart`; RN's test files were renamed to `*.rn.test.tsx`, still matched by jest's default discovery, verified via `--listTests` + a full lane run). Verified: react-native + flutter each discover **0 real projects** (fixture-only), dart no longer sees flutter, node lanes no longer see RN, swift still sees swift-ios. Wired across all 5 surfaces (`run-lang-tests.sh` · `lib/lang-fixtures.sh` · `run-lang-scan.sh` · `quality-tools.yaml` — 31 lang-scan tools, `flutter-analyze` added + `eslint` reused for RN · `.woodpecker.yml` `test-react-native`/`test-flutter`/`scan-react-native`/`scan-flutter`). **UPDATE 2026-09-11 — 4th mobile path added: `swift-tokamak` ("Swift without iOS").** Tokamak is a SwiftUI-compatible API that compiles via SwiftWasm to WebAssembly and renders to the DOM — the SwiftUI programming model + build chain proven entirely on Linux at $0, the Linux-verifiable half of the parked swift-ios UI. Spiked-then-built in Docker on rogueone: `carton test --environment node` (wasm XCTest in Node.js — a native `swift test` fails because off-wasm Tokamak needs its GTK backend) 2/2 pass · self-check `-Xswiftc -DSELFCHECK` fails (compile-flag-gated, fail-closed — carton test has no --filter and WASI does not forward host env) · `carton bundle` → `Bundle/` (index.html + 12MB .wasm + JS glue). **Stack PINNED/frozen** (Tokamak's last release 0.11.1/Feb-2023, carton's images stop at 0.20.1; lane image `ghcr.io/swiftwasm/carton:0.20.1` = SwiftWasm Swift 5.9.1; `Package.resolved` committed). **TEST-ONLY for lang-scan** (swift-format shipped with Swift 6.0, absent from 5.9.1; the swift/vapor-app lane already lints Swift — no un-runnable scanner). Collision handled like the others: `is_excluded()` keeps the swift lane off it, and a `*.tokamak.swift` test glob keeps its lane off vapor/swift-ios (verified: swift-tokamak 0 real projects; swift lane sees only vapor + swift-ios). Lane `test-swift-tokamak` added (no scan lane). Local guards green (`check-quality-tools` parity 31 · discovery all-clean · `.woodpecker.yml` parses, 65 steps). **CI-CONFIRMED 2026-09-13:** all 5 mobile lanes green (#126). Matrix now **48 (44 service + 4 mobile)**. The on-device iOS/SwiftUI UI (macOS + Xcode hardware gate) is **carved out to [B168] (Very Low)** — not a B164 blocker, since `swift-tokamak` covers the Linux-verifiable Swift-UI half.

**On-demand golden-path smoke capability (delivered 2026-09-13).** Build+serve SPECIFIC golden paths in minutes, skipping the ~75-min lane suite: `woodpecker-cli pipeline create edtbl76/weyland-lab --branch main --var SMOKE_ONLY=smoke --var GOLDEN_PATH_ONLY='<lang/fw …>'`. ONE workflow, step-level gate (`when: *skip_on_smoke` = `SMOKE_ONLY != "smoke"`) on every heavy lane; only `golden-path-smoke` runs, scoped by `GOLDEN_PATH_ONLY`. Proven #133 (dart/shelf, ~1 min) + #134 (all 9 B164 paths, ~15 min). Documented in `docs/runbooks/woodpecker.md`. Hard-won gotchas (all documented there): the sentinel is the STRING `smoke` not a boolean (a bool is YAML-coerced → string compare errors → empty pipeline / HTTP 204 / CLI `error: EOF`); it's a single workflow because Woodpecker feeds a `--var` only to the FIRST workflow's `when` (a `.woodpecker/` split can't gate a second workflow on it); `pipeline deploy`/`deployment`-event selection is a 403 permission wall for this token. Plain `create` (no var) + the nightly cron still run the full pipeline — manual-full preserved.

### B165 — Kindle HIGHLIGHTS → knowledge-management RAG — MEDIUM (↓ High→Medium 2026-09-20: full-text is Amazon-locked, re-scoped to highlights; 2026-09-09, Linear EMA-222)

**RE-SCOPED 2026-09-20 — full text is dead for this account; corpus is now HIGHLIGHTS.** The full-text route the spike flagged as the real risk hit its wall, proven the hard way in a throwaway Windows VM: Amazon has locked the old **DeDRM-strippable Kindle-for-PC (1.x)** out of *downloading* — **1.17** wouldn't even sign in; **1.36** signed in and synced the 188-book library but **refused every download** ("This version is out of date"). The current app (2.x) downloads but its DRM can't be stripped. So there is no $0 full-text path in 2026. **Re-scoped to the highlights floor:** the owner's own highlights/notes from `read.amazon.com/notebook` (no DRM, fully legal) → one `.txt` per book → MinIO `kindle-corpus` → the **existing** ingestion. **Kept + proven:** the cluster-side pipeline (`kindle_text_parse` + `datasets_kindle_{books_land,transform}` → chunk → bge → `kindle` Qdrant/Weaviate collection, definitions load in the dagster image, 40 light-lane tests + a live Gutenberg-EPUB parse). **Removed:** the whole Windows-VM / DeDRM machinery (`nodes/rogueone/kindle/*` — provision/autounattend/setup/extract/autorun/bootstrap). **Remaining (Medium, manual):** extract highlights (manual copy or a Playwright scrape of the notebook) + curate → land in `kindle-corpus` → run the `kindle` land/hydrate jobs + eval. Docs: [runbooks/kindle-rag.md](runbooks/kindle-rag.md) · [diagrams/flow-kindle-rag.md](diagrams/flow-kindle-rag.md) · [concepts/kindle-rag-eval.md](concepts/kindle-rag-eval.md). Linear: EMA-222.

**Original (full-text) scope below — retained for the record.**

Ingest the owner's full personal Kindle library as a RAG corpus for knowledge management. **Personal use, LAN-only, $0 — books the owner already bought.** The build is 90% existing infra; the risk is entirely in **stage 1, text extraction**, which is the hard part and the reason this is not Low.

**Stage 1 — extraction (the landmine).** Kindle books are DRM-wrapped (KFX/AZW3), so raw text is not sittable-on-disk. Options, ordered by how likely they still work on a `$0` Linux box in late 2025:
- **(a) Full text via de-DRM** — the classic path: an *older* Kindle-for-PC/Mac (≤ 1.x, pre-KFX-only) downloads AZW3, then **Calibre + the DeDRM plugin** strips protection → EPUB/TXT. **The catch:** Amazon has been forcing KFX and (Feb 2025) removed "Download & transfer via USB" for many accounts, so the pinned-old-app trick is fragile and account-dependent. This needs a **live spike** to confirm it works for *this* library before committing — do not assume it does (this is the exact "verify the real thing once before encoding an assumption" trap). Runs on a throwaway Windows/Mac VM or `wine`, NOT in-cluster.
- **(b) Highlights + notes only** — `read.amazon.com/notebook` exports the owner's own highlights/annotations per book (no DRM circumvention at all). **Legit and easy, but partial** — only what was highlighted, not full text. A strong *complement* to (a) and a fine *fallback* if (a) dies: highlights are the highest-signal passages anyway.
- **(c) Personal documents** — anything sent via Send-to-Kindle is already the owner's own unprotected file; harvest those directly.
- Decide the mix at the spike; **(b)+(c) are the guaranteed-legal floor**, (a) is the full-corpus stretch.

**Stage 2 — ingestion (reuse, don't build).** Extracted EPUB/TXT → the **existing** section-aware chunker + text→vector fan-out from the finance-filings RAG ([[b113-finance-filings-rag]]), embedded with **bge-base (768)** ([[b74-retrieval-precision]]), landed in **Qdrant/Weaviate** ([[vector-store-hydration-b1]]) as a new `kindle` collection. Books have real structure (chapters/sections) so the section-aware chunker fits directly. Per-book metadata (title/author/ASIN) as payload for filtered retrieval. Register the corpus + lineage in DataHub like any other dataset.

**Stage 3 — the KM surface.** Query via the existing eval/retrieval stack; expose through the operator or a small MCP so "what do my books say about X" is answerable across the whole library. Grade retrieval against a hand-built golden set ([[eval-golden-set-b96]]) as with every other RAG lane.
...
**DONE 2026-09-11.** All 7 candidates evaluated; **Serena + Sourcebot + Zoekt adopted, Joern on-demand, OpenGrok/Hound/Livegrep dropped** (operator decision). Serena wired local (`.mcp.json`, LSP→MCP) + eyes-on proven (find-refs on `verdict.py` `Decision`, 15 files, native `mcp__serena__*`). Sourcebot + standalone Zoekt deployed as GitOps services (Argo Synced+Healthy, reuse weyland-postgres+Valkey; zoekt daily index cron), search verified in-cluster. **All 3 operator eyes-on gates cleared 2026-09-11:** Sourcebot web-UI sign-up + search, 2 Kuma monitors (sourcebot/zoekt), `/etc/hosts` on rogueone. MCP two-plane separation (shared read-only fleet vs. local agent-side Serena) documented in mcp-fleet runbook + arch.md. 8-pillar DoD swept, all guards green (doc-counts 81 · app-registry · onboarding 61 · image-provenance 126 · cron-budgets 18 · mermaid 179 · linear-sync). Follow-on **B167** (Plannotator) added HIGH. Progress detail below.

**Progress (2026-09-11):** **Eval DONE** → [concepts/code-intelligence-eval.md](concepts/code-intelligence-eval.md). Headline (verified, not assumed): **Sourcegraph is out for a $0 lab** (enterprise-only + source-closed since 2024; Cody Free killed 2025-07); the 2026 $0 answer is **MCP-native code-intel servers for agents** + OSS search engines. **Owner picked a bake-off of 7:** Serena · Sourcebot · Zoekt · OpenGrok · Hound · Livegrep · Joern. **Serena — DONE:** `oraios/serena` (LSP→MCP: find_symbol/find_referencing_symbols/symbol-edit), verified on real code (`serena project index services/weyland-guard` → 25 files, exit 0), **wired into `.mcp.json`** (repo root, `ide-assistant`, stdio; `.serena/` gitignored). MCP loads on Claude Code restart → eyes-on test = find-refs on the duplicated `guardrails/verdict.py` `Decision`. Runbook [runbooks/code-intelligence.md](runbooks/code-intelligence.md). **Search bake-off — DONE (2026-09-11):** stood up **Sourcebot** (3 throwaway containers: Postgres 16 + Redis 7 + app on isolated `sb-net`) and **proved Zoekt search on the real repo** — `GuardrailPipeline` → **9 files in 0.19ms** over a single 59.7 MB index shard (queried the internal `zoekt-webserver` at `:6070`; the v5 web UI/API are auth-gated). Key cost finding: **Sourcebot v5 dropped embedded Postgres** — now a 3-service deploy needing external Postgres+Redis + `SOURCEBOT_ENCRYPTION_KEY`+`AUTH_SECRET`, heavier than the "one container" the box implies. **Standalone Zoekt** = that same engine minus the wrapper (1 process + 1 shard, no DB/auth) — proven as the $0 fallback if the node can't spare 3 services. **OpenGrok/Hound/Livegrep** documented (not deployed): all regex/xref search, **no symbol resolution + no MCP**, lose to Sourcebot with no offset (Hound/Livegrep also under-maintained). Results table in the eval doc; deploy + eyes-on in the runbook. **DECISION (operator, 2026-09-11): adopt Serena + Sourcebot + Zoekt; Joern on-demand; drop OpenGrok/Hound/Livegrep.** Serena = agent semantic (already wired, zero footprint); **Sourcebot AND standalone Zoekt both stood up as real LAN services** (Sourcebot = human web UI + MCP; standalone Zoekt = the un-authed lightweight JSON search engine for scripts/agents) — impl half now = GitOps manifests for both. **Joern — DONE (2026-09-11, separate track):** ran `joern-cli` v4.0.626 (no public image → 1.8 GB release zip in a JDK-21 container; v4 targets ≤21, host JDK is 25); `pysrc2cpg` on `weyland-guard` → **CPG 26 files / 292 methods / 3136 calls**; queries returned what no search engine can — call-graph fan-out, a risky-sink inventory (9, incl. the real `exec(compile(path.read_text()))` in `test_verdict_contract.py:48` + `json.loads` in `policy.py`), and **9 taint flows** from request `payload` → async validator via `reachableByFlows`. Confirms Joern is a **security/dataflow tool beside B47/B106, on-demand not a service** (JVM + per-language CPG build, no agent MCP). Runbook has reproducible build+query. **All 7 candidates now evaluated; decision made (above).** **Impl half — GitOps manifests BUILT + locally validated (2026-09-11):** `k8s/sourcebot/` (app Deployment reusing `weyland-postgres` [dedicated `sourcebot` DB, meshed sidecar + hold-for-proxy] + `valkey.data-mesh`; config/repos ConfigMaps; 5Gi PVC; forward-auth ingress `sourcebot.weyland.lab`) + `k8s/zoekt/` (standalone `zoekt-webserver` [pinned image, verified `/healthz`] + daily `zoekt-index` CronJob [03:35 NY, no sidecar] + un-authed ingress `zoekt.weyland.lab`). Full cascade done: 2 Argo apps (`subdir-apps.yaml`), 2 registry entries (`applications.yaml`, `code-search`/`code-navigation`/`mcp` capability terms added to `datahub_emit.py`), LikeC4 placement (2 components), `image-signatures.yaml` allow-list (`sourcegraph/`; `ghcr.io/` root already covers Sourcebot), schedules row + `ScheduledJobStale`/`ScheduledJobFailed` budgets for the cron, the 14 app-count doc bumps (79→81), a `flow-code-intelligence` diagram, demo `code-intelligence.md` + ledger #68. **Guards GREEN:** kubectl client dry-run (11 resources), app-registry (81), doc-counts (81), onboarding-completeness (61, LikeC4 placed), image-provenance (126 imgs), cron-freshness-budgets (18), port-iac, quality-tools, sa-automount, mermaid (179), linear-sync (170). (`opa` rego guard = CI-only here — sandbox blocks chmod; the identical-rule Python provenance guard passed.) **DEPLOYED + LIVE (2026-09-11):** operator ran the deploy — `sourcebot` DB created, secret sealed, pushed; **both Argo apps Synced + Healthy** (sourcebot 2/2, zoekt-webserver 1/1); zoekt index seeded (`zoekt-index-now` Job: 2686 files / 27641 symbols) and **search verified in-cluster** (`GuardrailPipeline` → 12 files / 1.6ms); Sourcebot backend cloned + indexed weyland-lab (59.8 MB shard, serving 307→login). **Serena eyes-on DONE** — `find_referencing_symbols` on `verdict.py` `Decision` resolved 15 files of LSP-accurate refs, natively via `mcp__serena__*` (project MCP loaded on restart; local plane, NOT the gateway fleet — separation documented in [runbooks/mcp-fleet.md](runbooks/mcp-fleet.md) + arch.md). **DoD swept:** docs (eval/runbook/api/hosts/arch/mcp-fleet), LikeC4 (2 components), `flow-code-intelligence` diagram, demo #68, schedules + `ScheduledJobStale`/`ScheduledJobFailed` budgets, image allow-list, registry + capability vocab. **Guards GREEN:** doc-counts 81 · app-registry · onboarding 61 · image-provenance 126 · cron-budgets 18 · mermaid 179 · linear-sync. **Remaining = operator manual gates only (why this is NOT flipped DONE — the B104 UI-eyes-on lesson):** (a) **Sourcebot web-UI eyes-on** — sign up at `sourcebot.weyland.lab` + run a cross-repo search (CLI-green ≠ UI-correct); (b) **2 Kuma monitors** (ingress gate, manual UI) for sourcebot + zoekt hosts; (c) **`/etc/hosts` on rogueone** for both subdomains — the workstation resolves `*.weyland.lab` per-host, so the wildcard only covers cluster/mother (corrects an earlier "no DNS change" note). Flips DONE when (a)–(c) are confirmed. OSS alternatives to the closed/research tools all documented (Sourcebot/Zoekt/OpenGrok/hound/livegrep for search; Joern/Semgrep for CodeQL; RepoGraph/CodexGraph research code is OSS — CodexGraph's Neo4j-code-graph approach borrowable since the lab runs Neo4j).

Evaluate + stand up a real **code-intelligence / code-graph** layer — structural understanding of the codebase (symbols, defs/refs, call graphs, code-property graphs) for BOTH **developer navigation/search** AND **feeding the lab's coding agents structural context** — the "who calls this / what breaks if I change X" question that grep + embeddings answer poorly. **$0 / OSS / self-hostable** lens.

**Why HIGH + why now:** it's a lever on two things at once — day-to-day code nav AND coding-agent quality (structural context beats grep/embeddings for cascade/impact questions, exactly what the DoD Pillar 8 code-half asks). Ties to **B104** (coding-agent eval — measure whether a code graph improves agent outcomes), the **MCP fleet** (a code-graph MCP server would join the read-only fleet the agents already consume), and the existing **`graphify.sh`** (the lab's nascent, advisory, TS/Py-only, shell-blind dep graph — [[graphify-code-cascade]]).

**Candidates to weigh:**
- **Sourcegraph** — self-hostable code search/nav, SCIP-backed. Build candidate if the community/self-host tier is genuinely $0.
- **SCIP** (Sourcegraph Code Intelligence Protocol) + OSS per-language indexers (`scip-python`/`-typescript`/`-java`/…) — the indexing layer; feeds Sourcegraph or a custom consumer.
- **Aider RepoMap** — tree-sitter ranked symbol map (Apache-2.0), lightweight/local. The cheapest "structural context for an LLM" win; pairs directly with B104's agents.
- **Joern** — OSS **code-property graphs** (CPG, Scala query DSL); the deepest structural + security analysis.
- **CodeQL** — semantic queries; CLI free for OSS but **engine proprietary** → comparison/limited-use, not fully $0.
- **CodexGraph / RepoGraph** — research (LLM-agent ↔ code-graph); experiments/comparison, not production tools yet.

**Deliverable:** a per-tool finding (self-hostable? $0? what it *uniquely* gives — search vs agent-context vs CPG security queries?) + a bounded plan to stand up the **1–2 highest-value OSS ones** (likely **Aider RepoMap** for agent context + **Sourcegraph/SCIP or Joern** for nav/analysis), wired where the lab consumes it (agent context · MCP fleet · a `graphify` successor). **Overlap to resolve:** `graphify.sh` already builds a dep graph (advisory, TS/Py only, shell-blind) — this either matures it or replaces it, not a parallel third graph. Relates **B104** (dev-tooling survey + coding-agent eval), **B17/B19** (MCP fleet), **B106/B47** (the review/scan stacks Joern's security queries would sit beside).

---

### B167 — Evaluate + integrate Plannotator — DONE 2026-09-11 (Linear EMA-228)

**DONE 2026-09-11.** Evaluated → **adopt, hardened**; installed pinned/attestation-verified binary (v0.27.14, `--no-extras --model-invocable none`), LAN-hardened (`PLANNOTATOR_AI`/`PLANNOTATOR_SHARE=disabled`), wired into Claude Code (plugin active). **UAT:** plan-review round-trip via the `ExitPlanMode` hook (agent plan → browser review → approve → execute); `/plannotator-review` diff path launches + returns. **Kept as a workstation tool** (beside the agent, like Emdash — no cluster footprint). DoD (repo-tooling lens): eval [concepts/plannotator-eval.md](concepts/plannotator-eval.md) · runbook [runbooks/plannotator.md](runbooks/plannotator.md) · flow [diagrams/flow-plannotator.md](diagrams/flow-plannotator.md) (mermaid parses) · demo [demos/plannotator.md](demos/plannotator.md) + ledger #69 · tracking (this flip + EMA-228) · ops N/A (no cluster workload) · scan N/A (docs only) · cascade — no shared code touched. Progress detail below.

**Progress (2026-09-11): EVAL DONE** → [concepts/plannotator-eval.md](concepts/plannotator-eval.md). Verified from source: **all three lab drivers supported** (Claude Code first-class via a plugin **or** a `PermissionRequest`/`ExitPlanMode` hook — the native plan gate; Codex auto Stop-hook; OpenCode json plugin). **Browser UI** (local server), not GTK → no Bruno-snap-tofu class. **$0/OSS** (MIT/Apache-2.0); **local by default** but Ask-AI + link-sharing egress unless disabled → **lab hardening: `PLANNOTATOR_AI=disabled` + `PLANNOTATOR_SHARE=disabled`**. Installer **inspected**: user-space `~/.local/bin`, no sudo/eval, **checksum + GitHub attestation verified** before chmod — trustworthy; lab-safe form = pinned `--version` + `--no-extras --model-invocable none`. **Workstation tool** (like Emdash), zero cluster footprint. **Recommendation: adopt, hardened.** **Remaining = operator batch:** binary install (pinned/verified/hardened) + Claude Code plugin-or-hook wiring + optional VS Code ext + eyes-on UAT round-trip (plan Deny→revise; diff annotate→session) → keep/skip + runbook/demo. Original scope below.

Evaluate and integrate **Plannotator** ([plannotator.ai](https://plannotator.ai/), `backnotprop/plannotator`, OSS) — a **local visual review + annotation surface for AI coding agents**. It gives a human a real UI to (1) **review an agent's proposed plan** before it executes and mark it up (pen tools, arrows, highlighting), and (2) **annotate git diffs line-by-line** on the code an agent wrote — then send those annotations back to the agent as **structured feedback in one click**, no copy-paste/retype, and the agent revises against exactly what was said. Ships as a plugin/hook + a **VS Code extension** (opens reviews in editor tabs) and integrations for Claude Code, OpenCode, Codex, Pi, and Droid. Runs **entirely local, keeps review data local, no telemetry/analytics** — so it clears the lab's **$0 / self-hosted / LAN-only** bar by construction.

**Why HIGH + why now:** it closes the human-in-the-loop gap the rest of the coding-agent stack opened. **B104** surveyed the tooling and stood up **Emdash** (run many agents in parallel worktrees) + the coding-agent eval harness; **B166** is giving the agents structural context (**Serena**). Plannotator is the missing **review/steer** layer over all of it — the place a human actually reviews the plan and the diff and pushes back precisely, instead of the plan-approval gate being a wall of terminal text. It pairs most directly with **Emdash** (supervise the fleet → review each agent's plan/diff here) and with Claude Code's own plan mode.

**Deliverable:** install it (the plugin/hook path + the VS Code extension); a **per-harness fit finding** — which of the lab's actual drivers (Claude Code primary; OpenCode/Codex secondary, per **B15**) it wires to cleanly on this machine, and any confinement/GTK/rendering gotchas (cf. the B104 Bruno-snap tofu lesson — prefer unconfined `.deb`/AppImage/extension over snap); an **eyes-on UAT** driving a real weyland plan + a real git diff through a round-trip (annotate → agent revises); and a **keep/skip call** with rationale. If kept, document the recipe in a runbook (beside `parallel-agent-supervisor.md`) + a demos-ledger row. **$0 / OSS / LAN-only** lens throughout. Relates **B104** (dev-tooling survey + Emdash), **B166** (Serena/agent context), **B15** (local-model coding agents), **B66** (operator — a different human-in-the-loop surface).

---

### B168 — iOS/SwiftUI on-device golden path (hardware-gated) — VERY LOW (2026-09-11, Linear EMA-229)

A **known limitation**, carved out of **B164** so B164 can close without an open hardware gate hanging off it.

**The gate.** A golden path that actually runs a SwiftUI app on an **iOS simulator or device** requires **macOS + Xcode** — Apple's iOS SDK, the Simulator, `xcodebuild`, and code-signing all ship only on macOS. The lab is **$0, Linux-only, LAN-only**; it has no Mac and no macOS CI runner. Cloud-macOS options exist (GitHub Actions `macos-*` runners, Xcode Cloud, MacStadium, Cirrus) but every one of them is a paid/off-LAN service that breaks the lab's founding constraints, so none qualifies. This ties the lab's **hardware-gated set** (B149/B150) — work that is authorable but cannot *run* here until specific hardware exists.

**Why this is a limitation, not a coverage hole.** The Swift-UI story is already covered on Linux by B164, two ways:
- **`golden-paths/mobile/swift-tokamak`** — [Tokamak](https://github.com/TokamakUI/Tokamak) is a **SwiftUI-compatible** API (same `View`/`@State` declarative model) that compiles via the **SwiftWasm** toolchain to WebAssembly and renders to the DOM. `carton test --environment node` + `carton bundle` prove the **SwiftUI programming model + the full build chain** at $0 on Linux. (Pinned/frozen stack — Tokamak 0.11.1 + `swiftwasm/carton:0.20.1`; see `docs/design/golden-paths.md` §"Workstream 1".)
- **`golden-paths/swift/vapor-app`** — server-side Swift as a first-class HTTP service (B164 language wave), Linux-native.

**Exactly what B168 would add and nothing else:** on-device / simulator **iOS execution**, **UIKit / SwiftUI-on-iOS** rendering behavior, and **App Store packaging** (`.ipa`, signing, TestFlight). None of that is reachable without a Mac.

**Disposition:** parked at **Very Low**; revisit **only if a macOS runner ever enters the lab** (a physical Mac, or a decision to relax the $0/LAN bar for a paid cloud-macOS runner). No work is planned. Documented as a limitation in `docs/design/golden-paths.md`. Relates **B164** (parent — the mobile workstream that shipped the Linux-verifiable Swift paths), **B149**/**B150** (the hardware-gated set).

### B155 — API lifecycle management — DONE 2026-09-07 (Linear EMA-212)

**DONE.** Full lifecycle governance (design → published → deprecated → retired) from one machine-readable catalog, [concepts/api-lifecycle.md](concepts/api-lifecycle.md). **Catalog** `apis.yaml` (sibling to applications.yaml) — 12 APIs, each declaring owner (resolves to a real service) · kind · status · version · consumers · captured `spec` + live `spec_source`. **6 contract snapshots** captured live into `docs/api/specs/` (tool-server 17 paths · realm-http 14 · guard 7 · agent 4 · operator 4 · realm A2A card 24 skills). **Breaking-change engine** `scripts/lib/api_spec_diff.py` — classifies removed operation/response-field/enum-value + new-required-field as BREAKING, additive as COMPATIBLE, cross-kind as cannot-compare (never a silent pass); handles OpenAPI 3.x + A2A cards. **Lifecycle guard** `scripts/check-api-lifecycle.sh` (repo-guards, fail-closed 0/1/2) — owner resolves, kind/status/version declared, published typed API carries a snapshot, deprecated⇒retire_by+successor, retired⇒no consumers. **Drift cron** `api-drift` (03:20, ns monitoring, no RBAC — HTTP only) — fetches each API's live spec, diffs vs the committed snapshot with the engine, breaking drift → Job fail → Telegram, unreachable → skip. **PR-time breaking-change enforcement** (added after review): a **contract lock** `docs/api/specs/contract-lock.json` records each API's approved baseline + version; the guard fails when a snapshot changed vs its lock, and re-locking (`gen-api-contract-lock.sh`) **REFUSES a breaking change without a MAJOR bump** (git-independent — the baseline is in the lock). So breaking changes are a merge gate, not just nightly. **Exercised live 2026-09-07**: all **6** deployed contracts fetched (tool-server NodePort + guard/agent/operator/realm via kubectl-exec) and diffed vs their snapshots — **0 drift**; the lock's refusal proven by removing an operation (guard LOCK exit 1, generator REFUSED exit 1, restore OK). Versioning policy (breaking ⇒ MAJOR bump + re-lock) + deprecation policy documented. Supersedes the cancelled **B12** (static registry — the lifecycle framing it lacked). Ties to **B152** (contract tests = the other enforcement arm). **DoD:** `flow-api-lifecycle.md`, `demos/api-lifecycle.md` (ledger row 62), `concepts/api-lifecycle.md`, `apiDrift` LikeC4 node, schedules row 03:20 + freshness budget + ScheduledJobFailed, api.md governance cross-ref (B12→B155). **36 bats** (engine 10 + lifecycle 14 + drift 6 incl. embedded byte-identity + contract-lock 6), shellcheck 0, bandit 0 (B310 on the runner's urlopen justified with `# nosec`), promtool SUCCESS, kubectl dry-run OK. arch.md gained an API-governance note at the service-boundary section. **Honest scope:** only typed contracts (OpenAPI/A2A) are snapshotted + drift-checked; MCP/OpenAI-shape APIs are catalogued (owner/version/status) but not snapshotted — the guard only requires a snapshot for published openapi/a2a, so it's explicit not silent; MCP tool-schema capture is a follow-on. Original scope below.
...
### B156 — Audit data mesh against Nick Tune's 5 data-platform capabilities — DONE 2026-09-07 (Linear EMA-213)

**DONE.** Verification audit → [docs/concepts/data-platform-capabilities-audit.md](concepts/data-platform-capabilities-audit.md), the capability lens complementary to B158's plane lens. Each of Nick Tune's five capabilities mapped to concrete weyland service(s), graded coherent/partial/gap, live-verified 2026-09-07 (kubectl against the k3s control plane on `mother` + the operator fleet Trino MCP): **(1) Discovery + dictionary** — DataHub (7 domains / 14 products / lineage / glossary; GMS+frontend+actions Running) + 6 fleet MCPs + storage grid + per-store query cookbooks; **(2) Observability** — Soda (finance 22/22) + GE + DataHub schema/freshness + the coverage quartet (scrape/dashboard/alert + `datahub-coverage`, nightly cron verified running); **(3) Control** — Ranger default-deny Trino authz (admin pod Running) + Gatekeeper 4 constraint templates live (required-labels/no-latest-tag/mem-limit/image-signature, all dryrun with real audited counts) + sealed-secrets + Argo GitOps + `applications.yaml` registry + SA-noautomount RBAC + istio STRICT mTLS; **(4) Data onboarding** — DomainConfig→factories + B158's paved edges (build_land_asset/build_domain_jobs/ensure_repo/autodiscovery), fetch/parse source-specific by nature; **(5) Interoperable data** — Iceberg/Nessie open format + Trino federation (live: catalogs iceberg/postgresql/system, iceberg exposes all 3 domains + dbt/analytics/eval) + 11-store hydration + format notebooks. **Verdict: all five already satisfied**, as the item predicted — a verification audit, not a build. **No new blocking gap; nothing needed fixing in-pass.** Residual gaps all pre-tracked or ripe promotions: DataHub-MCP-search timeout (known B113 GMS-token, UI discovery fine), column lineage = dbt-marts-only (B158, by design), user-code-not-GitOps + new-store-forces-k8s (B158 plane-1), and one **ripe promotion** — image-signature admission sits at **0 violations in dryrun** (every running image already from `registry.weyland.lab`), so the estate is ready for dryrun→deny, gated only on the B88 supply-chain enumerate-and-sign step (deliberately not flipped: the policy file documents it as the single most destructive change). **8-pillar DoD (repo-tooling lens):** 1 docs — the audit page + B158↔B156 cross-links; 2 diagrams — LikeC4 N/A (deploys nothing), inline capability-map mermaid (166 blocks parse); 3 demo — the audit IS its evidence, all claims live-verified 2026-09-07; 4 cleanup — read-only; 5 this flip + EMA-213; 6 ops N/A (no workload; governance-by-CI it confirms runs on its own timers); 7 scan N/A (pure doc, no code/deps); 8 cascade — cross-refs resolve, no capability claim contradicts the live cluster. Original scope below.

Audit the built data mesh against the 5 data-platform capabilities from Nick Tune's *Architecture Modernization* (the data-mesh platform enablers). **Likely already satisfied** — this is a verification audit that maps each capability to the concrete weyland service(s) providing it, calling out any gap as its own follow-up: **(1) Discovery + dictionary** (DataHub catalog/search + glossary, the storage grid, query cookbooks); **(2) Observability** — source + source-schema availability + data quality (DataHub schema/freshness, Soda/GE, the coverage-guard trilogy); **(3) Control** — onboarding + data-store control (Ranger authz, sealed secrets, GitOps/Argo, the `applications.yaml` onboarding registry, OPA admission); **(4) Data onboarding** — the paved path for a new source/dataset (dagster loaders, lakeFS/Iceberg landing, the hydration pattern, DataHub emit); **(5) Interoperable data** — usable across engines (Iceberg/Nessie open table format, Trino federation, the format notebooks, multi-store hydration). Deliverable: a short capability→service audit doc. Relates to **B1** (the mesh) and the **B78** maturity bucket.

### B157 — Evaluate ODCS (Open Data Contract Standard) + bring all data contracts to spec — DONE 2026-09-05 (Linear EMA-214)

**DONE (via B158 follow-up E + its tail).** **DoD pillars 2 (diagram) + 3 (demo): N/A** — audit/decision item (as sibling B156): the evidence is the ODCS decision doc + the shipped `check-odcs-contracts.sh` gate (10 bats) + the live `--check-schema` pass + dbt `contract: enforced`, not a diagram/demo. Decision: **adopt ODCS v3, a justified lab subset** — decision doc [docs/concepts/data-contracts-odcs.md](concepts/data-contracts-odcs.md). Shipped: **all three domains contracted** — 10 `services/weyland-dagster/contracts/{finance,music,health}/*.odcs.yaml` (finance hand-authored as the pattern, music/health produced by `scripts/gen_odcs_contract.py` from Trino columns+types / Soda rules / dbt descriptions); a **structural CI gate** `scripts/check-odcs-contracts.sh` (10 bats, shellcheck clean, in `.woodpecker.yml` repo-guards); a **live `--check-schema`** column-vs-Trino pass (validated: all 10 conform, a bogus column is caught); a **products-without-contracts** pytest; and **dbt `contract: enforced`** on the finance marts (verbatim Trino `data_type`s). **All three domains' marts now carry `contract: enforced`** with verbatim Trino `data_type`s, confirmed by live `dbt build` (finance PASS=16; music/health PASS=32) — nothing outstanding. Original scope below.

Evaluate the **Open Data Contract Standard (ODCS)** (Bitol / Linux Foundation) and adopt it (or a justified subset) as the canonical shape for the estate's data contracts, then audit every existing contract against it. The lab already has contract-ish surfaces — DataHub data contracts, dbt model contracts/tests, Soda/GE checks — but they are not expressed against one standard; ODCS is the emerging open spec to converge on. Every contract must carry: **fundamental elements** (name, version(s), description, owner, links to tutorials/docs/runbooks); **schema of the exposed data** — both **logical** and **physical**, including the explicit mapping between the two worlds; **data-quality rules + governance policies**; **SLAs** (freshness/availability/latency/retention); **security** (roles + stakeholders, classification); and **extensibility** (room for custom attributes without breaking the standard). Deliverable: an ODCS evaluation/decision (adopt / subset / defer, with rationale), and if adopted, existing contracts migrated + a conformance check for new ones. Pairs with **B152** (contract tests = the enforcement arm), **B155** (API lifecycle = the API analogue), **B156** (the Nick Tune capability audit), and the DataHub governance layer.

### B158 — Ensure the data mesh exposes the three self-serve platform planes — DONE 2026-09-05 (Linear EMA-215)

**DONE.** **DoD pillars 2 (diagram) + 3 (demo): N/A** — audit item (as sibling B156): the evidence is the planes-audit doc + the six shipped gap-closure gates/tests (`check-datahub-coverage.sh`, `build_land_asset`, `build_domain_jobs`, `ensure_repo`, ODCS, the asset-registration guard), not a diagram/demo. Audit ran (3 parallel plane passes + live verification) → [docs/concepts/self-serve-platform-planes.md](concepts/self-serve-platform-planes.md), grading each plane's surfaces coherent/partial/gap. Verdict: infra invisible-at-use (leaks only at provisioning), product-experience the weakest (declarative middle, hand-assembly at the edges), mesh-experience strong-on-see / weak-on-govern-by-CI. **Two live defects found + fixed inline** (finance land assets swept into the 15-min ingestion cron; the `data-mesh-map` product tile drifted from the emit's real `_PRODUCTS`). **All six gaps then closed rather than merely filed** (per fix-don't-file): **A** `check-datahub-coverage.sh` (data-estate governance-by-CI, nightly CronJob, live 111/111) · **B** `build_land_asset`+`land_core` (paved ingestion scaffold; 4 landers migrated) · **C** `build_domain_jobs(cfg)` (operate plane generated, land/transform split single-sourced) · **D** `lakefs_repo.ensure_repo` (repo self-provisions on first land, contract live-validated) · **E** ODCS adopted (finance contracted + `check-odcs-contracts.sh` gate; B157 foundation) · **F** asset-registration guard (silent-load footgun loud at CI). Each TDD'd (bats/pytest) + live-validated where the cluster was involved; dagster-level pieces validate at the code-server load on deploy. `arch.md` §datasets-lib + the planes/ODCS concept docs place them in the layers. **All six deferred follow-ons then also completed (2026-09-05), nothing left:** full autodiscovery — shipped `git-f5659901`; the dagster runtime-type trap it exposed (`AssetChecksDefinition` subclasses `AssetsDefinition`, so `isinstance` autodiscovery double-registered every check → `Duplicate asset check key` at code-server load) was fixed and regression-guarded (`_collect.py` + `tests/test_collect.py`, caught live by the ship SMOKE/TXN gate); ODCS other-domains + live `--check-schema` conformance (all 10 contracts, music/health generated); dbt `contract: enforced` across all finance/music/health marts (`dbt build` green). **B157/B158 fully shipped + code-server-verified.**

Original scope — Audit the mesh against Dehghani's **three self-serve data-platform planes** and make each a real, coherent surface: **(1) infrastructure/utility plane** (MinIO/Iceberg-Nessie/lakeFS · k8s/Argo · Dagster · Trino — the always-on foundation, invisible to a product author); **(2) data-product experience plane** (the declarative interface to build/operate a data product — the `DomainConfig` → `build_transform_assets`/`build_asset_checks`/`build_store_load_assets` machinery, the paved path; is onboarding *declarative* or still hand-assembly?); **(3) mesh experience plane** (the global cross-product view — discovery/lineage/governance/SLOs across all products: DataHub catalog + domains/products + lineage, the coverage guards, the operator fleet MCPs, ODCS contracts; can you see + govern the mesh AS a mesh?). Deliverable: a plane→surface audit doc, gaps become follow-ups. Complementary lens to **B156** (the 5 capabilities); **B157** (ODCS) is a mesh-experience-plane artifact; **B113** (a new domain) is the live test of the product-experience plane. Relates **B1**.

### B171 — Emit deployment events → Port deployment entities (DORA deployment-frequency + lead time) — DONE 2026-08-27 (Linear EMA-172)

**Backlog-entry added 2026-09-20.** Existed only in Linear (EMA-172, project `Weyland Lab`, parent EMA-154) since 2026-08-12 with no B-number in its title, so it fell outside the backlog↔Linear reconciler (whose orphan check keys on the title carrying a weyland number) and was never in this file. The weyland counterpart of the DORA seed captured on B46.

**DONE.** `scripts/ship-images.sh` records a Port `deployment` entity after the gates pass — verified live end to end, test entities cleaned up. **Two corrections the work surfaced:** (1) it is a `deployment` entity, not an `environment` entity — the `environment` blueprint has no properties (one relation to `k8s_cluster`), so `environment` is a *dimension on* a deployment, not the thing a frequency metric counts; (2) the gates it was filed as cross-gated on (B39/B57) had already dissolved. **The lead-time trap:** the first cut measured PR-created→merged, but this loop opens and merges its own tag-bump PR, so that is ~24 seconds — a flattering, meaningless `0.0h` forever. Fixed to **author-date → deploy time** (the image tag is `git-<short-sha>`, so the source commit is in hand): same ship measured **0.4h**. **PR is a URL property, never a relation** — B144's reaper deletes closed `githubPullRequest` entities nightly and this loop closes the PR it merged, so a relation would dangle. Changed: `ship-images.sh` (`commit_iso`/`lead_time_hours`/`deployment_payload`/`emit_deployment`, emit after FR1.5 + TXN; a failed emit is loud, cannot abort a ship); `tofu/port/b137_blueprints.tf` (`deployment` gains `lead_time_hours` + `pull_request_url`, full plan clean); `ship-images.bats` (+15 → 72 total, shellcheck clean). DoD: `runbooks/ship-images.md` § DORA emit, `flow-ship-loop.md`, `demos/ship-images.md`, ledger row 48. Relates **B137** (Port blueprints), **B135** (the ship loop), **B144** (the PR reaper).

### B172 — Evaluate/adopt Graphify — folder → queryable knowledge graph skill — DONE 2026-08-27 (Linear EMA-191)

**Backlog-entry added 2026-09-20.** Same reason as B171: tracked only in Linear (EMA-191, project `Weyland Lab`) since 2026-08-18 with no B-number in its title, so it was invisible to the reconciler and absent from this file.

**DONE — adopted.** Graphify (the OSS folder→knowledge-graph skill; local Tree-sitter AST + Leiden community detection, no vectors/embeddings) evaluated against the lab and adopted as an **advisory** code-cascade aid. Shipped `scripts/graphify.sh` — `install` / `build` / `affected` / `god-nodes` / `verify`, **32 bats**, wired into DoD **Pillar 8** (the CODE half of the graphify cascade). Pinned `graphifyy==0.9.50`; **local AST only, no LLM, no network**; measured 13,051 nodes / 21,163 edges in ~16s on the repo. Constraint that carries forward: build the venv with **python < 3.13** or Leiden silently degrades to networkx louvain (`venv_python()`/`leiden_status()` handle + report it). Full evaluation, measurements and staged plan: [docs/concepts/graphify-adoption.md](concepts/graphify-adoption.md). The deferred Stage 4/5 capabilities are **B173**. Relates **B15/B106** (coding-agents lane), **B1** (the Neo4j graph store).

### B173 — Graphify deferred capabilities — in-lab semantic pass, Neo4j export, MCP server — LOW (2026-08-27, Linear EMA-208)

**Backlog-entry added 2026-09-20.** Same reason as B171/B172: tracked only in Linear (EMA-208, project `Weyland Lab`) since 2026-08-27 with no B-number in its title, so it never reconciled and was absent from this file. Stages 4-5 of the Graphify adoption (**B172**), deliberately **not built** — nothing consumes them yet — but real capabilities with clear triggers, not dropped work.

**Stage 4 — in-lab semantic pass.** `graphify extract --backend openai` honours `OPENAI_BASE_URL`, so the semantic pass can run against LiteLLM / Ollama / vLLM with **zero egress**. Unlocks path-qualified node IDs (fixes ~3.8% ambiguous code symbols concentrated in `main()`/`run()`/`__init__()`) and concept-level edges across docs (8,693 `.md` nodes today have structure but no semantic relationships). **Deferred because** the only adopted query is `affected` on code, which semantic extraction does not improve. **⚠ Overlaps a doc-semantic-pass concern — decide ownership before either is built:** Graphify was adopted deliberately as structure-only (graph-RAG keeps retrieval), so a semantic pass over `docs/` would quietly reverse that decision. **Re-open when** doc/concept relationships are wanted, or `affected` starts being wrong in a way path-qualified IDs would fix. **Stage 5 — integrations.** `--neo4j` emits `cypher.txt` (code graph beside the data graph — re-open for cross-graph queries, pairs with B82); `--mcp` is a stdio MCP server making the code graph agent-queryable through the **B17/B19** gateway (the most plausible of the three — the operator and Realm of Agents have no way to ask code-structure questions today). **Do NOT install `graphify hook install`** — the operator owns all git operations here. Relates **B172** (the adoption).

---

### B174 — Investigate Jev / TypeSafe AI (System One decision model) for the lab — HIGH (2026-09-21, Linear EMA-232)

Investigate **Jev** (TypeSafe AI, typesafe.ai) — evaluate for the lab and decide keep / skip / where-it-fits. Prompted by the user 2026-09-21.

**What it is:** TypeSafe AI (SF lab, out of stealth 2026-09-15, ~$40M seed) shipped **Jev**, its first **"System One"** model — deliberately NOT an LLM, generates no text. You send **program state + typed questions** (a situation + a list of allowed actions) and it returns a **decision** directly, with a probability/confidence score, answering all questions in one parallel pass in **~70-500ms**. Claimed **up to 200× faster / ~400× lower cost** than comparable LLMs on classification/decision tasks. Built to run inside real-time loops (games, robots, simulations) and as a small component wherever code needs **one judgment** while the rest stays ordinary code.

**Why it fits here:** the lab spends an LLM (or a small model) on a **structured decision/classification** at several seams — exactly Jev's niche: the **operator** tool-routing/intent (B66), the **weyland-guard / guardrails** allow-block verdict (`verdict.py`, B70/B115), the **MCP gateway `policy.gate`** (B17/B19), and eval/label classification (B84/B96).

**Eval scope:** **(1) constraint fit FIRST — the gate:** is Jev self-hostable / free-tier / LAN-runnable, or cloud-API-only + paid? The lab is $0 / self-hosted / LAN-only — cloud-only + paid means weigh it like Port/Linear (justified cloud) or skip; pin hosting / pricing / licensing before anything else. **(2)** per-seam overlap vs the existing decision points (replace / complement / skip). **(3)** if a free/self-host path exists, a micro-benchmark on ONE real lab decision (guard allow/block, or operator tool-select): latency + cost + accuracy vs the current path. **(4)** integration shape — a `wl-*` client, an MCP tool, or inline in weyland-guard. **Deliverable:** an eval/decision doc (`docs/concepts/`) — what it is, constraint fit, a per-seam overlap table, an adopt-where / complement / skip call, and if adopted a bounded follow-on. Relates B66 (operator), B70/B115 (guardrails), B17/B19 (MCP gateway + policy.gate), B84/B96 (eval).
...
**Why for the lab:** the lab already runs many repeatable agentic loops **ad-hoc and uncodified** — the DoD 8-pillar pass, code-scan-suite triage ([[code-quality-scan-triage]]), the pr-lifecycle reconcile (B131), golden-path verify (B153/B160), the eval loops (B84/B96), the operator incident sweep (B45/B66), machine-inventory onboarding (B169), and this very session's MCP-fleet debug loop. Codifying them as a **versioned, discoverable library** — each with an explicit terminal condition — gives repeatability, shareability, and a stopping-condition discipline that matches the Claude Code `/loop` command's self-pacing.

**Where it fits (the eval):** the lab already has the **Bifrost prompt repo** (`register_bifrost_prompts.py`, [[bifrost-prompt-repo]]) + **skills repo** (`register_bifrost_skills.py`, [[bifrost-skills-repo]]) — a loop library is the natural extension, since a loop *is* a prompt with a terminal condition. The `/loop` command + the AIDLC stages are also loop-shaped. So decide the **SoT + discovery surface**: register loops into Bifrost (like prompts/skills) vs a curated `docs/` catalog vs both; then **seed** it with the lab's real recurring workflows, each written with its explicit stopping rule.

**Deliverable:** a loop library (SoT + discovery surface) seeded with the lab's real recurring workflows, each carrying a terminal condition; per-loop entry = title/category/description/prompt/stop. **$0** (existing Bifrost / docs infra). Relates [[bifrost-prompt-repo]], [[bifrost-skills-repo]], the `/loop` command, the AIDLC stages, B84/B96 (eval loops). Ref: https://signals.forwardfuture.com/loop-library/

---

### B152 — Audit CI/CD test categories: AOP, contract, and architecture (ArchUnit-style) tests — DONE 2026-09-06 (Linear EMA-209)

**Extended 2026-09-17.** The 2026-09-06 pass shipped only the **Python** `import-linter` half of the architecture category and left the **Java ArchUnit** it named "applicable — BUILD now" (finding-matrix row 1) unbuilt. Closed that plus two extensions — each self-checking (a planted violation trips the rule by reason), verified through the full `test-python` + `test-java` lanes + coverage ratchets in the CI images (no regression: python held/improved across 15 projects, java across 5): **(1)** Flink **ArchUnit** on `health-job` + `sql-runner` (`archunit-junit5`; no-standard-streams + package rules; planted `StdoutOffender`; `SqlRunner`'s `System.out.println`→slf4j so the rule holds), riding the existing `test-java` lane with no new step; **(2)** dagster `.importlinter` +2 contracts (`resources-are-independent`, `leaves-are-resource-free`); **(3)** framework-free `import-linter` for **weyland-guard** + **weyland-tool-server** (guardrails/validators + the byte-shared `verdict.py` must not import fastapi/starlette — complements, does not duplicate, `check-verdict-sync.sh`). Details in the [concepts/ci-test-categories-audit.md](concepts/ci-test-categories-audit.md) addendum. (Linear status is yours to set.)

**DONE 2026-09-06.** Audited the CI pipeline for whole test CATEGORIES B88's unit/scan/integration lanes miss, evaluated ALL seven against every language/service (per the user: no subset, no deferrals), and resolved each in-pass — [docs/concepts/ci-test-categories-audit.md](concepts/ci-test-categories-audit.md) (finding matrix + status). **Built:** **architecture** — `import-linter` static-AST contracts (`.importlinter`: pure leaves + `_collect` must not import dagster; leaves must not import the factories) enforced in the slim no-dagster lane, `tests/test_architecture.py` (real holds + a planted-violation fixture breaks the contract by reason), and `scripts/check-verdict-sync.sh` + `verdict-sync.bats` (the byte-duplicated guard↔tool-server `verdict.py` wire contract, fail-closed 0/1/2, wired into `repo-guards`); **property-based** — `tests/test_property_based.py` (hypothesis) pinning `domain_job_plan`'s single-sourced land/transform split invariant + `_collect`'s disjointness/flatten; **fuzz** — `tests/test_fuzz.py` (hypothesis) on `edgar_text_parse` (the untrusted-HTML parser — never crashes, 8-key chunk contract holds, chunker terminates); **mutation** — `scripts/run-mutation.sh` (on-demand, NOT a blocking lane — slow/expensive for a $0 lab), `_collect` proven 5/5 mutants killed. **Evaluated → COVERED (no new lane):** **contract** (the top RPC seam is `weyland-guard/tests/test_verdict_contract.py` + the new sync guard; data by ODCS/B157, API by B155, live by the B88 integration tier) and **AOP** (guard `test_pipeline`/`test_policy`/`test_metrics` already assert the hook/mode gating, enforcing act gate, and metrics aspect; infra aspects Istio-mTLS/forward-auth are N/A at unit level). **Documented N/A:** **load** (no perf NFR, capacity-bound node) and the **fixture-only languages** (Go/Rust/TS/JS/React/Next carry no production code, so enforcement would test nothing today — per-language tools named for when real code lands). Arch/property/fuzz tests ride the existing python lane (`import-linter`+`hypothesis` in `requirements-test.txt`). **8-pillar DoD:** 1 docs — audit concept doc + `arch.md` CI clause + demos ledger row 58; 2 diagrams — `flow-ci-test-categories.md` (LikeC4 N/A, deploys nothing), 165 mermaid blocks parse; 3 demo — `demos/ci-test-categories.md`, CLI-walkthrough IS the test, RUN 2026-09-06 with the negative cases (planted arch violation → exit 1, verdict drift/missing → 1/2); 4 cleanup — read-only, `.mutmut-cache/` gitignored; 5 tracking — this flip + EMA-209; 6 ops (repo-tooling restated) — arch/property/fuzz run in the python CI lane, `check-verdict-sync` in repo-guards, `run-mutation` by-hand on-demand (recorded); 7 scan — shellcheck 0, bats 406, bandit 0 Med/High on the new python; 8 cascade — new scripts covered by the shellcheck lane, new test deps pinned in requirements-test.txt, no new app/timer/host; reverse sweep — the abandoned `/aidlc` B152 workflow was parked then cleared (statusline back to `[AIDLC] ready`, intent registry emptied). Suite green in CI images: python **120** · bats **406** · shellcheck **0** · `.woodpecker.yml` parses. Original scope below.
...
### B176 — PR-lifecycle reconciler: cover ALL pr-lane repos + Loki audit log — DONE (2026-09-23, Linear EMA-234)

Extends the B131 reconcile half (`scripts/check-pr-lifecycle.sh` + the `pr-lifecycle-reconcile` CronJob) from
weyland-lab-only to **every `lanes.pr: true` repo in `repos.yaml`** — the B138 pr-lane set (8 repos: weyland-lab,
stud.io, Algopedia, ServiceTransformation, emangini-tailwind-nextjs-contentlayer, startme-curator, freejack,
MyBodyGraph; `midi_real_book` excluded as `stale`). Motivated by the B119 Linear walk (the Reviews/Inbox queue
was full of dependabot PRs across many repos, but the reconciler only watched one).

**What shipped (code complete + verified 2026-09-23):**
- **Multi-repo loop**, byte-identical repo set to `pr-staleness.yaml` so ONE `check-repo-coverage.sh` regex guards
  both consumers against the SoT (the new `pr(recon)` lane). Each repo runs in a subshell → per-repo fail-closed
  isolation: one unreachable/unclassifiable repo is collected and forces exit 2, never a silent shrink of the set
  (the same posture `pr-staleness` uses). `--repo`/`PR_REPOS` still scope it. Verified live in advisory across all
  8 (16 open managed PRs: 3 stale, 1 superseded, 10 mergeable, 2 needs-human).
- **Audit log → Loki.** Every run + every applied action emits a structured `pr-lifecycle-audit` line to stdout;
  Alloy already ships weyland pod logs to Loki, so the per-PR resolution history is queryable in Grafana. Metrics
  are derived via LogQL (documented in the runbook). **No Pushgateway** and the Job **stays unmeshed** — the Loki
  ruler is alerting-only (no `remote_write`), so native Prometheus counters are deferred (would need remote_write
  or a Pushgateway, which would re-mesh the Job). Reconcile *failures* already page via `ScheduledJobFailed`.
- **Tests:** `pr-lifecycle.bats` 14/14 (added multi-repo aggregation, per-repo fail-closed isolation, audit-line
  assertions), `repo-coverage.bats` 13/13 (added reconcile-drift + no-REPOS fail-closed) — both green in the CI
  image. shellcheck (`--severity=warning`), `check-doc-counts`, `check-cron-freshness-budgets`, manifest YAML, and
  the embedded-ConfigMap byte-identity all green.
- **Docs:** `docs/runbooks/pr-lifecycle.md` (multi-repo, audit log, LogQL queries, token reach), `repo-coverage.md`
  (pr(recon) lane), `repos.yaml` comment.

**Operational follow-up (runbook-captured — not a completion blocker; no private-repo PRs exist today):** the sealed `pr-lifecycle-reconcile-github` PAT is
scoped `Pull requests: write` on **weyland-lab only**. All-repos `--apply` needs it re-scoped to `Pull requests:
write` + `Issues: write` on all 8 pr-lane repos — including private `freejack` + `startme-curator` — then re-sealed
on mother (secret `pr-lifecycle-reconcile-github`, key `token`, ns `weyland`; see `docs/runbooks/sealed-secrets.md`).
Until then, advisory reads all 8 fine; `--apply` fails closed (exit 2 → Telegram) on any private repo it can't write.

**Not done (deliberate):** native Prometheus metrics (LogQL-from-Loki instead); auto-merge (the reconciler never
merges — that stays a human action).
 ...
 **DONE 9th — superset `connections.db_pass` (EMA-84), fixed 2026-07-20.** The first attempt took Superset down and produced a **wrong** diagnosis, recorded here for a week: that the chart renders `db_pass` into the container's `env:` and *shadows* `superset-env`. It does not. Inspecting the live Deployment showed exactly one explicit env var (`SUPERSET_PORT`); `DB_PASS` reaches all three consumers (`superset`, `superset-worker`, the `superset-init-db` Job) **only** via `envFrom: superset-env`. The outage was the plain cause — placeholdering `db_pass` put the placeholder into the chart-templated Secret (`secret-env.yaml`: `DB_PASS: {{ include "superset.db.password" . }}`).
...
**Why.** The team intake email (Team settings → General → Create issues by email) is enabled; anyone holding the address
can file issues with no authentication, and this repo is public. It is B184's alert → Linear fallback, so it needs a
sealed home before anything uses it. Found in the B119 settings walk.

**Scope.** (1) Owner adds `LINEAR_INTAKE_EMAIL` to the gitignored `scripts/.env` — never pasted into chat, commits,
issues or docs. (2) Create Secret `monitoring/linear-intake` (key `LINEAR_INTAKE_EMAIL`) from `.env`, add it to the
`SECRETS=(…)` allow-list in `scripts/seal-secrets.sh`, seal per `docs/runbooks/secrets.md` § "Add a NEW secret to the
sealed set". (3) Index it in the secrets runbook with the rotation step (refresh in Linear → update `.env` → re-seal).

**Technical context.** Linear team settings (value + rotate control); `scripts/.env`; Secret → SealedSecret in
`k8s/sealed-secrets/sealed/`; `scripts/seal-secrets.sh`; `docs/runbooks/secrets.md`.

**Acceptance criteria.**
- [ ] Set in `scripts/.env` and nowhere else in the tree (repo-wide search for the address returns nothing).
- [ ] Stored Secret value length matches `.env` (verify the stored value, not just `DATA 1`).
- [ ] SealedSecret committed and Synced; the plain Secret never committed.
- [ ] Listed in the secrets runbook with its rotation step.

**Edge cases.** Empty/truncated values are accepted silently (verify length); leak → rotate in Linear, update `.env`,
re-seal; the B147 placeholder guard must classify it correctly.

**Out of scope.** Wiring the Alertmanager email receiver (B184's fallback); other Linear credentials.

Relates B184, B69, B119.

### B194 — Nightly Linear workspace export to MinIO (backup/DR + the Linear data source) — HIGH (2026-09-26, Linear EMA-253)

**Why.** Nothing backs up Linear. Postgres, MinIO and rogueone are backed up, but Linear holds every item's status plus
comments, projects, initiatives (soon the B119.1 OKRs), labels, templates and views; `docs/backlog.md` holds the ordered
list and detail, not status history or comments. Same class of gap B137 found in Port. Found in the B119 walk via
**Cloudback** and **SimpleBackups** (hosted Linear backups; free tiers unverified). The same export is the **Linear data
...
(4) Lakehouse view (Iceberg / dbt) for B185 / EMA-172 / B119.1. (5) Runbook + demo.

**Technical context.** Linear GraphQL API → a Dagster asset in `weyland_pipeline` → MinIO `linear-backup` bucket;
read-only key (`LINEAR_API_KEY_RO`) as a SealedSecret; freshness rule; `docs/schedules.md`, `docs/runbooks/backups.md`,
a dbt model.

**Acceptance criteria.**
- [ ] Snapshot issue count equals a live Linear count taken at the same time.
- [ ] Comments, state history, projects, initiatives, labels, templates, views each present and non-empty.
- [ ] A missed/failed run alerts within one period (tested by skipping a run).
- [ ] Restore drill recreates sample issues + comments in a scratch team; non-restorable fields documented.
- [ ] Read-only key from a SealedSecret — never pasted, never in a UI.
- [ ] One example lakehouse query each for B185, EMA-172, B119.1.

**Edge cases.** API complexity limit (10,000 — page small, as check-linear-sync learned); a failed page fails the whole
run closed; include archived items and record deletions; revoked key → clear auth error + alert, never an empty
snapshot; retention for growth.

**Out of scope.** Paid hosted backups (comparators only); writing back to Linear beyond the scratch-team drill; the
manual CSV export as primary (fallback only).

Relates B119, B185, EMA-172, B119.1, B137.

### B193 — Spike: share Linear outward, read-only (project links vs initiative roadmaps) — LOW (2026-09-26, Linear EMA-252)

**Why.** Found as Linear integrations in the B119 walk's Customer Experience category. The one need there that doesn't
wait on users: show the lab's work to people who aren't in Linear — family (before they're Linear users) and a
portfolio showcase beside B191's YouTube demos. Linear shows projects only to members or paid guests.

| Tool | What it shares | Free tier |
|---|---|---|
| **Lindie** | private read-only links to a Linear project, no Linear account needed | **1 shareable project** |
| **Helium Rooms** | Linear boards in read-only "rooms" for outsiders | verify |
| **Feedvote** | client portal + public roadmap on Linear | verify |
| **Roadmark** | turns a **Linear initiative into a shareable roadmap** — shares goals + progress, not raw issues (fits B119.1's OKRs; sidesteps the sensitive-issue risk) | verify |
| **SteelSync** | private / public board views of Linear data | verify |
| **Alignear** | client portal + automated cycle reports (plus client-facing AI/requests, out of scope) | verify |

**Technical context.** The Linear project to share (e.g. Weyland Lab); the tool's hosted viewer (OAuth to Linear); the
verdict in `docs/concepts/linear-evaluation.md`.

**Acceptance criteria.**
- [ ] Free tiers verified from each pricing page (what is shared, how many projects, expiry).
- [ ] One project shared with the top candidate, opened in a private window with no Linear login, showing current status.
- [ ] Read-only confirmed: no edits, no comments into Linear, no other projects visible.
- [ ] Verdict recorded with the chosen project and link policy.

**Edge cases.** Links leak → must be revocable (test it); issues can hold internal hostnames / sensitive notes → pick
(or filter) a safe project first; reject any tool needing write OAuth scope.

**Out of scope.** Customer portals for the product apps (B188); Linear seats for anyone; paid tiers.
...
overlaps to settle (PostHog errors vs GlitchTip, PostHog flags vs Unleash). Customer chat/support, if needed: **Chatwoot**
(open source, self-hostable, links conversations to Linear issues) is the lab-native option — seen in the B119 walk's
Customer Experience category, where every other entry was a hosted support/feedback/CRM tool waiting on users.
...
### B186 — Configure GitLens in VS Code (Community; Pro only if it earns its cost) — WON'T DO (2026-09-26; was HIGH 2026-09-25, Linear EMA-244 canceled)

**WON'T DO 2026-09-26 (owner).** GitLens isn't used enough to be worth paying for, and Pro's first-year deal doesn't change that: the full price after year one won't be kept up. Nothing was configured, so there is nothing to undo. The detail below is kept as the record of what was considered.
...
| **Pro** | ~$8/user/mo billed annually (~$96/yr) — **not $0** | the above on private repos; Launchpad (PRs across GitHub/GitLab); Cloud Workspaces/Patches; GitKraken AI (250k credits/week). **No Linear integration** (owner-verified 2026-09-25) — issue tracking stays in Linear via the Linear MCP |

**Initial read.** Community likely covers the lab — weyland-lab and most active repos are public. Pro's extras mostly
duplicate what's in place: PR triage (`check-pr-lifecycle.sh` + Linear Reviews), AI (Claude Code /
...
| Timely | **automatic** AI time tracking with a Linear integration (seen 2026-09-26) | yes | paid | the only automatic tracker in the directory besides ActivityWatch — but not free |
| Everhour / TrackingTime / TMetric | timers on Linear issues (Linear-directory integrations, seen 2026-09-26; Everhour adds estimates + budgets) | yes | unverified | more manual-timer options — same category as Clockify |
| Traqq | manual start/stop timer from Linear issues (browser extension; a Linear-directory integration, seen 2026-09-26); Linux desktop app; does **not** auto-track app/URL use | **yes** (appears on free) | free: 3 users, **1 project** | manual timer — same category as Clockify, with a tighter project cap |
...
2. **Audit every component** for its Slack integration: what it sends/receives, what it would duplicate (Telegram
   ...
   (Slack as a second front door), **Linear initiative updates → a Slack channel** (Settings → Initiatives → Slack
   notifications; posts only for workspace-level initiatives or ones led by a public team — both true here; pairs with
   B119.1's OKR check-ins), and the Linear-directory entries that need Slack (Graph, Alkemi, Blocks, **Axolo** —
...
**Linear → Slack, per team (Team settings → Slack notifications; added 2026-09-26).** Connect a channel for the EMA
team and pick events: project update posted · issue added · completed/canceled · status change · comments · added to
triage. Sized for the lab's rate (~30 issues/hour in a session): "issue added", "status change" and "comments" would
flood a channel — start with **completed/canceled**, **added to triage** (pairs with the triage rotation) and **project
updates** (pairs with initiative updates + Pulse); record the choice in the channel map. The intake email (B195) lands
issues in the same team, so triage notifications cover those too.
...
deduplicate — repeat firings file repeat issues). The team intake address is **enabled** (Team settings → General) and is
a **secret**: anyone holding it can file issues unauthenticated, and this repo is public — keep it out of git, store it
in a SealedSecret if Alertmanager uses it, and rotate it (the refresh control beside it) if it leaks.
...
| **SuperDuty** | automatically assigns the on-duty member to Linear **triage** (seen 2026-09-26) | verify | a direct triage-rotation tool — candidate for the exercised rotation |
...
built-in tool list, and on this Linux workstation most of it can't work: the terminal launchers (Claude Code, Codex
CLI, Amp, OpenCode, custom script) need the Linear desktop app, which exists only for macOS/Windows ("Linux not on the
roadmap"); the cloud agents (Devin, Factory, Lovable, …) are paid or duplicate Claude Code. Only **Cursor** is enabled
(its `cursor://` handler is registered on rogueone). The app we'd use for greenfield prototyping — **Figma Make**
(Figma Pro: prompt → functional React app + Supabase backend + GitHub push, 3,000 AI credits/mo) for OJay Floyd and
...
3. Configure the ones that do as Linear custom-link tools with a prompt template; list the ones with no URL entry
   point (they stay on Copy-as-prompt).
4. Record the verdict + working URL templates in `docs/concepts/linear-evaluation.md` so the setup is reproducible.

$0 (existing Figma Pro + Linear's built-in feature). Relates B119, B39 (Figma ↔ code), OJay Floyd / MyBodyGraph.

### B180 — Track host systemd units (mother, rogueone, weyland) in a YAML SoT + the Port inventory — HIGH (2026-09-25, Linear EMA-238)

**Why.** Found 2026-09-25 while adding `weyland-docker-prune` on mother (the Docker image store had grown to ~270 GiB
because the existing `weyland-image-prune` only prunes k3s's containerd). The lab creates host-level systemd units
routinely and tracks them nowhere. The repo holds **15 unit files across 3 machines**:

| Host | Units in the repo |
|---|---|
| mother | `weyland-image-prune.{service,timer}` · `weyland-docker-prune.{service,timer}` (`nodes/mother/host/systemd/`) |
...
timer is **silent**: k8s CronJobs get `ScheduledJobStale` / `ScheduledJobFailed`, host timers get nothing unless they
push a Kuma heartbeat (only `restic-backup` and `machine-inv-drift` do).

**Scope.**
1. **Audit** mother, rogueone, weyland — every lab-authored unit installed/enabled vs. the repo, both directions.
2. **SoT YAML** (`host-units.yaml`, the `repos.yaml` / `applications.yaml` pattern): host, unit, type, schedule,
   purpose, source path, owning B-item.
3. **Port** — catalog them with the B129 machine inventory (a per-machine relation), emitted from the YAML.
4. **Drift guard** — repo ↔ SoT in CI; host ↔ SoT via the B129 nightly `machine-inv-drift` collector (already SSHes
   each host).
5. **Failure visibility** for every host timer (Kuma push heartbeat, or a node-exporter systemd collector + a rule).
6. **`docs/schedules.md`** rows for every timer; `weyland-image-prune` (Sun 11:00 NY) predates Design Rule #5 and
   should move pre-dawn.
...
**Questions to answer:**
- **What it actually is** — the model (agent orchestration? multi-agent build pipelines? a hosted IDE/terminal-agent?), how it runs, the problem it claims to solve.
...
### B178 — CI resilience: a Port (external SaaS) outage must not hard-block the whole pipeline — RETIRED (2026-09-24, Linear EMA-236)

**What happened.** On 2026-09-24 Port's API went down (`api.port.io` eu-west-1: TCP+TLS completed, then its ALB — `Server: awselb/2.0` — returned 503 / hung, i.e. no healthy targets; status.port.io stayed green). `port-iac-coverage` is an always-run, hard-fail step, and its auth `curl` had no timeout, so it hung ~45s+ and fail-fast killed the whole pipeline.

**Built, CI-verified, then reverted (same day, operator call: "the outage path was temporary — remove it").** The stopgap gave `port-iac-coverage` an exit 2 = "Port unreachable" and `linear-sync` an exit 3 = "Linear unreachable" (with a local-HTTP-stub bats harness), had both CI steps WARN-and-continue on those codes, and made `notify-port` best-effort. It worked (lean runs #178/#179 green) — and was removed because it contradicts the lab's fail-closed rule (`project.md` c14): a run in which a guard verified nothing must not come out green, whatever the cause. An outage blocking CI is the accepted cost.

**Kept (not an outage path):** bounded curls — `--connect-timeout 10 --max-time 25/30/20` on `port-iac-coverage`, `linear-sync` and `notify-port` — so a hung vendor fails the step in seconds instead of pinning it for curl's multi-minute default. Still a hard fail. Everything else is back to the pre-B178 contract (`port-iac-coverage` 18 bats; `linear-sync` 43 bats, exit 2 on any unreachable/non-200; plain `bash scripts/...` CI steps; shell-test steps without `curl`).

**Lessons that survived the revert:** lint `.woodpecker.yml` with `woodpecker-cli lint`, not python yaml (a `: ` in a step command passed PyYAML and broke the pipeline); diagnose a vendor outage by layer (DNS → TCP → TLS → HTTP, with a control host) — the response headers name the component that failed.
...
- `ci-langs.yaml` — manifest: `production_lanes` (always run) + `fixture_trigger_paths` (a change under any of these = run the full fixture matrix).
- `scripts/ci/select-fixtures.sh` — emits `RUN_FIXTURES=0|1` from a git diff; **fail-closed** (undeterminable/empty diff, any trigger-path change, or a missing/unparseable manifest → `1`); `scripts/tests/ci-select-fixtures.bats` (12, incl. the empty-diff-is-ambiguous case). shellcheck 0.
- `.woodpecker.yml` — the 46 fixture lanes flipped to `when: *fixture` (`SMOKE_ONLY != "smoke" && RUN_FIXTURES != "0"`), reusing the PROVEN `--var`+`evaluate` mechanism (NOT the LAN-unreliable `when: path`). An unset var / the nightly cron → full matrix (safe default, preserving B160's "CI is the proving ground"). The 6 production lanes keep `when: *skip_on_smoke`. `golden-path-smoke` (the ungated all-~44-paths build) is lean-gated the same way — `when: 'SMOKE_ONLY == "smoke" || RUN_FIXTURES != "0"'` — so a lean run skips it too (it still runs in smoke mode and in the full/nightly run); this was found when a lean run's `golden-path-smoke` did the full build and flaked on a transient apt mirror.
- **Verified end-to-end (lean run #177, 2026-09-24):** `port-iac-coverage` + `linear-sync` + `shell-tests` pass, the production lanes (`test-python/shell/java`, `scan-java`) RUN, and the 46 fixture lanes are **skipped-by-`when`** (not instantiated — Woodpecker's `ps` shows 22 steps, not 67). The gate works.
- Runbook: `docs/runbooks/woodpecker.md` § "Lean language matrix" (compute `RUN_FIXTURES` before push, pass `--var`).

**Phase 2 — make the FULL matrix OOM-safe (DONE 2026-09-24 — closed on evidence, not on new infrastructure).** Each planned lever was checked against live data before building anything:
- **(1) Memory-request the lanes — already in place.** The `woodpecker` ns carries the B93 `default-memory` LimitRange (128Mi request / 2Gi limit) for every step that declares no resources (verified live); the heavy compile lanes carry explicit 4–6Gi limits. The plan's "58 best-effort lanes" read only the YAML and missed the namespace default — corrected. Measured #168 step peaks (per-pod `container_memory_working_set_bytes` joined to step windows): most < 1 GiB, heaviest ~1.2–2.8 GiB.
- **(2) Move the full run off the batch window — no benefit.** `node_memory_MemAvailable` over 3 days is flat day and night (~4–7 GiB free); there is no quieter window to move to. The 01:00 `nightly-images` cron stays.
- **(3) Reclaim headroom — not warranted by the data.** Across **19 recorded nightly cron runs, 0 were killed**; every failure was a code-level lane (sonar-gate ×4 — cleared by the #168 fix — plus scan-erlang, test-haskell, linear-sync, test-python). Mother's kubelet already reserves 2Gi system + 1Gi kube with `eviction-hard memory.available<1.5Gi`.
- **What actually killed #169/#170:** they were ad-hoc FULL runs launched ~23:43–00:45 as the Dagster pre-dawn batch started; `node_memory_MemAvailable` dipped to **1.36 GiB at 00:42** (below the eviction threshold) and `node_vmstat_oom_kill` recorded a kernel OOM kill — the node never went NotReady and never reported MemoryPressure (the kubelet's ~10s eviction poll is outrun by fast growth, as `k3s/config.yaml` already notes).
- **Outcome:** runbook guidance (`docs/runbooks/woodpecker.md`) — trigger ad-hoc runs LEAN; if the full matrix is genuinely needed on demand, run it outside 00:00–01:00 or leave it to the 01:00 nightly. **Residual:** if the nightly ever starts getting killed, the only remaining lever is capacity on mother — a sizing decision, not a CI change. Relates B88, B93, B153/B160, B138, and the mother-ram-ceiling constraint.
...
## WARNING — RUN 7 BASELINE OVERTURNED B74's PREMISE — read this before building hybrid retrieval
...
## DONE RETRIEVAL-DEPTH EXPERIMENT COMPLETE — three runs, same exam, only `EVAL_ASK_LIMIT` changed
...
if a single heavy job still OOMs at 8Gi (then raise again, or investigate a specific job). WARNING — An earlier attempt
...
### B128 — Wire rogueone DCGM GPU telemetry → Prometheus/Grafana (dcgm-exporter) — DONE 2026-09-15 (Linear EMA-223)
...
**DONE 2026-09-15 (EMA-223) — verified end-to-end.** Static scrape job `dcgm-exporter-rogueone` (`192.168.1.230:9400`) in `kube-prometheus-stack-values.yaml` (mirrors the ray-worker scrape); Grafana dashboard `k8s/monitoring/dcgm-exporter-dashboard.yaml` (**rogueone GPU (DCGM)**); fault/thermal alerts `k8s/monitoring/dcgm-gpu-alerts.yaml` (`GpuXidError` critical, `GpuHighTemp` warning — no dedicated down alert: the blanket `TargetDown` covers the sleep-flap, same as ray-worker). Docs: observability runbook §dcgm-exporter · api.md · platform-map · demo `gpu-telemetry.md` (row 80) · `flow-gpu-telemetry`. **Verification:** exporter enabled on rogueone (`snap start --enable dcgm.dcgm-exporter`), all `DCGM_FI_DEV_*` serving on `:9400` incl. `XID_ERRORS` (in the default field set, so `GpuXidError` isn't blind); Prometheus target UP (`up{job="dcgm-exporter-rogueone"}=1`); metrics flowing (`GPU_TEMP` 56C, `XID_ERRORS` 0 / No Error); dashboard ConfigMap + PrometheusRule Argo-synced (`monitoring-extras`). RTX 5000 Ada Laptop, driver 595.71.05.

---

### B129 — Catalog installed tools/packages per machine in Port.io — DONE 2026-09-14 (Linear EMA-188)

**Shipped:** a git SoT + Port catalog of installed software across the three lab machines. `scripts/collect-machine-inventory.sh <host>` (read-only; local or SSH — per-host user, **weyland=root**, else emangini; fail-loud `sources:` note; **host-tag guard** so `collect A | merge B` fails closed rather than mislabeling) → `scripts/machine_inventory.py merge` folds into `machine-inventory.yaml` (add-only + preserve decisions; dep-dominated **apt/pip** + transient **images** default `status: system`, discretionary **snap/flatpak/npm** default `unreviewed` = the review queue + drift signal) → `machine_inventory.py emit all` upserts the committed SoT to Port (`host` + `installed_package` blueprints, `tofu/port/machine_inventory.tf`). **Collected + emitted 2026-09-14:** rogueone **720** (7 documented KEEPs seeded from the 2026-08-15 audit + 58 discretionary review queue), mother **79**, weyland **739** (apt-only baseline) → **3 hosts + 1538 installed_package entities in Port**. Docs: [runbooks/machine-inventory.md](runbooks/machine-inventory.md) · [diagrams/flow-machine-inventory.md](diagrams/flow-machine-inventory.md) · [demos/machine-inventory.md](demos/machine-inventory.md) (row 79). Scan: bandit 0 (B310 urlopen accepted — fixed https Port URL), shellcheck in CI. The discretionary review queue is a **living** surface — new installs land `unreviewed`; per-machine curation is ongoing (rogueone's queue is the user's to work through; the box's audit doc is the reference). Onboarding more clients = **B169 (EMA-230)**. Original scope below.

**Original scope + the 2026-08-15 rogueone audit (retained for reference):** HIGH (↑ Medium→High 2026-09-06; was MEDIUM 2026-08-15, Linear being the status source of truth)
...
### B170 — Keep the machine-inventory catalog fresh: nightly drift → inventory PR — DONE 2026-09-19 (B129/B169 follow-on, Linear EMA-231)

**Shipped:** a nightly drift check that keeps the catalog current **without any manual cataloging** — the design converged over the session from "alert you to go log it" (rejected: no good human interface + it's just more work for you) to **the bot logs it in a PR and you merge**. `scripts/machine-inv-drift.sh` runs on **rogueone** as a user systemd timer (`nodes/rogueone/systemd/machine-inv-drift.{service,timer}`, ~03:45 NY, `Persistent=true`). Each run: `emit`+`verify` the committed SoT → Port (B169 gate); for each **reachable** host `collect | machine_inventory.py merge --prune` **inside an isolated git worktree** off `origin/main` (your checkout is never touched); if the catalog changed, open/update **one** inventory PR (`chore/machine-inventory-drift`) — **merging the PR IS the cataloging**, zero typing; push a **Kuma heartbeat** → Telegram (`down` on drift or an unreachable host, `up` when clean; the missed-ping window is the dead-man's-switch), with `pr-staleness` backstopping an ignored PR.

**Key decisions:** (1) runs on rogueone, NOT a cluster CronJob — that's where installs happen and it already has the fleet SSH keys + `gh` auth + creds, so the whole "root-capable SSH key in a pod" surface **evaporated** (self-inflicted by putting it in the wrong place). (2) `merge --prune` (new flag) also **removes** rows for uninstalled software — but only from a host actually scanned; an **unreachable host is skipped, never pruned** (merge refuses empty stdin). (3) Reopened B129's "refresh is by-hand on purpose" stance: the old worry was a timer piling up `unreviewed` with nobody deciding — moot when the bot writes the change into a PR you merge rather than nagging you to type. **Half 1 (a DoD "log it before done" line) was dropped** — the nightly PR subsumes it. **Validated live** (`--dry-run`, RUN 2026-09-19): caught a real `maven` image on rogueone that was never catalogued (the exact "installed and forgot" case), worktree left no trace. Tests: `merge --prune` (add/remove/preserve/empty-safe) in `machine-inventory.bats`; `decide_signal` (up only when clean AND all reachable) in `machine-inv-drift.bats`; shellcheck-clean. Setup (one-time, on rogueone): a Kuma push monitor → `KUMA_INVENTORY_PUSH_URL` in `scripts/.env`, then enable the timer. Docs: [runbooks/machine-inventory.md](runbooks/machine-inventory.md) · [diagrams/flow-machine-inventory.md](diagrams/flow-machine-inventory.md) · [demos/machine-inventory.md](demos/machine-inventory.md). Linear: EMA-231. **Original scope below.**
...
**Full design: `design/b132-startme-curator-design.md`.** Chrome MV3 extension that files a new bookmark into the right start.me widget **at save time**, against the 1,968-link / 147-widget taxonomy rebuilt 2026-08-05 → 08-08 (2,214 → 1,968 links; 85 → 147 widgets; `Misc.` 195 → **0**; descriptions 16% → **100%**). **The problem is entropy at the edges:** every new link is a decision against 147 widgets, and the cost of deciding is high enough that links get parked instead of filed — parking is precisely what produced `Misc.` the first time. The vendor Bookmarker extension makes it worse: it saves to a start.me **inbox**, i.e. a `Misc.` that fills itself. Requirement (operator-stated): the decision happens at save time and the item is **done** — no queue, no second pass.
...
**SECOND POST-CLOSE DEFECT, surfaced by fixing the first (2026-08-24) — an unreachable guard.** With detection fixed, the next run reached `→ triggering pipeline` and returned to a clean prompt: no pipeline number, no gate line, no error, and **no pipeline created** (confirmed against the Woodpecker API — latest was #25 from the previous night's cron). Cause: `num="$(woodpecker-cli pipeline create … 2>/dev/null | wp_field 1)"` is a **bare assignment** under `set -euo pipefail`. When the CLI exits non-zero, `pipefail` makes the pipeline non-zero and `set -e` terminates the script *at that line* — so the `[ -n "$num" ]` guard on the very next line, written for precisely this failure, **could never run in the case it names**, and `2>/dev/null` had already discarded the reason. (`poll_pipeline` carried the same shape one level down, where an unanswerable CLI is indistinguishable from a slow build for the full 30-minute timeout.)

The trigger was environmental and is itself a consequence of the first fix: `woodpecker-cli` reads a `.env` from its **working directory**, so once the loop became safe to run from anywhere, running it from `nodes/.../tofu/port` handed the CLI Port's credentials file (4 keys, no `WOODPECKER_*`) while the real credentials sat unsourced in `scripts/.env`.

Fixed: exit status captured explicitly with stderr kept in a separate file so it cannot be mistaken for output; the pipeline number validated as **numeric** (guarding the documented `--output json`-silently-ignored trap, whose human table would otherwise be polled as a pipeline that does not exist); `poll_pipeline` bails after 3 consecutive CLI failures with the CLI's own message; and `ship-images.sh` now loads `scripts/.env` itself, so the run depends on neither the caller's shell nor whichever `.env` happens to sit in the current directory. Six new bats cases (**87 green**), shellcheck clean, and `load_env` + `woodpecker-cli repo ls` verified from the directory that failed.
...
**DoD:** 1 DONE (no stale version refs; no component/endpoint/host/timer change, so arch/api/hosts/schedules/platform-map/Port are N/A) · 2 DONE N/A · 3 DONE N/A · 4 DONE · 5 DONE · 6 DONE N/A (no new service) · 7 DONE · 8 DONE (all three images in `images.tsv`; readinessProbes on all four bumped workloads; the `feast-server` ride-along is documented above).
...
**RESCOPED 2026-08-23 — the original plan (bump to 1.12.x) was BLOCKED, and it did not need to happen anyway.** Two packages independently forbid 1.12: `dagster-dbt` pins `dbt-core<1.12` (still true at its latest, 0.29.19) and `dbt-metricflow 0.13.0` pins `dbt-core<1.12.0`. So the prescribed path could never have been executed — this item was not merely unscheduled, it was unrunnable as written. **`dbt-core 1.11.14` — a PATCH bump inside the pin we already hold — relaxes the cap to `sqlparse<0.7.0`,** and that single line is the *entire* dependency diff between 1.11.12 and 1.11.14 (verified against PyPI metadata; every other requirement is byte-identical). No minor bump, no adapter churn, no sibling moves: `dbt-adapters 1.24.5` (needs ≥1.15.5), `dbt-common 1.38.0` (≥1.37.3), `dbt-extractor 0.6.0`, `dbt-protos 1.0.541`, `dbt-semantic-interfaces 0.9.0` and `dbt-trino 1.10.2` (only needs `dbt-core>=1.8.0`) all already satisfy 1.11.14.

The premise behind the deferral — "dragging a dbt-core *minor* in couples a security fix to the tool that builds all 7 marts" — was sound, and is simply no longer the situation. That reasoning is why the item existed; the rescope retires it rather than overriding it.

**What it unblocks:** 5 DoS-class CVEs accepted in `osv-scanner.toml` (8 osv findings) — CVE-2026-59893, CVE-2026-54284, CVE-2026-71491, CVE-2026-59894, GHSA-cfqr-cjx5-5jcm, all quadratic-CPU paths in the lexer/grouping/reindent. On the 2026-08-23 scan these were **3 of trivy's 4 highs and 8 of osv's 9** — the single largest remaining accepted-findings cluster. Real exposure is low (dbt parses **our own** model SQL from the repo; these CVEs need attacker-controlled input), which is what made the deferral defensible — but the fix is now cheap, so there is nothing left to defer. `services/genre-trainer` has no dbt and has been on 0.6.0 throughout.

**Resolution verified before editing, not assumed.** A `pip install --dry-run` on **python:3.11-slim** (the image's actual base — marker evaluation is interpreter-sensitive) resolved the dbt/dagster cluster cleanly with 1.11.14 + sqlparse 0.6.0, holding every other pin at its current version. The same run with `dbt-core==1.11.12` was executed first as a **control** and failed with pip naming the reason (`dbt-core 1.11.12 depends on sqlparse<0.6.0`) — reproducing build 19. Without that control a green candidate run would not have distinguished a real fix from a subset that simply did not contain the conflict.

**Acceptance:** `dbt-core==1.11.14` + `sqlparse==0.6.0` in `services/weyland-dagster/requirements.txt`; **DELETE the `[[PackageOverrides]]` sqlparse block from `osv-scanner.toml`** (it is scoped to `version = "0.5.5"` so it expires by itself, but a dead exception left behind is drift — replaced with a dated removal note so the next reader can tell a met deadline from a live accept); rebuild `weyland-dagster-user-code` (the image bakes `dbt deps` + `dbt parse`, so a broken manifest fails the build — necessary but **not** sufficient); confirm `weyland_dbt_assets` materialises, the mart tests pass (**the real check**), and dbt-docs still renders. Rides the ship loop automatically — `weyland-dagster-user-code` is in `images.tsv`.

**Cascading changes (DoD Pillar 8), both checked:**
- **`requirements.in` needs no edit.** It leaves `dagster-dbt` / `dbt-trino` / `dbt-metricflow` *unpinned* and never names `dbt-core` or `sqlparse` — both are transitive, materialised only into the compiled `requirements.txt`. A future `pip-compile` would independently land on 1.11.14 anyway, since `dagster-dbt<1.12` + `dbt-metricflow<1.12.0` make it the ceiling. Hand-pin and recompile agree.
...
|---|---|---|---|
| **core 12** | 6/7 | userspace SIGSEGVs (pytest, pip, Ray, uvicorn, apparmor_parser, tracker-miner — unrelated binaries) | 38 of 42 segfaults on that pair (~7x expected); ~40h offline → **0** (Poisson p ≈ 1e-7) |
| **core 8** | 4/5 | kernel oopses → whole-box freeze | 4 of 4 captured crash dumps on that one core (~1 in 50,000 by chance); **~49h offline → 0** |

**VERIFIED 2026-08-27 — the core-8 test PASSED.** `uptime -s` = 2026-08-25 10:58:49 (2 days 1:43), `lscpu` Off-line = **4-7** (both pairs down, the condition the test required), newest `/var/crash` dump = `202608251057` — freeze #10, one minute *before* this boot, and nothing since. Immediately prior the box was freezing every **6-11 minutes**. Uptime is itself the proof: a freeze wedges the box and forces a power cycle, so 49h continuous uptime cannot coexist with one.

**The acceleration was the strongest hardware signal.** Uptime before each freeze: #6 6h05m · #7 8h19m · #8 9h44m · #9 52m · #10 11m — hours to minutes across three reboots. No software explanation fits that curve; progressive silicon degradation does.

**Closed with evidence — do not re-run:** RAM (memtest86+, 3 passes, 128 GB, **zero** errors) · microcode 0x133 + BIOS N3TET64W 2.00 (both current) · kernel version (reproduced on BOTH `6.17.0-35` and `7.0.0-28`) · slab use-after-free (`slub_debug=FZP` armed across two crashes, silent → **not** slab; FZP has since been removed) · container/docker-veth churn (appears in traces only because container test loops were running; the operator-observed trigger across #8/#9/#10 was a **window-focus switch**).

**ACTION — the Lenovo mainboard-replacement case is now complete.** The CPU is soldered, so a failing core means the mainboard. The writeup has every element support will ask for: RAM clean, firmware current, two kernel versions, kernel debug allocator silent, crashes clustering on two adjacent P-cores, accelerating over time, and both symptom families extinguished by offlining those exact cores.

**BLOCKER — the RMA needs a replacement machine first (operator, 2026-08-27).** rogueone IS the laptop; sending it to Lenovo means going without it for the duration, so the repair cannot be scheduled until something takes its place. **This inverts the dependency with [B149]:** the diagnosis is finished and the case is filed-ready, so B150 is no longer gated on investigation — it is gated on a *purchase*, which is B149's subject. B149 is therefore on the critical path to closing B150, not the other way round.

**The offlined-cores workaround is what makes the wait survivable, and it is holding** — no failure since 2026-08-25. It is a workaround, not a fix: the cores are degrading, so treat the wait as bounded, not indefinite.

**LIVE OPERATIONAL HAZARD — `/sys/devices/system/cpu/*/online` is NOT persistent.** Every reboot silently re-onlines 4-7 and the box returns to freezing within minutes. Re-apply immediately after any reboot:
```
echo 0 | sudo tee /sys/devices/system/cpu/cpu4/online /sys/devices/system/cpu/cpu5/online /sys/devices/system/cpu/cpu6/online /sys/devices/system/cpu/cpu7/online
```
...
### B151 — B88 follow-on: deferred integration-tier depth (Flink MiniCluster + future test-tier deferrals) — MEDIUM (↑ Low→Medium 2026-09-14 rebalance; 2026-08-29) [Linear EMA-224]
...
### B149 — Personal AI lab GPU strategy: cheap local NVIDIA box + burst-rent cloud GPUs (hybrid) — MEDIUM (2026-08-16; backlogged 2026-08-27)

**Backlog-entry added 2026-08-27.** This existed only in Linear (EMA-190) from 2026-08-16 — no B-number, no backlog row — so it was invisible to `docs/backlog.md` as the ordered source of truth and to every doc that reads from it. Full research write-up (provider table, price survey, build spec, break-even arithmetic, ASCII topology) lives in the Linear issue; this row is the durable pointer plus the parts that constrain lab decisions.

**Recommendation as filed:** don't pick a single winner — a cheap local NVIDIA box for persistent experimentation, burst-rented cloud GPUs when VRAM or serious compute is needed. The "build a workstation to save money" logic is **not true** for an intermittent personal lab at current prices.

**The load-bearing numbers.** MA residential power is 28.82c/kWh (May 2026), so a ~250W box costs ~$0.072/hr to run. A ~$1,100 local box against a $0.44/hr rented 4090 breaks even at roughly **3,000 GPU-hours** — about three years at 20 hrs/week — and that comparison already flatters the local box, since the rented 4090 has 24GB against 16 and far more compute. A used-3090 build (~$1,700 all-in) needs 4,500-5,400 hours. **The constraint is VRAM, not FLOPS:** 10 hours on a rented A100 80GB is $13.90, and designing a whole machine around an event that rare is the mistake the item exists to name.

**Tiers:** RTX 5060 Ti 16GB (~$580, 180W) = the sensible local lab GPU · used 3090 24GB at today's ~$1,200 = no (great at $600-700) · 5090 32GB at $3,350+ = rent, never own. Cloud ranking for this use case: Vast.ai (cheapest raw) · RunPod (best UX; A5000 24GB $0.27/hr, A6000 48GB $0.53/hr) · GPU.ai (cheapest without marketplace hunting) · Lambda/CoreWeave/hyperscalers = wrong audience.

**Two policy tensions to settle before this becomes work, NOT resolved here:**
- **Burst-renting is paid compute**, and the lab's standing constraint is `$0` (free tiers + self-hosted). This item is a genuine exception request, not an oversight — treat it as one.
- **The local half may already be satisfied.** rogueone is the current GPU box (16GB) and B111 already runs on-demand vLLM/SGLang plus llama-guard-8b on it. So "buy a cheap local NVIDIA box" is arguably DONE, and what remains is the burst-rent question plus whether rogueone is the right box at all — which is entangled with its unresolved freeze investigation (EMA-186, memory `rogueone-gpu-freeze-vram`; 8 incidents, first positive detection 2026-08-25, parked mid-investigation).

**Open question (decide before buying any GPU):** map 16 / 24 / 32 / 48 / 80GB VRAM to the actual workloads this lab runs — RAG, agents, inference servers, fine-tuning, diffusion/video, GPU-programming learning — and decide whether buying *any* GPU makes sense versus pure burst-rent.

**COUPLED TO [B150] / EMA-186 — and B149 IS THE BLOCKING SIDE (linked in Linear 2026-08-27).** B150's investigation is **finished**: two failing P-cores, both tests passed, the Lenovo mainboard-replacement case is complete. What blocks it now is that **rogueone IS the laptop** — shipping it for repair means going without it, so the RMA cannot be scheduled until a replacement machine exists (operator, 2026-08-27). That purchase is this item. **B149 is on the critical path to closing B150.**

**STOPGAP OR STRATEGIC? — answered 2026-08-27: BOTH (operator).** The machine has to carry the lab through rogueone's RMA *and* still earn its place afterwards.

**That constraint invalidates the shape of the recommendation above, which must be revised before any purchase.** The build spec is a **desktop** (Ryzen 5/7 + RTX 5060 Ti 16GB + 2TB NVMe, ~$1,000-1,200). A desktop cannot cover the stopgap half — rogueone is a laptop, and portability is the reason a replacement is needed at all.

**But "both" also mostly dissolves the GPU question, by this item's own thesis.** The constraint is VRAM, not FLOPS, and burst-rent covers the spikes. Mobile GPUs are the worst dollar-per-VRAM tier available — power-limited, 8-12GB, sold at a premium — so paying the laptop-GPU tax buys less VRAM than the 16GB rogueone already has and *returns with* after the mainboard swap. The likely shape of the answer is therefore: **a solid portable machine with no GPU premium** — cheap enough to be a sane stopgap, durable enough to be strategic, settling in as the daily driver while rogueone resumes as the GPU host and burst-rent handles anything above 16GB.
...
**DoD:** **1 OK** `trino.md` + `observability.md` + `schedules.md` (02:45 row; the "nine k8s CronJobs"
count → ten); api/hosts N/A, verified not assumed. **2 CORRECTED 2026-08-27** `flow-servicemonitor-coverage.md`; mermaid parses. **LikeC4 placement was MISSED at close-out.** `servicemonitor-coverage` is a DEPLOYED CronJob, so Pillar 2 wanted it in the model, and it was in neither the model nor `deliveryWatchdogs` — whose description still read "three CronJobs". It does not belong under `deliveryWatchdogs` either: that is ns `weyland` PR/CI lifecycle, this is ns `monitoring` observability. Now a component in the Observability zone with two relationships (reads Prometheus targets via the ClusterIP; depends on Argo selfHeal for `.spec.replicas` to mean anything), model re-validated `Valid`. Found by asking "were any diagrams created?" AFTER the item was closed — the reverse sweep Pillar 8 describes, applied to the gate itself. Original text: mermaid
parses. **3 OK** `demos/servicemonitor-coverage.md` steps 1-6 RUN including the negative case (exit **1** vs
...
**4 OK** guard is read-only; the CronJob has no destructive verb and no write permission at all.
**5 OK** Linear EMA-207, backlog flipped. **6 OK** new timer documented + budgeted + failure-covered,
...
kept distinct.
**7 OK** shellcheck clean, 30 new bats (197 total, all green), scan surface = the usual CronJob
findings shared with the pr-lifecycle siblings. **8 OK** cascade: schedules row + budget + failure rule
...
```
kubectl apply (real creds) -> len 32, HTTP 200 DONE
kubeseal -> apply -> verify -> HTTP 200 DONE
rollout restart the consumer -> ~90 s
kubectl get secret -> len 7, "YOUR_ID" [FAIL] reverted
```
...
**DoD:** **1 OK** `secrets.md` (rotate section rewritten + the guard documented); api/hosts/schedules N/A, verified not assumed. **2 OK** `flow-secret-placeholders.md`; 126 mermaid blocks parse. **3 OK** `demos/secret-placeholders.md`, RUN end to end including the negative case; ledger row 51. **4 OK** read-only steps marked; the one fixture has a teardown; step 5's upserts are the pipeline's real output and are idempotent. **5 OK** Linear EMA-206, backlog flipped, B148 filed as EMA-207, memory updated with the selfHeal trap. **6 OK** secret restorable **and now actually valid** — which is the distinction this item exists to draw. **7 OK** shellcheck clean, 20 bats, no new scan surface (one shell script; the suite ran earlier the same day). **8 OK** cascade: `seal-secrets.sh` allow-list unchanged (port-creds was already in it — that is *why* the placeholder was faithfully preserved), `secrets.md` count unchanged at 56, no new timer, no new endpoint.
...
sealed DONE, in git DONE, Argo-applied DONE, `DATA 2` DONE — while what it restores authenticates to nothing. Exactly
...
**Verified in both directions**, because a rule that stops ignoring too much is as broken as one that ignores too much: `.aidlc-clone-id`, `.aidlc-sessions`, `spaces/*/codekb`, `spaces/*/knowledge` and the per-intent artifact directories all still ignored [YES]; the 10 tracked files plus the 2 `.claude` skill files all visible [YES]; `git status --untracked-files=all aidlc/` offers **0** new files [YES]; the CLAUDE.md @-import chain still resolves [YES].

**WARNING — I introduced a regression while fixing this, and caught it in the same pass.** The scan came back `gitleaks: 2C` — up from 0 on both earlier runs the same day. Both were **mine**, in `scripts/tests/secret-placeholders.bats`: a synthetic 32-char token fixture (`generic-api-key`) and a fake PEM (`private-key`). Neither is a real credential, but gitleaks cannot tell and neither can a human reading the report — **which is precisely the disease this item exists to remove**, pointed at a different scanner. Fixed rather than suppressed where possible: the token fixtures were rewritten to be unmistakably synthetic while exercising the identical code path; the PEM keeps a single `# gitleaks:allow` with its reason, because the PEM *shape* is what that test asserts and disguising it would destroy the test. Gitleaks now reports **0 repo-wide**, verified locally, and the suite is still 20/20.
...
**WARNING — ONE ACCEPTANCE ITEM PENDING THE PUSH.** `secret-files` still reported **12** on the 19:28 scan, because the scan-suite clones from **git** and the fix is not yet pushed. The check was reproduced locally with scan.py's exact logic (`git ls-files | git check-ignore --no-index --stdin`) and returns **0**. It will read 0 on the first scan after the push — that is the confirmation to look for, not a re-verification of the local result.

**DoD:** **1 OK** the `.gitignore` block IS the documentation and now carries the full rationale; B133's entry corrected. **2 DONE N/A** — no workflow, no component, nothing to diagram. **3 DONE N/A** — a gitignore rule has no UI or CLI walkthrough; the verification is the two-direction check above, which is recorded in full. **4 DONE N/A** — creates nothing. **5 OK** Linear EMA-205, backlog flipped, `aidlc-docs-gitignored` memory clarified. **6 DONE N/A** — no deployment. **7 OK** scan RUN (`s3://scan-reports/2026-08-25T19-28-59Z/`); it caught my own gitleaks regression, now 0 repo-wide. **8 OK** cascade above.
...
**Jupyterhub replicaSet residue — root-caused + fixed (2026-09-05).** The "known residue" above was the last standing Port audit-failure stream: `weyland-cluster` pod→`k8s_replicaSet` CREATE failures, ~1.5/hr (200 rows over 5.6 days), all jupyterhub `hub`/`proxy`. Root cause is an eventual-consistency ordering artifact, not a mapping gap — those two Deployments roll constantly (11+ `hub-*` replicasets in ~14h), so within one resync a pod entity is occasionally written before its brand-new replicaset entity lands and the hard relation errors "does not exist in blueprint k8s_replicaSet"; it self-heals next sync (all 563 replicasets exist), but the permanent FAILURE noise trains the reader to ignore Port failures (the same lit-alert problem as the 12 false-critical `secret-files`). Fixed with `createMissingRelatedEntities = true` on the `weyland_cluster` integration `config` — the tofu-safe mapping zone, and the exact flag the `linear` + `sonarqube_direct` integrations in `b137_integrations.tf` already carry. Applied live via the Port API (PATCH `config`, verified `True`) **and** codified in `b137_integrations.tf` so the next `tofu apply` converges to a no-op — no git drift. Verified by mechanism: since the flag + a full resync (cutoff 15:55:01Z), **zero** new pod→replicaSet failures over 35+ min while jupyterhub kept rolling (baseline would have produced ~1). Also re-confirmed the same run: `github-weyland` PR ingestion is live (12 `githubPullRequest` entities, repo relations resolving, `incrementalSyncEnabled: True` — not left in override), and `k8s_workload` carries 40 Job + 14 CronJob entities (the batch/v1 mapping holds). **`tofu apply` is the one remaining operator step to converge git↔live (no-op expected).**

**DoD:** **1 OK** `port.md` · `opentofu.md` · `secrets.md` · `applications.yaml` + the ten count surfaces. **2 DONE N/A** — no new component in the C4 sense; the exporter was already modelled as the `weyland-cluster` integration. **3 DONE N/A** — codifying an existing workload adds no new user-facing workflow; the verification is the helm-template diff, recorded above. **4 DONE N/A**. **5 OK** Linear EMA-204, backlog flipped. **6 OK** — this pillar *is* the item: now reproducible from git, secret restorable and sealed, monitored by the existing exporter path. **7 OK** yamllint clean; no new code (values + an Argo app). **8 OK** cascade above.
...
|---|---|---|
| **A timer** | `schedules.md` row (NY time · cadence · weight · owner); off-hours + no-heavy-on-heavy checks; a freshness signal | DONE — and **05:00 was the wrong slot**, taken by the DataHub dbt scan. Corrected to 05:15. Picked by reading the neighbouring rows instead of the table, which is the documented mistake; worth recording because `check-cron-freshness-budgets.sh` asserts a row **exists**, not that the time is free, so CI would never have caught the collision |
| **A CronJob** | `ScheduledJobStale` (26h daily budget) + **exactly one** `ScheduledJobFailed` rule | DONE guard reports `ok`/`ok`, 11 CronJobs. The **failure** rule matters more than freshness here: because it fails closed, a broken run exits non-zero having reaped nothing while the catalog keeps accumulating — freshness stays quiet as long as a later run succeeds |
| **A secret** | `seal-secrets.sh` allow-list (an allow-list, **not** a filter) + the header count + the documented total in `secrets.md` | DONE 27→28 weyland, 55→56 total |
| **A shared Argo app** | `applications.yaml` `pr-lifecycle` reason text said "**TWO** CronJobs" | DONE → THREE, with the destroys-data caveat. Same stale-reason-text cascade B135 hit on this exact entry |
| **A new workflow** | sequence diagram + demo + ledger row | DONE `flow-port-pr-reconcile.md`, `demos/port-pr-reconcile.md` (RUN), ledger row 50 |
| **The runbook** | `pr-lifecycle.md` opened "**Two** CronJobs share the Argo app" | DONE → Three, new § with the two dead-end mechanisms, the fail-closed design, and the verify-by-comparison step |
| **The C4 model** | LikeC4 `deliveryWatchdogs` said "**two** CronJobs … Both POST synthetic alerts" | DONE → three, split into two that WATCH and one that ACTS |
| **`arch.md`** | §10b named both watchdogs; also carried "**62 bats**" from B135 | DONE third job added; bats count corrected to **147**, counted rather than guessed |
| **An endpoint / host / image / dataset / repo?** | none | DONE N/A, verified not assumed |

**DoD:** **1 OK** `pr-lifecycle.md` · `schedules.md` · `secrets.md` · `arch.md` · `applications.yaml`; api/hosts N/A (no endpoint or host change). **2 OK** `flow-port-pr-reconcile.md` + LikeC4 corrected; 125 mermaid blocks parse. **3 OK** demo RUN end-to-end including the destructive step. **4 OK** read-only steps marked; the destructive step has a backup and a documented restore. **5 OK** Linear EMA-203 (promoted Medium→High on both surfaces in the same step, per the human's call), backlog flipped, B147 filed as EMA-206. **6 OK** reproducible from git (directory-based Argo app picks the file up); secret restorable — **see the outstanding item below**; monitored (stale + failure rules); triggered (05:15 daily). **7 OK** `code-scan-suite` RUN 2026-08-25 (`s3://scan-reports/2026-08-25T15-34-32Z/`). kubescape went **534H → 540H**; all six land on this manifest and all six are triaged in its header. Five (C-0009/C-0050/C-0270 CPU+resource limits · C-0211 security context · C-0237 image signature) are **byte-identical to what both sibling CronJobs already produce** — verified by diffing the failed-control sets, not assumed. The sixth, **C-0012 'Applications credentials in configuration files', is novel and a false positive**: it flags the ConfigMap because the script embeds the literal JSON `{"clientId":…,"clientSecret":…}` in its token payload, where the siblings use a single `${TOKEN}` var. Proven phantom — the real id and secret are both absent from the file (grepped against the live values) and gitleaks + detect-secrets both return **0** across the repo on the same run. **A methodology note worth keeping:** the first attempt to attribute these highs returned a clean **zero**, which was wrong — it intersected against `resourceIDs.failedResources`, a field kubescape leaves empty. A zero from an unverified query is not evidence; the mechanism was checked and the query redone from the per-resource results. Also green: yamllint, kubeconform 2/2, shellcheck, **147 bats**. **8 OK** table above.

**WARNING — ONE ITEM OUTSTANDING:** `port-pr-reconcile-creds` exists in the cluster and authenticates, but has **not yet been sealed** — `scripts/seal-secrets.sh --seal` has not been run, so `k8s/sealed-secrets/sealed/weyland__port-pr-reconcile-creds.yaml` does not exist. Until it does, the CronJob will not start on a cluster rebuilt from git. The allow-list entry and the docs are in place; only the seal + rsync remain.
...
**WARNING — THE AGENT IMAGE TAG HAS NEVER MATCHED ITS BINARY — verified, not assumed.** Ran `--version` inside the images:

```
woodpeckerci/woodpecker-server:v3.18.0 -> "3.18.0" DONE
woodpeckerci/woodpecker-agent:v3.18.0 -> "next-6d14586713" [FAIL] a NIGHTLY
woodpeckerci/woodpecker-agent:v3.17.0 -> "next-b6e16d5aec" [FAIL] (the live agents)
```
...
**All three parts shipped.** The `TXN` gate runs one real transaction per shipped service after `FR1.5`/`SMOKE` and before `[YES] shipped`; failure alerting covers all 10 CronJobs; the cadence-drift guard closed earlier.
...
**Acceptance:** one real transaction per shipped service, run after FR1.5 and before `[YES] shipped` — e.g. a Dagster GraphQL query asserting the code location loaded without error (this is the one that closes the TCP-probe gap), and a `POST /get-online-features` against Feast for a known entity key.
...
**MEASURED 2026-08-23 — no collision. Do NOT move the cron. Downgrading to MEDIUM LOW / watch-only.**
...
### B138 — Repo coverage parity: every active repo tracked across the whole toolset — **DONE 2026-09-18 (Linear EMA-225)**

**VERIFIED + DONE 2026-09-18.** Operator verification (the "can't be tested from the repo" gap) completed live: `check-repo-coverage.sh` **rc=0** (5/6 lanes ✓ parity; `ci` deferred/documented). All 3 SonarQube CronJobs run green — `sonar-scan-repos` **6/6** repos incl. all **3 private** (`freejack`/`startme-curator`/`MyBodyGraph` cloned + scanned, so the `pr-lifecycle` token access is confirmed — **no grant needed**); `sonar-scan-gradle` compiled ServiceTransformation + `ANALYSIS SUCCESSFUL`. The scan-suite image was already `SCAN_TARGET`-aware (the "needs rebuild" note was stale — `scan.py` at the deployed `git-69d22237` matches HEAD). **Bug found + fixed on first live run:** `sonar-scan-repos` failed on `stud.io` ("File … can't be indexed twice") because the runner forced `-Dsonar.sources=.` over stud.io's own `sonar-project.properties` main/test split → the runner now **defers to a repo's own `sonar-project.properties`** when present; re-run green. DoD: `demos/repo-coverage.md` (ledger row 81, RUN 2026-09-18 with the stud.io negative case) · schedules rows (14:00/15:00 UTC) + cron-freshness + `ScheduledJobFailed` · `sonarqube` LikeC4 desc refreshed for the 8-repo split · `check-linear-sync` green. Deferred (documented exception in `repos.yaml` `enforce:`): the `ci` lane (per-repo `.woodpecker.yml`, bespoke).

**The problem:** the lab has **eight active repos** — `edtbl76/Algopedia`, `ServiceTransformation`, `emangini-tailwind-nextjs-contentlayer`, `startme-curator`, `stud.io`, `weyland-lab`, plus `freejack` (**PRIVATE** — added 2026-09-15) and `MyBodyGraph` (added 2026-09-15) — and every tool that watches repos watches a *different* subset. Nobody decided that; each tool was onboarded once, against whatever set was current that day, and none of them agree now. **That the count went 6→8 the moment someone looked is itself the proof:** the set was never authoritatively known. `freejack` being private matters — public-repo tooling and integrations silently skip private repos, so it is the likeliest to be missing everywhere.
...
**IN PROGRESS 2026-09-15 (EMA-225).** Audit done (Explore sweep): the lanes ranged **1→6 repos** — `tofu/github` + all scan/CI orchestrators = weyland-lab only; pr-staleness = 6; Port = 6 (incl. `midi_real_book`, omitting `startme-curator`); backup = 4; and **`freejack` [PRIVATE] + `MyBodyGraph` were tracked NOWHERE.** **Built:** `repos.yaml` (root) = the canonical SoT (9 repos — 8 active full-parity + `midi_real_book` `status: stale`/catalog-only per owner call; `enforce: [pr, catalog, backup, iac, scan]`, ci reported as onboarding-pending so the guard ships incrementally). `scripts/check-repo-coverage.sh` = the drift guard (in the `repo-guards` CI step; enforced-lane drift=exit 1, fail-closed=exit 2; bats `scripts/tests/repo-coverage.bats`, mutation-verified). `scripts/onboard-repo.sh <repo>` = the onboarding capability (per-repo checklist from the SoT incl. private-repo access grants; the repo slice of [B154]). Runbook [runbooks/repo-coverage.md](runbooks/repo-coverage.md). **Reconciled + ENFORCED:** pr-staleness `REPOS` → 8 active (+bats asserts `midi_real_book` excluded); Port `github-weyland` both selectors → +startme-curator/freejack/MyBodyGraph; backup allow-list `backup-repos.conf` → all 8 active repos' rogueone checkout paths (`freejack` is checked out under `~/Documents/Education`, not `freejack` — so the SoT carries a `backup_path` per repo and the guard matches by PATH, not name); iac `tofu/github/` → all 8 codified (repo.tf's weyland_lab + `repos_generated.tf`, generated from the LIVE repos via `import {}` + `tofu plan -generate-config-out`, then cleaned of the experimental generator's read-only cruft — etag/fork/create-only nulls — so plans don't perpetually diff); scan the **code-scan (vuln) suite** generalized from weyland-lab-only to all 8 — a `scan-all.sh` wrapper (ConfigMap) loops the 21-tool suite over `SCAN_REPOS`, cloning public+private with the `Contents:read` token, `SCAN_TARGET` per repo → per-repo Port counts + MinIO report (scan.py made `SCAN_TARGET`-aware; **needs the scan-suite image rebuilt**). **The iac import surfaced a real correction:** `startme-curator` is PRIVATE (SoT had it public) → its Port/scan/CI tokens need access like freejack. **SonarQube multi-repo — BUILT** (3 pipelines split by language, all 8 covered): `sonar-scan` (weyland-lab, Maven/Flink deep — unchanged) · `sonar-scan-repos` (the 6 non-Java repos — alpine/git clones each, sonar-scanner source-level per repo) · `sonar-scan-gradle` (ServiceTransformation — confirmed Gradle multi-module via GitHub, so `./gradlew classes` compile → sonar-scanner with `java.binaries`). All weekly Sun, cron-freshness rules + schedules rows added. **Operator verify (can't be tested from the repo):** rebuild the scan-suite image (SCAN_TARGET) + a first live run of all 3 Sonar jobs — `sonar-scan-gradle`'s Gradle build (JDK/wrapper/deps) especially needs eyes. **Deferred (onboarding-pending):** ci only — per-repo `.woodpecker.yml` + Woodpecker activation, bespoke per project. **Operator prereq before this deploys:** grant the Port github-ocean token + pr-lifecycle PAT access to **freejack (private)** + MyBodyGraph, or those lanes silently skip them.

Linear: EMA-225. Relates **B131** (surfaced the divergence), **B137** (same disease — live-only config nobody can diff), B106/B118 (the review-stack parity claim this would verify), B57a/B57b (Woodpecker's mixed fleet), B60/B82 (Port catalog completeness).
...
|---|---|---|
| `check-port-iac-coverage.sh` | run against the live API, not a snapshot | DONE 51/21/30 blueprints · 8/8 scorecards · 4/4 integrations, 0 missing |
| `tofu plan` | no drift after every apply | DONE `No changes` |
| owner-less pod relation | guard present in the LIVE integration config | DONE read back from `GET /v1/integration/weyland-cluster` |
| `batch/v1/cronjobs` | CronJob entities exist | DONE 0 → **10** |
| Job entities | present in `k8s_workload` | DONE 27 |
| `nightly-images` | `enabled` true AND `next_exec` in the FUTURE | DONE both, read from the Woodpecker API |
| `githubPullRequest` | count matches `gh pr list` | **8 vs 7** — see below |
...
| Trigger | Cascaded to | Status |
|---|---|---|
| A **guard script** (`check-port-iac-coverage.sh`) | bats suite · shellcheck · a CI step · runbook usage block | DONE 18 tests (mutation-verified), CI step `port-iac-coverage` **blocking** |
| A **new workflow** | `flow-port-iac-coverage.md` sequence diagram + `demos/port-iac-coverage.md` + the demos ledger row | DONE all three; demo RUN, including the negative case |
| **Port catalog changed** (DoD §1 names Port explicitly) | 21 blueprints · 8 scorecards · 4 integrations codified; `k8s_workload` gained 10 CronJob entities | DONE asserted live, not inferred from a plan |
...
| `platform-map.html` | OpenTofu card said "Port blueprints + 5 Proxmox guests"; Port.io card said "K8s / GitHub / Linear exporters" | DONE both refreshed |
