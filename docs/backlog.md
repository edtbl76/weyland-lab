...
### B157 — Evaluate ODCS (Open Data Contract Standard) + bring all data contracts to spec — HIGH · FOUNDATION DONE 2026-09-05 (Linear EMA-214)

**FOUNDATION DONE (via B158 follow-up E), stays open for the tail.** Decision made: **adopt ODCS v3, a justified lab subset** — decision doc [docs/concepts/data-contracts-odcs.md](concepts/data-contracts-odcs.md). The finance domain's four data products are expressed as `services/weyland-dagster/contracts/finance/*.odcs.yaml` (mapped from the existing dbt/Soda/DomainConfig substance; columns verified against Trino), and `scripts/check-odcs-contracts.sh` is the CI conformance gate (10 bats, shellcheck clean, wired into `.woodpecker.yml` repo-guards). **Remaining (the reason this stays open):** migrate music + health contracts the same way; a live pass asserting each declared column exists in Trino; turn on dbt `contract: enforced`; and optionally generate the ODCS from the substance to keep them in lockstep. Original scope below.
