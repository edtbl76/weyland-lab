...
(B113)**, wrapping a conversational surface (Open WebUI / chat), NOT the tool-calling operator. This closes the eval; the
buildout lives in **B113**.
...
- **B113** — **Guardrails platform (defense-in-depth, 4 paths)** — ▶ **STARTED 2026-08-03.** A four-layer guardrails stack, each a distinct path doing one job (the industry-standard pattern; stacks run 2–3, complementary not competing). All OSS/self-host → **$0**. Design: `aidlc-docs/guardrails-platform.md`. Paths: (1) **Scan** = weyland-guard (≈ LLM Guard) — I/O sanitization, already LIVE (B14/B34/B35); (2) **Classify** = **Llama Guard** (Meta) — a self-hosted safety-classifier model on rogueone (served via vLLM, the B111 GPU-inference pattern) that the Scan layer can call; (3) **Dialog** = **NeMo Guardrails** (adopted from B32) — Colang topical/flow control wrapping the Open WebUI / chat lane, NOT the tool-calling operator (`scripts/nemo-guardrails-trial/`); (4) **Structure** = **Guardrails AI** — output-schema validate + re-ask on structured-output tasks. Compose: Scan (always-on) → Classify → Dialog (conversational surfaces) → Structure (structured outputs); alongside the operator's pre-action-authorization (confirm-rails + `policy.gate`) + offline eval (B84). **Build order:** Llama Guard (first) → Guardrails AI → NeMo → augment Scan. Relates B14, B34, B35, B32 (NeMo adopt), B70 (weyland-guard), B111 (rogueone serving).
