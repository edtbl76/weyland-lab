...
### B133 — Migrate off the bespoke "Method" onto AWS AI-DLC v2 — 🟡 **BUILT + DoD-SWEPT 2026-08-20; one gap open**
...
**⚠ REMAINING before ✅.** **Pillar 7 (scan triage)** — the `code-scan-suite` run was blocked all of 2026-08-20 (first by the CPU-requests wall → B134, then by a semgrep OOM); once it completes, triage the new criticals/highs. **The demo's Part B (the full gated run) is authored but unexecuted, and its live validation is delegated to B87** (the batched demo-validation sweep, ledger row 47) rather than blocking here — it is the same 🟡 authored-pending-validation state as E1–E12. Everything else — engine health (`doctor` **47 passed / 0 failed**), graph integrity (0 cycles, 33 stages topo-ordered, `scope-table --check` clean), the deviation checks, and the decoupling regression — was **RUN live on rogueone 2026-08-20**. Per the DoD ("a demo written but not executed is not done"), B133 stays **🟡** until a person runs it; it cannot run unattended, since every stage ends in a human turn-stop.

**Part B was reshaped 2026-08-20 after a failed first attempt, and the failure is the lesson.** The original draft said "run on a deliberately trivial task" and picked `poc` scope. That stalls: `intent-capture` (scopes `enterprise`/`feature`/`mvp`/`poc`) is a **discovery** stage — it asks what business problem this solves, who the stakeholders are, what success looks like — and an invented task has no honest answers to any of it. Two fixes, both about what a demo *is*: (1) **a demo demonstrates, reproducibly, on demand** — it is not a vehicle for real work, and "run it against the next real backlog item" was rejected because that makes it non-reproducible and turns the pillar into a promise; (2) **use `bugfix` scope** (7 stages: reverse-engineering → requirements-analysis → code-generation → build-and-test), which **skips discovery entirely**, so its questions are about the defect and expected behaviour rather than business intent. Part B now creates its own **disposable fixture in `/tmp`** (a shell script that divides by zero on an empty file — defect reproduction **verified** 2026-08-20: prints `2`, then `division by 0`, exit 1), fixes it through the workflow, and tears both fixture and intent record down, leaving no repo trace. The first attempt also surfaced two real engine hazards, both fixed: `intent-create` **auto-migrated a legacy flat workspace**, sweeping the entire gitignored `aidlc-docs/` (79 files) into a tracked intent dir on a public repo (restored; the Method-era `aidlc-state.md` that triggered it is renamed `aidlc-state.method-archive.md`), and Method-era state has no `State Version` for the v8 graph so it could not advance.

**⛔ Pillar 7 blocker (infra, not code) — the scan-suite could not be scheduled.** `./scripts/run-scan-suite.sh` timed out at 900s with empty logs: the pod sat **Pending 40+ min**, never scheduled — `FailedScheduling: 0/1 nodes are available: 1 Insufficient cpu`, no preemption victims. Root cause is **CPU-request saturation, not contention**: mother is **16 cores allocatable with ~17.1 reserved** while running at **~19% actual usage** (busiest pod in the cluster: **0.12 cores**; top-10 combined ≈ 0.58). ~14 cores of reservation are fiction. Same wall the Flink JobManager already carries a comment about (`k8s/data-mesh/flink-session.yaml`: *"node CPU-requests are saturated"*), and the same class as [[istiod-request-node-wall]]. **Fix applied (GitOps, 2 manifests):** `llama-guard` `cpu: "1"` → **100m** (reserved a full core, uses <0.03, and has **no CPU limit** so inference still bursts freely) and `code-scan-suite` `cpu: 250m` → **100m** (a batch job with no latency SLO gains nothing from a large *reservation*). Frees ~1.05 cores. **Measurement caveat worth keeping:** naive `sum by (pod)` over `kube_pod_container_resource_requests` **double-counts** — both kube-state-metrics *and* opencost publish it — and a `node=` filter still includes terminal pods that no longer reserve; pin `job="kube-state-metrics"` and exclude Succeeded/Failed, or just trust the scheduler's own `FailedScheduling` verdict. The **systemic** over-reservation across ~50 pods remains open and is worth its own item.
