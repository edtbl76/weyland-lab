...
- **B48** — **Observability: unified logs + traces in Grafana (Loki + Tempo)** — currently NO log aggregation (only `kubectl logs`); traces are Jaeger-only (viewable in Grafana via Explore, but not the Drilldown app). Stand up **Loki** (+ a collector — Alloy/Promtail) for logs and **Tempo** for the unified trace Drilldown. Both new standing services. Deferred deliberately after Code Quality. Jaeger+Alertmanager datasources already surface traces+alerts in Grafana as the interim.
