...
- **B60** — **Master + build out Port** (B59 gave the catalog; Port does far more). **Phase 1 — audit & learn the portal surface (do first, understand before building):** (1) **finish the setup wizard** — kill the "3 of 4 tasks completed" nag (find + complete the 4th task); (2) **Quick Access walkthrough** — set up **ownership**, tune **automatic discovery**, learn **"Plan my day"** + the **Services / Workloads / Environments** dashboards; (3) **Catalog sidebar audit** — every page (our Software Catalog folder + the Port-default/integration tables) → keep/prune/organize; (4) **Self-service audit** — existing actions are mostly Port stock, decide what's ours; (5) **Builder sidebar audit** — automations, org settings, the admin surface. **Phase 2 — build the high-value stuff:** (6) **custom scorecards** for *our* standards (vs Port's stock DORA/quality — "has runbook", "meshed to STRICT Postgres", lab maturity tiers) — fast win on the fresh catalog; (7) **self-service actions** — golden-path scaffolding via Port's *own* engine (e.g. "new service" → GitHub repo/PR; **sidesteps the node-fetch bug that killed B42**); (8) **Port workflows**. **Polish:** operator-cockpit dashboard (cost+CI+errors+uptime); deeper entity coverage (Dagster pipelines / Argo apps / MLflow runs). **Start order:** wizard quick-win → the audits → scorecards → actions → workflows. **Big step — own block.**
