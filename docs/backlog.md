...
20. **B9** — **Codebase refactoring (Python / Go / Rust / …)** — 🔴 **HIGH (↑ Medium→High 2026-08-11 rebalance — CodeScene hotspots `loaders.py` 4.46 / `build_store_load_assets` cc=56 give it a real driver; was ↑ Low→Medium 2026-08-08; rebranded from "Python→Go refactor").** Language/perf refactoring of any service where a rewrite earns its keep (Go or Rust for hot paths, etc.) — conditional; revisit when operational pain is real or agents are heavily modifying the codebase. No driver today; the Python services run fine. See detail below.
...
### B127 — Migrate rogueone off Docker Desktop → native Docker Engine — 🟡 MEDIUM (2026-08-12)
**Root cause (conclusive 2026-08-12):** the recurring rogueone "idle GPU freeze" is **SYSTEM-RAM OOM**, not the GPU. Docker Desktop runs the engine inside a **qemu VM** that leaks **~28 GB of unreclaimable shmem** (`oom_reaper` can't free it — shmem-rss survives the kill) which, with an overnight Chrome session, exhausts the 125 GB box over ~2 days → repeated `global_oom` → CPU **soft-lockups** (chrome stuck ~9000s) → hard wedge (no-iGPU, so the display freezes). The **GPU/RTD3-D3cold theory was a multi-session red herring** — RTD3 was confirmed `Disabled` + PCI pinned and it STILL froze; `nvidia` appeared only in the OOM victim table. Memory `rogueone-gpu-freeze-vram`.
