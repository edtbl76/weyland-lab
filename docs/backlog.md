...
### B104 — AI dev-tooling landscape evaluation ("tech nerd" survey) — HIGH (↑ Low→Medium 2026-08-11 rebalance; ↑ Medium→High 2026-08-27 — UPGRADED, not demoted. The categories already covered by shipped work [B15 assistants · B106/B118 code review · B47/B89/B120 SAST-SCA] are *evidence the survey pays off*, not evidence it is spent; the landscape has moved substantially since this list was written and the list itself needs refreshing before the next pass. Genuinely open + unbuilt: **load/perf testing** [k6 · JMeter · Gatling, all OSS] — there is no throughput or latency baseline for the tool-server, AI gateway, or Trino today. Pairs with [B86], reopened the same day)
...
### B76 — Full MSD + Music4All + MTG-Jamendo — HIGH (↑ Medium→High 2026-09-06; data expansion)
...
### B88 — Per-language build lanes: tests + scanners + software supply chain — HIGH (ALL WORK COMPLETE 2026-08-30 — Track B gaps #1–#6 done + all Track A tails A1–A4 verified; ready to flip to Done pending the Linear status update) (REOPENED 2026-08-28 — closing it was then premature; see OUTSTANDING below) (2026-08-05; gated on Stud.IO CI track, B57/B118; ↑ Medium→High 2026-08-27; SCOPE EXPANDED 2026-08-27 from "test runners" to the whole per-language build surface — see the three phases below)
...
### B159 — Onboarding services: represent the onboarding flow as real services (B154 Phase 2) — MEDIUM (2026-09-07, Linear EMA-216)
...
### B160 — Extend the golden-path language suite (B153 follow-on) — HIGH (2026-09-07, Linear EMA-217)

Extend **B153**'s golden paths to a second wave of languages, each conforming to the SAME framework-agnostic contract ([docs/design/golden-paths.md](design/golden-paths.md) — `/health` `/ready` `/metrics` `/hello` + lane self-test + selfcheck + coverage + buildkit Dockerfile + run-to-completion Job + onboarding-declaration template; runnable, ephemeral, extendable). **Languages:** Angular (frontend/TS) · C#/.NET · C/C++ · PHP · Ruby on Rails · Elixir · Clojure · Kotlin · Scala. Multiple relevant frameworks per language, locked at build time like B153's matrix — e.g. **.NET** ASP.NET Core (minimal API + MVC) · **PHP** Laravel/Symfony/Slim · **Ruby** Rails/Sinatra · **Elixir** Phoenix/Plug · **Clojure** Ring+Compojure/Pedestal · **Kotlin** Ktor/Spring/http4k · **Scala** Play/Akka-HTTP/http4s/ZIO · **C++** Drogon/Crow/Pistache · **C** libmicrohttpd/civetweb · **Angular** SPA + Angular Universal SSR. Each also needs a **new CI test + scan lane** in `run-lang-tests.sh`/`run-lang-scan.sh` (most are net-new toolchains → new runner images). Depends on **B153** establishing the contract + the ephemeral-Job harness + the golden-paths-as-fixtures switch. Relates **B88** (per-language lanes), **B154** (onboarding gates the paths pass), **B155** (API lifecycle captures their OpenAPI).

---

### B161 — Investigate Dify — HIGH (2026-09-08, Linear EMA-218)

Evaluate **Dify** (open-source LLMOps platform — visual workflow/agent builder, RAG pipeline, prompt IDE, model/provider management, dataset management, built-in tracing) against the lab's existing AI plane, and produce a keep/skip call. **Method (per the reverse-engineering rule — read the governing reality first):** map Dify's capabilities onto what the estate already runs and find the actual delta, not the marketing surface. Existing coverage to compare against: **gateway** (Bifrost + LiteLLM, 232 MCP tools, prompt/skills repos), **agents** (LangGraph — B66 operator, B17 Realm of Agents, B70 agentic-RAG), **RAG** (Qdrant/Weaviate/LanceDB + bge embed + the streaming indexer), **eval** (promptfoo B84, MLflow gateway, golden-set B96), **observability** (Langfuse B103, MLflow Traces), **workflow** (n8n, Dagster). Open questions: (1) does Dify's low-code workflow/agent canvas give a non-code authoring surface nothing else does, or duplicate LangGraph + n8n? (2) is its RAG/dataset layer additive or a redundant fourth store? (3) can it front the existing Bifrost/Ollama providers rather than its own? (4) $0 + self-hosted + LAN-only (Keycloak forward-auth, meshed Postgres/Redis) — does it fit the node capacity budget? **Deliverable:** a per-capability overlap table + a recommendation (replace X / complement as low-code front door / skip as duplicate) with rationale. Relates B17+B19, B103, B84, B66.

---

### B162 — System-complexity bucket: reward DEEP modules, re-evaluate method line-count thresholds (Ousterhout) — MEDIUM (2026-09-08, Linear EMA-219)
...
### B163 — Golden-path capability coverage: exercise each framework's modules semi-exhaustively — HIGH (2026-09-08, Linear EMA-220)
...
### B129 — Catalog installed tools/packages per machine in Port.io — HIGH (↑ Medium→High 2026-09-06; was MEDIUM 2026-08-15, Linear being the status source of truth)
