...
**Added 2026-07-23.** MLflow is deployed (v3.14 — B10/B16/B47: experiment tracking + model registry) and B70 just lit up **Tracing** for `weyland-agent`. But MLflow 3.x is a full **GenAI platform** — this is the B60/B65-style "master the tool" buildout to exploit ALL of it. Four pillars:
- **Tracing (integrations) — extend B70's start.** MLflow auto-instruments a long framework list (OpenAI, LangChain, **LangGraph**, LlamaIndex, DSPy, Anthropic, **LiteLLM**, Gemini, Bedrock, AutoGen, CrewAI…). Wire MLflow **Traces** into EVERY AI surface: `weyland-agent` (B70), the tool-server RAG `/context/ask`, the eval harness, Hermes/B66 agents — one pane of per-step GenAI observability (prompt/context/answer/tool spans) that the mesh traces (Tempo) can't give.
...
- **Prompts** — MLflow **Prompt Registry** (versioned, managed prompts + diff/rollback). Promote the RAG system prompt + the agent's grade/reflect prompts + the eval questions from inline strings to first-class versioned artifacts.
- **AI Gateway** — MLflow's **AI Gateway / deployments server** (OpenAI-compat routing, provider abstraction, rate-limit/cost). **Overlaps LiteLLM (B26 — the current model gateway)** and the B70 Phase-B "LiteLLM→vLLM" plan. Evaluate MLflow AI Gateway vs LiteLLM and pick ONE gateway — don't run two.
