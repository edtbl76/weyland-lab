...
9. **B3** — IDP / Backstage — **slices A+B DONE 2026-06-19** (learning project). **⚠️ DECOMMISSION IN PROGRESS** — being replaced by Port.io (see B43). Backstage stays live until Port.io migration is complete; then `k8s/weyland-idp/` and related infra (ConfigMaps, Postgres role, MinIO `techdocs` bucket, Dagster `weyland_techdocs_job`) all get torn down. See detail below.
...
11. **B1** — Data mesh — three concrete data products: (1) model eval product (continuous judge-panel runs, time-series leaderboard); (2) data store inventory product (Data-as-a-Product for each backend); (3) model tuning feed → fine-tuning → custom models. **Includes central IDP/SSO — Keycloak** (decided 2026-06-22, **chosen over Okta**: LAN-only lab must not depend on cloud auth — a WAN outage can't be allowed to lock you out of your own lab; on-brand with self-hosted-over-SaaS). Keycloak (k8s + meshed Postgres, `keycloak.weyland.lab`, `weyland` realm) replaces the scattered dev-password basicAuth + per-app logins: OIDC for capable apps (Grafana/GlitchTip/MinIO/Open WebUI, repoint Woodpecker) + Traefik **forward-auth → Keycloak** for the non-OIDC ones (Kiali/MLflow/NeoDash). Slotted here because multi-service access governance is when SSO earns its keep. See detail below.
