...
20. **B9** — **Codebase refactoring (Python / Go / Rust / …)** — 🟡 **MEDIUM (2026-08-05; ↑ Low→Medium 2026-08-08 rebalance — earned: CodeScene scored `loaders.py` 4.46, `build_store_load_assets` cc=56; rebranded from "Python→Go refactor").** Language/perf refactoring of any service where a rewrite earns its keep (Go or Rust for hot paths, etc.) — conditional; revisit when operational pain is real or agents are heavily modifying the codebase. No driver today; the Python services run fine. See detail below.
