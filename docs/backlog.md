...
guardrails at the edge (weyland-guard + confirm-step), not inline. **Remaining Phase 3:** **Bifrost** (agent-edge:
aggregate `/mcp` + `/mcp-fleet`, OAuth-terminate for B15 coding agents — NEXT); **B17 = A2A eval** (later). First fleet
