...
### B154 — Self-service onboarding for applications / repos — HIGH (↑ Medium→High 2026-09-06, Linear EMA-211)
...
**GUARD EXTENDED 2026-09-07.** Evaluated the other DoD §6 surfaces for the guard and added only the ones that are cleanly file-checkable and non-duplicative — the honest outcome, not a box-check. The guard now runs **three** checks: **SCHEMA** (every registry entry declares a boolean `deployed` — a missing field would silently exclude a service from the placement check, the absent-reads-as-not-deployed footgun this guard family kills), **PORT** (every deployed service declares a `port_component`), and **PLACEMENT** (LikeC4, the original). **Deliberately NOT added, with reasons probed live:** ServiceMonitor/dashboard/alert are already owned by the live coverage guards (re-checking = duplication); **Kuma** is UI-configured, not in git; **arch.md §6** is a curated subset with no clean predicate — only 14 of 31 ingress services live in the §6 table, the rest documented in their own arch.md sections, so a `§6-row` check would be ~17 false positives. Re-checking any of these would add fragility or noise, not coverage. Live invariant holds (66 declare `deployed`; 59/59 have a Port component + a LikeC4 placement); `onboarding-completeness.bats` now **12** cases (SCHEMA + PORT negatives added), shellcheck 0. EMA-211 left open for the human to flip once the surface set is judged sufficient.
