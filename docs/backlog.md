...
32. **B43** — **Port.io IDP migration** (replacing Backstage) — **IN PROGRESS 2026-06-20.** Port.io (SaaS, EU, Free tier — `app.port.io`, org `org_KyCTEN4PVUv1D3TM`) is the zero-maintenance IDP replacement for Backstage. **Live:** K8s exporter (`weyland-cluster` — full cluster topology: namespaces/nodes/workloads/replicasets/pods + Istio Gateway/VirtualService CRDs), GitHub exporter (`github-weyland` — 6 repos, Port-side IN() filter to exclude public repo GitHub-app loophole). **Live exporters:** K8s (`weyland-cluster`), GitHub (`github-weyland`), **Uptime Kuma** (`uptime-kuma` webhook → `uptime_monitor` blueprint, **16 monitors**, `kuma.weyland.lab`; [runbooks/uptime-kuma.md](runbooks/uptime-kuma.md) — needs LAN CoreDNS + mkcert CA mount). **In progress:** code quality (SonarQube CE), and additional categories being wired up one at a time. **Decom trigger:** Backstage (`k8s/weyland-idp/`) torn down when Port.io covers the same catalog surface — including: removing the `weyland_techdocs_job` Dagster asset, MinIO `techdocs` bucket, IDP Postgres role, ConfigMaps, and k8s manifests. B42 (Backstage scaffolder) is moot once Backstage is decommissioned.
