...
- **What works:** `weyland_pipeline/datahub_emit.py` — walks `all_assets`, emits a Dataset + UpstreamLineage per asset to GMS via the `datahub` REST SDK. Wired as `datahub_catalog_emit_job` (op) + hourly `ScheduleDefinition`. `requirements.txt`: dropped the plugin → plain `acryl-datahub`. Sensor + its imports removed from `definitions.py`. Idempotent (DataHub upserts).
