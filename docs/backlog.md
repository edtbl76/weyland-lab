...
- **B50** — **Port as launcher (not status board)** — DONE 2026-06-21 (recording the model change). Retired the Kuma→Port `uptime_monitor` flow (status went stale — Kuma webhook is event-only); Port now = `endpoint` blueprint (28 entities) + **Launcher** dashboard (one-click UIs/APIs). Kuma (`kuma.weyland.lab`, Telegram paging) is the live status board. Catalog cruft left as-is (istio_gateway/VS empty, k8s_pod churn, DORA defaults) — deep-clean only if Port gets noisy.
...
### Extras / Optimization
- **B57** — **Build farm: real pipelines + Stud.IO migration** — deferred from B56. (a) Flesh out `.woodpecker.yml` into actual CI/CD for the weyland images (build tool-server/dagster → `k3s ctr import` → rollout; validate manifests with kubeconform) so the farm replaces the manual scp+build loop; add **cron** triggers (the LAN box gets no push webhooks). (b) Migrate **Stud.IO** off its local Woodpecker (+ GH Actions runners on rogueone) onto weyland's shared dev/build resources — weyland Woodpecker is natively multi-repo, so add the Stud.IO repos. Keep both parallel until the weyland farm is proven.
