...
| `pr-lifecycle-reconcile-github` | `weyland` | GitHub PAT that GENUINELY holds `Pull requests: write` + `Issues: write` on **all 8 pr-lane repos** (recreate posts a comment, close closes a PR) — incl. private `freejack` + `startme-curator`, or those repos fail closed; distinct from the read-only token above, least-privilege per job (see § "Multi-repo coverage") | `k8s/sealed-secrets/sealed/weyland__pr-lifecycle-reconcile-github.yaml` |

> ⚠ **Token gotchas, both found on the first live cron run (2026-09-22):** (1) `PR_TOKEN` (machine-inv-drift's PAT) is the WRONG token here — `GET repos/.../` shows it `admin/push/pull`, but that reflects the *repo owner's* access, not the fine-grained PAT's *granted* permissions; it lacks the `Pull requests: write` grant and every write 403s `Resource not accessible by personal access token`. Use a PAT that actually has that grant (the Woodpecker `github_token` fine-grained PAT does — Contents:write + Pull requests:write). (2) The guard's writes use the **REST API** (`gh api` on `issues/<n>/comments` + `pulls/<n>`), NOT `gh pr comment` / `gh pr close` — those are GraphQL mutations (`addComment` / `closePullRequest`) that fine-grained PATs cannot access even *with* the write grant. REST is fully supported for fine-grained PATs; GraphQL is not.
