...
| `pr-lifecycle-reconcile-github` | `weyland` | GitHub PAT that GENUINELY holds `Pull requests: write` on `edtbl76/weyland-lab` (recreate posts a comment, close closes a PR) — distinct from the read-only token above, least-privilege per job | `k8s/sealed-secrets/sealed/weyland__pr-lifecycle-reconcile-github.yaml` |
