...
- **In-process bge query embedding** (`OnnxBge`, raw ONNX Runtime, bge-base 768 — U13 dropped torch; byte-identical class to the tool-server, guarded by `check-onnx-sync.sh`) — same model the tool-server uses and the
