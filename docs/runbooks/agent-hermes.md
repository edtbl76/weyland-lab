...
- **What it is:** NousResearch/hermes-agent (MIT) — autonomous agent. Brain → Ollama `/v1`; system
  view (planned) → tool-server MCP. Sized lighter than the inference CTs because it
...
- **Base URL:** `http://192.168.1.230:11434/v1` · **API key:** any non-empty string (`ollama`).
...
## Ollama tuning (on rogueone — see [model-serving-ollama.md](model-serving-ollama.md))
Driven by this agent, applied on the Ollama host (rogueone since B79). Two env vars in rogueone's Ollama drop-in:
...
prompt." Ollama diagnostics now run on **rogueone** (`ssh edwardmangini@rogueone`, since B79 moved
Ollama there); the reachability check runs from the hermes CT.

```bash
# Loaded model? served CONTEXT? pinned? (UNTIL=Forever means KEEP_ALIVE=-1 held)
ssh edwardmangini@rogueone ollama ps

# Prefill rate + EXACT prompt token count of recent turns — the "why slow" ground truth.
ssh edwardmangini@rogueone journalctl -u ollama --no-pager -n 80 | grep -iE 'prompt processing|prompt eval|tokens per second'
...
ssh edwardmangini@rogueone top -bn1 | head -12
ssh edwardmangini@rogueone bash -lc 'ps -eo pcpu,comm --sort=-pcpu | head -4'

# Verify Ollama env vars actually applied (HOST, MAX_LOADED_MODELS, CONTEXT_LENGTH, KEEP_ALIVE)
ssh edwardmangini@rogueone systemctl show ollama -p Environment

# Network reachability hermes CT (104) -> ollama (rogueone .230)
pct exec 104 -- curl -s http://192.168.1.230:11434/v1/models
```
...
searchable, not pinned — the "0 active" count is normal).
...
First DM after a (re)start cold-prefills the ~18K prompt (17K base + MCP reg + your msg). Watch it on
rogueone (Ollama's host) — `ssh edwardmangini@rogueone journalctl -u ollama --no-pager -n 30 | grep -iE 'prompt eval|progress'`
...
`../design/b27-kanban-design.md` (repo-local; not published to the docs site).
...
- A `custom` provider **`weyland-model-gateway`** → `http://192.168.1.243:30400/v1`, key = the LiteLLM
  ...
  **⚠ the `hermes model` wizard sets this as the DEFAULT** — revert the default back to local:
  ...
  hermes config set model.base_url http://192.168.1.230:11434/v1
