...
| **A deployed service** | `applications.yaml` registry entry (or an explicit `kind: plumbing` with a reason) → Port `component`; **Kuma monitor**; a `*Down` **PrometheusRule**; **ServiceMonitor + Grafana dashboard** if it exposes `/metrics`; logs reaching Loki; LikeC4 placement; `arch.md` §6 inventory row. **`scripts/check-onboarding-completeness.sh` (repo-guards) is the standing gate for this row (B154):** each deployed service declares `deployed`/`metrics`/`ingress`; the guard VERIFIES the file-checkable gates (registry+`port_component`, LikeC4 placement, declaration completeness) and ACCOUNTS the rest per service — `--list` prints the DoD §6 matrix where ServiceMonitor/dashboard/`*Down` are LIVE (reconciled by servicemonitor-/dashboard-/alert-coverage, new services included), Kuma is MANUAL (UI-configured), logs are AUTO (Alloy), and `arch.md` §6 is curated. An undeclared conditional gate fails the guard — nothing is silently skipped. Onboard a new service with `scripts/onboard-service.sh` so it lands complete. |
