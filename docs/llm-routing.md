...
Most chains end on a **free, always-on** rung so a call lands for $0. Exceptions, faithful to config: `wl-search`
(perplexity → xAI, both paid — no free tail) and `wl-default` (whose last rung is the funded `wl-agentic`/Anthropic
alias, though free Gemini precedes it). The media lane `wl-tts` inverts the pattern — its **primary** Kokoro is the
free, self-hosted, always-on source. For the full interactive view see the
...
flowchart LR
  ...
  wspd(["wl-speed"]):::alias --> wspd1["groq · gpt-oss-120b"]:::free --> wspd2["cerebras · gpt-oss-120b"]:::metered --> wspd3["gemini · 2.5-flash"]:::free
  wrsn(["wl-reason"]):::alias --> wrsn1["ollama · qwen3:30b-a3b (local)"]:::free --> wrsn2["deepseek · v4-pro"]:::metered --> wrsn3["groq · gpt-oss-120b"]:::free
  wbig(["wl-big-oss"]):::alias --> wbig1["openrouter · minimax-m3"]:::metered --> wbig2["groq · gpt-oss-120b"]:::free

  %% Code & agents
  wcod(["wl-coding"]):::alias --> wcod1["opencode-zen · kimi-k3"]:::funded --> wcod2["anthropic · claude-haiku-4.5"]:::funded --> wcod3["deepseek · v4-flash"]:::metered --> wcod4["groq · gpt-oss-120b"]:::free
  wagt(["wl-agentic"]):::alias --> wagt1["anthropic · claude-haiku-4.5"]:::funded --> wagt2["openai · gpt-4o-mini"]:::metered --> wagt3["cerebras · gpt-oss-120b"]:::metered --> wagt4["gemini · 2.5-flash"]:::free
  ...
  wsch(["wl-search"]):::alias --> wsch1["perplexity · sonar (web)"]:::funded --> wsch2["xai · grok-4.5 (web)"]:::metered
