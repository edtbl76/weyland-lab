...
/**
 * B152 architecture-test category — ArchUnit rules for the Flink SQL-runner module, run as ordinary JUnit
 * tests in the existing test-java lane. ArchUnit reads bytecode statically, so no Flink cluster is needed.
 *
 * The load-bearing rule: NO access to System.out/err/printStackTrace. A streaming job must log through slf4j
 * (Flink's logging), never raw stdout. This module's two `System.out.println` logging lines were moved to
 * slf4j LOG.info in this same pass so this rule holds — and now guards against a stdout regression. A
 * structural rule pins the package.
 *
 * Mirrors the dagster import-linter lane: the production classes obey the rules (Green), and a planted offender
 * is PROVEN to trip the rule (asserting the offending class by name — fail-closed, never a bare boolean).
 */
