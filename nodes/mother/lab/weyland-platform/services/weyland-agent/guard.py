"""Fail-open client to the shared weyland-guard service (B70) — same contract the tool-server uses."""
import os
...
def guard(hook: str, request_id: str, payload: dict, actor: str | None = None) -> dict | None:
    """POST a hook to weyland-guard. Returns the blocking verdict dict if the guard blocked (enforcing mode), else
    None. FAIL-OPEN: any error/timeout/unreachable → None (allow) — a guard outage must never take an answer offline."""
