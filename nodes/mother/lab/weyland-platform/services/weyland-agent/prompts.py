...
def render_prompt(name: str, fallback: str, **variables) -> str:
    """load_prompt + str.format for templated prompts. If a fetched template can't be formatted with `variables`
    (e.g. a bad placeholder edit), fall back to formatting the baked template — a request never breaks on a prompt edit."""
    template = load_prompt(name, fallback)
    try:
