...
def _read_spotify(mc):
    ...
    return df[df["track_genre"].isin(keep[keep >= 20].index)]
...
def mlflow_genre_from_silver(context):
    out = _train_and_log(_read_spotify(io.client()), "silver", context.log)
...
def mlflow_genre_from_feast(context):
    ...
    labels = _read_spotify(io.client())[["track_id", "track_genre"]]
